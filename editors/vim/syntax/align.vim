...
syn keyword alignType i8 u8 i16 u16 i32 u32 i64 u64 f32 f64 bool char str string array slice soa box raw builder writer reader buffer array_builder file rng regex regex_match tcp_conn tcp_listener udp_socket child http_request_ctx http_headers response_builder http_stream Option Result Error Num Ord Eq vec2 vec4 vec8 vec16 mask2 mask4 mask8 mask16
syn match alignType "\<json\.\(doc\|kind\|scanner\)\>"
syn keyword alignBuiltin map par_map where reduce scan partition group_by agg sort sort_by_key chunks zip to_array map_into to_soa dict_encode sum min max count any all dot sum_where select fma abs sqrt floor ceil round trunc pow hash64 hash128 Some None Ok Err error map_err spawn wait print
