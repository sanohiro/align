...
if [[ "$(wc -l < "$work_dir/base" | tr -d ' ')" != 296 ]]; then
  echo "test-runtime-abi-exports: declaration golden does not contain 296 base symbols" >&2
  exit 1
fi
...
perl -ne '
  sub native_type {
    ...
    my @types = ($text =~ /\b(void|ptr|i64|i32|float|double)\b/g);
...
if [[ "$(wc -l < "$work_dir/golden-abi" | tr -d ' ')" != 296 ]] \
  || [[ "$(wc -l < "$work_dir/runtime-abi" | tr -d ' ')" != 296 ]]; then
  echo "test-runtime-abi-exports: normalized base ABI does not contain 296 rows" >&2
  exit 1
fi
diff -u "$work_dir/golden-abi" "$work_dir/runtime-abi"
printf 'runtime-abi-signatures base 296\n'
...
audit_case base "" 296
audit_case alloc alloc-count 300
audit_case par par-map-probe 300
audit_case task task-group-probe 296
audit_case all alloc-count,par-map-probe,task-group-probe 304
