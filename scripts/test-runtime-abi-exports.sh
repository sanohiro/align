...
if [[ "$(wc -l < "$work_dir/base" | tr -d ' ')" != 288 ]]; then
  echo "test-runtime-abi-exports: declaration golden does not contain 288 base symbols" >&2
  exit 1
fi
...
if [[ "$(wc -l < "$work_dir/golden-abi" | tr -d ' ')" != 288 ]] \
  || [[ "$(wc -l < "$work_dir/runtime-abi" | tr -d ' ')" != 288 ]]; then
  echo "test-runtime-abi-exports: normalized base ABI does not contain 288 rows" >&2
  exit 1
fi
...
audit_case base "" 288
audit_case alloc alloc-count 292
audit_case par par-map-probe 292
audit_case task task-group-probe 288
audit_case all alloc-count,par-map-probe,task-group-probe 296
