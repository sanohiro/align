...
if [[ "$(wc -l < "$work_dir/golden-abi" | tr -d ' ')" != 286 ]] \
  || [[ "$(wc -l < "$work_dir/runtime-abi" | tr -d ' ')" != 286 ]]; then
  echo "test-runtime-abi-exports: normalized base ABI does not contain 286 rows" >&2
  exit 1
fi
diff -u "$work_dir/golden-abi" "$work_dir/runtime-abi"
printf 'runtime-abi-signatures base 286\n'
