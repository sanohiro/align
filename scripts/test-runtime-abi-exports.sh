...
if [[ "$(wc -l < "$work_dir/base" | tr -d ' ')" != 386 ]]; then
  echo "test-runtime-abi-exports: declaration golden does not contain 386 base symbols" >&2
  exit 1
fi
...
if [[ "$(wc -l < "$work_dir/golden-abi" | tr -d ' ')" != 386 ]] \
  || [[ "$(wc -l < "$work_dir/runtime-abi" | tr -d ' ')" != 386 ]]; then
  echo "test-runtime-abi-exports: normalized base ABI does not contain 386 rows" >&2
  exit 1
fi
diff -u "$work_dir/golden-abi" "$work_dir/runtime-abi"
printf 'runtime-abi-signatures base 386\n'
...
audit_case base "" 386
audit_case alloc alloc-count 393
audit_case par par-map-probe 390
audit_case task task-group-probe 386
audit_case all alloc-count,par-map-probe,task-group-probe 397
