...
define <4 x i32> @blend(<4 x i1> %0, <4 x i32> %1, <4 x i32> %2) #0 {
bb0:
  %_0 = alloca <4 x i1>, align 1
  %_1 = alloca <4 x i32>, align 16
  %_2 = alloca <4 x i32>, align 16
  store <4 x i1> %0, ptr %_0, align 1
  store <4 x i32> %1, ptr %_1, align 16
  store <4 x i32> %2, ptr %_2, align 16
  %load = load <4 x i1>, ptr %_0, align 1
  %load1 = load <4 x i32>, ptr %_1, align 16
  %load2 = load <4 x i32>, ptr %_2, align 16
  %vsel = select <4 x i1> %load, <4 x i32> %load1, <4 x i32> %load2
  ret <4 x i32> %vsel
}
...
bb0:
  ...
  %call = call <4 x i32> @blend(<4 x i1> %load2, <4 x i32> %load3, <4 x i32> %load4)
  store <4 x i32> %call, ptr %_3, align 16
