...
tag-tested Drop, move/null handling, focused analysis coverage, and an alloc-count benchmark. The
next implementation slice is L1b only: Move tagged payloads through Option/Result/user sums. Do not
begin a SQLite/PostgreSQL driver or add database-named compiler variants before L1a–L7 are
complete. The remaining required order is L1b Move tagged payloads through Result, L2 borrow
