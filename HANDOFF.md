...
benchmark-evidence prerequisite implementation on `agent/request7-pinned-git-reader`. The
...
The current slice fixes the exact `git cat-file --batch` response boundary on top of the raw
object identity codec. It does not traverse repositories, inspect config, execute Git, Docker,
`ssh-keygen`, or the benchmark; pinned process/config isolation, revision graph binding, installed
replacement protection, profile, image recipe, and adversarial owners follow. Host qualification
and the full controller/verifier implementation remain later work.
