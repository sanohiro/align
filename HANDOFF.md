...
- **Next language capability:** `pkg.csv` has an accepted design for one explicit typed in-memory
  ...
  for sequential use, but its required `region` is non-Send; the shared parallel-worker provenance
  gate follows callable targets/environments and helper summaries recursively, rejecting both
  `spawn` and `par_map` transfers before publication or allocation. Implementation is next. The design changes no shipped
