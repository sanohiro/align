...
#1130 (plan 71 ledger), and #1131 (plan 71 PR 1). Plan 71 PR 2 is implemented
...
1. **plan 71 implementation** for #1076/#1077/#1078 (sum-type layout, aggregate
   ...
   PR 2 is the current candidate: a serialized per-parameter drop-state effect
   ...
   drop-state simplifier also removes only proved-dead move-out nulling. The
   remaining ordered capabilities are target-selected byval and split cleanup-result transport; then
...
new issues (1054/1055/1057): the total text boundary predicate (1057) is merged in PR 1058,
...
subdirectory; a shared Codex/agy owner closes that case. An independent
inspection-only continuation completed the uncertain and changed scope with no
new findings. The original native run remains INCOMPLETE; its raw CLEAN response
is not a completed host checkpoint.
...
are implemented and active at their joint capability boundary in PR #933. `pkg.csv` is implemented
...
- **Previous package capability:** `pkg.csv` implements one explicit typed in-memory direct-to-SoA
  ...
  seed-0 wyhash projection; Absent mode has no schema-count cap. Clean strings borrow input,
