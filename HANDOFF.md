...
implementation head is `2858d63`, with the new alias/call-result spelling contract repair in
progress. The earlier
...
Align PR #704 is pushed through `2858d63`; preflight and final attestations are still
...
- In progress: the independent review of `2858d63` found one remaining P2: inferred scanner aliases and generic-call results lose producer-owned spelling before a later generic scanner argument, including when captured by a lambda. The closure matrix is reopened again. The design now requires checker-only spelling propagation through inferred locals, transparent generic-call results, and captures; implementation and exact owners are next. The broad host review command was stopped after more than 30 minutes of repeated full-diff/context-compaction inspection without a verdict; it is not review attestation evidence.
