...
| `producer-evidence-propagation` | 5 | 3 | Three-plus events: automation required. Identities must come from producer-owned evidence for the resource actually in use; a loader-owned mapped image, not its replaceable pathname, owns dynamic-library build identity. #895 adds parameterized observation owners for opened-inode rearm, graph-root identity, and one stable checked-metadata observation. Require an exact producer-to-consumer evidence owner for every new resource identity. |
