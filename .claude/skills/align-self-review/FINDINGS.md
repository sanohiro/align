...
| `cross-stage-abi-exactness` | 14 | 6 | Covered by Gate 4 and exact ABI owner matrices; #772 additionally requires generated count evidence and canonical cross-driver context bytes, #909 fixes null/zero-length byte views at every native boundary, and #921 requires wire alignment to agree with the actual `Vec<u8>` owner plus every accessor to read only fields present in its fixed scalar layout. |
