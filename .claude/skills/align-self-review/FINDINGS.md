...
|---|---:|---:|---|
| `validation-phase-completeness` | 19 | 10 | Recurred past the added owner (#727: static-validation-after-native-prepare; unvalidated v3 tail-reserved field; #770: batch layout checked after native advancement; #772: prepared-option, generation-metadata/cardinality, lease/measurement, and static/vector ordering; #773: descriptor-v6 count-thunk preflight; #798: exact response-head syntax before chunked framing selection; #800: recognizable malformed response framing before the next blocking transport read; #866: root grammar must complete before any relative-view validation/allocation). Required next: deduplicate copied header validators into one shared function so an ABI version bump has one owner, extend the parameterized phase-order owner across prepared/transaction paths, and retain a root-first multi-invalid owner for every retained-root filesystem constructor. |
