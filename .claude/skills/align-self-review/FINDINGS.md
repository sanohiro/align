...
|---|---:|---:|---|
| `validation-phase-completeness` | 16 | 7 | Recurred past the added owner (#727: static-validation-after-native-prepare; unvalidated v3 tail-reserved field; #770: batch layout checked after native advancement; #772: prepared-option, generation-metadata/cardinality, lease/measurement, and static/vector ordering; #773: descriptor-v6 count-thunk preflight). Required next: deduplicate copied header validators into one shared function so an ABI version bump has one owner, and extend the parameterized phase-order owner across prepared/transaction paths. |
