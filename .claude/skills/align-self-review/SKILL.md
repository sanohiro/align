...
- **Fallible resource lifecycle**: arm cleanup immediately after acquiring a descriptor, child, or pathname — before the next fallible validation step. Route interruptible reads, status probes, and unlink cleanup through one retry-on-`EINTR` authority, and own failure immediately after acquisition plus an injected interrupt for every operation class. (#895)
...
- **Tests/bench portability**: atomically acquire one private temporary directory, arm RAII cleanup immediately, keep every artifact below it, and remove only that owned directory; a predictable PID/nonce path is not exclusive ownership (#20, #37, #132, #134, #933). A spawned child gets one deadline immediately after spawn; post-spawn pipe/fd setup, bounded `EINTR` retries, execution, kill, and reap all share it, and an unwind-safe process-group/direct kill-and-reap guard exists before the next fallible step (#870, #933). Branch on `cfg!(target_os/arch)`, don't hardcode `.so` / drop `.exe` / x86-only `target-cpu` on ARM (#128, #152, #167); a process exit code is the **low byte** on Unix but full 32-bit on Windows — `cfg!(windows)` the expected value (#287).
