...
- [ ] **`try_from` is not enough for `align`.** An alignment must be a **non-zero power of two** *and* the handle non-null before you dereference/allocate — `usize::try_from(align).ok().filter(|a| a.is_power_of_two())` + `if arena.is_null() { return null }`. (#293)
