...
fn main() {
    ...
    // Six is the common 13-byte response's ceiling. Larger BODY= probes intentionally exercise
    ...
    if default_response {
        for sample in &sa {
            assert!(
                sample.allocs <= 6.0,
                "http.get allocation regression: {:.2}/request exceeds the pinned 6-allocation ceiling",
                sample.allocs
            );
        }
    }
