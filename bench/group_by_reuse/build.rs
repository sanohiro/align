// Link the Align-compiled kernel object (`a1`/`a2`) + the Align runtime into this harness. `run.sh`
// produces the object via `alignc emit-obj` and points these env vars at it and at the runtime's
// build dir. The kernel calls the runtime dict-encode / group / gather / lookup primitives; they come
// from `libalign_runtime.so` (a cdylib — linked dynamically over the C-ABI so its bundled std doesn't
