...
have exactly one free. Separate rows cover `Some→Some→None→Some` field replacement, a conditional
replacement that moves the old field on only one path, and an early `?` after the first owned field
was initialized. `run.sh` also records the raw LLVM tag-guard count.
