// Link the Align-compiled kernel object (`build`) + the Align runtime into this harness. `run.sh`
// produces the object via `alignc emit-obj` and points these env vars at it and at the runtime's
// build dir. The kernel calls `align_rt_builder_*`; it comes from `libalign_runtime.so` (a
