...
#
#   bench/par_map/run.sh [baseline|v3|native|threshold|filter]   (default: native)
set -euo pipefail
cd "$(dirname "$0")"
...
case "$mode" in
  ...
  threshold|filter) align_tgt="native"; rust_tgt="native" ;;
  ...
  baseline)
    ...
    ;;
  *) echo "usage: run.sh [baseline|v3|native|threshold|filter]" >&2; exit 2 ;;
esac
