...
- **Next action: M3** (`docs/impl/07-roadmap.md`) — the memory model: move semantics for owning types + use-after-move errors, explicit `clone()`, pass-by-value of small structs (lint large copies), `arena {}` → `align_runtime` arena alloc + bulk free, and arena-view escape checking. This is where struct values stop being slot-only (whole-struct pass/return/copy) and the explicit-allocator-arena open question is decided.
