...
5. Record the reviewer and checks against the final commit with
   `scripts/pre-pr.sh`. Open the draft only through `scripts/open-pr.sh`; direct
...
4. Rerun `scripts/pre-pr.sh` after any follow-up and refresh the PR attestation
   with `scripts/update-pr-preflight.sh`. Wait for CI and only then merge.
