...
- **Next action: M4** (`docs/impl/07-roadmap.md`) — the array-processing core: `array<T>`/`slice<T>`, `out` args, `map`/`filter`/`where`/`reduce`/`sum` chains, **loop fusion** in MIR, and field projection (`.score`). This is Align's protagonist; needs the minimal generics from `open-questions.md` (before M4).
