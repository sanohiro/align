...
- **M1 in progress** (`docs/impl/07-roadmap.md`). Done: `if`/comparison/`bool`, `mut` + reassignment, multi-arg fns + calls, **builtin `print`** (integers only, decimal + newline, via `align_rt_print_i64`), and **structs** (keyword-less `Name { field: Type }` decl, `Name { field: value }` literal, `base.field` read/write). The runtime builds as a staticlib (`crate-type = ["lib","staticlib"]` → `target/<profile>/libalign_runtime.a`) and the driver links it (+ `-lpthread -ldl -lm`) into every executable; `print` is a sema builtin (not a user fn) and codegen widens its integer arg to i64. **Struct M1 cut:** primitive fields only; a struct lives in its slot (construct via `:=` → field stores; field read = GEP+load; `mut base.field = v`). Passing/returning/copying a whole struct value is rejected for now and waits for the move model (M3). Field access `p.x` is parsed as a 2-segment `Path` and resolved in sema (`Ty::Struct(id)` indexes `Program.structs`). **Next action: M1 remainder** — full primitives (`f32`/`f64`/`char`, float lexing + arithmetic). Each feature threads through all stages; do not build one stage out ahead of the skeleton.
