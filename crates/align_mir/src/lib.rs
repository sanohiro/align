...
pub enum Rvalue {
    ...
    /// `r.max_response_body_bytes(limit)` — mutate the request-local receive cap in place.
    ...
    /// `cl.max_response_body_bytes(limit)` — mutate the client-default receive cap in place.
                ...
                ) && let Some(elem_in) = elem_in
                {
                    ...
                    for stage in stages {
                        ...
                        stage_records.push(ParMapStage {
                            ...
                            capture_tys,
                            elem_in: stage_elem_in,
                            elem_out: stage.out_ty,
                        });
...
fn lower_array_sort(b: &mut Builder, source: &hir::Expr, stages: &[hir::Stage], elem: Ty, sort_key: Option<SortKey>) -> Operand {
    ...
    // per-unit `impl_hash` fingerprints the structural MIR program, so reading it here makes the object-cache
...
/// Decode a client-response status while reserving the HTTP-private `-1` sentinel for
/// `Error.Code(-1)`. Every non-negative status continues through the shared errno/category table.
fn make_http_client_error_from_status(b: &mut Builder, status: ValueId, result_ty: Ty) -> Operand {
    let error_id = match result_ty {
        Ty::Result(_, align_sema::Scalar::Enum(eid)) => eid,
        _ => 0,
    };
    let error_ty = Ty::Enum(error_id);
    ...
    let limit_bb = b.new_block();
    let ordinary_bb = b.new_block();
    let join = b.new_block();
    let slot = b.new_slot(error_ty);
    b.terminate(Term::Branch(
        Operand::Value(is_limit),
        limit_bb,
        ordinary_bb,
    ));

    b.cur = limit_bb;
    let minus_one = b.fresh_value(status_ty());
    b.push(Stmt::Let(
        minus_one,
        Rvalue::Use(Operand::Const(Const::Int(-1, status_ty()))),
    ));
    let limit_error = make_error_code(b, minus_one, result_ty);
    b.push(Stmt::Store(slot, limit_error));
    b.terminate(Term::Goto(join));

    b.cur = ordinary_bb;
    let ordinary_error = make_error_from_status(b, status, result_ty);
    b.push(Stmt::Store(slot, ordinary_error));
    b.terminate(Term::Goto(join));

    b.cur = join;
    let result = b.fresh_value(error_ty);
    b.push(Stmt::Let(result, Rvalue::Load(slot)));
    Operand::Value(result)
}
    ...
    match &e.kind {
        ...
        hir::ExprKind::CommandRun { command } => {
            ...
            lower_http_response_result(
                b,
                Rvalue::CommandRun { command: cm, out },
                out,
                Ty::RunOutput,
                e.ty,
                false,
        ...
        hir::ExprKind::CommandRunBytes { command } => {
            ...
            lower_http_response_result(
                b,
                Rvalue::CommandRunBytes { command: cm, out },
                out,
                Ty::RunBytes,
                e.ty,
                false,
...
fn lower_http(b: &mut Builder, e: &hir::Expr) -> Operand {
    match &e.kind {
        ...
        // `r.max_response_body_bytes(limit)` → set/clear the request-local receive cap; no value.
        hir::ExprKind::HttpRequestMaxResponseBodyBytes { req, limit } => {
            let rq = lower_required!(b, lower_expr(b, req), Operand::Const(Const::Unit));
            let lim = lower_required!(b, lower_expr(b, limit), Operand::Const(Const::Unit));
            let v = b.fresh_value(Ty::Unit);
            b.push(Stmt::Let(
                v,
        ...
        // `cl.max_response_body_bytes(limit)` → set/clear the client-default receive cap; no value.
        ...
        hir::ExprKind::HttpAccept { server } => {
            ...
            lower_http_response_result(
                b,
                Rvalue::HttpAccept { server: s, out },
                out,
                Ty::HttpRequestCtx,
                e.ty,
                false,
...
fn lower_regex_expr(b: &mut Builder, e: &hir::Expr) -> Operand {
    match &e.kind {
        hir::ExprKind::RegexCompile { pattern } => {
            ...
            lower_http_response_result(
                b,
                Rvalue::RegexCompile { pattern: p, out },
                out,
                Ty::Regex,
                e.ty,
                false,
...
fn lower_http_response_result(
    ...
    b.cur = err_bb;
    let errv = b.fresh_value(result_ty);
    let ec = if maps_http_body_limit {
