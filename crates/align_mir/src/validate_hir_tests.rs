...
fn live_process_records_and_writable_backing() -> Result<(), &'static str> {
    ...
    assert!(!is_empty(&lower_program(&base)));
    for name in ["start","read","result","signals","next","close","memory","write","seal","length","positional","executable","image_length","image_read","image_command","current","namespace","inherit","inherit_ns","scope_start","scope_id","scope_owner","scope_status","scope_try","scope_wait","scope_stdout","scope_stderr","scope_poll","scope_kill","scope_group","scope_children","scope_reap","scope_release","member_kill","member_finished"] {
        for mutation in 0..5 {
            let mut bad=base.clone();
    ...
    for name in ["process.wait_result","process.readiness","process.snapshot","process.signal_set","process.member_info","process.reaped"] {
