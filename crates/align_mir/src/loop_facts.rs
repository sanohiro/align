...
fn rvalue_facts(rv: &Rvalue) -> RvalueFacts<'_> {
    ...
    match rv {
        ...
        Rvalue::VecSum { vec, elem: _, n: _ } => Pure(vec![vec]),
        Rvalue::VecDot { a, b, elem: _, n: _ } => Pure(vec![a, b]),
        Rvalue::VecMinMax { vec, elem: _, n: _, max: _ } => Pure(vec![vec]),
        Rvalue::VecSumWhere { vec, mask, elem: _, n: _ } => Pure(vec![vec, mask]),
...
fn rvalue_operands_mut(rv: &mut Rvalue) -> Option<Vec<&mut Operand>> {
    Some(match rv {
        ...
        Rvalue::VecSum { vec, elem: _, n: _ } => vec![vec],
        Rvalue::VecDot { a, b, elem: _, n: _ } => vec![a, b],
        Rvalue::VecMinMax { vec, elem: _, n: _, max: _ } => vec![vec],
        Rvalue::VecSumWhere { vec, mask, elem: _, n: _ } => vec![vec, mask],
