    ...
    for s in &b.stmts {
        match s {
            ...
            Stmt::DropField(slot, path) => {
                let _ = writeln!(out, "    drop_field _{slot}{}", path_str(path));
            }
...
fn rvalue_str(rv: &Rvalue) -> String {
    match rv {
        ...
        Rvalue::JsonEncode {
            ...
            "json.encode({} pieces, max={}, -> _{out})",
            pieces.len(),
            max_bytes.as_ref().map(operand_str).unwrap_or_else(|| "unbounded".to_string())
        ),
        ...
        Rvalue::ReaderBuffered(r) => format!("io_buffered({})", operand_str(r)),
        Rvalue::ReaderReadLine(r, buf) => format!("io_read_line({}, {})", operand_str(r), operand_str(buf)),
        Rvalue::BytesAsStr { bytes, out } => format!("bytes_as_str({}, -> _{out})", operand_str(bytes)),
        ...
        Rvalue::RegexCaptures { regex, text, out } => {
            format!("regex_captures({}, {}, -> _{out})", operand_str(regex), operand_str(text))
        }
        Rvalue::RegexGroupCount { regex } => format!("regex_group_count({})", operand_str(regex)),
        Rvalue::RegexGroupIndex { regex, name } => {
            format!("regex_group_index({}, {})", operand_str(regex), operand_str(name))
        }
        Rvalue::CapturesGroup { caps, index, out } => {
            format!("captures_group({}, {}, -> _{out})", operand_str(caps), operand_str(index))
        }
        ...
        Rvalue::HttpHeadersCount { headers, name } => format!("http_headers_count({}, {})", operand_str(headers), operand_str(name)),
        Rvalue::HttpHeadersTokensValid { headers, name } => format!("http_headers_tokens_valid({}, {})", operand_str(headers), operand_str(name)),
        Rvalue::HttpHeadersContainsToken { headers, name, token, exact } => format!(
            "http_headers_{}({}, {}, {})",
            if *exact { "contains_token_exact" } else { "contains_token" },
            operand_str(headers),
            operand_str(name),
            operand_str(token),
        ),
        Rvalue::HttpCtxUpgradeReady { ctx } => format!("http_ctx_upgrade_ready({})", operand_str(ctx)),
        ...
        Rvalue::HttpRespondUpgrade { ctx, rb, out } => format!("http_respond_upgrade({}, {}, -> _{out})", operand_str(ctx), operand_str(rb)),
