...
pub use static_inputs::{
    ...
    STATIC_INPUT_MANIFEST_FORMAT_VERSION, STATIC_INPUT_MANIFEST_MAGIC, StaticConsumerKind,
    StaticDescriptorInputError, StaticDescriptorInputErrorCause, StaticInput, StaticInputError,
    StaticInputManifest, compose_codegen_impl_hash, metadata_logical_path, metadata_path,
    resolve_inline_static_input, resolve_static_descriptors, resolve_static_file,
    snapshot_checked_metadata,
};
...
fn install_static_descriptor_data(
    ...
    for descriptor in descriptors {
        ...
        let first_binder = binder_fields
            ...
            .ok_or_else(|| {
                format!(
                    "static descriptor `{}` has no driver",
                    descriptor.descriptor_id
                )
            })?
            .binder
        ...
        let binder_supported = !first_binder
            .iter()
            .any(|field| field.shape.kind != GeneratedValueKind::I64 || field.shape.nullable);
        let row_supported = match &artifact.runtime {
            GeneratedStaticRuntime::Query(runtime) => {
                !runtime.decoder.fields.iter().any(|field| {
            ...
            match option {
                ...
                StaticDescriptorOption::PostgreSQLParameterType {
                    ...
                    let fields = descriptor
                        ...
                        .ok_or_else(|| {
                            "generated parameter contract is not a struct".to_string()
                        })?;
...
mod tests {
    ...
    fn libpq_closure_is_ordered_after_unrelated_crypto_requests() {
        ...
        assert_eq!(
            ...
            [
                "crypto", "z", "zstd", "sqlite3", "pq", "ssl", "pq", "ssl", "crypto", "zstd", "z",
            ]
        );
    }
