...
pub fn run_size(path: &str, target: BuildTarget, profile: Profile, rt_lto: bool, jobs: usize, cache_stats: bool) -> ExitCode {
    ...
    if let Err(code) = crate::build_per_unit_to(path, &exe, target, profile, rt_lto, jobs, cache_stats) {
        return code;
    }
