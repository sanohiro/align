...
fn gate7_cross_process_second_build_all_hit() {
    ...
    assert!(hot_err.contains("prelink: 7 hit, 0 miss"), "second build: all function partitions prelink-hit:\n{hot_err}");
    assert!(hot_err.contains("backend: 7 hit, 0 miss"), "second build: all function partitions backend-hit:\n{hot_err}");
