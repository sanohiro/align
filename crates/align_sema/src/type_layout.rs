...
/// Natural ABI `(size, align)` for a resolved type.
///
/// Aggregate traversal is iterative, memoized by nominal/tagged id, cycle-safe, and bounds-safe.
/// Definition validation owns diagnostics for malformed graphs; this consumer only has to terminate
/// deterministically before sema lints and LLVM field ordering inspect the result.
    ...
    while let Some(item) = work.pop() {
        match item {
            Work::Enter(ty) => match ty {
                ...
                Ty::Tagged(id) => {
                    if let Some(&layout) = tagged_cache.get(&id) {
                        built.push(layout);
                    } else if !active_tagged.insert(id) {
                        built.push((0, 1));
                    } else if tagged_types.get(id as usize).is_none() {
                        active_tagged.remove(&id);
                        built.push((0, 1));
                    } else {
                        work.push(Work::ExitTagged(id));
                        work.push(Work::Enter(expand_tagged_ty(Ty::Tagged(id), tagged_types)));
                    }
                }
                Ty::Struct(id) => {
                    if let Some(&layout) = struct_cache.get(&id) {
                        built.push(layout);
                    } else if !active_structs.insert(id) {
                        built.push((0, 1));
                    } else if let Some(definition) = structs.get(id as usize) {
                ...
                Ty::Enum(id) => {
                    if let Some(&layout) = enum_cache.get(&id) {
                        built.push(layout);
                    } else if !active_enums.insert(id) {
                        built.push((4, 4));
                    } else if let Some(definition) = enums.get(id as usize) {
