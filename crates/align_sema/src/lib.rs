...
impl<'a, 't> Checker<'a, 't> {
    ...
    fn check_fn(&mut self, f: &ast::FnDecl) -> Fn {
        ...
        if f.name.name == "main" {
            // `main`'s error type — the `E` in `-> Result<(), E>` — is restricted to the builtin
            // `Error`. The C-`main` wrapper's exit-code lowering reads the `Err` payload as the
            // builtin `Error` enum's specific `{ i32 tag, i32 code }` shape (`Code(c)` → `clamp(c)`,
            // a category → `tag + 1`); a user-defined error enum at that position has a different
            // layout and no defined exit-code mapping, so it would miscompile. Reject it here with a
            // clear diagnostic. (Relaxation waits on the full `Error` design — see `open-questions.md`
            // "Error type design".)
            if let Ty::Result(_, e) = ret
                && !matches!(e, Scalar::Enum(eid) if eid == self.error_enum_id)
            {
                self.diags.error(
                    ...
                    f.ret.as_ref().map(|t| t.span()).unwrap_or(f.span),
                );
            }
            // `main` takes no arguments, or exactly `args: array<str>` (argv, draft.md §19) with a
            // `Result<(), _>` return — the latter is the only form the C-`main` wrapper marshals
            // argv into (an `-> i32` argv `main` is a later follow-up). The `E` is validated
            // separately above, so this checks only the parameter and `Ok`-payload shape.
            if !f.params.is_empty() {
                let args_ok = param_tys.as_slice() == [Ty::DynArray(Scalar::Str)]
                    && matches!(ret, Ty::Result(Scalar::Unit, _));
                if !args_ok {
                    self.diags.error(
