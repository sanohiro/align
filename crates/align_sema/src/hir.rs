...
/// One checked value pattern in an integer, character, or exact-string `match` arm.
#[derive(Clone, Debug, PartialEq, Eq, serde::Serialize)]
...
/// = a simple arm, many = an or-pattern (`A | B`). `values` = checked integer/character literals,
/// ranges, or exact decoded string literals. `bindings` are the locals bound to the variant's payload (one per payload slot, in order);
...
pub enum ExprKind {
    ...
    /// parallel range kernel through its immutable call-scoped context; a primitive-scalar
    /// length-preserving `map` chain uses the same kernel, while filters and unsupported aggregate
    /// sources retain the sequential collect loop. Move captures are rejected by the ownership
    /// checks. Every callable that enters the parallel chain is required to be Pure (checked in the
    /// parallelism pass over the full call graph).
    ...
    /// [`crate::Ty::Command`] local; `name`/`value` are borrowed `str`s. A `name`/`value` with an interior
    /// NUL (or a `name` containing `=`) aborts at runtime. Impure.
    CommandEnv { command: Box<Expr>, name: Box<Expr>, value: Box<Expr> },
    /// `c.env_clear()` (`std.process` Slice 6) — start the child's environment empty (the child
    /// `clearenv`s before applying any [`CommandEnv`] pairs), mutating the handle (not consumed — like
    /// [`CommandCwd`]). The `ty` is [`crate::Ty::Unit`]. `command` is a bound [`crate::Ty::Command`]
    /// local; no arguments. Impure.
    CommandEnvClear { command: Box<Expr> },
    ...
    /// `array<u8>` of fixed length (20 for SHA-1, 32 for SHA-256, 64 for SHA-512). The `ty` is
    ...
    /// runtime — crypto.md "Fixed-size digests as `array<u8>`"). `algo` selects SHA-1, SHA-256 or SHA-512.
...
#[derive(Clone, Copy, Debug, PartialEq, Eq, serde::Serialize)]
pub enum HashAlgo {
    /// SHA-1 interoperability digest: 20 bytes via EVP.
    Sha1,
...
pub enum EncodingKind {
    /// Maximal-subpart UTF-8 replacement to independently owned text; encode-only.
    Utf8Lossy,
