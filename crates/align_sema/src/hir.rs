...
pub enum ExprKind {
    ...
    /// parallel range kernel through its immutable call-scoped context; staged, Move-capturing, and
    /// unsupported aggregate sources retain the sequential collect loop. `func` is required to be
    /// Pure (checked in the parallelism pass over
    /// the full call graph).
