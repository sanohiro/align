...
Align は AOT コンパイル方式のデータ指向プログラミング言語です。コードを書く**人間**、コードを生成する **AI**、コードを最適化する**コンパイラ**、コードを実行する**ハードウェア** —— この四者の足並みを、一度にそろえます。隠し事を一切許さない厳格なポリシーと、エラーや所有権の統一されたモデル、そしてデータ指向の配列・スライスを中心とした設計により、予測可能なパフォーマンスとキャッシュに優しい融合ループを通常のコードから実現します。
