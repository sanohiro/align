...
**A1.** `a.zip(b).map(fn (x, y) { x + y })`。パイプラインがうまく処理してくれます。
...
v3 := v1 + v2
```
これで `v3` は `vec4<i32>(11, 22, 33, 44)` になります。1つの命令で、4つの結果が出ます。
...
**A6.** 真偽値（boolean）のベクトルです。`m := v2 != vec4<i32>(0)` と書くと、`mask4(true, true, true, true)` が得られます。
...
ans := v1 / safe_v2
```
マスクが true の場所は `v2` が選ばれ、false の場所は `1` が選ばれます。これで割り算がパニックを起こすことはなく、CPUが分岐処理で立ち止まることもありません。
