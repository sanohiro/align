...
`align-repl` を本の横で開いておくと、このドリルのリズムに合います。空の状態から始まり、答えは 1 行で、問いごとに新しいファイルを作る代わりにプログラムが自分と一緒に育っていきます。答えを予想し、入力し、見比べてください。インタプリタではなくネイティブにコンパイルして実行するので、予想が外れたときは本物のコンパイラが正してくれます。`:list` はその章でここまでに組み上がったプログラムを表示し、`:save` はドリルが残しておきたい何かに育ったときそれをファイルにします。ガイドの [16 章](../../guide/ja/16-toolchain.md)を参照してください。
