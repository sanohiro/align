...
the bounded claim loop and ordered all-or-Err cleanup. The x86-64 Base64 encoder now runtime-dispatches
to AVX2 at the measured 32-byte crossover; its scalar oracle and exact destination are retained. A
baseline-NEON backend cross-compiles, but native aarch64 correctness/performance measurement and
production activation remain deliberately deferred rather than guessing a threshold. Remaining work
is that ARM Base64 gate plus a new hex SIMD probe and scalar vs SIMD stable compaction. Existing work
from documents 10/11 remains attributed there. The separate native aarch64 UTF-8 portability run also
remains deferred.
