...
`Debug < Info < Warn < Error < Off` and one nominal Move `log.logger` constructed by consuming an
explicit writer plus a minimum level. There is no process-global/default logger, ambient
...
one-line delimiting rather than terminal sanitization. Formatting remains ordinary
template/builder construction; no variadic or reflection-based log formatter is added.

`line` is best effort and returns Unit. It retains the first writer status, stops that record, and
suppresses later lines. Explicit `flush()` is the sole source-visible checkpoint: it returns the
first retained or new flush failure through the fixed std `Error` mapping; Drop keeps the ordinary
unobservable best-effort writer cleanup. There is no atomicity, delivery, durability, cross-logger
ordering, clock/source metadata, structured-field/JSON, multiline, rotation, async queue, or fatal
promise.

Record: `docs/impl/std-design/log.md`, `draft.md` §18.2, `docs/language-spec.md`
...
mode exists. Before cache, capability, or artifact work, test mode rejects every
catalog-reachable `ProcessCommand` across direct/imported/function-value/lifted/generic edges; an
unreachable production helper remains valid and no dynamic command supervisor ships. `align-repl`
rejects an entire test-bearing submitted entry before replacement resolution or session mutation.
The harness is the sole literal `main`; all permitted source-main ABIs use the existing
...
- **`match` (expression, mandatory-exhaustive).**
  ...
  ```
  An expression — every arm unifies to the `match`'s type (or all diverge). Patterns are **alternatives over one scrutinee**: unqualified sum-variant names (`Variant` / `Variant(b0, b1)`, binding payloads positionally), or literal values in the settled integer, `char`, and exact-string domains. **Exhaustiveness is mandatory from day one**: every finite sum/integer alternative is covered, or a `_` wildcard arm; `char` and string always require `_`. `match` expresses alternatives whose discriminator the compiler owns; arbitrary boolean conditions stay with `if`. This is the one-way boundary: there are no guards or nested predicate patterns. `A | B` or-patterns landed in S4; the value domains are fixed by the slice ledger below.
...
- **Representation.** `Ty::Enum(id)` is interned into `Program.enums` (mirroring `Ty::Struct`/`Program.structs`). The settled target is one explicit tag plus max-variant union storage for user sums, `Option`, and `Result`. User tags remain `i32`; builtin tags remain `i8`; variant ordinals do not change. Construction stores only the tag and active payload, while `match` and Drop select the tag before reading that payload. Inactive storage and padding are unspecified and unobservable. There is no niche encoding. [Plan 71](impl/71-aggregate-layout-transport-drop-state-plan.md) owns the exact target-layout formula, cross-unit identity, transition and acceptance matrix. The older flattened aggregate remains the implementation until plan 71 PR 1 lands.
...
Implementation slices: **S1 DONE** — tag-only + scalar-payload enums + `Type.Variant`(`(args)`) + exhaustive `match` with positional payload bindings (no guards/nesting); the first implementation uses a non-union tagged struct `{ i32 tag, <flattened payloads> }`, with its union-storage replacement scheduled as plan 71 PR 1. **S2/L1b-a/L1b-b DONE** — structs, sums, owned strings, and supported owned arrays may be payloads; recursively Move payloads are tag-switched and multiple owned payloads require one allocation mode. Tuple payloads still need a `Scalar::Tuple`. **S3 DONE** `match` on `Option`/`Result` (via a `match_variants` helper + a two-variant `IsSome`/`IsOk` branch reusing the existing unwrap rvalues); **S4 (or-patterns) DONE** — `A | B | ...` shares one arm (a new `|` token + `MatchPattern::Or`; `hir::MatchArm.variants: Vec<u32>`; MIR tests each tag in sequence into the arm block). An or-pattern lists **bare** variant names and binds nothing (a payload variant may appear, its payload unbound — binding in an or-pattern is rejected); it counts toward exhaustiveness like any arm. **Integer and character value patterns, inclusive ranges (`min..=max`), and literal or-patterns landed in #1055**: exhaustive over integer type domains or requiring `_`. **Exact string-literal patterns are settled by plan 72 / #1085**: `str` and `string` borrow one once-evaluated scrutinee, exact decoded literals and `|` are admitted, `_` is mandatory, duplicates reject, and strings have no ranges. Four or more literals lower through one MIR-owned exact dispatch and a deterministic length/byte tree with at most one full comparison per path; no hash contract or runtime export is added. **Guards and recursive (boxed) enums were reviewed and NOT adopted now:** *guards* (`P if cond`) cross the settled "`match` = variants, `if` = conditions" One-Way boundary (and are expressible via an `if` in the arm body) — declined on philosophy, not difficulty; *recursive enums* (`List { Cons(i32, box<List>), Nil }`) run against the data-oriented core (pointer-chasing over arrays) and need the `box<Enum>` rejection lifted + self-referential layout + boxed-recursion drop/region — deferred as its own larger track if a concrete need (e.g. an AST) arises.
...
- `wait()` — joins all spawned tasks; later `wait()?` is the single error boundary (tasks returning `Result`, lowest-index `Err` propagation).
...
Pure; AoS projection, field filters, and the recognised invariant `str.contains` filter use the
stable range path, while richer string-search and unsupported aggregate forms remain sequential.
Direct wrapping-integer
...
The shipped slices add stable callable primitive-scalar `where` compaction and the recognised
invariant `str.contains` filter via count/prefix/scatter.
...
than a production ABI change. SoA/richer string filters, aggregate extensions, and a broader
width/aggregate sweep
remain.
...
- **`import` is REQUIRED + verified for prefix-accessed builtin namespaces.** The initial slice
  covered exactly `json` (`core.json`), `fs` (`std.fs`), and `io` (`std.io`); every later qualified
  builtin module, including designed `std.log`, follows the same per-file rule. Using a qualified
  builtin without its matching import is a compile error; an import naming an unknown module is a
  compile error; an unused import is a lint. This makes a file's capability surface visible in its
  header — "Nothing hidden." The **language-syntactic core** (`Option`/`Result`/`?`/`else`, `arena`,
  the array pipeline `.map`/`.where`/`.reduce`/`.sum`/…, `x.abs()` math methods, `template "…"`)
  needs **no import** — requiring one would be requiring an import for syntax.
...
field. L1b-a admits direct existing-`Scalar` Move payloads per tagged arm and completes their
`match`/`else`/`?`/join behavior; L1b-b admits multiple Move payloads with protected partial
construction and one uniform allocation mode; L1b-c adds tagged-in-tagged representation and the
exact `Result<Option<Output>, MoveError>` acceptance. All three representation slices are
implemented; L2 still owns dynamic path-selected return cleanup bits.
...
File SQL uses a tagged root-relative logical-path identity. Inline SQL uses
