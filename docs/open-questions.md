...
Implementation slices: **S1 DONE** — tag-only + scalar-payload enums + `Type.Variant`(`(args)`) + exhaustive `match` with positional payload bindings (no guards/nesting); the enum lowers to a non-union tagged struct `{ i32 tag, <flattened payloads> }`. **S2** struct/tuple payloads; **S3 DONE** `match` on `Option`/`Result` (via a `match_variants` helper + a two-variant `IsSome`/`IsOk` branch reusing the existing unwrap rvalues); **S4** guards / `|` / recursive (boxed) enums (additive). (Deferred codegen optimization: a space-optimal union layout instead of flattened per-variant fields — no surface change.)
