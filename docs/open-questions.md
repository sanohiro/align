...
- **Zero-copy `json.decode`**: `str` / `array<scalar>` / `array<Struct>` decode region-tied to the input → **`draft.md` §19 runs end-to-end except the `fs`/`io` std boundary**.
