...
SSO is **not** adopted (its own Settled entry above). Scalar element indexing `recv[index]` (array/slice/owned array → scalar, bounds-checked) is implemented. Still open / separate tracks (not part of this decision): tuples / multi-value returns (for `partition`), `array<slice<T>>` (for `chunks`), `array<Struct>.clone()`, **struct-element** indexing (`users[i]` / `users[i].name`), and `out` params + `noalias` (below).
