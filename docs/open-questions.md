...
- **par_map thread-spawn overhead — FIXED 2026-06-27 (persistent pool).** `align_rt_par_map` spawned
  ...
  **Whole-range specialization SHIPPED 2026-07-25:** codegen now emits one typed range kernel with
  ...
  heavy bodies, while the heavier body approached parity around 82–98K. The checked-in probe reports
  balanced median ratios and must be rerun on a target host before another retune. The shared pool
