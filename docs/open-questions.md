...
**Implementation plan (2026-06-23), after closures ①–③ shipped.** Key simplification: **`task_group` does not need the region-owned env.** Because the scope is *structured* — `wait()` joins every spawned task before the frame returns — a spawned closure's captured environment is alive for the whole task lifetime even as the ②b-2 **frame-local stack env** (a thread reads it concurrently; the frame outlives the join). So `spawn` reuses the existing closure value as-is. The region-owned env (freed at scope end) stays the memory-tight ideal and is the mechanism for closures that escape the *frame* (a returned closure) — not required for `task_group`. Surface (all scalar `R` for now, matching the closure slices):
- `task_group { … }` — a block scope like `arena {}`; opens a task context; `wait()` (or scope end) joins.
- `spawn(fn { … })` — a builtin valid inside the scope (like an arena allocation refers to its arena). Takes a `fn() -> R` value (captures by value; may be impure); returns `Task<R>`. `Ty::Task(Scalar)`.
- `wait()` — joins all spawned tasks; later `wait()?` is the single error boundary (tasks returning `Result`, first-`Err` propagation).
- `t.get()` — reads the task's result `R` after the join.
Decomposition: **④a** scope + `spawn` + `Task<R>` + `wait` + `get`, tasks run **deferred-sequential** (run at `wait` in spawn order — matches the eventual "complete by `wait`" semantics, unlike eager-at-`spawn`); **④b** real threads (reuse the `par_map` thread runtime: a thread per task, join at `wait`); **④c** the `wait()?` error boundary.
