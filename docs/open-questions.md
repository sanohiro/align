...
- **Representation.** `Ty::Enum(id)` is interned into `Program.enums` (mirroring `Ty::Struct`/`Program.structs`); LLVM uses the settled non-union aggregate `{ i32 tag, every variant payload flattened in declaration order }`. Construction stores the tag and the live variant's fields; `match` branches on the tag and extracts only that payload. `Option`/`Result` retain their builtin tagged aggregate layouts.
