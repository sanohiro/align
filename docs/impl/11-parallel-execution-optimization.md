...
- `ArenaHandle` is non-Send at every reachable callable-environment depth and rejects before MIR
  even when the callable is Pure; backend validation additionally rejects a direct handcrafted-MIR
  occurrence before context/kernel/runtime publication;
...
1. checked HIR proves each captured Copy value transitively worker-Send through callable-target and
   environment provenance; codegen builds the typed call-scoped record and rejects a direct
   `ArenaHandle` before layout as a defensive malformed-MIR check;
