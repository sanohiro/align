...
expanding as a tree. Cycle detection uses ID-indexed active bits, so a deep nominal chain is linear
as well. Collection element eligibility is separate and does not follow merely from having a Drop
plan.
