...
`par_map` requires a Pure callable (`03 §8`). The implemented `pkg.csv` worker-sendability gate
independently rejects every capture whose completed callable-
target/environment provenance reaches `ArenaHandle` before any MIR node is formed. The rule follows
nested closures, moves, joins, and direct/imported/indirect helper transfer summaries; unavailable
provenance fails closed and a noncapturing function's empty environment passes. It
...
slot initializes a local `HttpUpgrade` only on zero status. Read validates arguments and selects
live/spent/poisoned state before clearing a live destination buffer generation, then publishes no
partial bytes on error. Spent read/write/deadline return Invalid without mutation/clock/I/O;
