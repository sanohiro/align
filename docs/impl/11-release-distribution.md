...
The corpus is the byte-exact first-party `pkg.db`, `pkg.web`, `pkg.frame`,
`pkg.auth`, `pkg.kv`, `pkg.csv`, `pkg.ws`, and `pkg.template` source at the
release tag. Those source trees remain separately vendored by
