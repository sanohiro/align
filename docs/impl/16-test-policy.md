...
consumed by the DB suites also qualify. Dedicated `db_*` and static
artifact/input/runtime/query-metadata production modules qualify by path. A
changed or deleted mixed compiler source or owner-test file qualifies when a
zero-context changed hunk or its function header names the `pkg.db`,
PostgreSQL/libpq, or SQLite boundary. Deleted direct DB paths qualify by path;
unrelated deletions do not. Uncomputable diffs fail closed. Other documentation,
tooling, applications, leaf tests, and mixed-source changes do not spend the
roughly fifteen-minute service gate. `scripts/db-ci-scope.sh` owns that classification and
`scripts/test-db-ci-scope.sh` pins its positive, negative, and fail-closed
cases.

Pull requests run the classifier from the current trusted base tip, never from
the proposed checkout; changing the classifier itself therefore requires the
service job under the preceding trusted rules. Its introduction has one
fail-closed bootstrap: while the base has no classifier, the service always
runs. Direct pushes likewise use the preceding main revision's classifier.
...
job-level skipping therefore cannot conceal a classifier failure. A main push
skips the service only when GitHub's commit-to-pull-request API identifies the
exact head as a merged PR into that branch. That PR already passed the same
required check against GitHub's test merge. An unassociated direct push still
classifies its exact diff, and manual dispatch always provisions both database
services.
