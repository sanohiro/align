...
path-local cleanup bit. `Option<MoveStruct>` and every other owned `Option` field remain explicit
L1b diagnostics. The focused ownership and analysis suites plus the alloc-count probe pin balanced
Some frees, zero None allocations, and the raw LLVM tag guard.
