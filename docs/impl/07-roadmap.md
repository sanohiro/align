...
- **Slice 5 — RE-SCOPED 2026-07-11 by a two-lens design review** (mechanics/soundness lens +
  ...
  **RETRACTED IN PART 2026-09-20 — narrow scalar ABI extension and range facts.**
...
## Execution, storage, and startup optimization track (S0A/S0B implemented)
...
S0B  current-plan explanation               implemented against the accepted exact design
...
S0A is implemented in PR #955; S0B's exact reporting extension is implemented
against its accepted design. Their benchmark and reporting failure domains
...
existing client. Explicit-expiry presigning is implemented under the accepted
contract and closure matrix in `pkg-design/s3-presign.md`. It returns owned method/URL/required headers without a body or
