...
declaration, and a repeated concrete node ends the cycle. Codegen retains one type-graph validator
and its completed-node sets across every retained definition and signature root in the program;
per-root active paths still distinguish a real inline cycle from a previously validated DAG.
