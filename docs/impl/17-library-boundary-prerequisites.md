...
| Input and schema | `value` is borrowed and must satisfy exactly the currently shipped `json.encode` input predicate, including its local-binding restriction, nested records, Options, arrays, unions, field order, and closed exclusions. This capability does not admit a shape rejected by `json.encode`. `max_bytes` is exactly `i64`; no integer coercion or default exists. |
...
| Allocation and ownership | The bounded builder starts empty and grows in the existing C allocator family, never reserving or retaining capacity above `max_bytes`. Success transfers its allocation directly into the `string` Ok payload. Negative-limit and over-limit paths free the complete builder buffer and leave the output slot zeroed before constructing Err. The borrowed source is never consumed or mutated. Result `match`, `?`, `else`, return, reassignment, and unused-value cleanup use the existing recursive tagged-payload Drop. |
...
| HIR/MIR contract | Add a dedicated `JsonEncodeBounded { parts, max_bytes }` HIR expression with stored result `Result<string,builtin Error>` and a matching MIR operation/control-flow lowering. Its `parts` obey the existing `Template` and `TemplatePart` ledger verbatim; the new envelope adds exact limit type, result type, borrow, and failure cleanup. Ordinary `Template` remains `str` and unchanged. No unchecked HIR can select an arbitrary formatter or claim an owned result without the fallible finish. |
...
| Request 13 composition | This request accepts only shapes `json.encode` accepts at this implementation head; it does not consume Request 13's hypothetical recursive owned graph. The encode-part constructor is deliberately shared, so the later reviewed Request 13 widening must parameterize both `encode` and `encode_bounded` from the same admitted graph and add its exact C6 byte/cap owners in that capability. There is no bounded-only schema and no need to revise this limit/result ABI. |
...
| Byte parity | For every currently admitted encode shape — scalars within records, nested records, required/optional fields, all-`None`, strings requiring every escape class and multibyte UTF-8, scalar/record arrays, unions, and empty values — exact-fit bounded output equals ordinary encode byte-for-byte. The not-yet-admitted Request 13 C6 graph is not fabricated here. | parameterized `m5` golden/parity owner over the current encode matrix |
| Limit boundaries | Cover negative, zero, `exact - 1`, exact, `exact + 1`, a breach in static text, escape expansion, numeric formatting, nested object, array separator, and closing delimiter. Prove no allocation grows above the cap and no partial Ok is constructible. | runtime builder failpoint/allocation instrumentation plus end-to-end result owners |
...
| HIR validation | Add the exact `JsonEncodeBounded` ledger row: existing valid encode parts, `max_bytes` exactly `i64`, result exactly `Result<string,builtin Error>`, borrowed accesses, deterministic child order. Mutate part kind/access/schema ids, limit type, error enum, Ok type, result type, and malformed reachable definitions; reject before MIR/runtime allocation. | whole/per-unit producer twins and parameterized `validate_hir_tests` mutations |
...
| Future graph composition | Keep schema selection and part construction single-sourced between ordinary and bounded encode. A source-inventory/negative owner proves no bounded-only field predicate exists; Request 13 later owns the exact recursive C6 graph parity and limit vectors when that graph becomes real. | shared-constructor call graph assertion plus current `string`/`array<string>` rejection twins for both surfaces |
