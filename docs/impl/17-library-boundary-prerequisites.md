...
closure, `map`, or `reduce` may capture a `region` under the ordinary lexical-region proof, while
every parallel worker uses one shared fail-closed `ArenaHandle` capture gate before worker
publication, MIR, generated identity, call, or allocation.
...
| parallel eligibility | Before any MIR lowering or generated identity, require every `ArrayParMap` and widened parallel-stage callable to resolve to the replayed complete `Pure` fact. Independently, reject `Ty::ArenaHandle` in every staged or terminal parallel capture through the same non-Send authority used by `spawn`; unknown, Impure, absent, incomplete, or region-capturing boundaries fail closed. Externs remain Impure. Codegen defensively rejects a handcrafted MIR recurrence before capture-context layout or any kernel/runtime publication. | Positive/negative `ArrayParMap` and widened-stage effect twins cover direct, indirect, closure, aggregate projection, imported, and open-world callback targets; direct/helper-wrapped region captures plus raw malformed `ArrayParMap` HIR and `ParMapParallel`/`ParMapReduce` MIR captures cover whole/per-unit and generic forms; the existing spawn negative proves the shared authority; sequential closure/map/reduce region positives remain; every failing path asserts all four lowerers return the same empty program before generated identity, context layout, kernel/runtime call, or allocation. |
...
- the `region` capability itself is rejected at both `spawn` and `par_map` worker boundaries,
  independently of purity and before worker publication or allocation;
...
| parallel-worker capture | Treat effect and sendability as distinct facts. One fail-closed source/HIR authority rejects an exact `ArenaHandle` captured by `spawn`, a staged `ArrayParMap` callable, or its terminal callable before lifted-worker publication, MIR, generated identity, runtime call, or allocation. Codegen defensively rejects the same type in handcrafted parallel MIR before context layout or kernel/runtime publication. Sequential direct calls, local closures, `map`, and `reduce` continue to use the ordinary lexical region proof. | Existing spawned-region diagnostic plus direct and helper-wrapped par-map captures; staged and terminal capture positions; raw malformed HIR plus `ParMapParallel`/`ParMapReduce` MIR `ArenaHandle` captures; whole-program/per-unit/generic parity; sequential Pure closure/map/reduce positives; no-publication assertions through all four MIR entries and backend error owners. |
