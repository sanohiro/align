...
| L2b-a2-ta | Extend the same read-only tagged projection through ordinary dynamic scalar/AoS-record arrays and admit an indexed Move element only at an explicit shared-`borrow` call | No new syntax, borrow mode, mutable element, partial move, interface field, or runtime ABI. The exact grammar, stable indexed base, lifetime, validation order, and exclusions are owned by `impl/28-borrowed-dynamic-aggregate-projection-plan.md`; the two producer-to-consumer halves land atomically because neither is a useful stable consumer alone | direct/nested/imported/generic array-bearing sum matrix, indexed shared-borrow evaluation/bounds/lifetime matrix, malformed HIR/MIR and LLVM no-copy owners, cache parity, and exact align-llm result/evidence adoption |
