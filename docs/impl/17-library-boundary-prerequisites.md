...
| callable and sum relations | Resolve `FnValue`/`Closure`/`CallFnValue`/`Call` against one exact stored/imported/extern signature and in-range `FnTy`; reject extern `FnValue`, invalid generic suffix shape, disabled modes, and mismatched arguments. A monomorph call strips only the exact encoded `$...` suffix, preserving any module `$` in the base name. Validate `EnumValue`, `Match`, `Option`, `Result`, `Else`, `Try`, and `ResultMapErr` against exact enum/tagged tables, payload/arm coverage, and fallthrough result types. Am-b1 does not replay effect or callable-origin facts owned by am-b3/b4. | Callable and tagged table rows in `hir_body_validator_expression_inventory`, `hir_body_validator_accepts_module_monomorph_call_name`, including generic/extern/origin mutations, `MatchArm` wildcard/or-pattern/duplicate/coverage/arm-result mutations, and explicit `Block` tail/result mutations; the valid closure is accepted without changing `FnTy.effect` cells. |
| structured control | Validate `TaskGroup`, `If`, `Block`, `Loop`, `Arena`, and `Unsafe` child structure, divergence/result compatibility, loop-local range shape, and lexical unsafe depth. `TaskGet` validates its exact `Task(T)` primitive-Copy type and child/result relation; `Wait` validates its task-group context and exact Unit/`Result<Unit,Error>` result shape. The sema `TaskProof`/`WaitProof` group, generation, completion, and successful-wait dominance state is not stored HIR and is deferred to am-w/am-b4. | Control-family table rows, nested unsafe-depth restoration, divergent branch/tail, loop break/result, retained-dead-child precedence twins, and explicit `TaskGet`/`Wait` type/context mutations; no task proof or effect cell is consulted. |
| type graph and stack | Reuse the am-d common explicit type traversal through every b1 type relation: function parameters/returns, locals, statement targets, struct field paths, tuple elements, enum/tagged payloads and matches, callable signatures, projections, and control-result joins. Accept a deep finite am-g-t-valid acyclic inline-struct graph rooted through a function parameter/return and reject a malformed later sibling deterministically without process-stack recursion. The body record depth ceiling remains am-d-owned; am-b1 adds no type-depth cap. | `deep_hir_body_core_type_dag_is_stack_bounded` exercises the deep parameter/return root with one valid fixture and one malformed later sibling via the direct helper on a 2 MiB stack; body-order versus type-graph precedence remains stable. |
| ownership, cleanup, effects, and allocation | Do not read, recompute, or compare `drop_locals`, `drop_individual_locals`, `drop_individual_exprs`, `Assign` drop cells, `FnTy.effect`, return-provenance facts, `TaskProof`/`WaitProof` analysis state, or allocation flags. Do not null, move, Drop, replace, return, or register any Align/runtime/native object. These exact relations remain am-b4-owned; b1 only validates the type/control envelope needed to host them. | `hir_body_validator_deferred_facts_are_not_consumed` mutates every am-b4-owned HIR field independently and asserts unchanged b1 acceptance; the am-b4 owner names remain the only activation/ownership/effect tests. |
