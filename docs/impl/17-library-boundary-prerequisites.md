...
L2 ships through seven conceptual milestones in sixteen closed implementation PRs; L2b-a2 is split
into product, MIR action-continuation, malformed-HIR global-type, malformed-HIR nominal/link,
malformed-HIR callable/header, malformed-HIR body, fixed-array, eager-receiver lifetime, pipeline,
and tagged verticals below. A
PR may add dormant representation or tighten existing provenance, but it must
...
| L2b-a2-am-g-t | Validate concrete roots through every global type table before MIR construction | No source, semantic, HIR, MIR, interface, or ABI change; direct handcrafted-HIR lowering returns a canonical empty program for an invalid type domain while nominal/link and callable/header validation remain the semantic-checker contract | exhaustive type-domain/root/reference/cycle mutation matrix across all lowering entrypoints and unchanged valid-program MIR |
| L2b-a2-am-g-n | Validate nominal identity, table metadata, and link-library records after global type validation | No public change; direct handcrafted-HIR lowering returns a canonical empty program for invalid nominal/link metadata while callable/header validation remains the semantic-checker contract | exhaustive nominal/table/link mutation matrix across all lowering entrypoints and unchanged valid-program MIR |
| L2b-a2-am-g-h | Validate the callable namespace, `extern`/imported declarations, and every body-independent stored-function header fact before MIR construction | No public change; direct handcrafted-HIR lowering returns a canonical empty program for invalid callable/header metadata while body-derived ownership facts remain the semantic-checker contract | exhaustive symbol/signature/header mutation matrix across all lowering entrypoints and unchanged valid-program MIR |
| L2b-a2-am-b | Validate every statement, expression type relation, symbol target, action discriminator, id, path, arity, and result wrapper before lowering a function body | No public change; direct handcrafted-HIR lowering now has the complete fail-closed boundary | exhaustive table-driven malformed-body matrix, per-function Unreachable-only isolation, and unchanged valid-program MIR/codegen |
...
L2b is implemented as twelve independently sound vertical PRs. L2b-a1 owns named/direct/imported
...
terminating eager expression child for semantically checked HIR. L2b-a2-am-g-t then closes the
global type domain for direct lowering; L2b-a2-am-g-n closes nominal, table, and link-library
metadata after that prerequisite; L2b-a2-am-g-h closes callable, declaration, and body-independent
stored-function-header validation; L2b-a2-am-b closes the separate body/expression/ownership-action
boundary. L2b-a2-af closes validated fixed-array formation and
...
| narrow malformed-HIR defense | Ac may replace a direct index or shape assumption touched by its continuation edits with a checked lookup, but this is defense in depth rather than a complete handcrafted-HIR contract. Missing indirect-function type metadata and an invalid element-field path terminate before the parent action. L2b-a2-am-g-t/am-g-n/am-g-h/am-b own the complete structural boundary. | `malformed_hir_continuation_metadata_fails_closed` covers exactly the indirect-function signature and fixed/dynamic/SoA element-field-path cases, including no dynamic/SoA length action before rejection; no broader malformed-HIR claim is attached to ac |
...
checks across every HIR family, so it is re-split as L2b-a2-am-g-t/am-g-n/am-g-h/am-b rather than
expanding ac further.

### L2b-a2-am-g-t/am-g-n/am-g-h/am-b malformed-HIR closure matrix

L2b-a2-am-g-t, L2b-a2-am-g-n, L2b-a2-am-g-h, and L2b-a2-am-b form the internal fail-closed boundary between
handcrafted HIR and MIR. Normal compiler input is unchanged: semantic checking still owns user
diagnostics and supplies valid HIR. Direct `lower_program` callers, tests, and future tooling may
construct inconsistent HIR, so am has four independently mergeable validation phases before MIR
construction. Am-g-t owns the global type domain and concrete roots. Am-g-n runs only after am-g-t
succeeds and owns nominal identities, table metadata, and link-library records. Am-g-h owns the
callable namespace, declarations, and body-independent stored-function headers. Any
type/nominal/header failure returns the canonical empty MIR `Program`—all vectors empty—before any
MIR record is copied or any function is lowered. Am-b then validates bodies, body-correlated
ownership facts, and actions by walking every stored `Program.fns` entry—source functions, generic
monomorphs, and lifted lambdas—in vector order and each body in HIR preorder. A body failure
preserves its already-validated signature but replaces only that function
with an entry block terminated by `Unreachable`; valid functions before and after it lower
normally. A valid caller may retain a call to that signature; entering the rejected callee reaches
only `Unreachable`. No prefix action, allocation, ownership transfer, native call, or partial
control graph from the invalid function is retained. No diagnostic or new public result type is
introduced. Valid HIR produces byte-for-byte identical MIR.

| Slice / malformed-HIR cell | Required closure | Exact owner evidence |
|---|---|---|
| am-g-t concrete type roots and total type domain | Before copying any record, validate tuples, structs, enums, and every type reachable from those tables, an `extern`/imported declaration, or a stored-function header. A tagged-type or function-type table entry is not a root merely because it is stored: concrete roots pull in the exact entries they reference. An unreachable tagged or function-type graph may contain `Scalar::Param`/`Ty::Param` and is permitted because the producer retains generic-template interner entries that MIR omits; every reachable abstract parameter rejects. An unreachable non-template tagged entry is validated as a concrete root, so a missing id, inline cycle, or invalid concrete scalar cannot hide in discarded state. Every tagged reference and every function-type reference must be in range even inside a permitted template graph. Traverse with an explicit enter/exit worklist and visit colors rather than native recursion. `IntTy.bits` is exactly `8`, `16`, `32`, or `64`; `FloatTy.bits` is exactly `32` or `64`; the same widths apply inside every `PrimScalar`. `Vec`/`Mask` accepts only an integer/float scalar and exactly `2`, `4`, `8`, or `16` lanes. Every `Struct`/`Enum`/`Tuple`/`Tagged`/`Fn` discriminator resolves to its matching table, every struct-bearing collection resolves a struct, and `DictEncoded(id, field)` resolves an in-range `str` key field. `Ty::IntVar`, `Ty::FloatVar`, `Ty::Error`, and HIR-reachable `Ty::StrFinder` reject. Fixed arrays, tuples, structs, enums, `Option`, `Result`, and nested tagged payloads extend the active inline-layout path and reject an inline cycle. `Box`, slices, dynamic arrays, `ArrayBuilder`, `Task`, dynamic struct arrays, SoA, scanners, dictionary headers, and function closures validate their referenced entries but break that inline path; header-mediated nominal recursion is valid. | one mutation for every `Ty`, `Scalar`, and `PrimScalar` discriminator; every width/lane boundary; missing/wrong-kind table id and dictionary field; inline-cycle rejection and `Box`/dynamic-array/task/function-header positive cycle twins; reachable/unreachable `Param` tagged and function-type twins; unused malformed non-template tagged entries; first/middle/final concrete roots; invalid results have every vector empty in all four entrypoints |
| am-g-n nominal, table, and link metadata | Run only after am-g-t succeeds: the source-shape walk may index struct, enum, tuple, tagged, and function-type references only because am-g-t has already proved their ids, widths, template reachability, and inline-cycle safety. Every struct/enum internal `name` and `source_name` is non-empty UTF-8 without U+0000; identity and collision use exact UTF-8 bytes with no normalization. Internal nominal names are unique across the combined struct/enum namespace. A repeated `source_name` is permitted only for the same nominal kind with an identical complete recursively id-free shape. That shape includes field/variant order, alignment/layout flags, and recursively source-visible function signature, mode, and return-summary facts, but excludes the analysis-private function-effect cell. Field and variant names match `[A-Za-z_][A-Za-z0-9_]*` and are unique within their declaration. Tuple element lists are unique. A struct alignment is `None` or a power of two in `1..=2^29`; every `layout(C)` field is an integer or float. Enum variants are in tag order; the first `field_base` is `1`, each next base is the checked sum of the preceding base and payload length, and the final flattened field count fits `u32`. Link-library records are unique and accepted exactly when non-empty, not starting with `-`, and made only of ASCII alphanumeric bytes or `._+-`; spaces, slash, controls including U+0000, and non-ASCII reject. | empty/NUL/colliding nominal identities; repeated source identities with equal and unequal recursive shapes, including function-valued members; invalid/duplicate field and variant names; duplicate tuples; zero/non-power-of-two/over-limit alignment; invalid `layout(C)` field; wrong/overflowing enum field bases; duplicate libraries and every invalid library byte class/boundary position; equal-shape origin-specific identity twins remain valid and canonicalized |
| am-g-h symbol-table identity | Build one canonical callable lookup and emitted-symbol namespace before lowering. Visit stored functions, externs, then imported declarations in vector order. Stored and imported names are non-empty UTF-8 without U+0000 and use exact bytes; extern names additionally match `[A-Za-z_][A-Za-z0-9_]*`. Lookup identities are unique across the three classes. The emitted identity of a stored `main` returning `Unit` or `Result` is `align_main` and also reserves the generated C wrapper `main`; every other stored, extern, and imported identity is its exact name. Runtime/native LLVM identities come from one compiler-owned exact-symbol registry consumed by both validation and codegen; only actual emitted identities reject, and the `align_rt_` prefix alone is not reserved. Emitted identities are unique across classes and disjoint from that shared exact set. Body-selected `$fnval`, `$clos`, task-trampoline, and parallel-kernel identities remain am-b-owned because this slice does not inspect a body. The first invalid byte, name, or collision returns the canonical empty program before any name reaches LLVM or a linker. | empty/NUL stored/import names; every invalid extern first/continuation byte; duplicate lookup/emitted identities in every class/order; `main`/`align_main` wrapper collisions for Unit, Result, and direct-`i32` main; one collision per shared runtime-symbol family plus a non-member `align_rt_` prefix positive twin; distinct UTF-8 byte names without normalization; valid same-source display names with distinct canonical symbols |
| am-g-h signature and stored-function header metadata | Validate every body-independent signature/header field before `lower_fn`. Function-value modes are parallel to parameters, admit `ByValue`/`Out`, and require every `Out` parameter to be a slice; their return summaries are `None` before L2b-b. Constructing or calling a first-class value with an `Out` parameter remains rejected at the am-b body-use boundary. Extern modes are parallel to parameters and `ByValue`; summaries are `None`; parameters are integer/float/raw, `str`, a numeric slice, or a non-empty `layout(C)` struct, and returns are Unit/integer/float/raw or a non-empty `layout(C)` struct. Stored/imported modes are parallel to parameters, admit only `ByValue`/`Out`, and `Out` is a slice; `Borrow`/`BorrowMut` remain disabled. Every root set is canonical non-empty sorted-unique, parameter roots are in range, capture roots are empty before L2b-b, borrow/region summaries agree exactly, and each rooted parameter plus the return can borrow. For each stored function, parameter ids are unique/in-range; `lifted_capture_count <= params.len()` partitions the trailing parameter records, every lifted parameter is `ByValue`, and a lifted function is not exportable. The HIR does not distinguish source functions from generic monomorphs when `lifted_capture_count` is `None`, so no unobservable origin-specific exportability claim is made; observable `main` and lifted functions are non-exportable. For a no-argument `main`, a `Result` return is exactly `Result<Unit, builtin Error>`; after ordinary am-g-t return-type validation, am-g-h imposes no additional restriction on any non-`Result` return. The builtin `Error` id resolves the settled declaration `Error { NotFound, Invalid, Denied, Timeout, Code(i32) }` with exact source/internal identity, variant order, payloads, and field bases. A parameterized `main` has exactly one `ByValue` parameter of type `DynArray(Str)` and returns exactly that same `Result<Unit, builtin Error>` shape. Each local id equals its vector ordinal; local names are non-empty UTF-8 without U+0000; parameter names match `[A-Za-z_][A-Za-z0-9_]*`. Local alignment is `None` or a power of two in `1..=2^29` and is present only on non-parameter fixed arrays of integer/float scalars. `drop_locals` and `drop_individual_locals` are canonical in-range sets and the latter is a subset of the former. Their exact semantic membership and every `drop_individual_exprs` span/value are body-correlated facts owned by am-b. | one mutation per observable header field; mode-count/class/Out-shape matrix, including a stored function type with an `Out` slice positive twin and a non-slice `Out` rejection; canonical/root-bounds/capture/agreement/borrow-capability summary matrix; extern FFI shape matrix; one no-argument positive twin for every am-g-t-valid concrete non-Result `Ty` discriminator plus exact Result, wrong Ok, wrong/malformed Error identity/shape, and zero/one/multiple/wrong argv plus non-ByValue argv mutations; local name/id/alignment; lifted partition and exportability; drop-set structural predicates; mixed valid/invalid function order returns the canonical empty program before lowering any body |
| am-b statement, local, ownership, and place metadata | Validate every local/slot id and its exact declared type, destination kind, struct/tuple/element path, field/lane/index discriminator, writable-place shape, and assignment result assumption reachable from statements, returns, breaks, and block tails. Correlate `drop_locals`, `drop_individual_locals`, and every `drop_individual_exprs` span/value with the body analysis facts they encode; a structurally valid flipped ownership bit or unrelated span rejects before any MIR for that function. | table-driven mutations for every statement and place discriminator, including first/middle/final positions; missing/extra/flipped body-correlated drop facts; each invalid function is Unreachable-only |
| am-b universal expression consistency | One total `ExprKind` match validates every child first and derives the only permitted result type. The traversal uses an explicit enter/exit worklist, bounded per-node result state, and no native recursion, so every semantically checked HIR up to the parser's expression-depth ceiling remains stack-bounded. Every raw type/id reference reached from a body—including `Expr.ty`—is resolved against the already validated am-g-t tables here; a missing, cyclic, template-only, or wrong-kind body reference is an am-b failure and never changes the program-global result. Require `Expr.ty`, referenced local type, operand types, operator discriminator, scalar width/sign, cast endpoints, vector lane/arity, unary/binary/int-arithmetic/math arity and type rules, and literal payload to agree exactly. `FnValue`, closure/lifted targets, and direct calls resolve body uses through the am-g-h canonical namespace; indirect calls agree with their validated function type. No wildcard arm or default type is permitted. | one missing/wrong-kind body type id, wrong-result, and wrong-operand mutation for every expression discriminator; every unary/binary/cast/int-arithmetic/math operator/type/arity boundary; unresolved/wrong-kind call and lifted body targets; valid local ids carrying wrong expression types; direct maximum-checked-depth HIR validation and end-to-end `within_limit_chain_compiles_and_runs` |
| am-b aggregate, capture, and call metadata | Validate body-local aggregate/type ids, field and payload ordinals, constructor arity, capture ids and types, direct/indirect callable signatures, parameter modes/counts, result types, and constant-array contents before lowering. | missing/wrong body-local ids, paths, arities, signatures, captures, and result types across array/struct/tuple/tagged/closure/direct/indirect call families; constructing and calling a first-class function value with an `Out` parameter reject here while its validated header remains accepted; every failure replaces only its function |
| am-b storage, view, vector, and pipeline metadata | Validate collection/view/source discriminators, element/layout facts, scalar widths, vector lanes, chunk/range shapes, stage/terminal kinds, callback signatures, accumulator/result shapes, and every producer-owned pipeline table reference. | exhaustive discriminator mutations for fixed/dynamic/SoA/view/vector storage plus every stage and terminal; no helper CFG or terminal state on rejection |
| am-b native and runtime metadata | Validate every helper discriminator, argument/result ABI shape, wrapper enum/tag id, and operation-specific checked record used by JSON, I/O, filesystem, network, process, regex, HTTP, task, crypto/compression, time/random, and unsafe lowering. | one valid and every invalid metadata form per helper family, including `Option`/`Result` wrapper mismatches; rejection occurs before runtime allocation or call |
| am-b structured-control metadata | Validate branch/result types, match variant and binding records, `else`/`?`/`map_err` wrapper shapes, loop target/result records, and arena/task-group/unsafe control metadata without trusting an ordinal or table index. | malformed fully terminating, mixed, and all-fallthrough control records; each rejects deterministically before any MIR for the function |
| am-b function isolation | A body-metadata failure changes only the invalid function body. Its validated signature, linkage, provenance summaries, and neighboring valid functions remain unchanged; invalid functions in first, middle, and final declaration positions behave identically. Program/header failures remain all-or-nothing because their records can shape every function and emitted declaration. | valid-before/invalid/valid-after triples for source, monomorphized, and lifted functions compare neighboring MIR byte-for-byte with the all-valid twin and require an Unreachable-only invalid body |
| am-g-t/am-g-n/am-g-h/am-b public, valid-input, and cost boundary | No source syntax, semantic acceptance, HIR/MIR variant, interface byte, fingerprint, cache key, ownership rule, LLVM ABI, or runtime ABI changes. Each validator is linear in its stored input plus referenced type metadata and allocates only compiler-owned validation worklists/maps. | `valid_hir_global_type_preflight_is_mir_identity`, `valid_hir_global_nominal_preflight_is_mir_identity`, `valid_hir_global_header_preflight_is_mir_identity`, and `valid_hir_body_metadata_preflight_is_mir_identity` compare validated and internal unchecked lowering paths; the complete `align_mir` owner suite and whole/per-unit codegen suite remain clean; the benchmark records each phase with the continuation-lowering row |

Am-g-t, am-g-n, and am-g-h are independently complete, sequential fail-closed tightenings:
am-g-t activates only its global type-domain all-empty rejection; am-g-n runs after it and activates
nominal/table/link all-empty rejection; am-g-h then activates callable/header all-empty rejection.
This split keeps each implementation and exhaustive mutation owner near the repository review
bound; none claims validation of a later phase. Am-b may exceed roughly
1,000 changed hand-written lines because
its one total expression/body validator must match every statement, expression, pipeline-stage,
terminal, and native-helper discriminator before the public `lower_program` entrypoints can claim
fail-closed handcrafted bodies. A partial family validator would still let another body
discriminator panic under the same claim and would duplicate traversal/type-derivation state.
The total match, Unreachable-only function replacement, exhaustive mutation matrix, valid-input
identity proof, and four-entrypoint parity therefore form one atomic body boundary. If am-b cannot
remain reviewable as one exhaustive match plus table-driven owners, reopen this matrix and split
dormant validator construction from activation; do not activate a partially complete body claim.
...
prerequisite above without changing a projection fact. L2b-a2-am-g-t then closes direct
handcrafted-HIR global type-domain validation, L2b-a2-am-g-n closes nominal/table/link validation
after that prerequisite, L2b-a2-am-g-h closes callable/header validation, and L2b-a2-am-b closes
body/action validation without changing valid HIR.
...
owners to the shared focused targets. All ten retain the scope-boundary row.
...
L2b-a2-s, L2b-a2-ac, L2b-a2-am-g-t, L2b-a2-am-g-n, L2b-a2-am-g-h, L2b-a2-am-b,
L2b-a2-af, L2b-a2-ar, L2b-a2-ap, and
L2b-a2-t are the smallest independently correct verticals. The first PR
...
PR closes the general MIR continuation invariant for checked HIR. The third PR adds the global
type domain, concrete-root, reference, template-reachability, and inline-cycle validation. The
fourth adds nominal identity, table metadata, and link-library validation after that prerequisite.
The fifth adds callable namespace, declaration, and body-independent stored-function-header
validation. The sixth adds total body, body-derived ownership, and expression consistency
validation. The seventh adds validated fixed-array formation plus exact/dynamic reads and writes on
that substrate. The eighth closes eager retained-storage lifetime for non-fixed reads. The ninth
closes the explicit pipeline stage/terminal state machine. The tenth PR replaces
...
| L2b-a2-am-g-n | `cargo test -p align_mir malformed_hir_global_nominal_metadata_fails_closed`; `cargo test -p align_mir valid_hir_global_nominal_preflight_is_mir_identity`; `cargo test -p align_mir`; `cargo test -p align_driver --test per_unit_codegen` | `bench/library_boundary/run.sh provenance`: `mir-global-nominal-validation`, `mir-continuation-lowering` |
| L2b-a2-am-g-h | `cargo test -p align_mir malformed_hir_global_header_metadata_fails_closed`; `cargo test -p align_mir valid_hir_global_header_preflight_is_mir_identity`; `cargo test -p align_mir`; `cargo test -p align_driver --test per_unit_codegen` | `bench/library_boundary/run.sh provenance`: `mir-global-header-validation`, `mir-continuation-lowering` |
| L2b-a2-am-b | `cargo test -p align_mir malformed_hir_body_metadata_fails_closed`; `cargo test -p align_mir valid_hir_body_metadata_preflight_is_mir_identity`; `cargo test -p align_mir max_checked_hir_depth_body_preflight_is_stack_bounded`; `cargo test -p align_mir`; `cargo test -p align_driver --test expr_depth within_limit_chain_compiles_and_runs`; `cargo test -p align_driver --test per_unit_codegen` | `bench/library_boundary/run.sh provenance`: `mir-body-validation`, `mir-continuation-lowering` |
