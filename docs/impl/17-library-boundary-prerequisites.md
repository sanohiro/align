...
| checked HIR and MIR | Add one typed scalar-callback-descriptor formation record carrying exact `ProgramCall`, descriptor id, signature, effect, return summaries, and generated-family version. The HIR validator re-derives all fields from stored/imported facts; MIR keeps one corresponding static-descriptor rvalue and no function-value environment. Every lowering entrypoint fails closed on a field mutation. | valid scalar record; mutation of every scalar/id/signature/effect/provenance/cleanup field; source/monomorph/imported target; empty-program and checked-error paths |
...
| scalar C trampoline | Generate exact C `void(ptr,i32,ptr)` ABI, nounwind, fixed 127-value stack scratch, null-context/database-handle hard abort, ordered native accessor/OOM validation, one direct Align target call, dynamic Move-return cleanup handling, and exactly one SQLite result/error path for non-null context. No callback frame view, source owner, or native pointer escapes. | LLVM ABI/body golden; null-context/database-handle subprocess owners; argc/argv/value/result/error/OOM mutation owners; return-provenance and cleanup-bit matrix; ordinary Err/hard-abort twins |
