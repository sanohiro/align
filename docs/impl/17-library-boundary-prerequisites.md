...
active `align_mir::hir_program_is_valid` gate invokes its pure row-graph check before lowering.
...
| ownership and activation | Do not replay body Drop sets, cleanup cells, allocation identity, region/borrow/escape provenance, effects, task proofs, or native state. The one structural exception is `JsonScan`'s canonical recursive Copy/DropPlan precondition, which is required to prevent a checked-HIR scanner row from reaching MIR; it performs no move, cleanup, or runtime action. The general b2b2 body validator remains dormant, but the scanner predicate is separately invoked by the active `align_mir::hir_program_is_valid` pre-lowering gate. | `hir_body_validator_json_scan_copy_row` covers the shared structural predicate and one direct/transitive Move mutation; `hir_program_json_scan_copy_row` proves the active lowering gate rejects the same mutation; `hir_body_validator_pipeline_deferred_facts_are_not_consumed` remains the owner for all unrelated deferred facts, and repository search confirms the active gate is the only production consumer. |
| source propagation and review gate | Synchronize this matrix, the am-b2 ledger rows, exact owner names, and `HANDOFF.md`. The b2b2 PR claims only Template/JSON/group/dictionary validation plus the narrow Request 6 scanner safety gate; it must not claim b3, b4, general body activation, or a public database surface. | Author matrix-to-diff pass, one fresh independent matrix review before coding, focused owner checks, bounded ordinary gate once, applicable Clippy, and SHA-bound pre/post attestations. |
