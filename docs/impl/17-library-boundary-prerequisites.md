...
### D14-A2 — static native-callback descriptors and generated SQLite C trampolines
...
does not expose an Align function value to C. `pkg.db.sqlite.function` and `collation_key` are two
trusted compile-time producers. Each accepts one exact named or noncapturing lifted target and emits
the 32-byte v1 descriptor fixed by `pkg-design/db.md` §23 plus one kind-specific generated C-ABI
...
|---|---|---|
| formation and type proof | Recognize only the two exact producer identities after ordinary import/visibility and argument formation. Resolve one direct source/imported/lifted target; require the kind signature, no captures/extern/open dispatch, complete effect/return provenance/cleanup, and Pure for collation. Preserve source-order diagnostics and emit no partial descriptor HIR on failure. | direct/imported/lambda positives; capture/dynamic/extern/unknown-effect/signature/multi-invalid negatives; whole/per-unit source parity |
| checked HIR and MIR | Add one typed callback-descriptor formation record carrying kind, exact `ProgramCall`, descriptor id, signature, effect, return summaries, and generated-family version. The HIR validator re-derives all fields from stored/imported facts; MIR keeps one corresponding static-descriptor rvalue and no function-value environment. Every lowering entrypoint fails closed on a field mutation. | valid record per kind; mutation of every scalar/id/signature/effect/provenance/cleanup field; source/monomorph/imported target; empty-program and checked-error paths |
| descriptor and identity | Serialize the exact 32-byte v1 record and NUL-terminated canonical identity; identity includes target, signature/modes, effect, return facts, descriptor kind/version, and generated-family version. Correlate descriptor, relocation, callback target, and generated trampoline before LLVM construction. | semantic-byte goldens; canonical graph/interface/cache keys; equal dedup and unequal noncollision; splice/length/NUL/relocation negatives |
| scalar C trampoline | Generate exact C `void(ptr,i32,ptr)` ABI, nounwind, fixed 127-value stack scratch, native value validation, one direct Align target call, dynamic Move-return cleanup handling, and exactly one SQLite result/error path. No callback frame view, source owner, or native pointer escapes. | LLVM ABI/body golden; argc/argv/value/result/error mutation owners; return-provenance and cleanup-bit matrix; ordinary Err/hard-abort twins |
| collation C trampoline | Generate exact C `i32(ptr,i32,ptr,i32,ptr)` ABI, nounwind; require null app data and valid byte views; call the one Pure target left then right; compare/destroy owned arrays and return only -1/0/1. | LLVM ABI/body golden; operand order; owned-array Move/Drop on all normal edges; invalid native product abort; comparison-law parameterized owner |
