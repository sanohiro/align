...
### L2b-a2-am-g-t immediate closure and am-r design gate

Only am-g-t is authorized for implementation by this section. Normal compiler input is unchanged:
...
compiler/runtime spellings as malformed HIR would itself be a hidden semantic change. Am-r must
therefore publish and pass a new public-contract ledger before any placement, nominal/link,
namespace, declaration/header, or body implementation begins. The ledger fixes the exact remaining
PR split and count; this section deliberately does not guess either.
...
point to the one declaration. The eight reserved probe identities reject every extern or export
claimant even when its ABI matches.
...
| extern or export equal to any of the eight reserved probe symbols | reject regardless of ABI |
...
and four distinct `par-map-probe` exports are never compiler-callable, but their names are
unconditionally reserved external identities. `task-group-probe` adds no unmangled export. The
...
The compiler registry is fixed at 286 base records with no feature or ambient input. All eight
probe names extend an unconditional 294-identity collision-reservation set but may never be a
RuntimeKey, callable declaration, or compatible-extern reuse target. Runtime feature selection
affects only export-set verification and changes no MIR/interface/artifact/cache identity.
...
|---|---|---|---|
| formation and validation | Source `main` is non-generic and has either no parameters or exact `ByValue array<str>`. No-arg return is Unit, exact signed i32, or `Result<Unit,builtin Error>`; argv requires that Result. After ordinary type formation, main return checks run before the parameter-shape check. An otherwise-valid non-Result return outside the admitted set emits exactly `main returns only (), i32, or Result<(), Error>; got <type>` at the return span. Existing wrong-Result Ok/Error diagnostics retain their order, followed by the existing argv/parameter diagnostic on a multi-invalid declaration. | Each active group owns compiler-only `group`, `current_generation`, `proof_epoch`, `completed_through`, `valid_from`, and a sparse ordered unresolved-Wait set. Every Spawn increments current generation and proof epoch, staling all old WaitProofs and resetting current completion. With an unresolved Wait it also clears that set and advances `valid_from` to invalidate old Tasks; otherwise old Tasks remain eligible for reauthorization. A fallible Wait produces `WaitProof { group, proof_epoch, wait, covers_through }`; Spawn produces `TaskProof { group, born_generation }`. Ok resolves only its id and advances completion only after every earlier id in that epoch is Ok. Err advances the proof epoch and `valid_from`, poisoning all covered Tasks/Waits. Infallible Wait completes through current generation. TaskGet requires `valid_from <= born_generation <= completed_through == current_generation`. | all accepted signatures plus every other graph-valid return/parameter mutation and multi-invalid diagnostic order; straight-line infallible/fallible/reset/generation-join, stale resolved-Wait alias after Spawn, unresolved-first Wait plus successful empty second Wait, failed-first Wait plus `wait()?` on an empty queue, missing/wrong/stale TaskProof, and nested-group mutations |
| construction and control | Exact i32 emits external `i32 @main()` directly. Unit/Result emit an internal Align body plus an external i32 wrapper; argv wrapper alone accepts `(i32,ptr)`. No other entry ABI is constructible. | A fallible-Wait Result carries exact group/proof-epoch/wait/coverage provenance. Bare local binding, copy/reassignment, block tail, `ResultMapErr`, and value-producing if/match/else/loop preserve it only when every reachable result predecessor has the same proof; unrelated overwrite clears it. Move Task handles transfer `TaskProof { group, born_generation }` through transparent local binding, move/reassignment, block tail, and value-producing control flow; there is no Task-copy path. Try, Result Match, and Result Else resolve or poison the exact still-active proof even while a nested group is active. Multiple aliases resolve idempotently; a stale proof has no effect. Passing a Copy Wait proof leaves the caller local intact but no callee/import/return/capture/aggregate result inherits either proof. Ordinary control joins intersect exact group state and proof records; distinct histories get fresh counters, unresolved Wait proofs do not remap, and a Task proof remaps only when every predecessor carries the same logical handle and authorizes its born generation. Entering a nested group retains outer state; exit removes all proofs naming the inner group. TaskGet checks its originating group/generation bounds, never the innermost position. Return, Try Err, process termination, and a diverging loop have no continuation. Lambdas/functions start empty. | whole/per-unit raw/optimized LLVM exact signatures and link/run exit behavior; direct and stored/copied/reassigned/map_err Wait proof; transparent Task move/reassignment/control proof and rejected Task-copy premise; unresolved first Wait plus second empty Ok remains unreadable; first Ok then second Ok authorizes; first Err invalidates the task and every second proof; Spawn with an unresolved Wait invalidates the old task generation; successful Wait→Spawn→successful Wait authorizes old and new handles; unrelated overwrite and opaque-boundary nontransport; nested-group, originating-diagnostic, exact/wildcard/or Result arm, terminating Else fallback, branch/loop/early-exit/lambda matrix |
...
| benchmark | No persistent validation pass is added; one constant-time header predicate replaces invalid backend construction. | Generation replacement invalidates a failed or unresolved batch in O(1). Wait resolution and control joins examine only sparse live unresolved/proof entries and are O(waits plus control edges plus proof entries examined); there is no runtime work. | compile-time task-count × unresolved-Wait × branch-count × live-proof-alias scale owner plus no regression in existing `mir-header-validation` and task-group rows; no new runtime benchmark |
...
| native/FFI and allocation parity | The compiler registry is the fixed 286-row base surface. All eight probe identities are unconditionally reserved for collision rejection but are never RuntimeKeys, callable declarations, or compatible-extern reuse targets. Runtime feature selection affects only export-set verification and is not a compiler input or cache identity. A compatible extern may reuse only an exact base row; incompatible ABI/attribute or any reserved external-name collision rejects before LLVM. Native calls, runtime ownership provenance, and success/error allocation counts are unchanged. | base/`alloc-count`/`par-map-probe`/all-feature bidirectional export sets, fixed 281 declaration ABI rows, base compatible/incompatible extern twins, eight probe-name extern/export collision negatives, cumulative native allocation owners |
...
ledger inserts am-e, am-f, am-w, am-v, am-u, am-d, am-p, am-n, am-h, am-b1–b4, and am-c before
...
L2b-a2-s, L2b-a2-ac, and L2b-a2-am-g-t are fixed completed verticals. The mandatory remaining
sequence is am-e, am-f, am-w, am-v, am-u, am-d, am-p, am-n, am-h, am-b1, am-b2, am-b3, am-b4,
am-c, af, ar, ap, and t, pending the clean am-r review. The first PR
...
type-domain validation. The fourteen am-r implementation verticals then apply the six producer and
stack-safety prerequisites before placement, nominal/link, header, total body metadata, and finally
