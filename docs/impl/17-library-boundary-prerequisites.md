...
| L2b-a2-am-b2b | Extend the same dormant validator through pipeline terminals, array views, templates, JSON, and group/dictionary records | No public entrypoint activation | exhaustive direct pipeline/view/template/JSON/group unit owners |
...
`StrTrimKind`. The am-b2 range is delivered as two contiguous, reviewable verticals on the same
dormant validator state machine: am-b2a owns `ExprKind::ArrayLit` through `ExprKind::VecLit` in
declaration order, and am-b2b owns `ExprKind::ArraySum` through `ExprKind::ArrayDictEncode` in
declaration order, plus every `StageKind`, `TemplatePart`, `GroupSource`, `GroupAgg1`, and
`GroupOp`. Am-b3 owns
...
| L2b-a2-am-b2b | `cargo test -p align_mir hir_body_validator_storage_pipeline_json`; `cargo test -p align_mir deep_hir_body_storage_type_dag_is_stack_bounded`; no public-entrypoint activation | none |
