...
## 12. `pkg.ws` HTTP Upgrade prerequisite (implemented 2026-09-04)
...
|---|---|---|
| header multiplicity/token query | Four Pure, allocation-free operations scan every repeated physical row, share one RFC token grammar, reject invalid source arguments before scan, and return no borrowed cursor/view. The two membership operations differ only in ASCII-case-insensitive versus byte-exact member comparison. Their native rows hard-abort detectable null/alignment/length/range defects in ctx/name/token order before safe view formation and invalid token bytes before table scan; readiness does the same for ctx, so malformed input never aliases zero/false. | token differential oracle; repeated-row/malformed-neighbor/exact-case matrix; direct-ABI subprocess aborts; HIR/runtime/whole-per-unit owners |
...
All eleven runtime keys reuse existing shapes; no new shape is introduced. Implementation is one PR
because no strict prefix leaves a useful consumer: the web variant
