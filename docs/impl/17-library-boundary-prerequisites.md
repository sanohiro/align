...
|---|---|---|
| structural activation order | The shared MIR gate must short-circuit in this order: checked-HIR depth, global type/placement/nominal/header validators, the body-only structural record validator (`body_core::validate`), then sema body-fact replay. A malformed local/type/callable id must never reach MoveCheck/EscapeCheck/effect/task-wait replay. Every nonparameter local must have exactly one `Let`, `LetTuple`, or match-payload binding; even an unused orphan local-table record fails closed. Declaration-free `print`, `hash64`, and `hash128` calls are validated by their exact builtin contracts before replay; source `String` operands must already be represented by `StrBorrow`/`Str`. | A body-structure plus body-fact simultaneous mutation proves the body-core failure wins; all four entrypoints return canonical-empty without replay side effects. `malformed_hir_unused_local_record_fails_closed`, `hir_body_validator_accepts_builtin_display_and_hash_calls`, `hir_body_validator_rejects_unborrowed_builtin_string`, `hir_body_validator_accepts_nested_tagged_payload_construction`, and the body owner matrix close the binding, builtin, callable-shape, and nested-payload inventory. |
