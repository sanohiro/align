...
- a syntactically reachable `break` rejected inside an `arena` or `task_group` nested in its loop
  ...
  and a diverging break payload that never reaches its enclosing break;
