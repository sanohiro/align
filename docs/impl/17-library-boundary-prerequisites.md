...
|---|---|---|
| header multiplicity/token query | Three Pure, allocation-free operations scan every repeated physical row, share one RFC token grammar, reject invalid call arguments before scan, and return no borrowed cursor/view. | token differential oracle; repeated-row/malformed-neighbor matrix; HIR/runtime/whole-per-unit owners |
| checked 101 transfer | Borrow ctx, consume builder, validate HTTP/1.1/status/body/Upgrade/Connection/framing completely before fd lift/write, publish only after full head write, retain spent ctx views, never park. | raw socket response and validation/write failpoints; Move/source-null/Drop matrix |
| upgraded handle carrier | Positive local/direct-Ok/by-value/borrow/borrow-mut grammar; exhaustive rejection of all other storage/capture/task/parallel/native/return edges. | no-wildcard type graph, future-variant tripwire, control-flow and malformed-HIR owners |
| exact transport operations | Mutable bare-buffer exact read without overread/partial publication, payload-copy-free write-all, one cumulative monotonic deadline with no per-call/frame reset, idempotent shutdown, sticky failure and close-only Drop. | split/coalescing/deadline/SIGPIPE/fd/allocation counters across Linux/macOS |
| web extension dispatch | Copy prepare/pump functions and opaque values slice, closed third Handler variant, route/middleware/HEAD/405/group behavior, pump only after successful 101. | mixed-route E2E and interface/cache mutation owners |
| package protocol | canonical handshake/private SHA-1, server-order subprotocol, RFC framing/masking/fragment/control/UTF-8/close under explicit bounds. | independent RFC/wire/browser oracle; randomized fragmentation and multi-invalid matrix |

All nine planned runtime keys reuse existing shapes; no new shape or active count is introduced by
