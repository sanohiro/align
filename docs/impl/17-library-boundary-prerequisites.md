...
| checked 101 transfer | Borrow ctx, consume builder, validate HTTP/1.1, no parser residual, status/body, complete header syntax, Upgrade/Connection/framing before fd lift/write; expose the shared readiness fact; publish only after full head write, retain spent ctx views, never park. | raw socket response and validation/write failpoints; version/residual matrix; Move/source-null/Drop matrix |
...
| exact transport operations | Mutable bare-buffer exact read without overread/partial publication, payload-copy-free write-all, one cumulative monotonic deadline with no per-call/frame reset, spent Invalid/no-I/O behavior, idempotent shutdown, sticky failure and close-only Drop. | caller-invalid x live/spent/poisoned state matrix and split/coalescing/deadline/SIGPIPE/fd/allocation counters across Linux/macOS |
