...
| G9 | A function called from a pipeline stage or a hot loop is inlinable across units | a non-generic `pub fn` admitted by plan 74's target-independent checked-HIR budget travels in the unit interface with its exact extern closure and is emitted in the consumer as `available_externally`; release/fast direct calls are `alwaysinline`, while dev and size profiles retain their established optimization contracts | whether an excluded body or indirect call can inline | `align_interface`, with sema, LLVM and the driver | plan 74's five-profile two-unit scalar, pipeline/explain-opt, extern-wrapper, symbol/link and cache corpus | 1066; plan 74 |
...
operation-local      a call site never marks strict callee operations;
                     reassociation may compose after inlining because LLVM
                     flags carry no scope identity; contraction is selected
                     only from a two-sided authenticated MIR pair and emits
                     explicit FMA, never raw LLVM `contract`
...
serialization, diagnostics, exact LLVM-flag admission and FMA-pair selection.
...
emits it in the consumer as `available_externally`. Release and fast mark the
authenticated consumer definition `alwaysinline`; dev remains unoptimized and
small/tiny preserve `optsize`/`minsize` profitability. It deliberately does not
