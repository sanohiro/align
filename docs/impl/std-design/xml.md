...
later accepted capability. `xml.event` uses the existing nominal enum grammar and consumes no new
type-codec discriminator. The producer-certificate field below changes the function-record grammar
and therefore bumps `align_interface::FORMAT_VERSION` from 8 to exactly 9. Unknown, truncated,
missing-payload, and trailing records reject before cache publication.
...
| Interprocedural producer certification | Move the sealed graph builder to one reusable MIR-owned validator invoked before codegen and before interface publication. First form exact local call-graph SCCs. Each SCC is checked coinductively under provisional copies of its members' declared contracts: every internal call edge must match the exact target, argument products, return type, borrow/region roots, callable identity, and cleanup mode, and every actual `Return`/`ReturnWithCleanup` equation must satisfy that provisional contract through the ordinary producer graph. The contract is certified for the whole SCC only if every member succeeds. This admits a well-typed non-returning `fn f() -> string = f()` and mutual-recursion equivalent without manufacturing a value seed, while any raw/contradictory return branch, mismatched internal call, or invalid non-call producer rejects the whole SCC. This coinductive function-contract check is distinct from the intra-function value graph, where an unseeded SSA/slot/check cycle remains invalid. A `Ty::Fn` argument is a protected leaf: its provisional argument fact is conditional on every direct/indirect caller proving the exact callable producer, and `FnAddr`, lifted closure, wrapper, store/load, projection, join, and returned callable preserve that identity only through certified local/imported targets. A local direct-call result may seed facts only after its callee SCC is jointly certified. A bodyless imported result may do so only from a format-9 `ValidatedBody` certificate bound to that exact function record. Extern/runtime/raw calls never receive this certificate and remain intrinsic-specific or rejected. | Whole/per-unit owners mutate local return operands and cleanup pairs, direct/wrapped/returned callable producers, callable parameter products and identities, declaration/body disagreement, imported certificate tag/presence, and each call-graph edge. Positive owners cover direct/indirect wrappers, borrowed and self-owned results, returned callables, generic revalidation, self and mutual non-returning protected-result recursion, seeded recursion, and serialized producer/consumer parity. Raw/contradictory SCC return branches, signature/cleanup edge mismatches, and unseeded intra-function value cycles reject before resource selection. |
| Operand requirements and atomic seeds | Every validation dependency carries one exact product `OperandRequirement { read, write, exclusive, move_value, callable }`; `ShapeOnly` is the all-false product and is legal only for metadata that is neither evaluated nor dereferenced. Access facts provide capabilities as follows: Owned = read/write/exclusive/move, Shared = read, Exclusive = read/write/exclusive, and Out = write/exclusive but unreadable and uninitialized. A requirement succeeds only when every requested capability is present; `callable` is orthogonal and, when set, must name the one canonical certified function identity. The complete mode mapping is: ByValue Copy = read, ByValue Move = read+move, Borrow = read, BorrowMut = read+write+exclusive, and Out = write+exclusive on the exact canonical destination with no read. A callable leaf adds its identity requirement to that mode product rather than replacing it; structural carriers apply the product per selected leaf. Constants are readable. Conditions, lengths, indices, template holes, fresh-producer inputs, and other evaluated operands require read or move according to MIR semantics, never shape-only by convenience. Selected-path access still excludes validation-only dependencies, but their product result participates in total validity. A fresh, view, or runtime out-slot fact is published only by the same fully validated rvalue equation that proves exact result type, every operand and option, primary definition, output slot, and access relation. No preliminary scan of a discriminant or output-slot number may seed a fact. | Parameterized whole/per-unit mutations cross all four argument modes, Copy/Move/callable leaves, and each capability product for conditions, length/index operands, constructors, calls, fresh allocators, template pieces/arena, grouped views, and every runtime out-slot producer. Each result/operand/option/slot/type mutation rejects. Shared→BorrowMut, Out→BorrowMut, every Out read, and noncanonical destinations reject; Out is accepted only by the matching write-only destination product. A producer whose primary equation is absent, duplicate, rejected, or invalid cannot make its output slot readable. |
| Semantic producer inventory | The exhaustive `Stmt`/`Rvalue` match returns semantic equations, not a broad syntactic `Rejected` bucket. An `Irrelevant` classification is legal only when the actual result and every written slot are structurally proven unable to contain `str`, `string`, `xml.reader`, `Ty::Fn`, or a carrier of those leaves. Every current variant that can yield or feed such a leaf has an explicit result/dependency equation. In particular, `CloneIn` requires an exact same-shaped supported RegionPlain input/result—including `str` and `slice<u8>`—plus a readable `ArenaHandle`, retains the arena-rooted readable view provenance, and can feed `StrClone`; template equations validate every piece and arena; every view validates its source/index/resource relation; and each native out producer validates its complete primary/out equation. | A compile-time variant sweep plus a generated semantic inventory enumerates all current protected-leaf-capable result and out-slot variants. Positive whole/per-unit owners feed every valid `str`/`string`/callable carrier—including `CloneIn(...).clone()`—to an XML checked edge. Per-variant result, dependency, option, and out-slot mutations reject; an `Irrelevant` mutation to a protected actual type fails closed. |
| Existing container/view producer closure | The unconditional producer validator gives exact typed equations to every existing operation that can project or preserve a protected leaf. Dynamic AoS element/field loads, SoA field views, and subslices preserve only the selected source path while independently validating the complete base/index/range/nominal shape. A by-value structural carrier requires move authority only for selected leaves whose own type is Move; Copy view leaves require readable provenance even when a different sibling makes the enclosing carrier Move. | Valid non-XML owners cover dynamic AoS whole/field string projections, SoA string-column views, slice-of-string subslicing followed by indexing/forwarding, and a Move record containing both owned `string` and shared `str`. Per-operation base/result/nominal/field/index/path mutations and per-leaf Copy/Move authority mutations reject before LLVM in whole/per-unit builds. |
...
| Interface/cache/generics | Encode exact nominal event plus type leaves 72/48, parameter modes, mutability, effects, returned view regions, cleanup, and one `ProducerCertification` byte immediately after `return_cleanup` in each exported function record. Exact tag `0 = RevalidateGenericBody` is valid iff type parameters are nonempty and `generic_body` is present; the instantiated body becomes a local function and must pass the same MIR validator. Exact tag `1 = ValidatedBody` is valid iff type parameters are empty and `generic_body` is absent, and interface construction may emit it only after validating that exact local MIR body. Other tags and either presence mismatch reject. Imported non-generic functions carry `ValidatedBody` into MIR; it authenticates the record's exact parameter/return/borrow/region/cleanup facts and is consumed at every bodyless call boundary. The byte is part of interface surface/hash, format 9 invalidates format 8 before record parsing, and private body edits that preserve certification and surface remain interface-hash stable. A return root is admitted from a by-value parameter only when its type itself may borrow; an opaque/self-owned Move value whose only capability is dynamic cleanup or non-cleanup ownership may root a returned view only through `borrow`/`borrow mut`. Apply that rule recursively to direct, concrete-sum, and generic carriers. Cleanup capability alone is never external borrow provenance. Include used capability/runtime implementation in object/link identity. | Bidirectional semantic-to-byte and byte-to-semantic v9 goldens pin the certificate position and both tags; v8, unknown tag, generic/body-presence mismatch, truncation, and trailing bytes reject. Direct and concrete/generic carrier summaries cross every parameter mode, with forged by-value XML roots rejected and borrowed XML roots accepted; producer/consumer signatures and certificates; generic whole/per-unit view/Move parity; two-build determinism; surface/private edit-and-revert cache matrix. |
...
| Array-backed producer closure | `Index`, `IndexField`, `MakeSlice`, `ConstArray`, `StoreIndex`, `StoreElemField`, and `StoreConstArray` have exact array/element/path, readable-index, constant-element, and source-access equations, including static `str` sharing. | Fixed and static string arrays exercise direct element, field, and slice-view forwarding without importing XML. Malformed slot/element/result/index/count/type and constant-element mutations reject before LLVM. |
...
The second producer-closure review reopens four coupled cells:

| Cell | Implementation closure | Owner evidence |
|---|---|---|
| Whole container and element paths | A fixed-array `MakeSlice` has both an exact whole-view equation and selected-element equations. A whole struct element assembled by `StoreElemField` validates every stored field equation and derives readability from its independent local inline storage. | A numeric fixed slice passed to a helper returning `str`, and a numeric struct-array element passed whole to such a helper, certify in whole/per-unit builds; path/result/field mutations reject. |
| Storage authority versus payload provenance | A canonical borrowed descriptor authenticates two facts: payload readability/callable identity from its selected producer path, and write/exclusive authority from its root storage. BorrowMut on a Copy carrier requires readable payload plus writable exclusive storage; BorrowMut on a Move carrier still requires the selected payload itself to be exclusive. Out requires only canonical writable exclusive storage. | Shared Copy payload copied into a fresh mutable local may be replaced through BorrowMut. The same carrier rooted in a shared parameter, and a shared Move payload copied into any local, reject. Owners cross local/ByValue/Borrow/BorrowMut/Out roots with Copy/Move carriers. |
| Physical inline-slot kind | `Index`, `IndexField`, `MakeSlice`, and their store equations admit only the exact inline fixed-array families that LLVM's slot GEP consumes: `Ty::Array` and `Ty::StructArray`. Slice, dynamic-array, box, builder, and task slots reject even when their semantic element type matches. | Each non-inline container is substituted into an otherwise valid indexing graph and must reject before LLVM; exact scalar and struct fixed arrays pass. |
...
The following revised implementation still stopped the proof at function boundaries, separated
check-only access requirements from validity, published several output seeds before validating their
producer equations, and classified valid `CloneIn` and other semantic carriers as rejected.
...
| Producer-graph implementation review P1: function parameters and results re-trusted declaration types, so a raw callable or self-owned value could be laundered across a local or imported function boundary. | One MIR-owned interprocedural validator jointly certifies each local call-graph SCC under provisional exact contracts, checks every actual return and call edge, and keeps that coinductive contract proof distinct from intra-function value provenance. A non-generic interface record carries exact `ValidatedBody=1` only after producer validation; a generic record carries `RevalidateGenericBody=0` and is validated after instantiation. Calls consume a certified local/imported target, never a declaration alone. | Whole/per-unit direct, indirect, wrapper, returned-callable, self-owned return, borrowed-root return, cleanup, self/mutual non-returning recursion, raw/contradictory SCC branch, certificate codec, and producer/consumer mutation owners close both call directions. |
| Producer-graph implementation review P1: validation-only edges checked type/presence but silently treated `Out` as readable. | Every dependency states a product of read/write/exclusive/move and certified-callable requirements. The exact mode × Copy/Move/callable map makes `Out` an unreadable write-only destination and `BorrowMut` readable+writeable+exclusive; a readable fact can be published only through a successfully validated producer equation. | Product owners mutate conditions, lengths, fresh inputs, captures, calls, and runtime operands in whole/per-unit builds; Shared→BorrowMut, Out→BorrowMut, and every Out read reject. |
| Producer-graph implementation review P1: template, grouped view, and runtime out-slot facts were seeded independently of complete producer validation. | Result and out-slot facts are outputs of one atomic semantic rvalue equation containing exact result, all inputs/options, primary definition, slot, and access relation. There is no discriminant-only producer scan. | Every affected rvalue receives independent result/operand/option/slot mutations; an invalid or absent primary equation never seeds its slot. |
| Producer-graph implementation review P1: the syntactic rejected bucket contained valid `CloneIn` and could omit other protected carriers. | Classification is by actual result/out-slot semantics. Protected-leaf-capable variants require explicit equations; `Irrelevant` is structurally guarded. `CloneIn` validates same-shaped input, arena handle, and arena-rooted readable result so a following clone yields owned parse input. | Compile-time and generated semantic inventories plus positive whole/per-unit owners cover every current protected carrier and `CloneIn(...).clone()`; guarded irrelevant and per-field mutations fail closed. |
...
| Producer-closure re-review P1: `MakeSlice` rejected an empty selection path, so an unprotected numeric slice argument could invalidate a helper whose result was protected. | A fixed-array view has a separately validated whole-view equation; selected element provenance remains path-specific. | A numeric fixed array passed as `slice<i64>` to a function returning `str` certifies, while a forged length or slot kind rejects. |
| Producer-closure re-review P1: one access state conflated a local Copy binding's write authority with the borrowed provenance of a view stored inside it. | Borrowed descriptors authenticate storage authority and selected payload provenance separately. Copy BorrowMut composes storage write/exclusive with payload read; Move BorrowMut requires payload exclusivity. | Fresh-local/shared-payload, shared-root, Move-root, Borrow, BorrowMut, and Out cases form one parameterized owner product. |
| Producer-closure re-review P2: a whole struct-array element had no equation when construction used only `StoreElemField` records. | The whole element validates all contributing field stores and derives readability from the exact local inline array slot; selected field paths continue to follow only their field producer. | Numeric-only whole elements passed to protected-result helpers certify; malformed field paths/types reject. |
| Producer-closure re-review P2: semantic element selection admitted slice/dynamic/box slots to `Index`, although LLVM performs an inline-array slot GEP. | One exact physical-slot predicate is shared by index/view/store equations and accepts only `Ty::Array`/`Ty::StructArray`. | Mutation owners substitute every other element-bearing slot family and require pre-LLVM rejection. |

The full post-redesign review reopens the existing-producer closure as one axis:

| Cell | Implementation closure | Owner evidence |
|---|---|---|
| Dynamic container projection | `SoaColumn`, `IndexFieldPtr`, `IndexPtr`, and every sibling dynamic container operation capable of carrying a protected leaf have exact result/base/nominal/field/index equations. A selected field or element path maps to only the corresponding base path; unrelated fields remain validation-only. | Non-XML dynamic AoS and SoA programs return and forward string-bearing fields and whole elements. Mutating the base/result/struct id/field/index or selected path rejects before LLVM in whole/per-unit builds. |
| Subslice selected-path preservation | `SubSlice` validates its whole relation once and maps an empty path to the whole base while mapping `Element` and deeper selected paths to the same base element path. Range operands remain readable validation-only dependencies. | `slice<str>` subslice followed by element indexing and slice forwarding passes; forged result/element/base/range/path records reject. |
| Leaf-local transfer authority | By-value mode derives `move_value` from the selected leaf type, not from an enclosing carrier that may be Move because of a different sibling. Container movement and cleanup remain independently validated by ordinary MIR ownership. | A Move struct containing owned `string` plus a shared `str` passes by value; a shared owned-string leaf and forged Copy/Move leaf classifications reject. |

| Post-redesign full review P1: dynamic AoS/SoA string-bearing projections remained in `Irrelevant`, so unconditional validation rejected valid non-XML producers. | The semantic inventory owns exact equations for every protected-capable dynamic projection and audits its sibling class rather than enumerating only the reported variants. | Dynamic AoS field/whole-element and SoA string-column positive owners plus per-field mutations run in whole/per-unit mode. |
| Post-redesign full review P2: `SubSlice` admitted only the whole result path and rejected a selected string-element path. | Subslice path transfer preserves the exact selected element/deeper path and excludes range operands from provenance. | Subsliced `slice<str>` indexing and forwarding pass; path/type/range mutations reject. |
| Post-redesign full review P2: by-value leaf checks inherited Move authority from the enclosing carrier. | Transfer authority is computed per selected type; only a Move leaf requires ownership, while Copy views require readability. | Mixed Move/Copy carrier owners distinguish an owned string sibling from a shared string-view sibling. |
| Existing-producer repair review P2: `MakeDynArray` assumed scalar/AoS layout and omitted column-major allocation and stores. | A SoA result binds the exact `SoaAlloc` nominal identity and length; selected fields follow matching typed `StoreColumn` writes. Allocation authority remains separate from stored string provenance. | Numeric and string-bearing `to_soa` consumers pass in whole/per-unit builds; allocation/store nominal mutations reject. |
| Existing-producer repair review P2: buffer elements produced by runtime column aggregation had no explicit pointer-store equation. | Numeric and string-key column aggregators contribute typed buffer writes alongside `PtrStore`; key views follow only the source key column, while numeric aggregate outputs carry no borrowed payload. | Numeric grouped results feeding a string-producing helper and grouped string-key cloning pass; forged runtime output identity/type rejects. |

| DB owner sweep: empty fixed-array views were treated as unseeded element graphs. | An exact zero-length `MakeSlice` has no payload producer; its readable empty-view seed is conditional on the physical slot type and exact zero length. Nonempty views still require their element producers. | Empty string-bearing arguments certify; changing the view length from zero to one rejects. Dynamic DB execution owns the source-reachable empty enum-slice case. |
| DB owner sweep: generated column-batch row/SoA views were classified irrelevant despite string-bearing results. | `ColumnBatchRow` and `ColumnBatchSoa` validate exact result, batch resource/row identity, payload, owner, and index operands, then expose shared reader-owned fields. Existing static callback validation retains the full producer-owned callback contract. | `pkg_db_q1::generated_runtime_data_is_producer_owned`, existing column-batch mutation owners, and the provisioned fourteen-suite DB gate. |

| DB owner sweep: a returned dependent Move resource inherited shared access from its retained parent region. | A certified Move return owns its shell; its region dependency still constrains lifetime but does not remove transfer/exclusive-borrow authority. Selected Copy view leaves continue to follow input roots. String/XML cleanup-bit checks remain unchanged. | `resource_ownership::returned_dependent_resource_retains_its_own_mutable_authority` covers whole/per-unit return then BorrowMut; existing parent move/mutable-borrow negatives preserve lifetime exclusion. |
