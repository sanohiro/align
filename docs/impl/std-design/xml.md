...
| MIR and runtime selection | Preserve typed operation/result/event/region facts, add all eight keys and only select reachable rows, reject wrong operation/result/enum/provenance before LLVM, and keep whole/per-unit parity. All seven callable XML operations are semantic MIR rvalues that return their final source result; no raw runtime status or native out slot escapes into generic MIR control flow. XML preflight derives one inseparable `(exact type, access class)` fact from producers, never combining access with an independently trusted side-table type. Function arguments begin as `Owned` for `ByValue`, `Shared` for `Borrow`, `Exclusive` for `BorrowMut`, and unreadable for `Out`. A fresh allocating producer begins `Owned`; a direct, indirect, or imported call result begins `Owned` only when its validated callable signature, return provenance, and required dynamic-cleanup companion jointly prove transferred ownership. Every admitted edge validates its complete type relation before propagating access: `Use` requires operand and result equality; store/load requires the operand or result to equal the resolved selected slot-path type; field, tuple, variant, option, and result construction/projection derives the selected child type from the canonical aggregate definition and validates every constructor arity and operand even though unrelated access facts are not merged; a control selection requires exact `bool` condition and equal alternative/result types but excludes the condition from provenance; and a call validates its canonical parameter modes, every argument, return type, selected return path, and cleanup companion. No cast, reinterpretation, `raw`, or otherwise unrelated producer can establish a `string` or `xml.reader` fact. Provenance is structural per selected aggregate path: field/tuple/variant/option/result projections follow only their selected component, while `if`/`match`/loop joins merge only alternative producers of that selected value and never their condition or unrelated siblings. Call projection first derives the selected return-path type: a selected self-owned `string`/`xml.reader` component is `Owned` only with its exact dynamic-cleanup return, while borrow/region roots apply only to a selected borrow-bearing component. A borrow-rooted selected result keeps its shared/exclusive class; mixed or unknown alternative producers reject. Validation uses an iterative memoized worklist over finite value, slot, and structural-path nodes and computes a monotone fixed point: seeded cycles converge with their seed, while missing nodes, duplicate definitions, unseeded cycles, or ill-typed nodes become `Unknown` without recursive descent or panic. `XmlParse` requires `Owned`, `XmlNext` requires `Owned` or `Exclusive`, immutable getters accept `Owned`, `Shared`, or `Exclusive`, and readable integer indices additionally admit an exact typed constant; every borrowed-place descriptor remains noncanonical. This rejects a wrong-mode or foreign-pointer argument even after laundering through a forged value, slot, wrapper, or projection. The semantic shape `XmlParse { input, error_enum, cleanup }` makes its `Result<xml.reader,error>` primary value and a distinct boolean cleanup value one atomic MIR definition, like the existing cleanup-returning calls. Preflight requires the cleanup id to be in range, `bool`, distinct from the primary id, absent from every other definition, and defined exactly once by that `XmlParse`; lowering attaches it to the Result, while codegen alone maps `0` to `(Ok(reader), true)` and `-1` to `(Error.Invalid, false)`. `XmlNext` carries the canonical event enum id and returns `Option<xml.event>`; the other operations return their exact final `str`, `string`, or bounded `i64` result. | Parameterized producer mutations cross `ByValue`/`Borrow`/`BorrowMut`/`Out`, same-layout `raw` and other wrong-typed arguments, fresh/direct/indirect/imported call results, wrapper/projection and control-result paths, typed and forged `Use`/store/load/field edges, joins, constants, and every borrowed-place family for each operation in whole/per-unit preflight. Projection owners distinguish an owned reader from unrelated shared tuple/enum siblings and a shared-origin condition, including local and cleanup-returning call results, while alternative owned/shared producers still reject as mixed. A long acyclic chain, seeded cycle, unseeded cycle, missing node, and duplicate node own worklist termination and fail-closed behavior. Missing, duplicate, aliased, out-of-range, or wrong-typed parse cleanup ids, result types, cleanup attachment, and error/event enum definition/id mutations reject before LLVM. LLVM/codegen structural owners exclusively prove both status-to-result/cleanup pairings and the default abort; runtime key inventory/bijection, unused import/no-selection, and per-operation exact selection remain exact. |
...
provenance was not structural. The matrix is therefore reopened on the **raw ABI representation
phase**, **interface provenance**, **typed MIR producer relation**, **structural projection**, and
**runtime-status/result identity** axes. Shape-only rejection, post-shape caller preconditions, and
ownership acceptance are distinct phases; cleanup capability is not borrow provenance; XML MIR
authenticates one typed producer fact per selected structural path; and each semantic XML rvalue
owns one final source result while LLVM lowering owns its complete checked status map.
