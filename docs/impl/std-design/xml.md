...
an exact-compatible caller must provide every nonnull pointer with provenance, lifetime,
dereferenceability, and read/write accessibility for its exact range; null is allowed only where the
ABI explicitly says so. An output header is nonnull, aligned, writable,
and exclusive from access not represented by this call. A nonempty parse input is one allocator-
compatible owned range. Supplied argument ranges may overlap only as a mechanically rejected call;
the runtime checks their required pairwise disjointness before access through a Rust reference or
slice. A reader argument addresses one live allocation intended as a shell; getters and count have
shared access, while `next` and nonnull `free` have exclusive access. When the operation will follow
the shell's stored input pointer, that live allocation and exact readable range are also caller
preconditions unless the shell was published unchanged by this runtime. Violating these caller
preconditions has no behavior guarantee; a nonnull integer address alone is not evidence that a
pointer is valid.
...
1. Validate the output-header address/range and the input length, pointer, and representable address
   range. Check that the output header is disjoint from the input range. A detectable failure returns
   `AL_INVALID`, leaves the output untouched, and leaves input ownership with the unsafe caller.
2. Store null to the output and accept responsibility for the input allocation.
...
| Parse construction and failure | Move the selected owned string path exactly once and null its source. Keep unsafe caller preconditions distinct from mechanical range/alias preflight; perform output/input checks before output zeroing, reference/slice formation, or ownership acceptance. Mechanical rejection leaves output untouched and ownership with the caller; compiler-produced public invalid frees input once with null output. Allocate/publish one shell only after complete success and map exactly `-1`. | Direct local/field/tagged/function-result, block/`if`/`match`/`else`/`?`/`map_err`/loop/early-return matrix; invalid corpus; null/alignment/range/output-input alias Cartesian owner; input/shell allocation, untouched/zeroed output, and publication counters. |
...
| LLVM/native ABI | Emit only the eight exact existing-shape declarations, status branches, out-header handling, enum aggregate construction, owned-result cleanup, and reader destructor. Enforce exact-compatible pointer/access/exclusivity preconditions at the unsafe boundary and mechanical range/internal/output-alias preflight before mutation or typed access. Preserve untouched-before-preflight and canonical-zero-after-preflight failure outputs. No hand-written declaration bypass or shape attribute mutation. | Exact declaration/call IR goldens; input/output/shell alias and malformed-field native owner; untouched/zeroed output owner; key/symbol reverse lookup; base/export/collision parity; status/enum exhaustive owners; rt-LTO on/off. |
