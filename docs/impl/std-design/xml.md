...
| MIR and runtime selection | Preserve typed operation/result/event/region facts, add all eight keys and only select reachable rows, reject wrong operation/result/enum/provenance before LLVM, and keep whole/per-unit parity. All seven callable XML operations are semantic MIR rvalues that return their final source result; no raw runtime status or native out slot escapes into generic MIR control flow. XML preflight derives an access provenance lattice from producers, not from an operand's declared type. Function arguments begin as `Owned` for `ByValue`, `Shared` for `Borrow`, `Exclusive` for `BorrowMut`, and unreadable for `Out`. A fresh allocating producer begins `Owned`; a direct, indirect, or imported call result begins `Owned` only when its validated callable signature, return provenance, and required dynamic-cleanup companion jointly prove transferred ownership. Canonical stores, loads, moves, option/result/enum/tuple wrappers and projections, and control joins propagate those facts to slots and values; a borrow-rooted call result keeps its shared/exclusive class, and mixed or unknown provenance rejects. `XmlParse` requires `Owned`, `XmlNext` requires `Owned` or `Exclusive`, immutable getters accept `Owned`, `Shared`, or `Exclusive`, and readable integer indices additionally admit an exact typed constant; every borrowed-place descriptor remains noncanonical. This rejects a wrong-mode argument even after it is laundered through a same-typed value or slot. The semantic shape `XmlParse { input, error_enum, cleanup }` makes its `Result<xml.reader,error>` primary value and a distinct boolean cleanup value one atomic MIR definition, like the existing cleanup-returning calls. Preflight requires the cleanup id to be in range, `bool`, distinct from the primary id, absent from every other definition, and defined exactly once by that `XmlParse`; lowering attaches it to the Result and maps only `0` to `(Ok(reader), true)` and `-1` to `(Error.Invalid, false)`. `XmlNext` carries the canonical event enum id and returns `Option<xml.event>`; the other operations return their exact final `str`, `string`, or bounded `i64` result. | Parameterized producer mutations cross `ByValue`/`Borrow`/`BorrowMut`/`Out`, raw arguments, fresh/direct/indirect/imported call results, wrapper/projection and control-result paths, load/store-derived same-typed values, joins, constants, and every borrowed-place family for each operation in whole/per-unit preflight. Missing, duplicate, aliased, out-of-range, or wrong-typed parse cleanup ids, result types, cleanup attachment, both status-to-result/cleanup pairings, and error/event enum definition/id mutations reject before LLVM; runtime key inventory/bijection, unused import/no-selection, and per-operation exact selection remain exact. |
...
authenticates producer-derived access rather than declared operand type; and each semantic XML
rvalue owns one final source result while LLVM lowering owns its complete checked status map.
