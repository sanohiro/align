...
| Interprocedural producer certification | Move the sealed graph builder to one reusable MIR-owned validator invoked before codegen and before interface publication. Its local-call fixed point validates every reachable `Return`/`ReturnWithCleanup` selected path against the declared return type, borrow/region roots, callable identity, and cleanup mode; no signature or cleanup declaration can certify its own body. A `Ty::Fn` argument is a protected leaf: an argument-mode seed is conditional on every direct/indirect caller proving the exact callable producer, and `FnAddr`, lifted closure, wrapper, store/load, projection, join, and returned callable preserve that identity only through certified local/imported targets. Recursive SCCs converge only from validated parameter, intrinsic, or imported seeds; an unseeded or contradictory call/return cycle is invalid. A local direct-call result may seed facts only after every callee return path is certified. A bodyless imported result may do so only from a format-9 `ValidatedBody` certificate bound to that exact function record. Extern/runtime/raw calls never receive this certificate and remain intrinsic-specific or rejected. | Whole/per-unit owners mutate local return operands and cleanup pairs, direct/wrapped/returned callable producers, callable parameter modes and identities, declaration/body disagreement, imported certificate tag/presence, and each call-graph edge. Positive owners cover direct/indirect wrappers, borrowed and self-owned results, returned callables, generic revalidation, seeded recursion, and serialized producer/consumer parity; unseeded recursion and a raw producer hidden in any local or imported return reject before resource selection. |
| Operand requirements and atomic seeds | Every validation dependency carries one exact requirement: `ShapeOnly`, `Readable`, `WritableDestination`, `MoveOwned`, or `Callable`. `Readable` accepts only present Owned/Shared/Exclusive state; `MoveOwned` only present Owned; `WritableDestination` validates the exact canonical slot/place and its Out/Exclusive write authority but never creates a readable fact; `Callable` additionally requires one canonical certified function identity. Constants are readable. Conditions, lengths, indices, template holes, fresh-producer inputs, and other evaluated operands are `Readable` or `MoveOwned` according to MIR semantics, never shape-only by convenience. Call arguments derive the requirement from their canonical parameter mode. Selected-path access still excludes validation-only dependencies, but their requirement result participates in total validity. A fresh, view, or runtime out-slot fact is published only by the same fully validated rvalue equation that proves exact result type, every operand and option, primary definition, output slot, and access relation. No preliminary scan of a discriminant or output-slot number may seed a fact. | Parameterized whole/per-unit mutations cross all four argument modes and each requirement for conditions, length/index operands, constructors, calls, fresh allocators, template pieces/arena, grouped views, and every runtime out-slot producer. Each result/operand/option/slot/type mutation rejects. Out is accepted only as the matching destination and rejected through every read path. A producer whose primary equation is absent, duplicate, rejected, or invalid cannot make its output slot readable. |
...
| Producer-graph implementation review P1: function parameters and results re-trusted declaration types, so a raw callable or self-owned value could be laundered across a local or imported function boundary. | One MIR-owned interprocedural validator certifies every local return path and callable argument obligation. A non-generic interface record carries exact `ValidatedBody=1` only after producer validation; a generic record carries `RevalidateGenericBody=0` and is validated after instantiation. Calls consume a certified local/imported target, never a declaration alone. | Whole/per-unit direct, indirect, wrapper, returned-callable, self-owned return, borrowed-root return, cleanup, recursive-SCC, certificate codec, and producer/consumer mutation owners close both call directions. |
| Producer-graph implementation review P1: validation-only edges checked type/presence but silently treated `Out` as readable. | Every dependency states an exact shape/read/write/move/callable requirement. `Out` is an unreadable writable destination and can publish a readable result only through a successfully validated producer equation. | Mode × requirement owners mutate conditions, lengths, fresh inputs, captures, calls, and runtime operands in whole/per-unit builds; every Out read rejects. |
