...
| MIR and runtime selection | Preserve typed operation/result/event/region facts, add all eight keys and only select reachable rows, reject wrong enum/type/status before LLVM, and keep whole/per-unit parity. Each XML operand has one canonical MIR form: reader/input operands are existing typed value or argument ids, integer indices additionally admit canonical typed constants, and every borrowed-place descriptor is rejected because the source surface requires a bound reader local. Result ids and output slots are range-checked and type-checked. `XmlNext` carries its canonical event enum id and returns the final `Option<xml.event>` aggregate atomically, so the runtime status table cannot drift into an unauthenticated generic control-flow subgraph. | Parameterized mutations of every operand form, missing/wrong result and output slot, event definition/id/status mapping, and whole/per-unit preflight; runtime key inventory/bijection; unused import/no-selection and per-operation exact selection tests. |
| LLVM/native ABI | Emit only the eight exact existing-shape declarations, status branches, out-header handling, enum aggregate construction, owned-result cleanup, and reader destructor. Inspect address shapes without dereference, then apply provenance/access/exclusivity preconditions only to pointers that pass and would be accessed. For getters, complete pointer-shape/shell-field/internal-range/output-alias preflight failure is untouched; zeroing follows that preflight, and later state/index failures retain zero. No typed input access precedes all validation. No hand-written declaration bypass or shape attribute mutation. | Exact declaration/call IR goldens; null/misalignment/noncanonical-empty/overflow/input-output-shell alias and malformed-field native owner; rejected-pointer no-dereference/no-free owner; sentinel output proves untouched for every preflight magic/scalar/internal-range/alias failure and canonical zero for every post-zero state/index failure; key/symbol reverse lookup; base/export/collision parity; status/enum exhaustive owners; rt-LTO on/off. |
...
three authenticated facts across permissive generic machinery. The matrix is therefore reopened on
the **raw ABI representation phase**, **interface provenance**, **MIR operand formation**, and
**runtime-status/event identity** axes. Shape-only rejection, post-shape caller preconditions, and
ownership acceptance are distinct phases; cleanup capability is not borrow provenance; XML MIR
accepts only its canonical operand forms; and one semantic `XmlNext` owns the complete status map.
...
| Implementation re-review P2: XML MIR operand checks trusted the declared type inside forged borrowed-place descriptors. | The XML preflight admits only the canonical value/argument forms produced for bound reader locals, plus typed integer constants for indices. It rejects every borrowed-place family rather than trusting embedded type fields, and range/type-checks every referenced result and slot before LLVM. | Every XML rvalue is crossed with absent/wrong values, arguments, slots, constants, and all borrowed-place families in both whole and per-unit preflight. |
| Implementation re-review P2: `XmlNext` status identity was split from generic comparisons, branches, and enum construction, allowing valid-typed Start/End swaps. | `XmlNext` becomes one semantic MIR operation carrying the canonical event enum id and returning `Option<xml.event>` directly. LLVM lowering implements exactly one checked status switch; no generic MIR node separately owns the mapping. | Status constants `0..=3`, invalid status, enum id/shape, and every event ordinal are mutated independently; whole/per-unit output remains identical. |
