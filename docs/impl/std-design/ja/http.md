...
- `http_read_stream` と `http_sse_stream` は dependent **Move** resource である。それぞれ一つの checked-out
  ...
  local/parameter/return として nameable で、builtin `Option`/`Result` の有限な nesting だけを carrier として
  許す。この carrier は active stream の client provenance を継承し、active payload だけを recursive に
  move/drop する。user struct field、user-sum payload、array/slice/box element、closure/task capture、またはそれらを
  作る generic substitution の reachable type 内には stream を置けない。`request_stream` は最初の stream を
  `Result` 内に作り、`sse()` は同じ owner/borrow を `http_sse_stream` へ移し、source を null にする。
...
| Ownership and cleanup | 両 stream type は Move の borrow-mutated cursor で shared client borrow を保持する。client pool は他の shared call に利用できるが stream より先に move/drop できない。`sse()` は全 connection/scratch/head/control owner と cleanup bit を一度だけ transfer。bare/builtin `Option`/`Result` construction、success、`?`、`else`、`match`、`map_err`、replacement、return、ordinary Drop は一つの owner と client provenance を保つ。user aggregate と captured/parallel carrier は拒否する。stream は creating client、event string は output-buffer generation を越えて escape できない。 |
| Carrier formation and provenance | stream は bare または builtin `Option`/`Result` の有限 nesting 内だけに置ける。`Some`/`Ok` は owner と creating-client dependency を active payload に格納し、`None`/`Err` は stream を所有しない。active payload の move/pattern unwrap は destination 初期化後に complete source tag/handle を clear する。replacement/Drop は全 nested tag を検査し、存在する active stream を exactly once release。direct/imported parameter/return summary は client dependency を保ち、control-flow join は可能な creating client を union するため、joined carrier の consume/drop より前にどちらも死ねない。formation は user struct/user sum/array/slice/box/closure-task capture/parallel element-result/forbidden generic instantiation 下の stream を recursive に拒否する。 |
...
|---|---|---|
| Formation and type classes | exact method/arity/receiver/output type、bound client/request/buffer、各 stream の bare/builtin `Option`/`Result` finite nesting acceptance、user struct/sum/array/slice/box/closure-task capture/parallel element-result/forbidden generic substitution 下の recursive rejection、三 view + inline Copy retry を持つ event nested Option record。 | sema + checked-HIR parameterized reachable-type sweep |
...
| Carrier provenance, move, and Drop | direct/nested `Option`/`Result` Some/None/Ok/Err、two-client join、direct/imported/generic parameter/return、move-in/out、pattern unwrap、`?`/`else`/`match`/`map_err`、replacement、branch/loop/early return、active-tag Drop が exact client dependency を保持し、destination 初期化後 source clear、close/pool once。user aggregate/capture/parallel negative owner は全 forbidden reachable shape を recursive に拒否。 | sema provenance + checked-HIR mutation sweep + MIR cleanup + runtime live counter |
| Ownership, replacement, and return | raw→SSE transfer source null、bare/builtin carrier が生存中の client Drop/move rejection、other shared client request acceptance、next/replacement/Drop/escape 後の output string view rejection と inline retry の ordinary survival、全 allowed exit の exact-once Drop。 | sema provenance owner + MIR cleanup + runtime live counter |
...
| Generic/imported/per-unit | direct/imported helper、admitted bare/builtin carrier だけの generic forwarding、dependency-summary substitution、forbidden carrier を作る substitution の recursive rejection、whole/per-unit interface transport、cache edit/revert、malformed effect/carrier/interface/checked-HIR の LLVM 前 rejection。 | interface/cache/validation owner |
...
1. parameterized dependent-stream carrier owner と `http_read_stream`
   ...
   対する bare/builtin-carrier provenance、active-tag Move/Drop、forbidden reachable carrier、effect、
...
は caller が受け取らなかった event を skip し得る。したがって上記 boundary は carrier provenance を first-PR
substrate、SSE block state を second-PR transaction とし、どちらの proof も後続 fixup に分散させない。
