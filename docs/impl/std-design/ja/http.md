...
このワークロードには利益が少ない)。操作ごとと文書化する。ハングしたリクエストが logical wait deadline へ達した後に
test の bounded detector tolerance 内で返る gate を、最も単純な表面で満たす。scheduler/kernel delay の厳密な
public wall-clock ceiling は約束しない。
...
正の `Duration` budget を記録し、その call と `F_SETFL(flags | O_NONBLOCK)` の両方を check する。
どちらかが失敗すれば `connect` 前に close/continue する。immediate call は zero を success、
`EINPROGRESS`/`EAGAIN`/`EWOULDBLOCK` を in-progress、その他の errno を terminal failure に分類し、どちらの immediate
terminal result も同時の budget exhaustion より優先する。absolute `start + budget` は作らないため、
`Instant::checked_add` overflow がどの正の i64 timeout も unbounded wait に変えられない。各正の remainder は
次の millisecond へ切り上げ、1回の `poll` に対し `i32::MAX` で saturate する。remainder が尽きて
いれば次の poll 前に `AL_TIMEOUT` を返す。`EINTR` は remainder を再計算する。poll の zero result は新しい
monotonic check を行い、時間が残る場合だけ re-poll する。`poll` が既に返した正の readiness/error event は
同時の budget exhaustion より優先し、`SO_ERROR` で解決する。immediate/polled success はどちらも HTTP が
socket を受け取る前に checked `F_GETFL` + `F_SETFL(flags & !O_NONBLOCK)` restoration を必要とし、
restoration failure はその candidate を close して次の address へ進む。

同じ prerequisite は、正の各 HTTP socket timeout を `ceil(ns / 1000)` microseconds に変換し、
`SO_RCVTIMEO` と `SO_SNDTIMEO` の両方で normalized
`timeval { tv_sec, tv_usec: 0..999999 }` を使う。exact microsecond は exact のまま、次の nanosecond は切り上げ、
最大 admitted value も表現可能で、zero は reused connection 上の shipped clear/block-forever の意味を保つ。
これらの変換は logical timeout を早期失効させない。scheduler/kernel delay により operation の return が
それより遅くなることはある。

acceptance owner は exact/next/maximum ns-to-ms と ns-to-us boundary、failed checked mode
...
**コネクションを accept するが決して応答しない**エンドポイントへのリクエストは、configured logical wait
deadline へ達した後に `Err(Timeout)` を返し、無期限にブロックしない(Request-2 の受け入れゲート)。
ブラックホール化された(決して accept しない)アドレスへの connect も同様。test は stall を検出する十分な upper
tolerance を使うが、それは public wall-clock guarantee ではない。`ns == 0` は現在のブロッキング挙動を保つ。通常の高速リクエストは影響
...
| Structural metadata | live な interim/final header table は最大一つ、survive する offset は final header だけ。trailer は bytes/offset 無しで final header-count remainder を消費。chunk-size line は scalar incremental grammar state で検証し、raw-line staging buffer を保持しない。decoder state は constant-size。body length、peer magnitude、chunk count、trailer volume 由来の capacity は無い。 |
...
| Fixed-length receive | leading-zero/equal-duplicate CL、exact cap、cap+1、above-target/global、malformed/conflicting/truncated precedence、excess declaration 後の reserve/read 無し。 | runtime parser/transport matrix |
| Chunked receive | exact/cap+1、tiny chunk、oversized valid magnitude、invalid extension/guard、co-read excess、limit 後の trailer read 無し、terminal trailer/reuse。 | Request 4 decoder owner + counter |
| Close-delimited receive | exact/cap+1、same/split read、one-byte probe、decision 後の read 無し、non-reuse。 | plaintext/TLS transport fixture |
...
| SSE storage | `next` は current block の de-framed source byte を一つの reusable stream-owned work allocation に retain する。bound はその call の output capacity `C` と `HTTP_MAX_SSE_METADATA` の和。blank line が commit boundary で、parser は追加 allocation 無しに retained block を scan し、field の validation/replacement と exact final event/data/ID size を計算してから、未公開 caller allocation に canonical order で successful event を materialize する。control-only block は retained state を update して work buffer を clear、data block は caller buffer を publish、terminal incomplete input は commit 無しに clear する。work allocation は新たに必要な call bound へ exact grow して reuse され、最大 supplied capacity が安定した後は event-frequency allocation が無い。stream はこれと別に committed last-event-id の一つの exact-capacity allocation と inline retry state を retain する。old ID capacity を `K`、commit candidate ID length を `N`、current output capacity を `C` とする。`N > C` は mutation 前に `Error.Code(-1)`。`N > K` なら exact に `K' = max(N, min(C, saturating_mul(K, 2)))` を allocate/initialize してから、まだ live な old allocation を free する。zero からは直接 `N` へ grow する。`M` をこれまで供給された最大 output capacity とすると、retained ID capacity は最大 `M`、retained block-work capacity は最大 `M + HTTP_MAX_SSE_METADATA`、steady capacity 合計は最大 `2 * M + HTTP_MAX_SSE_METADATA`。work-buffer growth と retained ID は厳密に `3 * M + 2 * HTTP_MAX_SSE_METADATA` 未満、ID growth と retained work buffer は厳密に `3 * M + HTTP_MAX_SSE_METADATA` 未満。後の小さい buffer はどちらも shrink せず、dispatch event は current call capacity に収まらなければならない。response-leading 32 KiB transport scratch と fixed head allocation は raw mode と共有する。 |
...
| Allocation | construction は一つの fixed `HTTP_MAX_HEADER_BLOCK = 262,144` final-head allocation、一つの fixed `HTTP_CLIENT_READ_CHUNK = 32,768` scratch、header-span table、checked-out connection を所有し、raw stream 内で body byte allocation は無い。request serialization scratch は construction 直後に client へ戻る。SSE は current largest `C + HTTP_MAX_SSE_METADATA` に bound された一つの reusable raw-block work allocation、current largest `C` に bound された一つの committed-ID allocation、inline parser/retry state を追加する。work allocation は prior block-local candidate を壊す前に後続 replacement field を validate するため必要である。例えば末尾近くの NUL により全体を ignore すべき `id` line は、同じ block の earlier valid `id` を保持しなければならない。一つの `C`-byte caller output だけを使う one-pass parser は arbitrary な二つの alternative を同時に retain できない。exact steady/growth ceiling は上の `SSE storage` row に従い、caller-owned output buffer、fixed handle/table metadata、allocator bookkeeping、opaque kernel/libssl storage を除く。peer Content-Length/chunk magnitude/cumulative body length/SSE field length は fixed または caller-selected bound を越える capacity を生じさせない。OOM は fatal/no-unwind。 |
...
| Control-state allocation | `C + 262,144` に bound された一つの retained raw-block work allocation、allocation-free block scan、`out.len == 0` の caller materialization、latest committed ID overwrite、earlier same-block candidate を失わない NUL/invalid retry ignore、repeated control-only commit、一つの exact persistent-ID allocation、checked candidate length、exact capacity growth/reuse、両方の old+new transient high water、smaller later output、rollback/error/Drop free、largest supplied capacity 安定後の event-frequency allocation 無し。 | allocation/failpoint/high-water instrumentation |
...
| ABI and layout | read-window capacity getter を含む ten declared symbol、shared free/head ABI、SSE state getter、64-byte envelope offset/reserved zero、null/malformed-input sentinel と validation order、output clearing、pre-consumption rejection、null-safe free、signed private status mapping、cross-target pointer/`i64` layout。 | ABI ledger tripwire + malformed direct-call matrix + MIR/LLVM structural assertion |
| Resource parity | fixed 262,144 head + 32,768 transport scratch、raw body allocation 無し、operation-local framing/SSE-work counter、output capacity と fixed metadata allowance に bound された reusable SSE block work、output capacity に bound された committed control storage、両方の old/new growth layout accounting、unbounded peer-derived allocation 無し、全期間一つの connection owner。 | runtime high-water/allocation instrumentation |
...
`respond_upgrade` は builder だけを consume する。byte write/fd ownership move 前の順序は HTTP/1.1、exact
status 101、body absent、既存 insertion-order header guard、nonempty token-list Upgrade row exactly one、
Upgrade token を持つ valid Connection row exactly one、Content-Length absent、Transfer-Encoding absent。
validation failure は ctx unspent。validation 後に head を serialize、fd を lift、ctx を
spent にして complete head を書く。success だけ Move handle を publish する。write failure は fd を close して ctx
spent のまま。ctx は pump view の request buffer を保持し、upgraded fd を park しない。

`http_upgrade` はその fd と sticky terminal error state を所有する。local/pump-only Move carrier で、direct
constructor Ok、by-value/borrow/borrow-mut parameter、local は可。user return、他の tag、aggregate、collection、
box、global、out、extern、capture、task、parallel value は拒否する。`read_exact` は mutable bare-local buffer と
...
idempotent。stateful failure は
close/poison し、後続 call は I/O 無しで同じ builtin Error を返す。Drop は protocol write 無しの close-only。

九 keyed row は `HttpHeadersCount` A37、`HttpHeadersTokensValid` A20、
...
test contract は `../pkg-design/ws.md`。
