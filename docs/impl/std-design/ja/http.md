...
events.next(out: mut buffer) -> Result<Option<http_sse_event>, Error>
                                             // WHATWG event。view は out の fresh generation を borrow
...
- `http_read_stream` と `http_sse_stream` は dependent **Move** resource である。それぞれ一つの checked-out
  ...
  は他の shared-borrow request operation に使えるが、stream より先に move/drop はできない。両 type は
  nameable で `Option`/`Result` payload として有効だが、element read が owner を duplicate し得る
  array/slice/box element にはできない。`request_stream` は最初の type を `Result` 内に作り、`sse()` は同じ
...
| SSE fields and state | `data` は value と一つの LF を append、`event` は current event type を replace、`id` は decoded value に U+0000 が無い場合だけ persistent last-event-id を replace し、含む場合は ignore する。`retry` は nonempty ASCII digit が `0..=i64::MAX` を表す場合だけ persistent reconnect delay を replace し、その他は ignore。unknown/case-mismatched field は ignore。data/event type は各 blank-line dispatch attempt 後に reset、last-event-id/retry は persist。初期値は empty/`None`。`events.last_event_id()` は current value の stream-bound view、`events.retry_ms()` は current Copy value を返し、control-only block の update も後で `next` が `None` を返す場合まで含めて観測できる。last-event-id view は value を replace し得る後続 `next` を越えられない。 |
| Dispatch and output | 少なくとも一つ `data` field がある blank line は `Some(http_sse_event)` を返す。`data` の無い block は persistent control state を更新するが何も返さず、同じ call で parse を続ける。accumulated data 末尾の LF を一つ除く。empty event type は `"message"`、それ以外は exact preserve。`last_event_id`/`retry_ms` は block 後の persistent state snapshot。parse 前に `out.len = 0`。`Some` では `out` が exact UTF-8 bytes を `event || data || last_event_id` 順に持ち、三 field はその fresh generation の zero-copy span。exact capacity は成功し、capacity を越えて必要になる最初の decoded/defaulted/copied state byte は `Error.Code(-1)`、stream close、`out.len == 0`、event 非公開となる。これが `id`/`event` control-only block も bound する。 |
| Empty and terminal cases | `data`、`data:`、一つの optional space が続く `data:` は empty `data` event を dispatch でき、二つの empty data line は一つの LF を dispatch する。comment と empty/control-only block は `Some` を作らない。final blank line 無しの HTTP completion は pending data/event block を discard し、partial event を一切公開せず `None` を返して normal connection verdict を適用する。以後の `next` は I/O 無しで常に `None`。 |
| SSE storage | event data は caller `out` へ直接書き、`next` は grow せず per-event body allocation を行わない。stream が保持できるのは latest last-event-id と current event-type byte string だけで、各々を admit した最大 `out.capacity` に bound し、再利用後 stream Drop で free する。UTF-8 carry は最大三 byte、line/parser counter は fixed-size。response-leading 32 KiB transport scratch と fixed head allocation は raw mode と共有する。 |
| Errors and precedence | request/setter validation が network work より先。received head/framing syntax と fixed guard が body-cap check より先で、complete valid magnitude が conversion/accounting より先。SSE parse 中は HTTP framing/transport outcome を newly available decoded byte の interpretation より先に決め、UTF-8 replacement と line/field interpretation を caller-cap comparison より先に行う。recognizable explicit cumulative body-cap excess は後続 body byte 公開前に勝ち、caller output-cap excess は event 公開前に勝つ。framing/truncation/`u64` excess は `Error.Invalid`、deadline は `Error.Timeout`、transport/TLS は既存 category、どちらの explicit cap も `Error.Code(-1)`。terminal error は connection を poison/close し、以後は I/O 無しで同じ stored error を返す。 |
| Allocation | construction は一つの fixed `HTTP_MAX_HEADER_BLOCK = 262,144` final-head allocation、一つの fixed `HTTP_CLIENT_READ_CHUNK = 32,768` scratch、header-span table、checked-out connection を所有し、raw stream 内で body byte allocation は無い。request serialization scratch は construction 直後に client へ戻る。SSE は上記二つの capacity-bounded control string だけを追加し、event byte は caller storage に置く。kernel/libssl buffer は opaque transport storage であり、peer Content-Length/chunk magnitude/cumulative body length/SSE field length から capacity を導出してはならない。OOM は fatal/no-unwind。 |
| Ownership and cleanup | 両 stream type は Move の borrow-mutated cursor で shared client borrow を保持する。client pool は他の shared call に利用できるが stream より先に move/drop できない。`sse()` は全 connection/scratch/head/control owner と cleanup bit を一度だけ transfer。success、`?`、`else`、`match`、`map_err`、replacement、client provenance を伴う return、ordinary Drop のすべてで owner は一つ。stream/event は creating client/output-buffer generation を越えて escape できない。 |
...
| Compiler/runtime owner | Sema は exact method/type、bound receiver/output check、Move transfer、client/output provenance、event field type を所有。checked HIR は全 new envelope と type/region fact を独立検証。MIR は construction/head access/read/conversion/next/status mapping/source nulling/Drop。LLVM は ABI declaration/call と Result/Option/event reconstruction。runtime は URL/request work、pool/TLS connection、framing state、cap/timeout accounting、SSE decoding、buffer write、cleanup。package owner は無い。 |
...
| Prerequisite and acceptance | Request 4 framing、Request 5 explicit cap、client TLS/pool/timeout、caller-owned `buffer`、nested `Result<Option<Record>, Error>`、dependent-resource provenance が shipped prerequisite。raw acceptance は fixed/chunked/close/bodyless/interim body を plaintext/TLS の every split boundary で stream し、bounded memory と pool/Drop fate、explicit cap 無しの 1 GiB 超 download を証明。SSE acceptance は WHATWG example に加え BOM/invalid UTF-8、全 line ending/split、empty/multiline/control-only event、id NUL/reset/inheritance、retry valid/invalid/overflow、unknown/case field、exact output cap/cap+1、incomplete EOF、cumulative body cap、reconnect-policy absence を持つ。 |
...
|---|---|---|
| Formation and type classes | exact method/arity/receiver/output type、bound client/request/buffer、両 Move type の named `Option`/`Result` payload acceptance と copy/array/slice/box rejection、event nested Option record、shared-client/fresh-buffer generation propagation。 | sema + checked-HIR parameterized type sweep |
...
| Explicit/unset limits | whole-body unchanged、streaming unset で 1 GiB 超、positive CL constructor exact/excess、chunked/close later exact/excess、SSE output capacity exact/excess、private negative status と byte count の non-alias。 | runtime cap table + MIR/LLVM discriminant owner |
| SSE interpretation | official example と complete BOM/UTF-8/line/field/blank/EOF Cartesian case、state reset/persistence、raw-prefix transition、output concatenation/span、invalid-byte replacement expansion/cap。 | parameterized runtime parser oracle + driver event consumer |
| Control-state allocation | latest id/event overwrite、NUL/invalid retry ignore、repeated control-only block、capacity growth/reuse、smaller later output、error/Drop free、no data allocation、fixed carry/counter。 | allocation/failpoint instrumentation |
| Ownership, replacement, and return | raw→SSE transfer source null、stream 生存中の client Drop/move rejection、other shared client request acceptance、next/replacement/Drop/escape 後の output view rejection、`?`/`else`/`match`/`map_err`/branch/loop/early return の exact-once Drop。 | sema provenance owner + MIR cleanup + runtime live counter |
...
| Generic/imported/per-unit | direct/imported helper、legal generic forwarding、whole/per-unit interface transport、cache edit/revert、malformed interface/checked-HIR の LLVM 前 rejection。 | interface/cache/validation owner |
| ABI and layout | nine declared symbol、shared free/head ABI、SSE state getter、64-byte envelope offset/reserved zero、null/error output、signed private status mapping、cross-target pointer/`i64` layout。 | ABI ledger tripwire + MIR/LLVM structural assertion |
| Resource parity | fixed 262,144 head + 32,768 scratch、raw body allocation 無し、explicit output capacity だけに結びつく SSE control storage、peer-derived transport/parser allocation 無し、全期間一つの connection owner。 | runtime high-water/allocation instrumentation |
...
1. `http_read_stream` construction/head/raw read、explicit-cap composition、ownership、TLS、pool closure。
   large download と任意の incremental protocol consumer に直ちに有用である。
2. consuming `http_sse_stream` transition、WHATWG parser、event projection、caller-buffer cap。
   最初の PR の唯一の transport/framing owner を再利用し、dormant な第二 decoder を増やさない。
