...
`fs.open_regular(path: str) -> Result<reader, Error>` は Impure で Linux/macOS に対応する。
通常の相対・絶対パス、ドット成分、シンボリックリンクを解決する。読み取り専用かつ
非ブロッキングで開き、開いた記述子が通常ファイルであることを検査し、ブロッキングを
復元して同じ記述子を所有する reader を返す。通常ファイル以外は FIFO の書き手を待たず
`Error.Invalid` となり、通常の open エラーは既存の対応を維持する。UTF-8 と埋め込み NUL
はファイルシステム操作前に検証し、空パスは通常の OS エラーとする。項目の作成や内容の
書き込みは行わない。結果はパスの寿命を保持せず、既存の reader Drop が close-on-exec の
記述子を閉じる。パスが差し替えられた場合も、実際に開いて検査した通常ファイルだけを返す。
サンドボックス、内容の不変性、リモートファイルシステム操作の中断可能性は保証しない。
通常ファイルという種別以外の受け入れ方針はアプリケーションが所有する。
