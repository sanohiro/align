...
| Event/cursor | explicit/synthesized start/end を exact order で emit。maximal adjacent `CharData` と single/consecutive `Reference` を ordinary text 1 個に coalesce。comment、CDATA、child、end-tag boundary で分割し、各 nonempty CDATA は separate。declaration/comment/outside S skip。current state は next まで保持、EOF idempotent。`XmlNext` は `Option<xml.event>` を返す one semantic MIR operation とし、exact `0=None`、`1=Start`、`2=End`、`3=Text`、other=abort mapping と canonical enum id を所有する。generic compare/branch/`MakeEnum` node に identity を分割しない。 | exact `<a>x&amp;y&#33;</a>` と `<a>&amp;&lt;</a>` golden、nested/empty/comment/CDATA/child/whitespace document、repeated getter/EOF、cursor one-field mutation。MIR owner は全 status、enum id/variant、invalid-status edge を mutate し valid event swap を拒否。 |
...
| MIR/runtime selection | typed op/result/event/region を保持、eight keyを追加、reachable row のみ select、wrong enum/type/status は LLVM 前 reject、whole/per-unit parity。各 XML operand は one canonical MIR form：reader/input は existing typed value/argument id、integer index は canonical typed constant も許可し、source surface が bound reader local を要求するため全 borrowed-place descriptor を拒否。result id/output slot は range/type check。`XmlNext` は canonical event enum id を持ち final `Option<xml.event>` aggregate を atomically return し、runtime status table を unauthenticated generic control-flow subgraph に分割しない。 | 全 operand form、missing/wrong result/output slot、event definition/id/status mapping の parameterized mutation と whole/per-unit preflight、key inventory/bijection、unused import/no-selection、per-op selection。 |
...
provenance**、**MIR operand formation**、**runtime-status/event identity** axis で再オープンする。
shape-only rejection、post-shape caller precondition、ownership acceptance は別 phase。cleanup capability は
borrow provenance ではなく、XML MIR は canonical operand form だけを accept し、one semantic `XmlNext` が
complete status map を所有する。
...
| Implementation re-review P2: XML MIR operand check が forged borrowed-place descriptor 内の declared type を信用した。 | XML preflight は bound reader local が生成する canonical value/argument form と typed integer constant だけを accept。embedded type を信用せず全 borrowed-place family を rejectし、LLVM 前に全 result/slot reference を range/type check。 | 全 XML rvalue を absent/wrong value、argument、slot、constant、全 borrowed-place family と whole/per-unit preflight で交差。 |
| Implementation re-review P2: `XmlNext` status identity が generic comparison/branch/enum construction に分割され、valid-typed Start/End swap が可能だった。 | `XmlNext` を canonical event enum id を持ち `Option<xml.event>` を直接返す one semantic MIR operation に変更。LLVM lowering は exactly one checked status switch を実装し、mapping を generic MIR node に分けない。 | status constant `0..=3`、invalid status、enum id/shape、全 event ordinal を独立 mutateし、whole/per-unit output identical。 |
