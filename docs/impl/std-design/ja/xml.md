...
current-view region を先に証明。raw ABI entry は全て `unsafe`。exact-compatible caller は全 nonnull pointer に
exact range の provenance、lifetime、dereferenceability、read/write accessibility を提供し、null は ABI が
明記した位置だけ許す。output header
は nonnull/aligned/writable で、この call に表現されない access から exclusive。nonempty parse input は
allocator-compatible owned range 1 個。supplied argument range の overlap は mechanically rejected call
としてだけ許され、runtime は Rust reference/slice access 前に required pairwise disjointness を検査する。
reader argument は shell 用の live allocation 1 個を指し、getter/count は shared access、`next` と nonnull
`free` は exclusive access。operation が shell 内の stored input pointer を追う場合、この runtime が publish
した shell を unchanged で使うのでなければ、その live allocation と exact readable range も caller
precondition。これら caller precondition の違反に behavior guarantee
はなく、nonnull integer address だけでは valid pointer の証明にならない。
...
1. output-header address/range と input length/pointer/representable address range を検査し、output header と
   input range の disjoint を確認。detectable failure は `AL_INVALID`、output untouched、input ownership は
   unsafe caller に残る。
2. output に null を store し、input allocation の責任を受ける。
...
out を持つ getter は最初に output range を確認し、stored input pointer を追わず fixed shell range、magic、
scalar field を検査し、stored shell/input range の internal disjoint と output が両方に disjoint であることを
確認する。detectable mechanical failure は output untouched。この preflight 後だけ `{ptr,i64}` output を
zero し、current state/index を検査して input view を形成する。borrowed/owned output とも later failure は
canonical zero のまま。owned getter は exact decoded length を検査し、once allocate/fill、completed
...
| Parse construction/failure | selected owned string path を exactly once move し source を null。unsafe caller precondition と mechanical range/alias preflight を分離し、output zeroing、reference/slice formation、ownership acceptance 前に output/input check。mechanical rejection は output untouched で ownership は caller、compiler-produced public invalid は null output で input once free。complete success 後だけ shell allocate/publish、exact `-1` map。 | local/field/tagged/result/block/`if`/`match`/`else`/`?`/`map_err`/loop/early-return matrix、invalid corpus、null/alignment/range/output-input alias Cartesian owner、input/shell allocation、untouched/zeroed output、publication counter。 |
...
| LLVM/native ABI | exact existing-shape declaration/status/out/enum/cleanup/destructor のみ。unsafe boundary の exact-compatible pointer/access/exclusivity precondition と、mutation/typed access 前の mechanical range/internal/output-alias preflight を実装。preflight 前 failure は untouched、後は canonical zero を保持。hand-written bypass/shape attribute mutation なし。 | exact declaration/call IR golden、input/output/shell alias と malformed-field native owner、untouched/zeroed output owner、key/symbol reverse、base/export/collision、status/enum exhaustive、rt-LTO on/off。 |
