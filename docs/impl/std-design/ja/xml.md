...
| Interprocedural producer certification | sealed graph builder を codegen/interface publication 前に呼ぶ reusable MIR-owned validator へ移す。local-call fixed point は全 reachable `Return`/`ReturnWithCleanup` selected path を declared return type、borrow/region root、callable identity、cleanup mode と照合し、signature/cleanup declaration が自分の body を certify することを禁止。`Ty::Fn` argument は protected leaf。argument-mode seed は全 direct/indirect caller が exact callable producer を証明する場合だけ有効で、`FnAddr`、lifted closure、wrapper、store/load、projection、join、returned callable は certified local/imported target 経由だけ identity を保持。recursive SCC は validated parameter/intrinsic/imported seed からだけ convergeし、unseeded/contradictory call-return cycle は invalid。local direct-call result は全 callee return path certification 後だけ seed。bodyless imported result は exact function record に bind された format-9 `ValidatedBody` certificate だけを使う。extern/runtime/raw call は certificate を持たず intrinsic-specific または rejected。 | whole/per-unit owner は local return operand/cleanup pair、direct/wrapped/returned callable producer、callable parameter mode/identity、declaration/body mismatch、imported certificate tag/presence、全 call-graph edge を mutate。positive owner は direct/indirect wrapper、borrowed/self-owned result、returned callable、generic revalidation、seeded recursion、serialized producer/consumer parity。unseeded recursion と local/imported return 内 raw producer は resource selection 前 reject。 |
| Operand requirement/atomic seed | 全 validation dependency は exact `ShapeOnly`/`Readable`/`WritableDestination`/`MoveOwned`/`Callable` requirement を持つ。`Readable` は present Owned/Shared/Exclusive のみ、`MoveOwned` は present Owned のみ、`WritableDestination` は exact canonical slot/place と Out/Exclusive write authority を検証するが readable fact を作らず、`Callable` は one canonical certified function identity も要求。constant は readable。condition、length、index、template hole、fresh-producer input、他 evaluated operand は MIR semantics に従い `Readable`/`MoveOwned` であり便宜的 ShapeOnly ではない。call argument requirement は canonical parameter mode から導出。validation-only dependency は selected-path access に寄与しないが requirement result は total validity に含む。fresh/view/runtime out-slot fact は exact result type、全 operand/option、primary definition、output slot、access relation を証明する同じ fully validated rvalue equation だけが publishし、discriminant/out-slot number の preliminary scan は seed 不可。 | parameterized whole/per-unit mutation は4 modeと各 requirementを condition、length/index、constructor、call、fresh allocator、template piece/arena、grouped view、全 runtime out producer で交差。各 result/operand/option/slot/type mutation は reject。Out は matching destination だけ acceptし全 read path で reject。primary equation が absent/duplicate/rejected/invalid の producer は output slot を readable にできない。 |
...
| Producer-graph implementation review P1: function parameter/result が declaration type を再信用し、raw callable/self-owned value を local/imported function boundary 越しに laundering できた。 | one MIR-owned interprocedural validator が全 local return path と callable argument obligation を certify。non-generic interface record は producer validation 後だけ exact `ValidatedBody=1`、generic record は `RevalidateGenericBody=0` を持ち instantiation 後に validate。call は declaration 単独でなく certified local/imported target を consume。 | whole/per-unit direct/indirect/wrapper/returned-callable/self-owned return/borrowed-root return/cleanup/recursive-SCC/certificate codec/producer-consumer mutation owner が両方向を close。 |
| Producer-graph implementation review P1: validation-only edge が type/presence を確認したが `Out` を暗黙に readable と扱った。 | 全 dependency は exact shape/read/write/move/callable requirement を持つ。`Out` は unreadable writable destination で、successfully validated producer equation 経由だけ readable result を publish可能。 | mode × requirement owner が condition、length、fresh input、capture、call、runtime operand を whole/per-unit で mutateし、全 Out read を reject。 |
