...
| LLVM/native ABI | exact existing-shape declaration/status/out/enum/cleanup/destructor のみ。address shape は dereference せず検査し、通過して access 対象となる pointer だけ provenance/access/exclusivity precondition を適用。mutation/typed access 前に mechanical range/internal/output-alias preflight を完了。preflight 前 failure は untouched、後は canonical zero。hand-written bypass/shape attribute mutation なし。 | exact declaration/call IR golden、null/misalignment/noncanonical-empty/overflow/input-output-shell alias と malformed-field native owner、rejected-pointer no-dereference/no-free owner、untouched/zeroed output owner、key/symbol reverse、base/export/collision、status/enum exhaustive、rt-LTO on/off。 |
