...
nominal enum grammar を使い new type-codec discriminator を消費しない。下記 producer-certificate field は
function-record grammar を変えるため `align_interface::FORMAT_VERSION` を 8 から exact 9 に上げる。
unknown/truncated/missing-payload/trailing record は cache publication 前 reject。
...
| Interprocedural producer certification | sealed graph builder を codegen/interface publication 前に呼ぶ reusable MIR-owned validator へ移す。最初に exact local call-graph SCC を形成し、各 SCC を member の declared contract の provisional copy の下で coinductive に検査する。全 internal call edge は exact target、argument product、return type、borrow/region root、callable identity、cleanup mode に一致し、全 actual `Return`/`ReturnWithCleanup` equation は ordinary producer graph で provisional contract を満たす必要がある。全 member 成功時だけ whole SCC を certify。これにより well-typed non-returning `fn f() -> string = f()` と mutual-recursion equivalent は value seed を捏造せず acceptし、raw/contradictory return branch、mismatched internal call、invalid non-call producer は whole SCC を reject。この coinductive function-contract check は intra-function value graph と別で、unseeded SSA/slot/check cycle は invalid のまま。`Ty::Fn` argument は protected leaf。provisional argument fact は全 direct/indirect caller が exact callable producer を証明する場合だけ有効で、`FnAddr`、lifted closure、wrapper、store/load、projection、join、returned callable は certified local/imported target 経由だけ identity を保持。local direct-call result は callee SCC joint certification 後だけ seed。bodyless imported result は exact function record に bind された format-9 `ValidatedBody` certificate だけを使う。extern/runtime/raw call は certificate を持たず intrinsic-specific または rejected。 | whole/per-unit owner は local return operand/cleanup pair、direct/wrapped/returned callable producer、callable parameter product/identity、declaration/body mismatch、imported certificate tag/presence、全 call-graph edge を mutate。positive owner は direct/indirect wrapper、borrowed/self-owned result、returned callable、generic revalidation、self/mutual non-returning protected-result recursion、seeded recursion、serialized producer/consumer parity。raw/contradictory SCC return branch、signature/cleanup edge mismatch、unseeded intra-function value cycle は resource selection 前 reject。 |
| Operand requirement/atomic seed | 全 validation dependency は exact product `OperandRequirement { read, write, exclusive, move_value, callable }` を持つ。`ShapeOnly` は全 false で、evaluate/dereference しない metadata だけに許可。access fact の capability は Owned = read/write/exclusive/move、Shared = read、Exclusive = read/write/exclusive、Out = write/exclusive だが unreadable/uninitialized。requirement は requested capability 全てを必要とし、`callable` は orthogonal で set 時に one canonical certified function identity を指定。complete mode mapping は ByValue Copy = read、ByValue Move = read+move、Borrow = read、BorrowMut = read+write+exclusive、Out = exact canonical destination 上の readなし write+exclusive。callable leaf は mode product を置換せず identity requirement を追加し、structural carrier は selected leaf ごとに適用。constant は readable。condition、length、index、template hole、fresh input、他 evaluated operand は semantics に従い read/move を要求し便宜的 ShapeOnly ではない。validation-only dependency は selected-path access に寄与しないが product result は total validity に含む。fresh/view/runtime out-slot fact は exact result type、全 operand/option、primary definition、output slot、access relation を証明する同じ fully validated rvalue equation だけが publishし、discriminant/out-slot number の preliminary scan は seed 不可。 | parameterized whole/per-unit mutation は4 mode、Copy/Move/callable leaf、各 capability productを condition、length/index、constructor、call、fresh allocator、template piece/arena、grouped view、全 runtime out producer で交差。各 result/operand/option/slot/type mutation は reject。Shared→BorrowMut、Out→BorrowMut、全 Out read、noncanonical destination は reject。Out は matching write-only destination product だけ accept。primary equation が absent/duplicate/rejected/invalid の producer は output slot を readable にできない。 |
| Semantic producer inventory | exhaustive `Stmt`/`Rvalue` match は broad syntactic `Rejected` bucket でなく semantic equation を返す。`Irrelevant` は actual result と全 written slot が `str`/`string`/`xml.reader`/`Ty::Fn` またはその carrier を含めないと structural に証明できる場合だけ。該当 leaf を yield/feed し得る全 current variant は explicit result/dependency equation を持つ。`CloneIn` は `str` と `slice<u8>` を含む supported RegionPlain の exact same-shaped input/result と readable `ArenaHandle` を要求し arena-rooted readable view provenance を保持して `StrClone` へ feed可能。template は全 piece/arena、view は source/index/resource relation、native out producer は complete primary/out equation を検証。 | compile-time variant sweep と generated semantic inventory が current protected-leaf-capable result/out-slot variant を全列挙。positive whole/per-unit owner は `CloneIn(...).clone()` を含む全 valid `str`/`string`/callable carrier を XML checked edge へ feed。per-variant result/dependency/option/out-slot mutation は rejectし、protected actual type の `Irrelevant` mutation は fail closed。 |
| Existing container/view producer closure | unconditional producer validator は protected leaf を project/preserve し得る既存 operation 全てに exact typed equation を持つ。dynamic AoS element/field load、SoA field view、subslice は selected source path だけを preserveし、complete base/index/range/nominal shape を別途 validate。by-value structural carrier は selected leaf 自身が Move の場合だけ move authority を要求し、別 sibling が enclosing carrier を Move にしていても Copy view leaf は readable provenance だけを要求。 | valid non-XML owner が dynamic AoS whole/field string projection、SoA string-column view、slice-of-string subslice 後の index/forward、owned `string` と shared `str` を含む Move record を cover。per-operation base/result/nominal/field/index/path mutation と per-leaf Copy/Move authority mutation は whole/per-unit build の LLVM 前に reject。 |
...
| Interface/cache/generic | nominal event、72/48、mode/mutability/effect/view region/cleanup と、各 exported function record の `return_cleanup` 直後に one `ProducerCertification` byte を encode。exact tag `0 = RevalidateGenericBody` は type parameter nonempty かつ `generic_body` present の場合だけ validで、instantiated body は local function として同じ MIR validator を通す。exact tag `1 = ValidatedBody` は type parameter empty かつ `generic_body` absent の場合だけ validで、interface construction は exact local MIR body の validation 後だけ emit。他 tag と presence mismatch は reject。imported non-generic function は `ValidatedBody` を MIR に運び、exact parameter/return/borrow/region/cleanup fact を authenticateして全 bodyless call boundary で consume。byte は interface surface/hash の一部、format 9 は record parse 前に format 8 を rejectし、certification/surface が同じ private body edit は interface hash stable。by-value parameter が return root になれるのは type 自体が borrow 可能な場合だけ。dynamic cleanup または non-cleanup ownership だけを持つ opaque/self-owned Move value は `borrow`/`borrow mut` 経由でのみ returned view の root になれる。direct/concrete-sum/generic carrier に再帰適用し、cleanup capability だけを external borrow provenance としない。used runtime implementation を object/link identity に含める。 | bidirectional semantic-to-byte/byte-to-semantic v9 golden が certificate position と両 tag を固定。v8、unknown tag、generic/body-presence mismatch、truncation、trailing byte は reject。全 parameter mode を交差する direct/concrete/generic carrier summary、forged by-value XML root rejection と borrowed XML root acceptance、producer/consumer signature/certificate、generic whole/per-unit、two-build、surface/private edit-revert cache。 |
...
| Array-backed producer closure | `Index`、`IndexField`、`MakeSlice`、`ConstArray`、`StoreIndex`、`StoreElemField`、`StoreConstArray` は static `str` sharing を含む exact array/element/path、readable index、constant element、source access equation を持つ。 | fixed/static string array は XML import なしで direct element/field/slice-view forwarding を実行し、malformed slot/element/result/index/count/type/constant-element mutation は LLVM 前 reject。 |
...
second producer-closure review は coupled cell 4個を再オープンする。

| Cell | Implementation closure | Owner evidence |
|---|---|---|
| Whole container/element path | fixed-array `MakeSlice` は exact whole-view equation と selected-element equation の両方を持つ。`StoreElemField` で構築した whole struct element は全 stored field equation を検証し independent local inline storage から readability を導出。 | numeric fixed slice と numeric struct-array whole element を `str` を返す helper に渡し whole/per-unit で certify。path/result/field mutation は reject。 |
| Storage authority/payload provenance | canonical borrowed descriptor は selected producer path の payload readability/callable identity と root storage の write/exclusive authority を別 fact として authenticate。Copy carrier の BorrowMut は readable payload + writable exclusive storage、Move carrier は selected payload 自体の exclusive を要求。Out は canonical writable exclusive storageだけを要求。 | shared Copy payload を fresh mutable local に copy 後 BorrowMut で replace可能。同じ carrier が shared parameter root の場合と shared Move payload を local に置く場合は reject。local/ByValue/Borrow/BorrowMut/Out root × Copy/Move carrier を交差。 |
| Physical inline-slot kind | `Index`、`IndexField`、`MakeSlice` と store equation は LLVM slot GEP が consumeする exact inline fixed-array family `Ty::Array`/`Ty::StructArray` だけを admit。semantic element type が同じでも slice/dynamic-array/box/builder/task slot は reject。 | 全 non-inline container を valid indexing graph の slot に substituteし LLVM 前 reject。exact scalar/struct fixed array は pass。 |
...
判明した。その次の revised implementation も proof を function boundary で止め、check-only access requirement
を validity から分離し、複数の output seed を producer equation 検証前に publishし、valid `CloneIn` と他の
semantic carrier を rejected と分類した。このため matrix を **raw ABI representation phase**、
...
| Producer-graph implementation review P1: function parameter/result が declaration type を再信用し、raw callable/self-owned value を local/imported function boundary 越しに laundering できた。 | one MIR-owned interprocedural validator が provisional exact contract の下で各 local call-graph SCC を jointly certifyし、全 actual return/call edge を検査し、coinductive contract proof を intra-function value provenance と分離。non-generic interface record は producer validation 後だけ exact `ValidatedBody=1`、generic record は `RevalidateGenericBody=0` を持ち instantiation 後に validate。call は declaration 単独でなく certified local/imported target を consume。 | whole/per-unit direct/indirect/wrapper/returned-callable/self-owned return/borrowed-root return/cleanup/self-mutual non-returning recursion/raw-contradictory SCC branch/certificate codec/producer-consumer mutation owner が両方向を close。 |
| Producer-graph implementation review P1: validation-only edge が type/presence を確認したが `Out` を暗黙に readable と扱った。 | 全 dependency は read/write/exclusive/move と certified-callable requirement の product を持つ。exact mode × Copy/Move/callable map により `Out` は unreadable write-only destination、`BorrowMut` は readable+writeable+exclusive。successfully validated producer equation 経由だけ readable result を publish可能。 | product owner が condition、length、fresh input、capture、call、runtime operand を whole/per-unit で mutateし、Shared→BorrowMut、Out→BorrowMut、全 Out read を reject。 |
| Producer-graph implementation review P1: template/grouped view/runtime out-slot fact が complete producer validation と独立に seedされた。 | result/out-slot fact は exact result、全 input/option、primary definition、slot、access relation を含む one atomic semantic rvalue equation の output。discriminant-only producer scan は廃止。 | affected rvalue ごとに result/operand/option/slot mutation。invalid/absent primary equation は slot を seed不可。 |
| Producer-graph implementation review P1: syntactic rejected bucket が valid `CloneIn` を含み、他 protected carrier も omit可能だった。 | classification は actual result/out-slot semantics に基づく。protected-leaf-capable variant は explicit equation 必須、`Irrelevant` は structural guard。`CloneIn` は same-shaped input、arena handle、arena-rooted readable result を検証し following clone が owned parse input を作れる。 | compile-time/generated semantic inventory と positive whole/per-unit owner が全 current protected carrier と `CloneIn(...).clone()` を cover。guarded irrelevant/per-field mutation は fail closed。 |
...
| Producer-closure re-review P1: `MakeSlice` が empty selection path を rejectし、unprotected numeric slice argument が protected result helper を invalid にした。 | fixed-array view は separate validated whole-view equation を持ち selected element provenance は path-specific のまま。 | numeric fixed array を `slice<i64>` として `str` return function へ渡すと certifyし forged length/slot kind は reject。 |
| Producer-closure re-review P1: one access state が local Copy binding の write authority と内部 borrowed view の provenance を混同した。 | borrowed descriptor は storage authority と selected payload provenance を別々に authenticate。Copy BorrowMut は storage write/exclusive + payload read、Move BorrowMut は payload exclusive。 | fresh-local/shared-payload、shared-root、Move-root、Borrow、BorrowMut、Out を one parameterized product で所有。 |
| Producer-closure re-review P2: `StoreElemField` だけで構築した whole struct-array element に equation がなかった。 | whole element は contributing field store 全てを validateし exact local inline array slot から readability を導出。selected field path はその field producer だけを追跡。 | numeric-only whole element を protected-result helper に渡すと certifyし malformed field path/type は reject。 |
| Producer-closure re-review P2: semantic element selection が `Index` に slice/dynamic/box slot を admitしたが LLVM は inline-array slot GEP を行う。 | one exact physical-slot predicate を index/view/store equation で共有し `Ty::Array`/`Ty::StructArray` だけ accept。 | 他の element-bearing slot family を全て substituteし pre-LLVM rejection を要求。 |

full post-redesign review は existing-producer closure を one axis として再オープンする。

| Cell | Implementation closure | Owner evidence |
|---|---|---|
| Dynamic container projection | `SoaColumn`、`IndexFieldPtr`、`IndexPtr` と protected leaf を carry し得る sibling dynamic container operation は exact result/base/nominal/field/index equation を持つ。selected field/element path は対応する base path だけへ mapし unrelated field は validation-only。 | non-XML dynamic AoS/SoA program が string-bearing field/whole element を return/forward。base/result/struct id/field/index/selected path mutation は whole/per-unit の LLVM 前 reject。 |
| Subslice selected-path preservation | `SubSlice` は whole relation を一度 validateし empty path を whole base、`Element` と deeper selected path を同じ base element pathへ map。range operand は readable validation-only dependency のまま。 | `slice<str>` subslice 後の element index と slice forwarding は pass。forged result/element/base/range/path record は reject。 |
| Leaf-local transfer authority | ByValue mode の `move_value` は enclosing carrier でなく selected leaf type から導出。container movement/cleanup は ordinary MIR ownership が別途 validate。 | owned `string` + shared `str` を含む Move struct の by-value pass は成功し、shared owned-string leaf と forged Copy/Move leaf classification は reject。 |

| Post-redesign full review P1: dynamic AoS/SoA string-bearing projection が `Irrelevant` のままで、unconditional validation が valid non-XML producer を rejectした。 | semantic inventory は protected-capable dynamic projection 全ての exact equation を所有し、reported variant だけでなく sibling class 全体を audit。 | dynamic AoS field/whole-element と SoA string-column positive owner + per-field mutationを whole/per-unit で実行。 |
