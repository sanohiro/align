...
| MIR/runtime selection | typed op/result/event/region を保持、eight keyを追加、reachable row のみ select、wrong operation/result/enum/provenance は LLVM 前 reject、whole/per-unit parity。seven callable XML operation は全て final source result を返す semantic MIR rvalue で、raw runtime status/native out slot を generic MIR control flow に出さない。XML preflight は operand の declared type ではなく producer から access provenance lattice を導出する。function argument は `ByValue=Owned`、`Borrow=Shared`、`BorrowMut=Exclusive`、`Out=unreadable` で開始。fresh allocating producer は `Owned` で開始し、direct/indirect/imported call result は validated callable signature、return provenance、required dynamic-cleanup companion が transferred ownership を共同で証明するときだけ `Owned`。canonical store/load/move、option/result/enum/tuple wrapper/projection、control join が slot/value へ伝播し、borrow-rooted call result は shared/exclusive class を維持、mixed/unknown は reject。`XmlParse` は `Owned`、`XmlNext` は `Owned`/`Exclusive`、immutable getter は `Owned`/`Shared`/`Exclusive` を要求し、readable integer index は exact typed constant も許可。全 borrowed-place descriptor は noncanonical。wrong-mode argument は same-typed value/slot 経由でも reject。semantic shape `XmlParse { input, error_enum, cleanup }` は primary `Result<xml.reader,error>` と distinct bool cleanup value を existing cleanup-returning call と同じ one atomic MIR definition にする。preflight は cleanup id の in-range、`bool`、primary id との distinct、他 definition からの absent、exactly-once definition を要求。lowering は Result に attach し、`0` だけ `(Ok(reader), true)`、`-1` だけ `(Error.Invalid, false)` に map。`XmlNext` は canonical event enum id と final `Option<xml.event>`、other op は exact final `str`/`string`/bounded `i64` を返す。 | 各 operation について `ByValue`/`Borrow`/`BorrowMut`/`Out`、raw argument、fresh/direct/indirect/imported call result、wrapper/projection/control-result path、load/store-derived same-typed value、join、constant、全 borrowed-place family を whole/per-unit preflight で交差。parse cleanup id の missing/duplicate/alias/out-of-range/wrong type、result type、cleanup attachment、両 status-to-result/cleanup pairing、error/event definition/id mutation は LLVM 前 reject。key inventory/bijection、unused import/no-selection、per-op selection は exact。 |
...
machinery に分割されていた。このため matrix を **raw ABI representation phase**、**interface
provenance**、**MIR operand formation**、**runtime-status/result identity** axis で再オープンする。
shape-only rejection、post-shape caller precondition、ownership acceptance は別 phase。cleanup capability は
borrow provenance ではなく、XML MIR は declared operand type ではなく producer-derived access を認証する。
各 semantic XML rvalue は one final source result、LLVM lowering は complete checked status map を所有する。
