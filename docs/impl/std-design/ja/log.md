...
append-only canonical type record は version 3 のままである。shipped asymmetric-key family は
payloaded tag `Ty::SignatureKey=63` と `Scalar::SignatureKey=39` を所有し、logging は次の leaf
tag `Ty::Logger=64` と `Scalar::Logger=40` を予約する。tag 65 と 41 はそれぞれ次の unknown
Ty/Scalar tag である。

| semantic root | exact canonical byte | exact reverse result |
|---|---|---|
| `Ty::Logger` | `[3, 0, 0, 0, 0, 64]` | `CanonicalTy::decode` は同一の `Ty::Logger` root を返す。 |
| `Ty::Option(Scalar::Logger)` | `[3, 0, 0, 0, 0, 4, 40]` | `CanonicalTy::decode` は同一の Option/Logger root を返す。 |

`[3, 0, 0, 0, 0, 65]` と `[3, 0, 0, 0, 0, 4, 41]` は unknown tag として拒否する。root
tag がない record、payloaded record の payload 欠落、valid record への trailing byte は cache
publication 前に拒否する。`log.logger` と `log.level` は既存 nominal named-type/enum grammar
を使うため log capability 自体は interface FORMAT_VERSION を上げない。canonical record version、2 個の
new tag、reverse decoding、malformed rejection、exact byte は compiler build fingerprint と
bidirectional golden に atomic に入る。
...
| `enabled` | unique enum id と one-field aggregate を HIR/MIR 全体で保持し、native-call lowering だけが field 0 を抽出する。eager single evaluation、pure borrow、complete threshold table、latch suppression、exact Bool result も保持する。 | threshold 25 cell と post-latch row、aggregate/extracted-tag parity、effect/no-I/O/no-allocation/wrong-HIR/MIR test。 |
...
| LLVM/native ABI | exact declaration/call と opaque pointer を emit し、validated level aggregate の field 0 を native i32 argument へ抽出して status にも i32 を使い、6 row すべてを typed registry と base export に置く。hand-written declaration bypass はない。 | exact aggregate-to-extern-type matrix、declaration/call golden、key/symbol reverse lookup、base/maximum export parity、rt-LTO on/off。 |
| Interface/cache/generics | `log.logger` と `log.level` を既存 nominal name で serialize し、canonical type-record-v3 leaf `Ty::Logger=64` と `Scalar::Logger=40` を append し、parameter mode/effect/return-region/return cleanup を保持する。imported generic user を runtime ownership の重複なしに instantiate し、capability/runtime fingerprint を含める。この capability は interface field を追加せず repository-wide current format を consumeする。 | exact bidirectional semantic/byte と unknown/truncated/trailing-tag golden、producer/consumer signature/return-region golden、generic whole/per-unit borrowed/static parity、two-build determinism、surface use による interface hash change と private span-only edit での不変。 |
...
| P2: `log.level` が HIR/MIR 全体で raw i32 と記述されていた | settled one-field `Ty::Enum`/`Scalar::Enum` aggregate を HIR/MIR 全体で保持する。checked validation が id/tag を証明し、LLVM native-call lowering だけが i32 を取る 4 ABI の field 0 を抽出する。 |
| P2: ABI owner が accepted logging shape を予約していなかった | complete planned six-row logging delta を runtime ABI ledger に追加し、shipped count を保持したまま A114–A117 を予約し、A118 を次の unreserved shape とする。 |
| P2: Logger に canonical type-codec discriminator がなかった | asymmetric-key の payloaded tag 63/39 に続く append-only record-v3 leaf `Ty::Logger=64` と `Scalar::Logger=40` を予約する。semantic/byte 両方向と unknown/truncated/trailing rejection をこの ledger と HIR inventory に固定する。 |
