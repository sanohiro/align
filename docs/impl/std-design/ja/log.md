...
| `log.new(output: writer, minimum: log.level) -> log.logger` | 正確に 2 個の positional argument を取り、default はない。argument は左から右へ評価する。`output` は完全な owned writer source でなければならず、両 argument の check 後にのみ consume される。成功 path では source slot を null にする。`minimum` は閉じた 5 tag のいずれかである。stdout/stderr/file constructor も environment/config lookup もない。 | 初期化済み logger を 1 個返す。Pure であり、write、flush、close、clock、process、thread、その他の externally visible I/O を行わない。ソースから形成した construction に recoverable error はなく、terminal OOM は言語全体の abort policy に従う。 | 返される Move handle は logger shell allocation 1 個と consume した writer pointer を所有する。writer は自身の fd と optional 64 KiB buffer の唯一の owner のままであり、logger がその writer の唯一の owner になる。path/text view は保持しない。Logger Drop は underlying writer を通常の best-effort flush-then-close path で free し、その後 logger shell を free する。 | `Ty::Logger`/`Scalar::Logger`、`ExprKind::LogNew`、MIR construction、keyed native constructor 1 本。runtime shell は private で `{ writer, minimum, first_error }` を持つ。公開型は structural ではなく nominal。`log.logger` は既存の `IType::Named` spelling として interface を越え、その reachability と `std.log` capability は既存の interface/implementation hash と compiler/runtime ABI fingerprint に入る。 | arity/type/import/shadowing、完全/不完全な全 writer source、local/function/branch/loop/`?`/`map_err` construction と source nulling、shell 1 個の allocation/free count、no-I/O constructor、malformed checked-HIR、whole-program/per-unit imported signature。 |
...
|---|---|---|
| local、by-value/`borrow`/`borrow mut`/`out` parameter、direct function return | 既存 Move 規則の下で許可する。by-value transfer は完全な source を null にし、borrow は null にしない。 | type formation、call mode、MoveCheck、EscapeCheck、return cleanup、interface round trip。 |
| user struct field、direct builtin `Option`/`Result` または user-sum payload | 既存 single-owner handle grammar が `writer` を許可する位置で許可する。enclosing value は Move になり active logger を正確に 1 回 recursive Drop する。field move-in/move-out、replacement、consuming match、`else`、`?`、`map_err` は既存 aggregate/tagged ownership に従う。 | field/payload formation、DropPlan、active-tag cleanup、complete-source nulling、replacement と branch/loop join。 |
...
| Drop、`process.exit`、abort、successful `process.exec` | ordinary scope exit と `process.exit` は logger、次に writer cleanup を実行する。既存の immediate abort と successful exec は cleanup を skip する。logging 固有 hook や global flush はない。 | 既存 lifecycle twin と logger fd/allocation probe。 |
...
`LogFree` は既存 ABI shape A03 と A62 を再利用し、他の 4 個の exact declaration は新しい
shape A110 から A113 である。6 個すべての key、symbol、declaration、definition、collision
...
314/331/339 keyed/base/maximum record を 320/337/345 に変更し、shape range を A113 まで
...
を返される non-null logger へ transfer し、compiler は source slot を null にする。null、
invalid tag、dangling pointer、または uniquely owned でない writer を渡す foreign call は、
この compiler-private ownership ABI の外である。terminal allocator failure は abort する。
`LogEnabled`、`LogLine`、`LogLineBuilder`、`LogFlush` は live unique logger pointer を要求し、
`LogFree` だけが null-safe である。`LogEnabled` は invalid level に対して mutation なしで 0 を
返す。`LogLine` と `LogLineBuilder` は successful/gated call 後に 0、既存 latch 後に stored
...
| Construction と move-in | argument を左から右へ check し、完全な writer source 1 個を要求し、`LogNew` を形成し、両方の check 後だけ transfer し、その source だけを null にして logger drop flag 1 個を初期化する。 | direct local、param、function result、block、`if`/`match`、`else`、`?`、`map_err`、loop-carried writer constructor matrix。 |
...
| HIR/replay/validation | depth、clone/replay、effect、ownership、borrow/region、finalization、traversal、cache/semantic projection、checked-HIR validation、malformed-input fail-closed switch に全 new expression を追加する。wildcard が new form を silent classification してはならない。 | variant sweep tripwire、child/result/enum id/effect/ownership fact の one-field mutation、replay identity。 |
