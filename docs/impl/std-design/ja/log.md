...
| P2: `log.level` が HIR/MIR 全体で raw i32 と記述されていた | settled one-field `Ty::Enum`/`Scalar::Enum` aggregate を HIR/MIR 全体で保持する。checked validation が id/tag を証明し、LLVM native-call lowering だけが i32 を取る 3 ABI の field 0 を抽出する。 |
