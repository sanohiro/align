...
を使うため interface FORMAT_VERSION は 8 のままである。canonical record version、2 個の
...
| Interface/cache/generics | `log.logger` と `log.level` を既存 nominal name で serialize し、canonical type-record-v3 leaf `Ty::Logger=64` と `Scalar::Logger=40` を append し、parameter mode/effect/return-region/return cleanup を保持する。imported generic user を runtime ownership の重複なしに instantiate し、capability/runtime fingerprint を含める。interface encoding grammar が変わらないため FORMAT_VERSION は 8 のまま。 | exact bidirectional semantic/byte と unknown/truncated/trailing-tag golden、producer/consumer signature/return-region golden、generic whole/per-unit borrowed/static parity、two-build determinism、surface use による interface hash change と private span-only edit での不変。 |
