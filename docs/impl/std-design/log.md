...
| `log.new(output: writer, minimum: log.level) -> log.logger` | Exactly two positional arguments and no defaults. Arguments evaluate left to right. `output` must be a complete owned writer source and is consumed only after both arguments check; its source slot is nulled on the success path. `minimum` is one of the five closed tags. There is no stdout/stderr/file constructor and no environment/config lookup. | Returns one initialized logger. Pure: it performs no write, flush, close, clock, process, thread, or other externally visible I/O. Source-formed construction has no recoverable error; terminal OOM follows the language-wide abort policy. | The returned Move handle owns one logger shell allocation and the consumed writer pointer. The writer remains the sole owner of its fd and optional 64 KiB buffer; the logger is now its sole owner. No path/text view is retained. Logger Drop best-effort frees the underlying writer through its ordinary flush-then-close path, then frees the logger shell. | `Ty::Logger`/`Scalar::Logger`, `ExprKind::LogNew`, MIR construction, and one keyed native constructor. The runtime shell is private and carries `{ writer, minimum, first_error }`. The public type is nominal, not structural. `log.logger` crosses an interface as the existing `IType::Named` spelling; its reachability and `std.log` capability enter the existing interface/implementation hashes and compiler/runtime ABI fingerprints. | Arity/type/import/shadowing; every complete/incomplete writer source; local/function/branch/loop/`?`/`map_err` construction and source nulling; one-shell allocation/free count; no-I/O constructor; malformed checked-HIR; whole-program and per-unit imported signatures. |
...
|---|---|---|
| Local; by-value, `borrow`, `borrow mut`, or `out` parameter; direct function return | Admitted under the existing Move rules. A by-value transfer nulls its complete source; a borrow never does. | Type formation, call modes, MoveCheck, EscapeCheck, return cleanup, interface round trip. |
| User struct field; direct builtin `Option`/`Result` or user-sum payload | Admitted where the existing single-owner handle grammar admits `writer`. The enclosing value becomes Move and recursively drops the active logger exactly once. Field move-in, move-out, replacement, consuming match, `else`, `?`, and `map_err` follow existing aggregate/tagged ownership. | Field/payload formation, DropPlan, active-tag cleanup, complete-source nulling, replacement and branch/loop joins. |
...
| Drop, `process.exit`, abort, successful `process.exec` | Ordinary scope exit and `process.exit` run logger then writer cleanup. Existing immediate abort and successful exec skip cleanup; no special logging hook or global flush exists. | Existing lifecycle twins plus logger fd/allocation probes. |
...
|---|---|---|---|
| `LogNew` | `align_rt_log_new` | A110: `ptr @SYM(ptr, i32)` | `unsafe extern "C" fn(*mut Writer, i32) -> *mut Logger` |
| `LogEnabled` | `align_rt_log_enabled` | A111: `i32 @SYM(ptr, i32)` | `unsafe extern "C" fn(*mut Logger, i32) -> i32` |
| `LogLine` | `align_rt_log_line` | A112: `i32 @SYM(ptr, i32, ptr, i64)` | `unsafe extern "C" fn(*mut Logger, i32, *const u8, i64) -> i32` |
| `LogLineBuilder` | `align_rt_log_line_builder` | A113: `i32 @SYM(ptr, i32, ptr)` | `unsafe extern "C" fn(*mut Logger, i32, *mut Builder) -> i32` |
...
`LogFree` reuse existing ABI shapes A03 and A62; the other four exact declarations are new shapes
A110 through A113. All six keys, symbols, declarations, definitions, collision reservations,
...
314/331/339 keyed/base/maximum records to 320/337/345 and extends the shape range through A113.
...
pointer into the returned non-null logger and the compiler nulls the source slot. A foreign call
that supplies null, an invalid tag, a dangling pointer, or a writer that is not uniquely owned is
outside this compiler-private ownership ABI. Terminal allocator failure aborts. `LogEnabled`,
`LogLine`, `LogLineBuilder`, and `LogFlush` require a live unique logger pointer; `LogFree` alone is
null-safe. `LogEnabled` returns zero without mutation for an invalid level. `LogLine` and
...
| Construction and move-in | Check arguments left-to-right, require one complete writer source, form `LogNew`, transfer only after both check, null exactly that source, and initialize one logger drop flag. | Direct local, param, function result, block, `if`/`match`, `else`, `?`, `map_err`, and loop-carried writer constructor matrix. |
...
| HIR/replay/validation | Add every new expression to depth, clone/replay, effects, ownership, borrow/region, finalization, traversal, cache/semantic projection, checked-HIR validation, and malformed-input fail-closed switches. No wildcard may silently classify a new form. | Variant sweep tripwire; one-field mutation for child/result/enum id/effect/ownership fact; replay identity. |
...
| Interface/cache/generics | Serialize `log.logger` and `log.level` through existing nominal names, preserve parameter modes/effects/return cleanup, instantiate imported generic users without duplicating runtime ownership, and include capability/runtime fingerprints. FORMAT_VERSION remains 8 because the encoding grammar does not change. | Producer/consumer signature goldens, generic whole/per-unit parity, two-build determinism, interface hash changes on surface use and not on private span-only edits. |
