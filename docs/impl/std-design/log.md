...
### Canonical type-codec delta

The append-only canonical type record remains version 3. The already-shipped asymmetric-key
family owns payloaded tags `Ty::SignatureKey=63` and `Scalar::SignatureKey=39`; logging reserves
the next leaf tags `Ty::Logger=64` and `Scalar::Logger=40`. Tags 65 and 41 are the next unknown
Ty and Scalar tags respectively.

| Semantic root | Exact canonical bytes | Exact reverse result |
|---|---|---|
| `Ty::Logger` | `[3, 0, 0, 0, 0, 64]` | `CanonicalTy::decode` returns the identical `Ty::Logger` root. |
| `Ty::Option(Scalar::Logger)` | `[3, 0, 0, 0, 0, 4, 40]` | `CanonicalTy::decode` returns the identical Option/Logger root. |

`[3, 0, 0, 0, 0, 65]` and `[3, 0, 0, 0, 0, 4, 41]` reject as unknown tags. A record missing
its root tag, a payloaded record missing its payload, and any valid record with a trailing byte
reject before cache publication. The log capability itself does not bump Interface FORMAT_VERSION:
`log.logger` and `log.level` use the existing nominal named-type/enum grammar. Canonical record version, the two new tags,
reverse decoding, malformed rejection, and exact bytes enter the compiler build fingerprint and
bidirectional goldens atomically.
...
| `enabled` | Preserve the unique enum id and one-field aggregate through HIR/MIR, extract field 0 only at native-call lowering, and retain eager single evaluation, pure borrow, the complete threshold table, latch suppression, and exact Bool result. | 25 threshold cells plus post-latch row; aggregate/extracted-tag parity; effect, no-I/O, no-allocation, wrong-HIR/MIR tests. |
...
| LLVM/native ABI | Emit exact declarations/calls and opaque pointers; extract each validated level aggregate's field 0 into the native i32 argument and use i32 status; keep all six rows in the typed registry and base exports. No hand-written declaration bypass. | Exact aggregate-to-extern-type matrix, declaration/call golden, key/symbol reverse lookup, base/maximum export parity, rt-LTO on/off. |
| Interface/cache/generics | Serialize `log.logger` and `log.level` through existing nominal names; append canonical type-record-v3 leaves `Ty::Logger=64` and `Scalar::Logger=40`; preserve parameter modes/effects/return-region/return cleanup; instantiate imported generic users without duplicating runtime ownership; and include capability/runtime fingerprints. This capability adds no interface field and consumes the repository-wide current format. | Exact bidirectional semantic/byte and unknown/truncated/trailing-tag goldens; producer/consumer signature and return-region goldens; generic whole/per-unit borrowed/static parity; two-build determinism; interface hash changes on surface use and not on private span-only edits. |
...
| P2: `log.level` was described as raw i32 through HIR/MIR | Retain the settled one-field `Ty::Enum`/`Scalar::Enum` aggregate through HIR and MIR. Checked validation proves its id/tag, and LLVM native-call lowering alone extracts field 0 for the four i32-taking ABIs. |
| P2: the ABI owner did not reserve accepted logging shapes | Add the complete planned six-row logging delta to the runtime ABI ledger, reserve A114–A117 while retaining shipped counts, and name A118 as the next unreserved shape. |
| P2: Logger had no canonical type-codec discriminators | Reserve append-only record-v3 leaves `Ty::Logger=64` and `Scalar::Logger=40` after the asymmetric-key 63/39 payloaded tags; pin both semantic/byte directions plus unknown, truncated, and trailing rejection in this ledger and the HIR inventory. |
