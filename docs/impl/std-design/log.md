...
reject before cache publication. Interface FORMAT_VERSION stays 8: `log.logger` and `log.level`
use its existing nominal named-type/enum grammar. Canonical record version, the two new tags,
...
| Interface/cache/generics | Serialize `log.logger` and `log.level` through existing nominal names; append canonical type-record-v3 leaves `Ty::Logger=64` and `Scalar::Logger=40`; preserve parameter modes/effects/return-region/return cleanup; instantiate imported generic users without duplicating runtime ownership; and include capability/runtime fingerprints. Interface FORMAT_VERSION remains 8 because its encoding grammar does not change. | Exact bidirectional semantic/byte and unknown/truncated/trailing-tag goldens; producer/consumer signature and return-region goldens; generic whole/per-unit borrowed/static parity; two-build determinism; interface hash changes on surface use and not on private span-only edits. |
...
| P2: `log.level` was described as raw i32 through HIR/MIR | Retain the settled one-field `Ty::Enum`/`Scalar::Enum` aggregate through HIR and MIR. Checked validation proves its id/tag, and LLVM native-call lowering alone extracts field 0 for the three i32-taking ABIs. |
