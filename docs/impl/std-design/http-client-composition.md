...
| Borrowed helper parameters | `borrow` and `borrow mut` use the existing parameter syntax and call-bounded lifetime inference. A borrowed owner cannot be consumed, returned as owned, or given callee function-exit Drop. Shared `borrow` forbids replacement. `borrow mut` permits whole-owner replacement under the existing rule: Drop the displaced value before storing the replacement, forward its cleanup bit to caller storage, and preserve all live-view/dependent-stream exclusions. A returned response view retains the input response root; existing dependent raw/SSE stream carrier and origin rules still apply. | No allocation, retain count or new native shell. The caller remains the owner. Mutable loans exclude overlapping access and move/Drop/replacement for their lifetime. Dependent streams retain a shared client loan, permitting further shared request operations while excluding client move/Drop/replacement and incompatible mutable helper access. | Sema borrow/move/region checks and imported ownership summaries. V1, M1, I1. |
...
| M1 | Shared network versus exclusive configuration authority for every client method; mutable versus shared request authority; shared response access; reject borrowed consumption and same-root overlap through helpers/aggregates; shared replacement rejection and mutable whole-owner replacement/cleanup | `borrow_authority_and_consumption_matrix`, `borrowed_whole_owner_replacement` |
...
| P1 | Checked HIR operand/result identity; exact native access/slot producer certification; sibling-type substitution, missing storage proof and forged borrowed consumption refuse before LLVM; recursively authenticate every HTTP owner even in a mixed carrier with str/string/callable leaves | `align_mir --lib hir_body_validator_native`; `align_codegen_llvm --lib http_mir_gate_preserves_owner_identity_and_authority` and `http_mixed_carriers_preserve_recursive_ownership_proof` |
...
## Recursive ownership and replacement closure

Every client/request/response leaf participates in the existing MIR producer
fixed point, including when a record, tuple, sum, Option or Result also contains
a protected str/string/callable leaf. A carrier-level Move fallback is not proof
of each field. The same selected-path obligation applies at construction,
return, by-value call, projection and borrowed parameter forwarding. Shared and
mutable borrowed origins cannot become an owned result or owned call argument.
The owning response-batch header must likewise remain owned when transferred;
indexing borrows an element and never transfers it.

P1's parameterized owner covers each HTTP type in direct and mixed/nested
...
domain; tuple and Option/Result function-type parameters remain rejected.

M1 follows `draft.md`'s existing borrowed-replacement rule and Request 39's
cleanup implementation. `borrowed_whole_owner_replacement` covers all three
owners through imported generic replacement, whole/unit compilation and
Dev/Release execution; live response views and dependent streams still block
incompatible replacement. No HTTP-specific replacement prohibition is added.
