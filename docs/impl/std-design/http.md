...
| Surface / state | Exact contract |
|---|---|
| Public methods | `cl.max_response_body_bytes(limit: i64) -> ()` mutates a bound `http.client`; `r.max_response_body_bytes(limit: i64) -> ()` mutates a bound `http.request`. Both are Impure setters and borrow rather than consume their Move receiver. A request remains consumed only by `cl.request(r)`. |
...
| Native status mapping | Runtime uses HTTP-private `AL_HTTP_BODY_LIMIT = i32::MAX`. Only the HTTP Result lowerer maps that sentinel to `Error.Code(-1)` before the shared category/errno mapping. The sentinel never enters the generic errno decoder and adds no `Error` variant or type-layout change. |
