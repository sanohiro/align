...
- `http_read_stream` and `http_sse_stream` are dependent **Move** resources. Each owns one checked-out
  ...
  before the stream. Both types are nameable as bare locals/parameters/returns and may be carried
  through any finite nesting of the builtin `Option`/`Result` only. Such a carrier inherits the
  active stream's client provenance and recursively moves/drops only its active payload. Neither
  stream may occur anywhere in the reachable type of a user struct field, user-sum payload,
  array/slice/box element, closure/task capture, or a generic substitution that forms one of those
  carriers. `request_stream` produces the first stream inside `Result`; `sse()` transfers the same
  owner and borrow into an `http_sse_stream` and nulls the source.
...
| Ownership and cleanup | Both stream types are Move, borrow-mutated cursors and retain a shared client borrow. The client pool remains usable by other shared calls but cannot be moved/dropped before them. `sse()` transfers every connection/scratch/head/control owner and cleanup bit exactly once. Bare and builtin `Option`/`Result` construction, success, `?`, `else`, `match`, `map_err`, replacement, return, and ordinary Drop preserve one owner plus its client provenance; user aggregates and captured/parallel carriers are rejected. A stream cannot escape its creating client and an event string cannot escape its output-buffer generation. |
| Carrier formation and provenance | A stream may be bare or occur under finite builtin `Option`/`Result` nesting only. `Some`/`Ok` stores the owner and its creating-client dependency in the active payload; `None`/`Err` owns no stream. Moving or pattern-unwrapping an active payload clears the complete source tag/handle after the destination is initialized. Replacement and Drop inspect every nested tag and release exactly the one active stream, if any. Direct and imported parameter/return summaries preserve the client dependency; a control-flow join unions possible creating clients, so neither can die before the joined carrier is consumed or dropped. Formation recursively rejects the stream under a user struct, user sum, array, slice, box, closure/task capture, parallel element/result, or forbidden generic instantiation. |
...
| Closure axis | Required implementation evidence | Owner |
|---|---|---|
| Formation and type classes | Exact method/arity/receiver/output types; bound client/request/buffer; each stream accepted bare and through finite builtin `Option`/`Result` nesting; recursive rejection beneath user struct/sum, array/slice/box, closure/task capture, parallel element/result, and forbidden generic substitution; event nested Option record with three views plus inline Copy retry. | sema + checked-HIR parameterized reachable-type sweep |
...
| Carrier provenance, move, and Drop | direct and nested `Option`/`Result` Some/None/Ok/Err, two-client joins, direct/imported/generic parameter/return, move-in/out, pattern unwrap, `?`/`else`/`match`/`map_err`, replacement, branch/loop/early return, and active-tag Drop retain the exact client dependency, initialize destination before source clear, and close/pool once. User aggregate/capture/parallel negative owners recursively reject every forbidden reachable shape. | sema provenance + checked-HIR mutation sweep + MIR cleanup + runtime live counters |
| Ownership, replacement, and return | raw→SSE transfer nulls source; client Drop/move rejected while any bare/builtin carrier survives; other shared client requests accepted; output string view rejected after next/replacement/Drop and on escape while inline retry survives normally; all allowed exits drop once. | sema provenance owners + MIR cleanup + runtime live counters |
...
| Generic/imported/per-unit | direct and imported helpers, generic forwarding only for admitted bare/builtin carriers, dependency-summary substitution, recursive rejection when substitution forms a forbidden carrier, whole/per-unit interface transport, cache edit/revert, malformed effect/carrier/interface/checked-HIR rejection before LLVM. | interface/cache/validation owners |
...
1. The parameterized dependent-stream carrier owner plus `http_read_stream`
   ...
   PR closes bare/builtin-carrier provenance, active-tag Move/Drop, forbidden reachable carriers,
   effects, and interface/cache transport for the raw type instead of leaving them for SSE. This is
...
The boundary above therefore makes carrier provenance a first-PR substrate and makes SSE block
