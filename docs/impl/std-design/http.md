...
| SSE interpretation | Official examples and full BOM/UTF-8/line/field/blank/EOF Cartesian cases; state reset versus persistence; raw-prefix transition; output concatenation/spans; invalid-byte replacement expansion and cap; per-`next` source-work exact `capacity + 262,144` and rejected-next-byte cases for comments, unknown fields, invalid retry, control-only blocks, and split chunked input. | parameterized runtime parser oracle + driver event consumer |
| Control-state allocation | latest id/event overwrite, NUL/invalid retry ignore, repeated control-only blocks, one shared control allocation, checked aggregate length, exact geometric capacity growth/reuse, old+new transient high water, smaller later output, error/Drop free, no data allocation, fixed carry/counters. | allocation/failpoint instrumentation |
