...
| ABI and identity | Add `void @align_rt_http_max_response_body_bytes(ptr, i64)` and `void @align_rt_http_client_max_response_body_bytes(ptr, i64)` to ABI row A66. No other signature, type tag, interface record, or handle ABI changes. Method spellings and MIR discriminants entered shipped interface-format-6 and cache identities; the accepted Request 9 format-7 bump preserves those records, and exact edit/revert restores the prior hash. |
