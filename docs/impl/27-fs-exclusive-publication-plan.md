...
3. Decode UTF-8, reject an empty path, and scan the complete byte view for an
   ...
   mutation. After validation, copy each path into an ephemeral
   NUL-terminated buffer with a fallible `len + 1` reservation. A reservation
   failure yields the existing runtime allocation status (`AL_CODE`) and makes
   no native call. `rename_no_replace` constructs the complete source copy
   before validating or allocating the destination copy.
...
recoverable input errors. The runtime's strict path helper then performs a
fallible `len + 1` reservation, copies the bytes, appends one NUL, and retains
the temporary storage only through the native call. For rename, the source
buffer is completely constructed before destination validation/allocation. A
temporary-copy allocation failure returns the existing runtime allocation
status (`AL_CODE`) with no native call; a source buffer is dropped if
destination construction fails.
...
transfers the descriptor into the existing `Writer`; any recoverable writer or
buffer construction failure closes the fd before returning its terminal
allocation status, preserving exactly-once ownership. The existing runtime
allocator's terminal behavior is otherwise unchanged, and no writer is
published before construction succeeds.
...
| Validation order | `out_writer == null` wins before path processing; checkable ABI length/null/UTF-8/empty/NUL failures have no side effect; rename fully validates and marshals source before destination; path-copy allocation failure has no native call. | Runtime malformed-view matrix with valid-range ABI preconditions, output-slot precedence, source/destination error order, NUL/empty/UTF-8 cases, fallible path-copy owner, plus Align diagnostics with two invalid operands and exact primary error. |
...
| Error mapping | One fixed errno table; EEXIST/EXDEV remain native `Error.Code`; path-marshalling allocation uses the existing `AL_CODE` runtime status; the pair consumer maps clean failures to `OUTPUT_WRITE` and cleanup failures to `OUTPUT_PAIR_CLEANUP_FAILED` without changing either primitive's error model. | Per-platform native-code assertions, allocation-status and `Error` reconstruction tests, including unsupported-volume `EINVAL`/`EOPNOTSUPP`, plus C6 pair-status assertions. |
...
| Native rename path storage was unspecified | Every native path receives an ephemeral NUL-terminated copy, built with fallible `len + 1` reservation; source storage is built before destination storage; `AL_CODE` has no native side effect. |
...
The review log remains outside the worktree under `.git`; this disposition is
the author-side finding-to-fix ledger. A changed-slice review must confirm the
six closures and the author-side ledger-to-prose pass before this design is
marked accepted.
