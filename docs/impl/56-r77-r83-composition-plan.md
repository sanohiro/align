...
| Fixed-array Copy-field shared calls | A computed index admitted by `check_index_field` reads the selected Copy leaf through `lower_index_field`, then borrows its call-local descriptor, as the existing dynamic Copy-field path does. Evaluate the index once, retain its early exit and bounds check, and preserve nested paths and text-view backing. The temporary is Copy: no move-in/out, source nulling, Drop, replacement or allocation is introduced. Literal fixed places and exclusive/Move calls retain their existing paths and restrictions. No IR shape, interface record, native ABI or public type rule changes. | `owned_borrowed_composition::fixed_copy_field_borrow_evaluates_computed_indices` covers scalar/text, nested paths, generic and imported calls, index effects/early return and source reuse in whole/per-unit execution; `fixed_copy_field_borrow_checks_bounds` closes negative/upper bounds; MIR `malformed_fixed_copy_borrow_receiver_fails_closed` rejects forged non-place receivers before indexing or bounds actions. Existing indexed Move and mutable-view rejection owners close the excluded-domain controls. |
