...
`par_map` remains Pure-required. Its worker-sendability gate independently rejects the non-Send
`region` capability from every staged and terminal worker capture, including through a
...
The shipped worker-sendability gate rejects a captured `Ty::ArenaHandle`, even when the arena
outlives the group. `pkg.csv` extends that rule through nested function values and to `par_map`
using one fail-closed worker-transfer provenance authority. It follows concrete
`ClosureTarget`/`ClosureCapture` paths through moves
...
Copy is not worker-sendability. Every staged and terminal capture rejects any `Ty::ArenaHandle`
recursively reachable through a function-value
