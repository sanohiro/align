...
4. validate the complete root as an absolute strict path, retain `/`, and walk every root component
   with no-follow observation/open/identity revalidation;
...
Linux randomness is a bounded `getrandom` loop that retries only `EINTR` and rejects a zero or
failed read through the fixed native-error mapping. macOS uses `getentropy` for the 16-byte request
and maps failure identically. This fallible constructor does not reuse the aborting
...
| Platform root and randomness | Linux `/tmp`; macOS `_CS_DARWIN_USER_TEMP_DIR`; no environment lookup. Exactly 128 random bits and lowercase hex; `EINTR` retry, failure mapping, and 128-attempt collision cap. | Platform runtime owner with injected root/random provider, first-name collision, terminal collision, short/zero/error random returns, and source audit forbidding environment reads. |
