...
| Parallel function-value effects | A lifted capturing closure omitted its effect, and a higher-order parameter had no `FnTy` fact; either could let I/O enter an accepted Pure `par_map` | The 2026-07-13 gate first added conservative closure edges and unknown-indirect propagation; #465 superseded that workaround with inferred `FnTy` effects for concrete named functions/closures/imports/FFI, mutable target joins, and type-bit consumption at indirect calls. Unknown HOF parameters remain fail-closed. `analysis_coverage.rs` and `per_unit.rs` pin both directions. |
| Nested parallel progress | Saturating the shared pool with `workers + 1` task-group tasks whose inner `par_map`s each queued a helper left every worker and the external caller waiting forever | `par_map` helpers and callers now drain a shared atomic range cursor; a total-range barrier joins every claim before return, and late helpers exit before touching call-scoped addresses. A forced-two-worker child test reproduces the old cycle under a watchdog. |
| Dynamic allocation byte arithmetic | Heap, arena, and SoA byte counts used wrapping LLVM multiply/add, so a large logical extent could reach an allocator as a smaller or non-positive size while generated loops retained the original count | Codegen uses checked unsigned multiply/add plus signed-ABI result guards before allocator calls; SoA checks every offset, alignment bump, and total. Raw/O2 IR gates cover negative, zero, largest-fitting, one-over, widening, and padding cases, with two independent mutation failures. |
...
| String byte view | The specified `s.bytes() -> slice<u8>` escape hatch for arbitrary-byte work was absent, forcing callers toward UTF-8-constrained string slicing | `str` and owned `string` now expose a zero-copy descriptor retype; the HIR retains borrow provenance, MIR emits no operation, and region/owner checks prevent dangling views. **Fixed 2026-07-15.** |
...
- `crates/align_runtime/src/lib.rs` (forced-worker nested parallel progress watchdog);
- `crates/align_codegen_llvm/src/lib.rs` (heap/arena/SoA allocation-size IR boundaries and mutation gates);
