...
| Compilation modes | whole-program, per-unit and function partition | preserve the same counted record, rotation and extent placement in each path | whole-program G3 owners, `g3_counted_fact_reaches_per_unit_and_function_partition_codegen` |
...
| Validation | malformed MIR / unsupported CFG / id exhaustion | diagnose earlier validation faults; otherwise roll back the entire function with no partial rotation or extent record | `analysis_coverage`, `loop_facts::structurally_valid`, existing id-limit rollback owner |
