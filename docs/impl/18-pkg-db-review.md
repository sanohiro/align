...
| Namespaced builtin type identity | non-entry same-module declarations win bare lookup over compiler-provided aliases; provider-qualified builtins such as `core.Error` remain explicit; entry canonical collisions reject | Q2 | local/qualified/builtin fallback/entry-reject whole/per-unit matrix | draft, language spec, design notes, open questions, DB EN/JA |
