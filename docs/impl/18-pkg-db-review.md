...
| Namespaced builtin type identity | Q2 established the data-driven rule with `Error`/`core.Error`, `argon2_params`/`crypto.argon2_params`, and `regex_match`/`regex.regex_match`; non-entry same-module declarations win bare lookup, exact core/std import rules preserve explicit builtins, `error(c)` stays builtin, and entry canonical collisions reject. The accepted pending asymmetric suite extends that same rule with six bare/`crypto.*` key pairs and owns their expanded sweep in the crypto ledger. | Q2 | parameterized local/qualified/builtin fallback/import/entry-reject whole/per-unit matrix plus local-Error `error(c)`; crypto implementation expands the data set to nine aliases | draft, language spec, core/std EN/JA, frontend and interface prerequisite plans, open questions, DB EN/JA, crypto EN/JA |
...
| D14 | driver-restricted dynamic rows; SQLite static-target scalar functions; separately designed persisted-identity SQLite collations and PostgreSQL callbacks | pre-send mismatch; callback capture/effect/ABI, invocation-view lifetime, registration/replacement/removal, thread/reentrancy/abort/cleanup | dynamic decode plus scalar callback overhead; later collation migration cost |
