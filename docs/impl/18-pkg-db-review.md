...
| Namespaced builtin type identity | the closed alias table is `Error`/`core.Error`, `argon2_params`/`crypto.argon2_params`, and `regex_match`/`regex.regex_match`; non-entry same-module declarations win bare lookup, exact core/std import rules preserve explicit builtins, `error(c)` stays builtin, and entry canonical collisions reject | Q2 | parameterized local/qualified/builtin fallback/import/entry-reject whole/per-unit matrix plus local-Error `error(c)` | draft, language spec, core/std EN/JA, frontend and interface prerequisite plans, open questions, DB EN/JA |
