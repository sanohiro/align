...
- external code implements `LinkOutputSink`; both `_with_output` functions
  ...
  reap when waitable; every external caller passes a private output and, after
  successful return, performs publication isolation into a distinct unexposed
  inode before making any executable path reachable, regardless of tool
  behavior, while error or unwind drops it unpublished;
...
- explicit `SIG_IGN`, `SA_NOCLDWAIT`, another `SIGCHLD` handler, and a second
  ...
  inherited descriptors or the private tool inode briefly after return, but
  cannot extend the bounded parent drain or reach an isolated publication inode;
- ordinary success, child failure, and sink failure observe direct exit without
  ...
  group-preserving and deliberately detached helpers may continue briefly after
  an accepted group signal but can mutate only the tool inode and cannot reach
  the isolated publication inode.
