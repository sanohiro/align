...
| 2c | Success-terse test and CI logs | Implemented — command and binary output remains captured; success emits phase/aggregate summaries, failure replays complete relevant logs with the original exit status, and explicit verbose modes retain local auditability |
...
`apps/web/pkg`, `apps/frame/pkg`, `apps/auth/pkg`, `apps/db/pkg`, `apps/kv/pkg`,
`apps/csv/pkg`, `apps/ws/pkg`, and `apps/template/pkg`. A consumer receives a hit
only for byte-identical reachable units
from the same tagged source. The release still does not embed or ambiently
resolve those source trees.
...
| C6 artifact graph | Warm only after final PGO compiler/runtime production, copy the exact bundle into tar/Debian/Homebrew layouts, and verify every manifest/blob/source-inventory edge in both directions. Derive the complete source-level native-link declaration set and require an explicit package mapping before any route is accepted. | `prepare-prebuilt-cache-project.sh`, `build-prebuilt-cache.sh`, `verify-prebuilt-cache-layout.sh`, and `prebuilt-cache-inventory.py` own the exact current 29-unit source/outcome/action/index/CAS graph; `release.yml` invokes them after PGO phase 3, archives `share`, carries it into Debian and Homebrew layouts, and includes the complete Debian link dependencies. `scripts/test-pr-workflow.sh` owns shared-corpus use and declaration-to-Debian/Homebrew parity. |
...
| Partition record | The driver/codegen seam is `ThinPartition<'a> { unit: &'a str, view: PartitionCodegenView<'a>, impl_hash: Hash128, preserve_symbols: Vec<String> }`. `PartitionCodegenView<'a>` is the sole MIR module-emitter and structural-fingerprint input: `Function { selected: &'a align_mir::Function, definition: ThinPeerDeclaration, peers: Vec<ThinPeerDeclaration>, peer_functions: Vec<&'a align_mir::Function>, shared: PartitionSharedCodegenView<'a> }` or `Support { thunks: Vec<SupportThunkRecord> }`. `peer_functions` is the physical MIR type source required by the existing LLVM ABI lowering, is paired one-for-one with `peers`, and contributes no body or second structural identity: before LLVM work, the emitter requires exact logical identity/order/cardinality and re-derives each physical function's complete canonical ABI equal to its peer record. Peer bodies are never emitted, and the canonical peer records remain the sole fingerprint input. `PartitionSharedCodegenView<'a>` has private fields, is constructible only as `PartitionSharedCodegenView::from_program(&'a Program)`, and borrows the complete canonical struct/enum/resource/tagged/function-type/tuple tables, extern/imported declarations, and callback-effect map while storing their exact shared fingerprint. Its accessors are internal to codegen; callers cannot pair a digest with different tables. `ThinPeerDeclaration { logical: ProgramCall, abi: CanonicalFnAbi, symbol: String, linkage: ThinFunctionLinkage }`; `SupportThunkRecord { drop_thunk: String, representation_version: u32, drop_abi_fingerprint: [u8; 16], owner: SupportThunkOwner }`; and `SupportThunkOwner = Owned { hook: ThinPeerDeclaration } | Imported` derive `Debug`. Support records are one per distinct validated Drop-thunk symbol, sorted by raw symbol bytes; compatible duplicate resource records collapse before both hashing and emission. The sole partition constructor is `function_partitions<'a>(units: &'a [PerUnitArtifact], exports: &[String]) -> Result<Vec<ThinPartition<'a>>, String>`: only `units` supplies the returned lifetime, while validated exports are consumed into owned symbol and preserve records. Before classification, every export must exactly name a stored function in the single entry unit, reusing `unknown_exports`; direct library callers therefore cannot bypass the CLI's fail-closed validation. Unknown names are sorted and deduplicated by raw UTF-8 bytes and return `unknown ThinLTO export roots:<count>:<byte-length>:<lowercase-hex-bytes>...`, with decimal fields free of leading zero except zero. `partition_function_symbol(unit: &str, function: &Function, exports: &[String]) -> Result<(String, ThinFunctionLinkage), String>` is the sole definition/declaration/hash producer. `Root` uses the existing canonical symbol; `UnitLocal` uses the exact unit-qualified record above. Records borrow validated unit MIR and own only derived metadata. `preserve_symbols` is raw LLVM symbol identity, sorted and deduplicated. Neither MIR module emitter may read the original `Program` after view formation; target/profile/runtime-LTO and preserve inputs remain separately keyed as specified below. | Structural owners reject duplicate/missing logical identities, duplicate root symbols, zero/multiple entry units, unknown/mixed known-and-unknown exports, and non-entry-only export names before side effects; normalize duplicate known exports; prove stable ordering and hash identity; and permit the same monomorph `ProgramCall` in two consumer units only with different unit-qualified symbols. The direct-library owner pins exact sorted/deduplicated unknown-export bytes while the existing CLI owner retains its user-facing entry/non-entry diagnostics. Peer-view owners mutate logical identity, order, cardinality, symbol uniqueness, and every ABI field and prove rejection before LLVM work. Shared-view owners mutate each shared field independently, prove every function hash changes, and prove the complete shared record is formatted and hashed only once per unit. Resource owners independently mutate every support-record field, local-hook symbol/ABI, owned/imported classification, order, and compatible duplicate and prove hash/emission agreement. A compile-time signature owner proves the result survives release of the shorter `exports` borrow but not release of `units`. Every other `Err` is a static-prefix internal-compiler diagnostic plus lowercase length-delimited offending identity; duplicate roots use `duplicate ThinLTO root symbol:<decimal-byte-length>:<lowercase-hex-bytes>`. No OS error text enters formation. |
...
`impl_hash` is a two-level structural hash. Once per source unit,
`PartitionSharedCodegenView::from_program` computes
`Hash128::of(format!("align-thin-partition-shared-v1\n{shared_tables:?}").as_bytes())`,
where `shared_tables` is an exact private record in this order: complete
canonical struct, enum, resource, tagged, function-type, and tuple tables;
extern and imported declarations; and callback-effect facts. Every partition
then computes
`Hash128::of(format!("align-thin-partition-impl-v2\n{view:?}").as_bytes())`.
The custom `Debug` view record contains `selected`, `definition`, `peers`, and
the stored `shared_fingerprint` for a function, or the complete `thunks` for
support. The shared view stores both the borrowed tables and fingerprint;
private fields plus its sole constructor prevent a digest/table mismatch. The
compiler build id already namespaces this internal representation, as it does
for the current complete MIR structural hash. This Merkle-shaped identity is required:
formatting the same large `pkg.db` type graph independently for every one of
604 function partitions made an optimized cache-hit build slower than the
pre-item-6 cold unit build before LLVM did useful work. A function identity
covers the selected full `Function`, its definition record, every referenced
local peer ABI declaration (logical name, canonical
parameter/return/borrow/region/cleanup ABI, `ThinFunctionLinkage`, and exact
raw symbol), the complete shared fingerprint, and the function role. It
excludes every unrelated local function body while any shared-table change
still invalidates every function in that unit.
...
1. validate the complete unit list, require exactly one entry unit, and validate
   partition-independent per-unit MIR envelope/table invariants;
2. collect raw local function identities and reject duplicate logical
   identities, a missing selected body, or a body/declaration ABI mismatch;
   then validate the complete export set against the entry unit, reject its
   sorted unknown-name set, and only then classify every root versus unit-local
   definition through the sole symbol producer and reject duplicate canonical
   root symbols or more than one C `main`; equal unit-local symbols in different
   units are impossible because the unit bytes differ;
...
step-2 function identity/ABI failures in logical-name byte order precede an
unknown-export set, which precedes root-symbol/main failures and the step-3
support-inventory failure; multi-invalid owners pin that product. Once
...
| Axis | Required closure | Owner |
| --- | --- | --- |
| Formation and validation | Every MIR function appears in exactly one function partition; every owned resource thunk appears in exactly one support partition; imported/resource/runtime declarations have no accidental definition. Exactly one entry unit exists, and every explicit export resolves in it before root classification can silently omit a requested symbol. Complete type and callable preflight still runs before lowering. Function identity/ABI and unknown-export errors precede support-inventory errors before side effects; producer failures use support-first partition order afterward. Malformed ids, tables, descriptors, duplicate names, and unknown thin-link module ids diagnose instead of panic. | partition inventory, direct-library and CLI unknown/non-entry export, function-plus-export-plus-support multi-invalid precedence, unknown/duplicate thin-link edge, codegen malformed-MIR, resource, callback, and ABI owners |
...
| Partition fingerprint symmetry | The sole partition constructor closes formation, hashing, and emission. `PartitionSharedCodegenView::from_program` borrows and fingerprints the exact shared tables once per unit behind private fields. Function views contain exactly the selected body, definition/peer records, their one-for-one ABI-validated physical peer sources, and the sealed shared view consumed by function emission; support views contain exactly the deduplicated thunk records consumed by support emission. Physical peer bodies are not emitted or independently fingerprinted; their logical identity/order/cardinality and canonical ABI must agree with the fingerprinted peer records before LLVM work. No emitter re-reads `Program` to rediscover a symbol, ABI, ownership class, declaration, or table. | total-field mutation sweep for both view variants and every shared-view field; peer source/record identity, order, cardinality, symbol, and ABI mismatch rejection; one-shared-format/hash-per-unit counter owner; owned/imported and duplicate-thunk resource mutations; hash-change iff emitted prelink changes; exact-revert CAS hit; source guard against post-view `Program` access |
...
The first optimized implementation probe on the fixed `pkg.db` corpus exposed
one pre-review design defect before publication. The candidate formed 604
function partitions: both a cold run with 604 backend misses and a hot run with
604 backend hits took about 14.4 seconds, while the pre-item-6 unit-ThinLTO cold
baseline took 13.7 seconds on the same host. Because misses and hits were equal,
the repeated formatting of each unit's complete shared tables—not LLVM
production—was the dominant fixed cost. The two-level shared fingerprint above
is the closure: exact shared invalidation remains fail-closed, but that large
record is formatted and hashed once per unit rather than once per function.
These diagnostic numbers identify the defect; the required alternating
seven-run benchmark remains the acceptance evidence for the corrected
implementation.

The corrected implementation passed that acceptance harness on 2026-08-29.
Across seven counterbalanced fresh-cache edit samples, the unit baseline was
`4.606917 s` median and the function path was `0.297229 s` (`6.45%`), with the
private leaf producing one prelink miss in each path and narrowing the backend
miss frontier from eight unit objects to seven function objects. Cold
end-to-end wall time was `16.244868 / 15.496857 = 104.83%`; peak RSS was
`288488 / 262696 KiB = 109.82%`. The final executable size ratio was `1.0001`,
unit/function runtime output agreed, and function cache-off/cold/hot
executables were byte-identical. `bench/function_incremental/run.sh` owns the
reproduction and thresholds; these measurements are not a CI gate.
...
each partition identity to its digest and phase outcomes. The seventh pass
found one P2 fail-closed validation omission: a direct library caller could
supply an unknown export. The existing entry-unit `unknown_exports` seam now
runs before root classification and before every side effect, with an exact
library error and direct owner; this is a local validation/diagnostic correction
to the reviewed boundary. The implementation author-side matrix-to-diff pass
then found two exact-seam omissions: LLVM ABI lowering still needs a borrowed
physical peer type source, and the public symbol producer had lost its settled
`Result` boundary. The ledger now names the one-for-one physical witness while
keeping canonical peer records as the sole structural identity, and the symbol
producer itself rejects malformed unit identity:
...
| A direct library caller could supply an export root absent from the entry unit and have it silently ignored | Reuse `unknown_exports` inside partition formation before symbol classification, define the sorted/deduplicated length-delimited library error, retain the CLI's existing entry/non-entry messages, and add direct plus multi-invalid owners. |
| The exact partition view omitted the physical MIR type source required to lower a peer declaration's LLVM ABI | Add `peer_functions: Vec<&Function>` as a one-for-one physical witness only. Before LLVM work, require canonical logical identity/order/cardinality, unique symbols, and a re-derived complete ABI equal to each fingerprinted peer record; never emit or independently fingerprint a peer body. |
| `partition_function_symbol` was specified as fail-closed but implemented as an infallible tuple producer | Restore the exact `Result<(String, ThinFunctionLinkage), String>` seam and reject empty/embedded-NUL unit identity in the sole public producer, even when called outside validated partition formation. |
