...
| compiler/cache identity | Packaged keys keep the exact existing tuple: `CACHE_KEY_FORMAT_VERSION=3`, `UNIT_KEY_FORMAT_VERSION=1`, `align_interface::FORMAT_VERSION`, and the hash of the running `alignc` bytes, plus every current source/dependency/target/profile/LLVM/runtime-LTO/PGO component. There is no release-version fallback. A repackager that changes the executable bytes gets clean misses, never cross-compiler reuse. | Each native runner warms with the final PGO-use `alignc` that is copied into that same archive. Cache-root paths, archive paths, mtimes, uid/gid, and compression are excluded as today. Target and LLVM identity make all three native bundles disjoint even before the compiler fingerprint. | Final-layout smoke runs the copied packaged binary and requires exact hits; a one-byte different compiler, key/schema bump, LLVM change, target change, and interface-format bump all miss. |
...
| applicability | Frontend entries may serve every existing profile/CPU/runtime-LTO/PGO choice because those inputs remain absent from `UnitKey`. Shipped codegen entries serve only the default release/baseline/runtime-LTO-on/PGO-off/non-ThinLTO tuple. Other backend tuples may still take a packaged frontend hit but must codegen-miss. ThinLTO prelink/backend entries are not distributed. | Exact existing keys enforce every case; no option is normalized toward the warmed tuple. Package private-body edits invalidate that unit's codegen; public/interface edits also invalidate importers according to the existing structural dependency closure. Exact revert may re-hit the packaged entry. | Cartesian owner crosses package unchanged/private edit/public edit with frontend/codegen and default/dev/fast/native/no-rt-lto/PGO/ThinLTO modes, pinning field presence and result order through the existing cache report. |
...
There are no detail levels, verification states, unavailable fields, native
handles, text/wire inputs, connection-global operations, or user-visible
record ordinals in this contract. The applicable products are the environment
matrix, lookup matrix, and build-configuration matrix enumerated above; their
orders and first failures are fixed there rather than delegated to an
installer.
...
| C7 identity parity | Whole-program/per-unit construction, generic interface serialization, runtime-LTO digest, target/LLVM resolution, compiler provenance, and allocation behavior reuse existing keys/codecs without a parallel format. |
