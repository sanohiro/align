...
| Build entry | The additive driver entry is `build_function_thin_lto(units: &[PerUnitArtifact], cache: &CacheContext, target: &BuildTarget, profile: Profile, exports: &[String], rt_lto: bool, jobs: usize) -> Result<FunctionThinLtoBuild, String>`. `FunctionThinLtoBuild { pub outcomes: Vec<CacheOutcome>, pub objects: Vec<PathBuf>, pub prelink_bc: Vec<PathBuf>, object_stage: ArtifactStage }` has a private stage field: external callers can read paths but cannot construct the result, and Drop removes every private path. Object cardinality and controlled names are discovered from MIR. The function validates the complete borrowed inventory and response-path representation before it creates `object_stage`; no caller can create the stage early through this seam. | Library owners cover zero, one, and many source units/functions, disabled and enabled caches, `jobs == 0` normalization to one, deterministic error selection, result construction privacy, and stage Drop. Existing `build_thin_lto` remains available for unit-granular tests and non-CLI callers until the hard cutover PR updates them coherently; no dual CLI path ships. |
...
The first independent adversarial review found three P2 contract gaps. They
share the `route-stage-ownership-and-native-path-encoding` axis, so the matrix
was reopened on that axis and the class was corrected in one pass:
