...
7  successful-build stdout line write and flush
...
`remove 'ENCODED_PATH': MESSAGE`, `backend: MESSAGE`, or
...
informational and has no control-flow meaning. Failure to write this line is
the W2 unavailable-stderr exit, not a recursive watcher error.
...
| D8 resource bound | Exact/rejected-next input, resolved-graph, symlink-traversal, encoded-path, and dynamic-message capacities; one-slot wake channel; one non-overlapping periodic scan; streaming snapshots/identity comparison; nonblocking special-file rejection; checked counters; and memo refusal are all reachable. | Parameterized limit/traversal/encoding/message/overflow/FIFO/sparse-file/RSS/timer/identity owners. |
| D9 observable transcript | Marker count/order, stdout/stderr write/flush order and failures, reversible single-line paths/messages, output-alias error, ordinary diagnostics, cache stats, stdout, object bytes, link-input order, executable behavior, and exit behavior are exact for success, failure, recovery, and watcher error. The linked executable itself is not byte-compared: macOS link products carry link-time `LC_UUID`/page-hash identity. | Runtime-loaded pipe-backed golden transcript plus all-byte path/message injection and object/link-input/program-output comparison with one-shot `alignc build`. |
