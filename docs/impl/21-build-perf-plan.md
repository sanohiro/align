...
|---|---|---|---|---|
| W1 | CLI | `alignc build FILE.align --watch`. `--watch` is a valueless, idempotent flag accepted only by `build`; every existing `build` flag and environment setting keeps its current meaning. `run`, `size`, inspection verbs, `cache clear`, and `db` reject it before reading source. `build --help` describes it as `rebuild on compiler-observed file changes; restart after toolchain or external linked-library changes`. There is no configurable interval/debounce, daemon, socket, or background option. | Arguments, resolved target/profile/jobs/cache/linker choice, PGO mode, current working directory, and external `-lNAME` library search environment are fixed at startup. The compiler/runtime/LLVM/linker/system libraries and every capability/user `-lNAME` result are explicit restart-only session preconditions: their paths, metadata, and bytes are not watched, and replacing one requires process restart. Invalid combinations exit 1 before watcher creation. No new environment variable. | CLI parser/help table and real-binary invalid-combination/non-impact owner. |
...
| W10 | Cache and artifact identity | No persisted format, namespace, cache key, compiler fingerprint, interface, object, runtime, or link identity changes. An attempt consults the existing persistent caches and process memo in their settled order. Source edit/revert is content identity, never mtime identity. `ALIGNC_CACHE=off` disables persistent reuse but leaves the already-settled in-process memo active. ThinLTO and PGO keep their existing key spaces and validation order. | The only long-lived allocation is the memo and watch state. Cache rejection markers remain process/persistence authoritative. A watch event cannot authorize a cache key or artifact. W1's external linked libraries affect the link result exactly as in one-shot builds but remain outside compiler/cache identity and require restart for watch-mode relinking. | Memo-on/off, cache-on/off, edit/revert, rejected-key, ThinLTO, PGO, and external-library restart owners reuse existing cache matrices plus one multi-revision driver owner. |
...
|---|---|---|
| D1 CLI parity | Every existing build flag/default/error composes with W1; every other verb rejects `--watch`. Startup state and the external-library restart boundary are resolved/described once. | `main.rs` parser/help table and `watch_build` real-binary matrix, including exact help and no revision for a replaced external archive. |
...
| D5 build control paths | Initial/repeated success; ordinary and ThinLTO frontend/codegen/link/publication, including the one-unit ThinLTO fallback and multi-unit ThinLTO path; semantic-instability/output-alias rejection before link; ordinary rehydration/PGO and one retry with both observation sets; cache on/off; every applicable cache-stat shape; edit/revert; malformed input. External linked libraries retain W1's restart boundary. | Existing ordinary/ThinLTO owners remain authoritative; `watch_build` adds revision sequencing, consuming retry merge, final semantic/topology validation, last-good publication, recovery, external-library non-revision/help, and observed-input parity for each route/cardinality. |
...
| D10 non-impact | One-shot build/run/size, `align-repl`, inspection verbs, release layout, and docs remain unchanged except for the additive flag/help text and its explicit external-library restart boundary. | Existing bounded gate, REPL owners, release workflow structural test, usage/help snapshot, and external-library replacement owner. |
...
| P2: project-supplied `-l` archives were absent from both observation and the restart boundary | W1/W4 make every capability/user linked library an explicit restart-only external input even under `LIBRARY_PATH`, and the exact help text states that limit. D1/D5/D10 own replacement-without-revision and restart behavior. |
...
- `build --help` names the external-library restart boundary; replacing a
  project archive selected through fixed `LIBRARY_PATH` starts no revision,
  while restarting watch links the replacement exactly as a one-shot build;
