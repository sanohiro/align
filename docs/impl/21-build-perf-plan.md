...
W3 exclusively precreates the randomized tool stage before W13, retaining its
open close-on-exec descriptor and identity while external tools receive only
the pathname and must preserve that inode. After W13 releases its child lease,
W3 first nofollow-verifies that retained ownership; only then does it create the
distinct publication stage through the same same-directory exclusive allocator.
...
decision. Permission mode in W3 is exactly `st_mode & 0o777`; no set-id or
other inode metadata is copied. W3 then nofollow-compares the tool-stage name
with its still-open descriptor: a matching entry is unlinked, a missing entry
needs no cleanup,
...
direct child; the parent then never consults the numeric PGID again. In-group
and deliberately detached descendants may continue briefly, and W3's separate
publication inode prevents either class from mutating the candidate that can
reach the public path.
...
- the child sees only a randomized tool-output stage. After successful child
  ...
  hash. Mutation before, during, or after the copy either
  selects exact `link
