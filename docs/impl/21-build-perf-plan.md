...
returned after child reap, group absence, and lease release. The watch sink retains its first W2
...
first-signal slot and maps SIGINT/SIGTERM to `SigInt`/`SigTerm`; it never locks,
formats, allocates, or writes a record. `None` is the only result for ordinary
external sinks unless they explicitly opt into the same stop contract. The
50-ms pump poll is the maximum detection delay while a captured child is live.
Forwarded stop is not a watcher error or `failed` attempt: mandatory child
cleanup returns its exact internal sentinel, then W9's already-stored signal
wins the post-return snapshot and emits the sole `stop` record.

The matching group signal retries `EINTR`. An accepted send starts the 250-ms
grace; `ESRCH` skips directly to reap and absence proof, while another error
records exact `child group cleanup: MESSAGE` and enters immediate SIGKILL
cleanup. For an external sink without W9's pending signal, that first cleanup
error outranks the stop sentinel; under watch, W9's already-stored signal
outranks both. Every stop path still proves group absence and reaps the direct
child when waitable before the stage, lease, or caller can proceed.
...
the parent enters the same SIGKILL/reap/group-absence protocol as every other
outcome. The fixed 10-ms group check retains the stage and captured-child lease
until `ESRCH`; elapsed time does not convert an unproved cleanup into return.
...
10 W13 captured-child lease and `SIGCHLD` disposition, pipe/group/spawn setup,
   child-output stop/write return/panic or status/poll/read result, forwarded
   stop/grace when selected, mandatory process-group quiescence, child reap when
   waitable, lease release, resumed sink panic or link and publication
...
forwards it to the private child group, escalates after 250 ms, proves group
absence, and returns without a `failed` marker before the parent drops stages
and emits `stop`. Terminal-generated and parent-only SIGINT/SIGTERM therefore
have the same bounded child-cleanup path.
...
| D4 event lifecycle | Startup-before-init, initialization, pre-`started`, normal/identical/burst/max-debounce/full-pipe/overflow/path-loss/watch-loss events, event during compile, W15 immediate/timed retry and tentative-generation compaction, the complete prior/current 4x4 W5 state product including readability recovery, every fatal class/first-error order, periodic audit, failed-set and repair-output merge, idle/active shared-pipe signal wake, signal/fatal/synchronous-transition precedence, and terminal cleanup wake/stop exactly once. | Injectable event/timer/error source, one shared pipe, dirty/uncertain/fatal/signal atomics, startup and every before/after-transition control snapshot, W15 retry budget/deadline/generation state and repeated-churn peak bound, total semantic classifier, logical/repair merge owner, bounded-read poll owner, backend shutdown, and process teardown; native adapters stay thin. |
| D5 build control paths | Initial/repeated success; ordinary/ThinLTO cardinalities; semantic/alias rejection and repair including W14's both-absent folded slot; retry/PGO/cache combinations; child output/success/failure/abort; edit/revert/malformed input; and both W1 trigger-excluded boundaries preserve ordinary pre-link/link/publication semantics. Both W13 `_with_output` entry points cover ordinary, PGO-use, PGO-instrument, ELF strip, and macOS strip without changing the legacy entry points. | Existing owners plus `watch_build` revision, retry, finalization, collation/repair recovery, exact five-symbol W13 inventory, external sink default/forwarded stop and return/panic with resumed payload, accepted/rejected `SIGCHLD`, captured-child overlap, pump/process-group quiescence, last-good publication, external/resource-freshness trigger, and route/cardinality parity owners. |
| D6 concurrency and cleanup | Worker panic cleanup remains; callbacks/handlers do not block; atomics select overflow/fatal/signal; evidence, repair tokens, W14 random probes, W15 partial handle sets, pump buffers/fds/process groups, captured-child lease, workers, stages, handles, control fds, and direct children do not outlive owners. Signal setup is singleton and rollback-complete before publication; afterward the still-installed handlers and shared pipe remain process-lifetime with no close/reuse race. Every watcher/control/probe fd is close-on-exec. Descendant-retained, continuously written, or permanently unreadable child-output pipes cannot release cleanup before proved group absence. | Existing panic owner plus finalization/repair Drop, W14 create/lookup/pre-unlink-identity/unlink/link-count/fd cleanup and cooperative-collision barrier, W15 reverse rollback/idempotent loss/tentative-generation compaction/unexpected failure, watcher atomics, every startup/signal barrier, backend shutdown while the process-lifetime pipe remains live, terminal `process::exit`, exec descriptor-table negative, W13 lease/disposition/partial-setup rollback, armed-child-guard Drop at every barrier, and unwind-safe sink cleanup/resumed unwind, blocking-write/nonblocking-read, every-outcome drain/group-KILL/reap/10-ms-absence, forwarded stop detected within 50 ms then matching signal/250-ms grace/KILL/reap/absence, permanent-error TERM/grace/KILL/reap/absence, two-publisher barrier, and bounded child owners. |
...
| D8 resource bound | Exact/rejected-next length/NUL/absolute-path/input/combined-input-repair-graph/evidence/finalization/symlink/encoding/message/frame/post-exit limits and each bounded offending identity; W14 descriptor `NAME_MAX`, 1,023-byte effective component limit, 36-byte suffix, 16 attempts, one directory fd plus one probe fd and two fixed 1,024-byte buffers; W15 eight immediate attempts, 50-ms deadline, one tentative generation, and pre-next-generation compaction within existing handle maxima; W13 one lease bit, one armed guard record, one caller-owned panic payload, fixed status record, existing 50-ms pump poll as the stop-detection bound, 250-ms abort grace, and one 10-ms group check with no cleanup timeout; shared wake pipe/fatal slot; non-overlapping scan; 64 KiB hash and fixed transcript/pump buffers; nonblocking special-file rejection; checked counters; and memo refusal are reachable. | Parameterized limit/repair/encoding/frame/pump/snapshot/overflow/fatal/FIFO/sparse/RSS/timer/probe/registration-generation owners assert the encoded logical path or path pair, or fixed length/hash for the overlong case, on every refusal. |
| D9 observable transcript | The single stderr protocol stream fixes startup-without-`started`, marker/prerequisite order, marker atomicity, write/flush/wait-setup/sink/status/poll/read/group-cleanup failure, no-stdout behavior, reversible frames for every nonterminal record, instability/alias/collation/resource/backend/wake/pump errors with offending identity, non-diagnostic W15 expected path/watch loss, decoded in-process/child logical channels, object/link order, behavior, and exits for success/failure/recovery/watcher error. Active-child SIGINT/SIGTERM forwards the same signal and emits only the terminal `stopped` line after cleanup, never `failed` or watcher error. Linked executables are not byte-compared on macOS. | One-pipe golden transcript and no-panic private-watch-sink owner with every startup checkpoint, kind/state/chunk boundary, all-byte and marker-shaped diagnostic/cache/message/path/child injection, decoded bounded parity, short marker writes, full/partial/flush/wait-setup/sink/status/poll/read/group-cleanup errors, terminal-generated and parent-only SIGINT/SIGTERM during every child phase, mutation/repair/collation/registration-compaction, exact limits/classes/identities, and one-shot object/link/program comparison. |
| D10 non-impact | One-shot build/run/size, both legacy inherited-stdio link entry points, `align-repl`, inspection verbs, release layout, and public input/output slots remain unchanged except for the additive flag/help text, W5 records, W13 `LinkStopSignal`/sink/link entry points and their explicit default-control and `SIGCHLD` caller contracts, W14's reserved private probe namespace, and the external-resource/runtime-freshness trigger boundaries. | Existing bounded gate, W14 public-slot nonmutation plus cooperative-concurrency, clean-path no-residue, and cleanup-failure residue-identity owners, legacy/new link-route parity, external-sink default/opt-in stop owners, REPL owners, release workflow structural test, usage/help snapshot, and parameterized external-resource/runtime-source mutation owner. |
...
- external code implements `LinkOutputSink`; both `_with_output` functions
  ...
  injected `SigInt` and `SigTerm` return their exact stop sentinels only after
  the captured group is absent and its direct child is reaped when waitable;
...
- explicit `SIG_IGN`, `SA_NOCLDWAIT`, another `SIGCHLD` handler, and a second
  ...
  closes both reads and enters group quiescence. A permanent poll/read failure
  ...
  250 ms when required, reaps the direct child when waitable, proves group
  absence, releases the lease, and emits the first pump error;
...
- ordinary success, child failure, and sink failure observe direct exit without
  ...
  pinned process group, reap only when waitable, and retry at 10-ms intervals
  until `ESRCH`; an injected in-group descendant cannot write the staged
  executable after return, while group-kill/check failure selects exact `child
  group cleanup` ahead of child status and retains the stage/lease until the
  group is absent;
- an external sink panic on either stream after zero or several callbacks runs
  ...
  immediately kills and reaps the child group, releases the lease, and resumes
  the identical payload on the calling thread without a second hook; the caller's resumed
...
- an injected unexpected unwind at every post-spawn pump barrier runs the armed
  guard's no-panic cleanup before crossing the public seam; the child/group and
  lease are absent, the caller's stage then drops, and a following captured call
  acquires the lease;
...
- idle and active SIGINT/SIGTERM return 130/143 only after every owned child,
  ...
  when required before group absence. They emit only the matching `stopped`
  line, never `failed` or watcher error. Signals at every terminal barrier still
  target the unreused process-lifetime pipe, and the kernel closes that pipe
  with the process.
