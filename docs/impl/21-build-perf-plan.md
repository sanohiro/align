...
W3 creates the publication stage only after W13 releases its child lease. It
uses the existing same-directory exclusive private-stage allocator; external
tools receive neither that name nor its descriptor. Isolation order is tool
stage nofollow-open and `fstat`, 8-GiB length check, first streaming source
hash, exclusive publication-stage creation, exact-length copy, executable-mode
application, second source `fstat`/hash, destination `fstat`/hash, equality
decision, close/unlink of the tool stage, nofollow comparison of the
publication-stage name with its still-open descriptor, close, then atomic
rename. Missing or unequal pre-rename identity preserves the foreign entry and
returns exact `publication stage ownership lost`. `EINTR` retries and a
premature source EOF or extra byte is `link output changed during isolation`.
The first operational error is retained; cleanup still closes and unlinks each
owned private stage in reverse ownership order, preserves an unequal current
entry, and a cleanup error replaces that result only when it is the first
error. An unexpected unwind drops both stage guards without publishing. A
...
TERM/grace/KILL/quiescence/reap path; an unexpected `ECHILD` status failure is
exact `child wait: MESSAGE` and enters that same quiescence path. A
...
`ECHILD` enters group quiescence and cannot release the stage or lease early.
...
The private watch sink's `stop_signal` performs only an Acquire load of W9's
first-signal slot and maps SIGHUP/SIGINT/SIGQUIT/SIGTERM to
...
they explicitly opt into the same stop contract. The 50-ms pump poll is the
maximum detection delay while a captured child is live. Forwarded stop is not
...
results close further user execution by the direct child and owned in-group
set; the parent then reaps the leader and never consults the
numeric PGID again. A deliberately detached process is outside W13's declared
owner, and W3's separate publication inode prevents it from mutating the
candidate that can reach the public path.
...
child case the private sink reports the parent's first signal within 50 ms, W13
forwards it to the private child group and direct pid, escalates after 250 ms,
sends final SIGKILL to both while the direct leader still pins their identities,
and returns without a
...
- external code implements `LinkOutputSink`; both `_with_output` functions
  ...
  reap when waitable; an external caller passes a private output and either
  performs publication isolation or supplies only group-preserving tools;
...
- the child sees only a randomized tool-output stage. After successful child
  cleanup, W3 copies it into a distinct unexposed publication stage and accepts
  only matching source-before/source-after/destination identity, length, and
  hash. Mutation before, during, or after the copy either selects exact `link
  output changed during isolation` with no publication or affects only the
  unlinked tool inode; injected open/copy/mode/hash/close/unlink/rename failures
  and pre-rename publication-stage replacement preserve the last-good output,
  while the replacement is not removed and exact `publication stage ownership
  lost` is selected; a stable copy preserves exact executable bytes and mode.
...
- ordinary success, child failure, and sink failure observe direct exit without
  ...
  child that changes group selects exact `child group changed`; an injected
  in-group descendant cannot execute further user code after the final signal,
  while a deliberately detached helper may mutate only the tool inode and
  cannot reach the isolated publication inode.
...
- an external sink panic on either stream after zero or several callbacks runs
  ...
  unwind then drops the already-quiescent stage;
- an injected unexpected unwind at every post-spawn pump barrier runs the armed
  guard's no-panic cleanup before crossing the public seam; no owned in-group
  process can execute further user code, the direct child is reaped when waitable, the lease is
  absent, the caller's private stage then drops, and a following captured call
  acquires the lease;
...
- idle and active SIGHUP/SIGINT/SIGQUIT/SIGTERM return 129/130/131/143 only
  ...
  observed within 50 ms, forwarded unchanged, allowed 250 ms, and escalated
  when required before pinned-group/direct-pid SIGKILL and direct reap. They
