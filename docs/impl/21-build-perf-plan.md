...
| W2 | Attempt protocol | Watch mode owns exactly one ordered protocol stream: process stderr. It writes nothing to process stdout. Revision 1 starts immediately. Each attempt writes exactly `alignc: watch: revision N started` before reading an input, then preserves the ordinary build's diagnostic/cache-stat/success-line meaning on that stream; W13 labels the original channel of child bytes. After successful atomic publication, the same owner writes `alignc: built executable: ENCODED_PATH`. Before either terminal marker, it flushes that one stream. While holding its sole lock, it then performs one unbuffered write on the same descriptor of the complete <=128-byte line `alignc: watch: revision N ready\n` or `alignc: watch: revision N failed\n`; `EINTR` retries, and a short/failed write exits 1 without another marker. On a pipe this write is below `PIPE_BUF` and atomic. Same-descriptor write order makes every preceding diagnostic, child frame, cache line, and success line observable before the marker. `N` starts at 1 and increments by one; exhaustion is a watcher error. Any prior protocol write/flush failure emits W9's error only if a later write remains possible, then exits without a terminal marker; another failure exits without recursion. Every watch-mode protocol write is fallible. | All filesystem paths use `WatchPath`: percent-encode each raw Unix path byte outside `[A-Za-z0-9._/-]` as uppercase `%HH` (including `%` as `%25`). Dynamic watcher-error messages use `WatchText`, which retains printable ASCII except `%` and percent-encodes every other UTF-8 byte. `WatchPath` and admitted `WatchText` values are reversible single-line encodings. `WatchText` accepts at most 16,384 input bytes; a longer message becomes the static replacement `message exceeds 16384-byte limit`. A private single-writer `WatchTranscript` routes in-process diagnostics, cache stats, success, child frames, errors, stop lines, and markers. Every spawned `cc`/linker/strip stdout and stderr is piped through W13, never inherited. Automation consumes only this stderr stream, accepts only a complete terminal line, and never infers readiness from quiet time. | One-pipe transcript owner covers initial success/failure, two rebuilds, write/partial-write/flush failures, same-stream ordering, no process stdout, atomic marker write/short-write/`EINTR`, success-line visibility, every-byte path round trips, accepted/rejected-next messages, dynamic newlines, arbitrary child bytes, and injected marker-shaped paths/messages. |
...
| D4 event lifecycle | Normal/identical/burst/max-debounce/full-pipe/overflow events, event during compile, every fatal class/first-error order, periodic audit, failed-set and repair-output merge, idle/active shared-pipe signal wake, signal/fatal precedence, and terminal cleanup wake/stop exactly once. | Injectable event/timer/error source, one shared pipe, dirty/uncertain/fatal/signal atomics, logical/repair merge owner, bounded-read poll owner, backend shutdown, and process teardown; native adapters stay thin. |
...
| D9 observable transcript | The single stderr protocol stream fixes marker/prerequisite order, marker atomicity, write/flush failure, no-stdout behavior, reversible path/message/child-byte frames, instability/alias/resource/backend/wake/pump errors, decoded in-process/child logical channels, object/link order, behavior, and exits for success/failure/recovery/watcher error. Linked executables are not byte-compared on macOS. | One-pipe golden transcript with all-byte/marker child injection and decoded bounded parity, short marker writes, full/partial/flush errors, mutation/repair, exact limits/classes, and one-shot object/link/program comparison. |
...
- an unrelated event and an identical-content rewrite trigger none after state
  comparison;
- one stderr-pipe reader observes every diagnostic, child frame, cache line,
  and ordinary success line before the matching `ready`; watch-mode stdout is
  empty, and injected protocol full/partial write or flush failure emits no
  false `ready` and exits 1 without recursion;
...
- each asynchronous backend class produces its exact static line; two errors
  ...
  EOF/error on the pipe fails closed;
