...
| C3 codegen read | Parameterize action/CAS lookup by policy; preserve digest verification, object materialization, primary corruption unlink, packaged unavailable-I/O clean miss, and no packaged mutation. | `try_hit`/`materialize_blob` take `ReadPolicy`; cache owners cover packaged hit, primary precedence/corruption fallback, unavailable packaged blobs, digest-bad packaged corruption immutability, and zero-producer hit. |
