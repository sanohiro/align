...
|---|---|---|---|---|
| W1 | CLI | `alignc build FILE.align --watch`. `--watch` is a valueless, idempotent flag accepted only by `build`; every existing `build` flag and environment setting keeps its current meaning. `run`, `size`, inspection verbs, `cache clear`, and `db` reject it before reading source. There is no configurable interval/debounce, daemon, socket, or background option. | Arguments, resolved target/profile/jobs/cache/linker choice, PGO mode, and current working directory are fixed at startup. Invalid combinations exit 1 before watcher creation. No new environment variable. | CLI parser table and real-binary invalid-combination owner. |
| W2 | Attempt protocol | Revision 1 starts immediately. Each attempt writes exactly `alignc: watch: revision N started` to stderr before reading an input, then preserves the ordinary build's diagnostics, cache-stat report, and success line. After successful atomic publication it flushes stdout before writing exactly `alignc: watch: revision N ready` to stderr; an ordinary build failure terminates with exactly `alignc: watch: revision N failed`. `N` starts at 1 and increments by one; exhaustion at `u64::MAX` is a watcher error. A stdout-flush failure instead emits the W9 watcher-error line and exits 1 without a false `ready`. | Marker strings are static. Formatting allocates one bounded line. stdout remains the ordinary build surface; a successful flush makes the stderr terminal marker a transcript boundary even when stdout is a pipe. Automation waits for that marker and never infers readiness from quiet time. | Pipe-backed transcript owner covers initial success/failure, two rebuilds, success-line-before-ready visibility, and injected flush failure. |
| W3 | Publication | Each attempt uses the existing applicable ordinary or ThinLTO per-unit pipeline and its current retry rule. Before a successful candidate may link/publish, the W6 resolver compares the fixed cwd/output directory-entry slot with both the lexical and final resolved slots of every observed input. If any match, the lowest platform-byte-sorted input emits exactly `alignc: watch: output 'OUT' aliases observed input 'INPUT'`, the revision produces `failed`, and no link or rename runs. Other successful candidates retain the existing link and same-directory atomic rename. Absent an external writer, any failed revision neither removes nor replaces an earlier executable. Only `ready` reports that this process completed publication; a pre-existing or last-good file may remain after `failed`. | `OUT` is the absolute lexical startup-cwd path for the existing `stem(FILE)` output. A slot is `(parent device, parent inode, final component bytes)` after resolving parent symlinks; an input contributes its lexical slot and, when resolution reaches a leaf, its target-side slot. Comparing slots rather than leaf inode rejects parent/symlink aliases that publication would replace while allowing a distinct hard-link name whose source entry survives. Each attempt owns and drops its private object and publish stages before waiting. Existing link/publication errors remain ordinary failures. Concurrent external mutation after the final alias snapshot retains W12's existing last-writer rule; the compiler provides no cross-process filesystem transaction. | Parameterized same-resource owner covers direct slot, parent-directory alias, leaf symlink to output, distinct hard link, and barriers before/after the final snapshot; real-binary owner also runs last-good output after later compile/link failure. |
| W4 | Observed build inputs | The ordinary route uses `align_driver::build_path_pipelined_observed`, which owns the entry read and one call to the same private pipeline implementation as `build_package_pipelined`. The ThinLTO route uses `align_driver::build_path_per_unit_observed`, which owns the same read/observation front half and returns the existing `PerUnitWalk` consumed by the current ThinLTO CLI branch. Both return the complete ordered `BuildInputSet` even when entry read or frontend fails. The set records the entry source, every present or missing user-module path, every validated file-backed static source and checked-metadata path, and, on the ordinary PGO route, the exact `--pgo-use` file. Inline static sources and static paths rejected before root/path validation add no target; their owning Align source is already watched and is the only input that can repair that error. Paths are absolute lexical paths rooted at the startup cwd/project root; missing paths are not canonicalized. The installed compiler directory, its baked runtime-LTO bytes, adjacent runtime archive, loaded LLVM image, linker, and system libraries are an immutable toolchain precondition for one session and require process restart when replaced. | Each record owns one `PathBuf` and a small semantic state. Regular-file state owns only `Hash128` and length, never file bytes. Missing, non-regular, and I/O states own no payload. The set is sorted by platform path bytes and deduplicated; observing one path under two different states sets `changed_during_attempt`. Native file identity belongs to the private watcher snapshot, not the build/cache identity. | One parameterized library owner runs the entry/import/static/metadata present, missing, replacement, invalid-path exclusion, invalid UTF-8, and duplicate-read matrix through both ordinary and ThinLTO front halves; the ordinary arm additionally covers PGO. |
| W5 | Input record | `pub struct BuildInput { pub path: PathBuf, pub state: BuildInputState }`; `pub enum BuildInputState { Missing, Regular { content_hash: Hash128, len: u64 }, NonRegular, Unreadable }`; `pub struct BuildInputSet`; `impl BuildInputSet { pub fn inputs(&self) -> &[BuildInput]; pub fn changed_during_attempt(&self) -> bool }`; `pub enum BuildSourceError { Missing, NonRegular, InvalidUtf8 { offset: u64 }, Io { message: String } }`; `pub enum ObservedBuildAttempt { SourceFailed { error: BuildSourceError, inputs: BuildInputSet }, Pipeline { build: PipelinedPackageBuild, inputs: BuildInputSet } }`; `pub enum ObservedPerUnitBuild { SourceFailed { error: BuildSourceError, inputs: BuildInputSet }, Walk { walk: PerUnitWalk, inputs: BuildInputSet } }`. These are the complete new `align_driver` exports. Constructors and mutation stay private. | `Hash128` is `Hash128::of` over the exact bytes consumed by the build. A failed read is recorded before its diagnostic/error returns. Invalid UTF-8 is regular-byte state and retains the hash; its first invalid byte is widened to `u64`. An embedded-NUL path becomes `Io` before any backend call for that path. Every observation receives a monotonically increasing producer ordinal before its read; the highest ordinal state wins a duplicate path, and any unequal pair sets `changed_during_attempt`. Retry ordinals follow all first-attempt ordinals. The ordinal is private and the public slice remains path-sorted/deduplicated. No `FileId`, borrowed source, descriptor, HIR, MIR, or object escapes except through the already-public `PipelinedPackageBuild`/`PerUnitWalk` fields. | Public-surface inventory and state/error/duplicate-order table. |
| W6 | Watch-set transition | A private topology graph resolves each absolute lexical W5 path with filesystem lookup semantics. It records every lexical and target-side component visited: missing; directory `(device,inode)`; symlink `(device,inode,raw target bytes)`; leaf regular target `(device,inode)`; or other. A relative symlink target is spliced at the symlink's containing directory, an absolute target restarts at its root, and resolution continues through nested symlinks until a leaf, missing suffix, loop, or the W8 traversal bound. Thus `/project/link/file` with `link -> /outside/missing` retains both `/project/link` and the deepest existing target-side directory under `/outside`. Before waiting, the watcher adds handles for every existing graph node and deepest existing directory above every unresolved suffix, then rebuilds the graph and W5 semantic snapshot. Only after both comparisons may it remove obsolete handles. After success the next logical set is exactly that revision. After failure, construct it from all `current_attempt` records plus only `last_successful` paths absent from current; current state therefore wins every overlap. Earlier failed sets never contribute. With no success it is current. Entry is always present. | The watcher owns all graph paths, raw target bytes, and native handles. A detected loop is stable topology for the current W5 `Unreadable` state, so changing any traversed symlink can repair it; exceeding the traversal bound is the W8 fail-closed resource error. Add, topology/snapshot, or removal failure is fail-closed: emit `alignc: watch: watcher error: MESSAGE` and exit 1. Changed semantic state, changed non-leaf directory/symlink/missing topology, or `changed_during_attempt` schedules an immediate revision. A leaf regular inode-only change with identical W5 state re-arms its handle but starts no revision. Add-before-remove plus post-add graph rebuild closes read-to-registration, ancestor-creation, and target-side dangling-symlink gaps. | Injected watcher owner covers overlapping-current-wins, last-success-only absent paths, repeated identical failure without a spin, mutation before registration/during build/during transition, two missing parents, relative/absolute dangling and intermediate symlinks, cycle repair, and removal failure. |
| W7 | Event and verification model | Linux uses inotify and macOS uses the native file-event backend selected by the implementation dependency for low-latency wakes. An event for any W6 node, rename/replacement, or overflow wakes the loop. Events are collected until 50 ms of quiet or 250 ms from the first event, whichever comes first. Independently, after every completed no-change comparison the loop arms one fixed two-second audit deadline. Either an event drain or that deadline rebuilds W6 topology, installs newly reachable handles, repeats topology, and streams every W5 regular input to recompute semantic state. A topology or semantic change starts exactly one revision; otherwise the next two-second deadline starts after that comparison completes. The audit is the correctness owner for dropped/native-unreported events, writable-`mmap` changes, and remote changes once ordinary reads expose their bytes; native events are the latency path, not correctness authority. Events received while compiling or comparing remain pending and cause a following comparison after the current attempt/scan completes. | The callback retains no event or path payload: it sets an atomic dirty bit and performs nonblocking `try_send(())` into a one-slot wake channel. A full channel is successful coalescing. A backend-overflow event additionally sets an atomic uncertain bit. Event and timer wakes share one non-overlapping comparison; audits never accumulate or overlap, and hashing retains only the W8 fixed buffer. The loop clears the dirty bit before, never after, a comparison so a concurrent callback cannot be erased. There is no user-configurable interval or event-only mode. | Deterministic event/timer owner covers quiet/max debounce, irrelevant/no-op events, wake coalescing, backend overflow, rename, dropped-event and no-event mutation, multi-level target-side missing creation, symlink topology, event-during-build/scan, and audit non-overlap. Native smoke covers replacement on each release OS and writable `mmap` on Linux. |
| W8 | Resource ceiling | An installed set admits at most 16,384 distinct logical inputs and 65,536 distinct W6 graph nodes, with at most 40 followed symlinks per logical input. The accepted limits and rejected next values emit `alignc: watch: too many inputs (maximum 16384)`, `alignc: watch: too many path components (maximum 65536)`, or `alignc: watch: too many symlink traversals (maximum 40)` and exit 1 before installing the oversized set. Add-before-remove may temporarily retain two accepted sets: at most 32,768 logical records and 131,072 topology registrations; the backend owns at most that many logical registrations plus its fixed control state, and graceful-stop owns one two-descriptor signal pipe. Memo retention remains the existing byte-accounted 768 MiB default and refuses new entries at its bound without changing build output. | Graph path and raw target bytes count against their node and each is bounded by the platform path limit. Snapshots retain private Linux/macOS device/inode identity to re-arm a replaced target, but W5 content is the leaf semantic identity: an atomic same-content regular-file replacement re-arms and starts no revision. Event and periodic hashing stream regular bytes through one fixed 64 KiB buffer and never allocate `len` bytes. Opens are nonblocking, then `fstat` must prove a regular target before any read. Arithmetic for counts/lengths is checked. | Exact-limit/rejected-next input, graph, and symlink-traversal owners; disjoint-set transition; absolute/relative dangling-target and cycle repair; identical-content inode replacement; sparse-large-file RSS; FIFO/device no-block; timer non-overlap; signal-pipe cleanup; and memo-budget owners. |
| W9 | Failure and process exit | Source, frontend-cache, W3 alias, codegen, PGO, link, and executable-publication failures produce `failed` and keep watching. Watcher initialization/transition/backend errors and W2 stdout-flush failure produce `alignc: watch: watcher error: MESSAGE` and exit 1 because future changes or a complete transcript cannot be promised. SIGINT/SIGTERM request a graceful stop: the first signal wins, an active revision finishes with its ordinary `ready`/`failed` result unless stdout flush fails, every child is waited and every stage drops, then the process emits exactly `alignc: watch: stopped by SIGINT` or `... SIGTERM` and exits 130 or 143. While idle, the signal pipe wakes the wait and performs the same cleanup/line/exit immediately. Later SIGINT/SIGTERM requests are coalesced; SIGKILL retains ordinary immediate OS semantics and no cleanup promise. | A process-global compare-exchange admits exactly one private signal installation. A second attempt fails with watcher error `signal handlers already installed` before changing a disposition or fd. Setup creates the nonblocking self-pipe, registers both async-signal-safe handlers, then publishes the active owner. If either registration fails, it unregisters any installed handler, closes pipe write then read ends, clears the singleton guard, and returns the setup error; success transfers those resources to the whole watch-process lifetime and exits rather than restoring them. Each handler compare-exchanges the first signal number and writes one byte; full pipe is successful coalescing. No handler allocates, locks, formats, closes, or touches compiler state. | Child-process-only owner covers successful singleton lifetime, failed second install, injected first/second registration failure and exact unregister/close/guard-clear order, then signals idle and every active phase; it proves child reaping and no staging residue/partial executable. SIGKILL remains the force-stop escape. |
...
resolved-slot output-alias validation
...
4  ordinary source/frontend/cache/codegen result
5  resolved topology/resource and output-alias validation
6  link and executable publication
7  successful-build stdout flush
8  ready or failed marker
9  live-input/topology limit
10 watch-handle addition
11 post-registration topology and semantic snapshot
12 obsolete-handle removal
13 wait/backend/timer delivery
```
...
`initialize: MESSAGE`, `stdout flush: MESSAGE`, `add 'PATH': MESSAGE`,
`snapshot 'PATH': MESSAGE`, `remove 'PATH': MESSAGE`, `backend: MESSAGE`, or
`revision counter exhausted`, each behind the single
`alignc: watch: watcher error: ` prefix. The underlying OS message is
informational and has no control-flow meaning.
...
| D2 input formation | Formation, validation, construction, move-in/out, return, producer ordinal, duplicate-state replacement, retry union, success replacement, and current-wins failure merge cover entry/import/static/metadata paths and every W5 state through both ordinary and ThinLTO front halves; the ordinary arm additionally covers PGO. | One private observer threaded through the existing read owners; `observed_build_inputs` parameterizes both public wrappers and asserts identical ordered records plus first-attempt/retry duplicate precedence. |
| D3 read/register/publication race | A change before, during, or after registration cannot leave the process waiting on bytes it did not build. Replacement, lexical and target-side missing-parent creation, relative/absolute dangling symlinks, nested symlinks, and cycle repair are included. Publication must not replace an observed input's lexical/resolved slot at the final snapshot; later external mutation is W12 concurrency. | One resolved-graph/slot authority serves prepublication alias validation and add-snapshot-remove transition; deterministic barriers and native replacement smoke cover both consumers. |
| D4 event lifecycle | Normal event, identical rewrite, burst, max-debounce traffic, full-channel coalescing, backend overflow, event during compile, backend disconnect, periodic no-event audit, failed-set current/absent merge, idle/active signal wake, and Drop each wake/stop exactly once. | Injectable event/timer source, one-slot event channel, merge owner, and self-pipe owner; native backend and async-signal-safe handler are thin adapters. |
| D5 build control paths | Initial/repeated success; ordinary and ThinLTO frontend/codegen/link/publication, including the one-unit ThinLTO fallback and multi-unit ThinLTO path; output-alias rejection before link; ordinary rehydration/PGO and one retry; cache on/off; every applicable cache-stat shape; edit/revert; malformed input. | Existing ordinary/ThinLTO owners remain authoritative; `watch_build` adds revision sequencing, resolved-slot validation, last-good publication, recovery, and an observed-input parity witness for each route/cardinality. |
| D6 concurrency and cleanup | Worker panic retains item 3 cleanup; watcher callback and signal handler never block; event overflow and first-signal selection are atomic; no worker, stage, handle, pipe, or child survives normal/error/graceful-signal return. Signal installation is singleton, a failed second install is side-effect-free, and partial setup restores in W9 order. SIGKILL alone has no cleanup promise. Independent publishers preserve complete-image atomicity and last-successful-rename semantics. | Existing pipeline panic owner plus watcher channel/Drop, signal setup/phase/pipe cleanup, two-publisher barrier, and bounded child-process owners. No process-global panic hook or output lock; signal handlers use W9's binary-owned singleton lifetime. |
...
| D8 resource bound | Exact/rejected-next input, resolved-graph, and symlink-traversal counts; one-slot wake channel; one non-overlapping periodic scan; streaming snapshots/slot comparison; nonblocking special-file rejection; checked counters; and memo refusal are all reachable. | Parameterized limit/traversal/overflow/FIFO/sparse-file/RSS/timer/same-slot owners. |
| D9 observable transcript | Marker count/order, stdout flush order/failure, output-alias path/error, ordinary diagnostics, cache stats, stdout, object bytes, link-input order, executable behavior, and exit behavior are exact for success, failure, recovery, and watcher error. The linked executable itself is not byte-compared: macOS link products carry link-time `LC_UUID`/page-hash identity. | Runtime-loaded pipe-backed golden transcript plus object/link-input/program-output comparison with one-shot `alignc build`. |
...
| Finding | Redesigned closure |
|---|---|
| P1: publication could replace a file-backed input that aliases the output | W3 now uses W6's single resolved-slot authority after complete observation and before link/publication. It rejects lexical, parent-alias, and resolved-target matches while allowing a distinct surviving hard-link slot; D3/D5/D8 own the operation matrix and timing barriers. |
...
- direct output/input slot identity, a parent-directory alias, and an input
  symlink resolving to the output all fail before link/publication; a different
  hard-link directory entry is allowed and retains its source bytes;
...
- a pipe reader observes the ordinary success line before the matching `ready`,
  while an injected stdout-flush failure emits no false `ready`; and
- a failed second signal installation changes nothing, each partial setup
  failure unregisters/closes/clears in W9 order, and a later clean child can
  install successfully; and
