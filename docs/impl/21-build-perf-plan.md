...
| W13 | Child-output pump | Every watch-mode `cc` (including its linker) and macOS `strip` child is spawned with separate blocking stdout/stderr pipe write ends. All four original ends are close-on-exec. Before spawn, the parent applies `O_NONBLOCK` only to its two read ends; read and write ends are distinct open-file descriptions, so command setup can map the still-blocking write ends onto child stdout/stderr, close every other child end, and leave only those two blocking standard descriptors open across exec. The parent closes its copies of both write ends immediately after spawn. One parent-owned loop checks direct-child status before every poll, polls both read ends for at most 50 ms, services stdout then stderr when both are ready, and reads at most 4,096 bytes per stream per cycle. Poll/read `EINTR` retries and `EAGAIN` returns to poll. Each nonempty read is one W2 `child-stdout` or `child-stderr` `end` record; concatenating its decoded payloads reconstructs each original stream. Per-stream order is fixed; cross-stream order is stdout-before-stderr poll order and has no semantic meaning. | The pump retains two 4,096-byte read buffers and borrows W2's common framed-record writer; it never retains whole output. It records the first read/transcript error but continues servicing both pipes while the direct child runs, discarding later chunks if the protocol stream is unavailable so the child cannot block. Immediately after reaping that child, it performs one `FIONREAD` query per pipe, stdout then stderr, for the exact queued byte count. Counts through 1,048,576 are accepted; a larger count records `child stdout read: buffered output exceeds 1048576 bytes` or the stderr sibling, and a negative count uses `invalid buffered-byte count`. The pump reads at most each accepted snapshot count in 4,096-byte chunks, so later descendant writes cannot extend the quota, then closes both read ends without waiting for EOF. Query/read failure or EOF before the snapshotted count records the matching `child stdout read: MESSAGE` or `child stderr read: MESSAGE`. The first recorded pump/transcript error wins over child status; child exit status otherwise retains ordinary build meaning. No child byte can become an unframed marker or another record kind. | Injectable pump owner covers blocking-child/nonblocking-parent flags, empty/arbitrary/all-byte/marker-shaped output, simultaneous streams, pipe-capacity backpressure, >buffer output, `EINTR`, `EAGAIN`, 50-ms no-output child exit, read/write/flush/query/early-EOF error, child failure, exact/over-limit/negative post-reap count, descriptor-table inheritance, descendant-retained and continuously-writing write ends, wait/snapshot/drain/close, and exact decoded parity with one-shot direct-child streams through the accepted bound. |
...
| D4 event lifecycle | Normal/identical/burst/max-debounce/full-pipe/overflow events, event during compile, every fatal class/first-error order, periodic audit, failed-set and repair-output merge, idle/active shared-pipe signal wake, signal/fatal/synchronous-transition precedence, and terminal cleanup wake/stop exactly once. | Injectable event/timer/error source, one shared pipe, dirty/uncertain/fatal/signal atomics, logical/repair merge owner, bounded-read poll owner, every before/after-transition control snapshot, backend shutdown, and process teardown; native adapters stay thin. |
| D5 build control paths | Initial/repeated success; ordinary/ThinLTO cardinalities; semantic/alias rejection and repair; retry/PGO/cache combinations; child output/success/failure; edit/revert/malformed input; and both W1 trigger-excluded boundaries preserve ordinary pre-link/link/publication semantics. | Existing owners plus `watch_build` revision, retry, finalization, repair recovery, W13 pump, last-good publication, external/resource-freshness trigger, and route/cardinality parity owners. |
...
| D8 resource bound | Exact/rejected-next length/NUL/absolute-path/input/combined-input-repair-graph/evidence/finalization/symlink/encoding/message/frame/post-reap limits; shared wake pipe/fatal slot; non-overlapping scan; 64 KiB hash and fixed transcript/pump buffers; nonblocking special-file rejection; checked counters; and memo refusal are reachable. | Parameterized limit/repair/encoding/frame/pump/snapshot/overflow/fatal/FIFO/sparse/RSS/timer/identity owners. |
| D9 observable transcript | The single stderr protocol stream fixes marker/prerequisite order, marker atomicity, write/flush failure, no-stdout behavior, reversible frames for every nonterminal record, instability/alias/resource/backend/wake/pump errors, decoded in-process/child logical channels, object/link order, behavior, and exits for success/failure/recovery/watcher error. Linked executables are not byte-compared on macOS. | One-pipe golden transcript with every kind/state/chunk boundary, all-byte and marker-shaped diagnostic/cache/message/path/child injection, decoded bounded parity, short marker writes, full/partial/flush errors, mutation/repair, exact limits/classes, and one-shot object/link/program comparison. |
| D10 non-impact | One-shot build/run/size, `align-repl`, inspection verbs, release layout, and docs remain unchanged except for the additive flag/help text and its explicit external-resource/runtime-freshness trigger boundaries. | Existing bounded gate, REPL owners, release workflow structural test, usage/help snapshot, and parameterized external-resource/runtime-source mutation owner. |
