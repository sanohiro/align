...
| Partition record | The driver/codegen seam is `ThinPartition<'a> { unit: &'a str, kind: ThinPartitionKind<'a>, mir: &'a align_mir::Program, impl_hash: Hash128, preserve_symbols: Vec<String> }`, where `ThinPartitionKind<'a> = Support | Function(&'a ProgramCall)`. The sole constructor is `function_partitions<'a>(units: &'a [PerUnitArtifact], exports: &[String]) -> Result<Vec<ThinPartition<'a>>, String>`: only `units` supplies the returned lifetime, while `exports` is consumed into the owned preserve set. Records borrow the validated unit MIR for the build attempt; they do not own or persist MIR. `preserve_symbols` is raw LLVM symbol identity, sorted and deduplicated. | Structural owners reject duplicate/missing function identities and prove stable ordering and hash identity without cloning the complete MIR once per function. A compile-time signature owner proves the result survives release of the shorter `exports` borrow but not release of `units`. Every `Err` is a static-prefix internal-compiler diagnostic plus lowercase length-delimited offending identity; no OS error text enters formation. |
| Build entry and completion | The additive driver entry is `build_function_thin_lto(units: &[PerUnitArtifact], cache: &CacheContext, target: &BuildTarget, profile: Profile, exports: &[String], rt_lto: bool, jobs: usize) -> Result<FunctionThinLtoBuild, String>`. `FunctionThinLtoMode = WholeUnit | Partitioned`; `FunctionThinLtoBuild { pub mode: FunctionThinLtoMode, pub outcomes: Vec<CacheOutcome>, pub prelink_bc: Vec<PathBuf>, objects: Vec<PathBuf>, profile: Profile, link_libs: Vec<String>, object_stage: ArtifactStage }` is externally non-constructible and keeps object paths plus link configuration private. `link_libs` is the deterministic first-seen union of every import-DAG unit's MIR-owned list, formed during validated inventory; `profile` is copied from the build input. An empty inventory returns `ThinLTO partition inventory is empty` before stage creation. Exactly one function partition with no support returns `WholeUnit`, uses the unchanged ordinary object-cache/codegen route, and has ordinary outcomes plus an empty `prelink_bc`; every other nonempty inventory returns `Partitioned` with partition-phase outcomes and bitcode. After inspecting `mode` and `outcomes`, a caller must consume the result through `FunctionThinLtoBuild::link_and_publish(self, exe: &Path) -> Result<(), String>` or `FunctionThinLtoBuild::link_and_publish_with_output(self, exe: &Path, sink: &mut dyn LinkOutputSink) -> Result<(), String>`. Both methods use only the stored profile and library union, write, flush, and close the private response file inside `object_stage` before spawn, then run the existing ordinary or captured-watch link and atomic-publication owner respectively; neither exposes or accepts an object slice, response path, response argument, profile, or library list. Consuming `self` makes completion single-use and drops every private path after success, returned error, or unwind. Object cardinality and controlled names are discovered from MIR. The build function validates the complete borrowed inventory and response-path representation before it creates `object_stage`; no caller can create the stage early through this seam. | Library owners cover empty rejection, one-partition byte identity and ordinary cache observability, multi-partition formation across one and many source units, exact stored-profile and MIR-library-union use, disabled and enabled caches, `jobs == 0` normalization to one, deterministic error selection, result construction privacy, response close-before-spawn, both completion methods, success/error/unwind stage Drop, and compile-fail attempts to construct the result, supply a second link configuration, or link its private objects directly. Existing `build_thin_lto` remains available for unit-granular tests and non-CLI callers until the hard cutover PR updates them coherently; no dual CLI path ships. |
...
4. form every peer ABI record, preserve set, partition structural hash, stable
   module identity, controlled ordinal output name, future randomized stage
   basename, and native response-path representation; reject embedded NUL,
   LF/CR, or representation overflow before creating a directory;
...
Three independent adversarial review passes found six P2 contract gaps. They
share the `route-stage-ownership-and-native-path-encoding` axis, so the matrix
was reopened on that axis and each complete finding set was corrected in one
coherent pass:
