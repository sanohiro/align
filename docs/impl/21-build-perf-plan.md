...
| 4 | Prebuilt optimized cache distribution | Shipped as #893 — exact native releases carry an adjacent immutable cache warmed from 20 first-party `pkg` modules plus one generated entry unit (21 total); compiler-provided `core`/`std` imports have no cacheable source unit |
...
`apps/web/pkg`, `apps/frame/pkg`, `apps/auth/pkg`, and `apps/db/pkg`. A consumer receives a hit only
