...
| W9 | Failure and process exit | Source, frontend-cache, codegen, PGO, link, and executable-publication failures produce `failed` and keep watching. Watcher initialization/transition/backend errors and W2 stdout-flush failure produce `alignc: watch: watcher error: MESSAGE` and exit 1 because future changes or a complete transcript cannot be promised. SIGINT/SIGTERM request a graceful stop: the first signal wins, an active revision finishes with its ordinary `ready`/`failed` result unless stdout flush fails, every child is waited and every stage drops, then the process emits exactly `alignc: watch: stopped by SIGINT` or `... SIGTERM` and exits 130 or 143. While idle, the signal pipe wakes the wait and performs the same cleanup/line/exit immediately. Later SIGINT/SIGTERM requests are coalesced; SIGKILL retains ordinary immediate OS semantics and no cleanup promise. | One async-signal-safe handler per signal is installed once before watcher construction. It compare-exchanges an atomic signal number and writes one byte to a nonblocking self-pipe; full pipe is successful coalescing. The handlers are process-global but the `alignc` watch binary owns the whole process, installs them before workers, never overlaps a second registration, and exits instead of restoring them. No signal handler allocates, locks, formats, closes, or touches compiler state. | Bounded child-process owner signals both idle and each active phase, waits for conventional exit, proves the link child was reaped and no staging residue/partial executable remains, and separately documents SIGKILL as the force-stop escape. |
...
`changed_during_attempt` rather than choosing one observation as authoritative.
...
4  ordinary revision through executable publication
5  successful-build stdout flush
6  ready or failed marker
7  live-input/topology limit
8  watch-handle addition
9  post-registration topology and semantic snapshot
10 obsolete-handle removal
11 wait/backend/timer delivery
```
...
| D2 input formation | Formation, validation, construction, move-in/out, return, duplicate observation, retry union, success replacement, and failure union cover entry/import/static/metadata paths and every W5 state through both ordinary and ThinLTO front halves; the ordinary arm additionally covers PGO. | One private observer threaded through the existing read owners; `observed_build_inputs` parameterizes both public wrappers and asserts identical ordered records. |
| D3 read/register race | A change before, during, or after registration cannot leave the process waiting on bytes it did not build. Replacement, lexical and target-side missing-parent creation, relative/absolute dangling symlinks, nested symlinks, and cycle repair are included. | Resolved-graph add-snapshot-remove transition with deterministic barriers and native replacement smoke. |
| D4 event lifecycle | Normal event, identical rewrite, burst, max-debounce traffic, full-channel coalescing, backend overflow, event during compile, backend disconnect, periodic no-event audit, idle/active signal wake, and Drop each wake/stop exactly once. | Injectable event/timer source, one-slot event channel, and self-pipe owner; native backend and async-signal-safe handler are thin adapters. |
| D5 build control paths | Initial/repeated success; ordinary and ThinLTO frontend/codegen/link/publication, including the one-unit ThinLTO fallback and multi-unit ThinLTO path; ordinary rehydration/PGO and one retry; cache on/off; every applicable cache-stat shape; edit/revert; malformed input. | Existing ordinary/ThinLTO owners remain authoritative; `watch_build` adds revision sequencing, last-good publication, recovery, and an observed-input parity witness for each route/cardinality. |
| D6 concurrency and cleanup | Worker panic retains item 3 cleanup; watcher callback and signal handler never block; event overflow and first-signal selection are atomic; no worker, stage, handle, pipe, or child survives normal/error/graceful-signal return. SIGKILL alone has no cleanup promise. Independent publishers preserve complete-image atomicity and last-successful-rename semantics. | Existing pipeline panic owner plus watcher channel/Drop, signal-phase/pipe cleanup, two-publisher barrier, and bounded child-process owners. No process-global panic hook or output lock; signal handlers are installed once under W9's binary-owned lifetime. |
...
| D8 resource bound | Exact/rejected-next input, resolved-graph, and symlink-traversal counts; one-slot wake channel; one non-overlapping periodic scan; streaming snapshots; nonblocking special-file rejection; checked counters; and memo refusal are all reachable. | Parameterized limit/traversal/overflow/FIFO/sparse-file/RSS/timer owners. |
| D9 observable transcript | Marker count/order, stdout flush order/failure, ordinary diagnostics, cache stats, stdout, object bytes, link-input order, executable behavior, and exit behavior are exact for success, failure, recovery, and watcher error. The linked executable itself is not byte-compared: macOS link products carry link-time `LC_UUID`/page-hash identity. | Runtime-loaded pipe-backed golden transcript plus object/link-input/program-output comparison with one-shot `alignc build`. |
