...
| W3 | Publication | Each attempt uses the existing applicable ordinary or ThinLTO per-unit pipeline and its current retry rule. Before a successful candidate may link/publish, `finalize_watch_inputs(inputs, Some(output))` resolves the current W6 graph and compares the fixed cwd/output lookup with every current and W4 read-time directory-entry node, including lexical prefixes, symlinks, target-side prefixes, the opened leaf, and the final leaf. Exact absolute lexical equality rejects even a missing slot; when the output lookup exists, matching its nofollow `(device,inode)` against any existing node or opened-leaf identity also rejects. The latter deliberately rejects distinct hard links as well as case-/Unicode-folded names: portable fail-closed safety takes precedence over admitting a rare harmless alias. Any pre-open/open/post-read/current identity disagreement sets `changed_during_attempt`; such a candidate emits exactly `alignc: watch: inputs changed during revision`, produces `failed`, schedules a comparison, and never links. Otherwise the lowest platform-byte-sorted alias emits exactly `alignc: watch: output 'ENCODED_OUT' aliases observed input 'ENCODED_INPUT'`, produces `failed`, and runs no link or rename. Other successful candidates retain the existing link and same-directory atomic rename. | `ENCODED_OUT` is `WatchPath` over the absolute startup-cwd path for the existing `stem(FILE)` output. Retaining the read-time path graph and opened-leaf identity until finalization means a node removed or retargeted after supplying bytes cannot disappear from alias validation. Requiring stable read-time-to-current identity means an absent final output lookup cannot authorize publication after such a removal. Concurrent external mutation after finalization retains W12's existing last-writer rule; the compiler provides no cross-process filesystem transaction. Each attempt owns and drops private object/publish stages; other link/publication errors remain ordinary failures and preserve last-good output. | Parameterized same-resource owner covers direct slot, traversed directory/symlink named as output, parent alias, leaf symlink to output, case- and Unicode-equivalent names on a supporting macOS volume, conservative distinct-hard-link rejection, output absent, and removal/retarget barriers before open, after open, after read, and before/after finalization. Real-binary owner runs last-good output after later failure. |
| W4 | Observed build inputs | The ordinary route uses `align_driver::build_path_pipelined_observed`, which owns the entry read and one call to the same private pipeline implementation as `build_package_pipelined`. The ThinLTO route uses `align_driver::build_path_per_unit_observed`, which owns the same read/observation front half and returns the existing `PerUnitWalk` consumed by the current ThinLTO CLI branch. Before retaining or issuing a filesystem call for any input, both form its absolute lexical path and reject more than 1,023 raw bytes with `ObservationFailed`. Otherwise they return the complete ordered `BuildInputSet` even when entry read or frontend fails. The set records the entry source, every present or missing user-module path, every validated file-backed static source and checked-metadata path, and, on the ordinary PGO route, the exact `--pgo-use` file. Inline static sources and static paths rejected before root/path validation add no target; their owning Align source is already watched and is the only input that can repair that error. Missing paths are not canonicalized. Under watch, all path fields passed to the diagnostic renderer use W2 `WatchPath`; non-path diagnostic meaning/order remains unchanged. | For each admissible read, the observer privately retains the bounded W6 graph immediately before open, the opened leaf's `(device,inode)` from `fstat`, and the graph immediately after open/read. The union is the read-time evidence of bytes actually consumed even if a link is removed/retargeted before W3. It retains at most 131,072 distinct nodes across one attempt/retry; the rejected next node is `ObservationFailed`. Each public record owns one bounded `PathBuf` and semantic state; regular state owns only `Hash128`/length. The installed compiler/runtime/LLVM/linker/system toolchain remains a restart-only session precondition. | Parameterized library owner runs entry/import/static/metadata present, missing, replacement during every pre-open/open/post-read barrier, invalid-path exclusion, 1,023/1,024-byte paths, invalid UTF-8, duplicate-read, read-time identity, and arbitrary-byte paths through ordinary/ThinLTO; ordinary also covers PGO. |
| W5 | Input and finalization record | `#[derive(Clone, PartialEq, Eq)] pub struct BuildInput { path: PathBuf, state: BuildInputState }`; `impl BuildInput { pub fn path(&self) -> &Path; pub fn state(&self) -> BuildInputState }`; `#[derive(Clone, Copy, PartialEq, Eq)] pub enum BuildInputState { Missing, Regular { content_hash: Hash128, len: u64 }, NonRegular, Unreadable }`; `pub struct BuildInputSet`; `impl BuildInputSet { pub fn inputs(&self) -> &[BuildInput]; pub fn changed_during_attempt(&self) -> bool }`; `pub struct FinalizedWatchInputs`; `impl FinalizedWatchInputs { pub fn inputs(&self) -> &BuildInputSet; pub fn alias_index(&self) -> Option<usize>; pub fn into_inputs(self) -> BuildInputSet }`; `pub struct BuildInputTopologyError`; `impl Debug + Display + Error for BuildInputTopologyError`; `pub fn finalize_watch_inputs(inputs: BuildInputSet, output: Option<&Path>) -> Result<FinalizedWatchInputs, BuildInputTopologyError>`; `pub enum BuildSourceError { Missing, NonRegular, InvalidUtf8 { offset: u64 }, Io { message: String } }`; `pub enum ObservedBuildAttempt { ObservationFailed { error: BuildInputTopologyError }, SourceFailed { error: BuildSourceError, inputs: BuildInputSet }, Pipeline { build: PipelinedPackageBuild, inputs: BuildInputSet } }`; `pub enum ObservedPerUnitBuild { ObservationFailed { error: BuildInputTopologyError }, SourceFailed { error: BuildSourceError, inputs: BuildInputSet }, Walk { walk: PerUnitWalk, inputs: BuildInputSet } }`. These are the complete new `align_driver` exports; fields/constructors/mutation/evidence stay private. | `finalize_watch_inputs` is called exactly once for every non-`ObservationFailed` result. `output=Some` resolves W3's final graph, compares it with all read-time evidence, and sets `changed_during_attempt` for any pre-open/open/post-read/current disagreement before checking aliases. `None` is required for an already-failed candidate: it resolves no topology and performs no alias comparison. Both paths consume and free evidence before returning the opaque finalized value; only the bounded semantic set survives into W6. `alias_index` indexes its path-sorted public slice and is always `None` for `output=None`. `BuildInputTopologyError` owns no unbounded path/message and its exact `Display` is one W2-safe line. `Hash128` covers consumed bytes. A failed read records state first. Invalid UTF-8 retains regular hash and widens offset to `u64`; embedded NUL is `Io` before backend work. A producer ordinal is assigned before each read; highest ordinal wins duplicates, unequal state sets `changed_during_attempt`, and retry ordinals follow attempt one. | Exact exported-surface/trait inventory; external construction/mutation negatives; finalization `Some`/`None`, evidence-consumed-on-every-result, alias-index, topology-error Display, state/error/duplicate/retry owners. |
| W6 | Watch-set transition | A private topology graph resolves each absolute lexical W5 path with filesystem lookup semantics. It records every lexical and target-side directory-entry node visited, not only the final path: missing; directory `(device,inode)`; symlink `(device,inode,raw target bytes)`; leaf regular target `(device,inode)`; or other. Every existing node's tuple comes from nofollow metadata for that entry, so W3 can compare the output lookup against a traversed symlink/directory and filesystem case/normalization is reflected by the kernel lookup rather than reimplemented from path bytes. A relative symlink target is spliced at the symlink's containing directory, an absolute target restarts at its root, and resolution continues through nested symlinks until a leaf, missing suffix, loop, or the W8 traversal bound. Thus `/project/link/file` with `link -> /outside/missing` retains `/project/link` plus every reached target-side prefix and deepest existing target-side directory. W3 finalization compares the current graph with W4's bounded read-time union, then consumes that evidence. Before waiting, the watcher adds handles for every existing current graph node and deepest existing directory above every unresolved suffix, rebuilds the graph and W5 semantic snapshot, and only then removes obsolete handles. After success the next logical set is exactly that revision. After failure, construct it from all `current_attempt` records plus only `last_successful` paths absent from current; current state wins every overlap. Earlier failed sets never contribute; with no success it is current. Entry is always present. | The watcher owns all current graph paths, raw target bytes, native identities, and handles; read-time evidence exists only through W5 finalization. A detected loop is stable topology for the current W5 `Unreadable` state, so changing any traversed symlink can repair it; exceeding the traversal bound is the W8 fail-closed resource error. Nofollow metadata, add, topology/snapshot, or removal failure is fail-closed with a W9 single-line encoded path. Changed semantic state, changed non-leaf directory/symlink/missing topology, or `changed_during_attempt` schedules an immediate revision. A leaf regular inode-only change with identical W5 state re-arms its handle but starts no revision. Add-before-remove plus post-add graph rebuild closes read-to-registration, ancestor-creation, target-side dangling-symlink, and traversed-slot gaps. | Injected watcher owner covers overlapping-current-wins, last-success-only absent paths, repeated identical failure without a spin, mutation before registration/during build/during transition, read-time/current disagreement, output-named traversed nodes, two missing parents, relative/absolute dangling and intermediate symlinks, cycle repair, and removal failure. |
...
finalize_watch_inputs(Some(output)): current topology/resource,
  read-time stability, and output-alias validation
...
marker. A failed candidate calls `finalize_watch_inputs(inputs, None)` after
...
5  ordinary source/frontend/cache/codegen result
6  finalize_watch_inputs(None): evidence consumption for a failed candidate;
   or finalize_watch_inputs(Some): current topology/resource,
   read-time stability, then output alias for a publishable candidate
7  link and executable publication
8  successful-build stdout line write and flush
9  ready or failed marker
10 live-input/topology limit
11 watch-handle addition
12 post-registration topology and semantic snapshot
13 obsolete-handle removal
14 graceful signal, fatal backend slot, then event/timer delivery
```
...
| D2 input formation | Formation, 1,023-byte validation before retention/I/O, private construction, read-only public access, move-in/out, return, producer ordinal, duplicate-state replacement, retry union, success replacement, and current-wins failure merge cover entry/import/static/metadata paths and every W5 state through both ordinary and ThinLTO front halves; the ordinary arm additionally covers PGO. `ObservationFailed` and `FinalizedWatchInputs` make evidence ownership and its exactly-once consumption explicit. | One private observer threaded through the existing read owners; `observed_build_inputs` parameterizes both public wrappers, rejects external construction/mutation, and asserts identical ordered records, first-attempt/retry duplicate precedence, 1,023/1,024 boundaries, finalization `Some`/`None`, and evidence drop on every result. |
| D3 read/register/publication race | A change before, during, or after an observed read or registration cannot publish over a resource that supplied bytes or leave the process waiting on bytes it did not build. Replacement, lexical and target-side missing-parent creation, relative/absolute dangling symlinks, nested symlinks, and cycle repair are included. Publication compares current topology with retained pre-open/open/post-read identities and rejects instability or an output alias before link; later external mutation is W12 concurrency. | One resolved-graph/nofollow-identity authority serves read-time evidence, finalization, alias validation, and add-snapshot-remove transition; deterministic removal/retarget barriers at every observation phase, folded-name/hard-link cells, and native replacement smoke cover all consumers. |
...
| D8 resource bound | Exact/rejected-next 1,023-byte path, 16,384 input, 65,536 current-graph, 131,072 read-time-graph, 196,608 combined-finalization, 40 symlink, 3,069-byte encoded-path, and 16,384-byte dynamic-message capacities; one-slot wake channel and one `AtomicU8`; one non-overlapping periodic scan; streaming snapshots/identity comparison; nonblocking special-file rejection; checked counters; and memo refusal are all reachable. | Parameterized path/hash/limit/traversal/encoding/message/overflow/fatal-slot/FIFO/sparse-file/RSS/timer/identity owners. |
| D9 observable transcript | Marker count/order, stdout/stderr write/flush order and failures, reversible admitted single-line paths/messages, over-limit static replacement, input-instability and output-alias errors, exact path/resource/backend-class errors, ordinary diagnostics, cache stats, stdout, object bytes, link-input order, executable behavior, and exit behavior are exact for success, failure, recovery, and watcher error. The linked executable itself is not byte-compared: macOS link products carry link-time `LC_UUID`/page-hash identity. | Runtime-loaded pipe-backed golden transcript plus all-byte path/message injection, 1,023/1,024 hashed-path errors, every static backend class, and object/link-input/program-output comparison with one-shot `alignc build`. |
