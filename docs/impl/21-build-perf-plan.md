...
| Build entry and completion | The additive driver entry is `build_function_thin_lto(units: &[PerUnitArtifact], cache: &CacheContext, target: &BuildTarget, profile: Profile, exports: &[String], rt_lto: bool, jobs: usize) -> Result<FunctionThinLtoBuild, String>`. `FunctionThinLtoMode = WholeUnit | Partitioned`; `FunctionThinLtoBuild { pub mode: FunctionThinLtoMode, pub outcomes: Vec<CacheOutcome>, pub prelink_bc: Vec<PathBuf>, objects: Vec<PathBuf>, object_stage: ArtifactStage }` is externally non-constructible and keeps object paths private. An empty inventory returns `ThinLTO partition inventory is empty` before stage creation. Exactly one function partition with no support returns `WholeUnit`, uses the unchanged ordinary object-cache/codegen route, and has ordinary outcomes plus an empty `prelink_bc`; every other nonempty inventory returns `Partitioned` with partition-phase outcomes and bitcode. After inspecting `mode` and `outcomes`, a caller must consume the result through `FunctionThinLtoBuild::link_and_publish(self, exe: &Path, link_libs: &[String], profile: Profile) -> Result<(), String>` or `FunctionThinLtoBuild::link_and_publish_with_output(self, exe: &Path, link_libs: &[String], profile: Profile, sink: &mut dyn LinkOutputSink) -> Result<(), String>`. Both methods write, flush, and close the private response file inside `object_stage` before spawn, then run the existing ordinary or captured-watch link and atomic-publication owner respectively; neither exposes an object slice, response path, or response argument. Consuming `self` makes completion single-use and drops every private path after success, returned error, or unwind. Object cardinality and controlled names are discovered from MIR. The build function validates the complete borrowed inventory and response-path representation before it creates `object_stage`; no caller can create the stage early through this seam. | Library owners cover empty rejection, one-partition byte identity and ordinary cache observability, multi-partition formation across one and many source units, disabled and enabled caches, `jobs == 0` normalization to one, deterministic error selection, result construction privacy, response close-before-spawn, both completion methods, success/error/unwind stage Drop, and compile-fail attempts to construct the result or link its private objects directly. Existing `build_thin_lto` remains available for unit-granular tests and non-CLI callers until the hard cutover PR updates them coherently; no dual CLI path ships. |
...
Two independent adversarial review passes found five P2 contract gaps. They
