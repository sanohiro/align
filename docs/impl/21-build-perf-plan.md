...
reconstructs the direct child's bytes through the accepted post-reap bound;
the watch framing means no child byte can become an unframed marker or another
record kind. A failed queued-byte query, read, or EOF before the snapshotted
count records exact `child stdout read: MESSAGE` or the stderr sibling. The
pump owner retains the 50-ms no-output child-exit case, output larger than one
buffer, exact/over-limit/negative counts, the child descriptor table, and the
complete wait/snapshot/drain/close sequence.
...
metadata and graph traversal failures remain fail-closed topology errors. The
...
9  successful-build protocol-line write
10 single protocol-stream flush, terminal-marker write
11 graceful signal, then fatal backend slot
12 live-input/combined-input-repair-topology limit
13 watch/repair-handle addition
14 post-registration input/repair topology and semantic snapshot
15 obsolete-handle removal
16 event/timer delivery
```

The step-11 control snapshot is repeated before and after every step 12–15
...
| D4 event lifecycle | Normal/identical/burst/max-debounce/full-pipe/overflow events, event during compile, the complete prior/current 4x4 W5 state product including readability recovery, every fatal class/first-error order, periodic audit, failed-set and repair-output merge, idle/active shared-pipe signal wake, signal/fatal/synchronous-transition precedence, and terminal cleanup wake/stop exactly once. | Injectable event/timer/error source, one shared pipe, dirty/uncertain/fatal/signal atomics, total semantic classifier, logical/repair merge owner, bounded-read poll owner, every before/after-transition control snapshot, backend shutdown, and process teardown; native adapters stay thin. |
| D5 build control paths | Initial/repeated success; ordinary/ThinLTO cardinalities; semantic/alias rejection and repair; retry/PGO/cache combinations; child output/success/failure; edit/revert/malformed input; and both W1 trigger-excluded boundaries preserve ordinary pre-link/link/publication semantics. Both W13 `_with_output` entry points cover ordinary, PGO-use, PGO-instrument, ELF strip, and macOS strip without changing the legacy entry points. | Existing owners plus `watch_build` revision, retry, finalization, repair recovery, exact four-symbol W13 inventory, external sink, pump, last-good publication, external/resource-freshness trigger, and route/cardinality parity owners. |
| D6 concurrency and cleanup | Worker panic cleanup remains; callbacks/handlers do not block; atomics select overflow/fatal/signal; evidence, repair tokens, pump buffers/fds, workers, stages, handles, control fds, and direct children do not outlive owners. Signal setup is singleton and rollback-complete before publication; afterward the still-installed handlers and shared pipe remain process-lifetime with no close/reuse race. Every watcher/control fd is close-on-exec. Descendant-retained or continuously written child-output pipes cannot block cleanup. | Existing panic owner plus finalization/repair Drop, watcher atomics, every signal flag/setup barrier, backend shutdown while the process-lifetime pipe remains live, terminal `process::exit`, exec descriptor-table negative, W13 blocking-write/nonblocking-read and post-reap snapshot/drain-close, two-publisher barrier, and bounded child owners. |
...
| D8 resource bound | Exact/rejected-next length/NUL/absolute-path/input/combined-input-repair-graph/evidence/finalization/symlink/encoding/message/frame/post-reap limits and each bounded offending identity; shared wake pipe/fatal slot; non-overlapping scan; 64 KiB hash and fixed transcript/pump buffers; nonblocking special-file rejection; checked counters; and memo refusal are reachable. | Parameterized limit/repair/encoding/frame/pump/snapshot/overflow/fatal/FIFO/sparse/RSS/timer owners assert the encoded logical path, or fixed length/hash for the overlong case, on every refusal. |
| D9 observable transcript | The single stderr protocol stream fixes marker/prerequisite order, marker atomicity, write/flush/sink failure, no-stdout behavior, reversible frames for every nonterminal record, instability/alias/resource/backend/wake/pump errors with offending identity, decoded in-process/child logical channels, object/link order, behavior, and exits for success/failure/recovery/watcher error. Linked executables are not byte-compared on macOS. | One-pipe golden transcript with every kind/state/chunk boundary, all-byte and marker-shaped diagnostic/cache/message/path/child injection, decoded bounded parity, short marker writes, full/partial/flush/sink errors, mutation/repair, exact limits/classes/identities, and one-shot object/link/program comparison. |
| D10 non-impact | One-shot build/run/size, both legacy inherited-stdio link entry points, `align-repl`, inspection verbs, release layout, and docs remain unchanged except for the additive flag/help text, W5 records, W13 sink/link entry points, and the explicit external-resource/runtime-freshness trigger boundaries. | Existing bounded gate, legacy/new link-route parity, REPL owners, release workflow structural test, usage/help snapshot, and parameterized external-resource/runtime-source mutation owner. |
