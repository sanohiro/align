...
| Partition fingerprint symmetry | The sole partition constructor closes formation, hashing, and emission. `PartitionSharedCodegenView::from_program` borrows and fingerprints the exact shared tables once per unit behind private fields. Function views contain exactly the selected body, definition/peer records, and sealed shared view consumed by function emission; support views contain exactly the deduplicated thunk records consumed by support emission. No emitter re-reads `Program` to rediscover a symbol, ABI, ownership class, declaration, or table. | total-field mutation sweep for both view variants and every shared-view field; one-shared-format/hash-per-unit counter owner; owned/imported and duplicate-thunk resource mutations; hash-change iff emitted prelink changes; exact-revert CAS hit; source guard against post-view `Program` access |
