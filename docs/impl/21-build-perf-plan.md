...
| W6 | Watch-set transition | A private topology snapshot records every lexical component from the filesystem root through each logical input: missing; directory `(device,inode)`; symlink `(device,inode,raw target bytes)`; leaf regular target `(device,inode)`; or other. Before waiting, the watcher adds handles for every existing component and for the deepest existing directory above each missing suffix, then re-snapshots topology and W5 semantic state. Only after both comparisons may it remove obsolete handles. After success the next input set is exactly that revision; after failure it is `last_successful ∪ current_attempt`, never earlier failed sets; with no success it is current. Entry is always present. | The watcher owns all topology bytes and native handles. Add, topology/snapshot, or removal failure is fail-closed: emit `alignc: watch: watcher error: MESSAGE` and exit 1. Changed semantic state, changed non-leaf directory/symlink/missing topology, or `changed_during_attempt` schedules an immediate revision. A leaf regular inode-only change with identical W5 state re-arms its handle but starts no revision. Add-before-remove plus post-add re-snapshot closes both read-to-registration and ancestor-creation gaps. | Injected watcher owner mutates before registration, during build, during transition, across two separately created missing parents, through an intermediate-symlink replacement, and at removal failure. |
| W7 | Event and coalescing model | Linux uses inotify and macOS uses the native file-event backend selected by the implementation dependency. An event for any W6 component, rename/replacement, or overflow wakes the loop. Events are collected until 50 ms of quiet or 250 ms from the first event, whichever comes first. Before deciding whether to build, the watcher recomputes topology, installs newly reachable component handles, and repeats the topology snapshot; a topology change follows W6 even while W5 remains `Missing` or content-identical. It then snapshots W5 state. If neither topology nor semantic state requires a revision, it waits with the refreshed set. Events received while compiling remain pending and are handled after the attempt's W6 transition. | The callback retains no event or path payload: it sets an atomic dirty bit and performs nonblocking `try_send(())` into a one-slot wake channel. A full channel is successful coalescing. A backend-overflow event additionally sets an atomic uncertain bit; the next drain performs unconditional topology and semantic snapshots and rebuilds only if identity cannot be proved unchanged. | Deterministic fake-event owner covers quiet/max debounce, irrelevant/no-op events, wake coalescing, backend overflow, rename, multi-level missing creation, symlink topology, and event-during-build. Native smoke owner covers replacement on each release OS. |
| W8 | Resource ceiling | An installed set admits at most 16,384 distinct logical inputs and 65,536 distinct W6 topology nodes. The accepted limits and rejected next values emit `alignc: watch: too many inputs (maximum 16384)` or `alignc: watch: too many path components (maximum 65536)` and exit 1 before installing the oversized set. Add-before-remove may temporarily retain two accepted sets: at most 32,768 logical records and 131,072 topology registrations; the backend owns at most that many logical registrations plus its fixed control state, and graceful-stop owns one two-descriptor signal pipe. Memo retention remains the existing byte-accounted 768 MiB default and refuses new entries at its bound without changing build output. | Topology target bytes count against the node allocation and each is bounded by the platform path limit. Snapshots retain private Linux/macOS device/inode identity to re-arm a replaced target, but W5 content is the leaf semantic identity: an atomic same-content regular-file replacement re-arms and starts no revision. Hashing streams regular bytes through a fixed 64 KiB buffer and never allocates `len` bytes. Opens are nonblocking, then `fstat` must prove a regular target before any read. Arithmetic for counts/lengths is checked. | Exact-limit/rejected-next input and topology owners, disjoint-set transition owner, two-level-missing and symlink-replacement owners, identical-content inode-replacement owner, sparse-large-file RSS owner, FIFO/device no-block owner, signal-pipe cleanup owner, and memo-budget owner. |
| W9 | Failure and process exit | Source, frontend-cache, codegen, PGO, link, and executable-publication failures produce `failed` and keep watching. Watcher initialization/transition/backend errors produce the W6 watcher-error line and exit 1 because future changes could be missed. SIGINT/SIGTERM request a graceful stop: the first signal wins, an active revision finishes with its ordinary `ready`/`failed` result, every child is waited and every stage drops, then the process emits exactly `alignc: watch: stopped by SIGINT` or `... SIGTERM` and exits 130 or 143. While idle, the signal pipe wakes the wait and performs the same cleanup/line/exit immediately. Later SIGINT/SIGTERM requests are coalesced; SIGKILL retains ordinary immediate OS semantics and no cleanup promise. | One async-signal-safe handler per signal is installed once before watcher construction. It compare-exchanges an atomic signal number and writes one byte to a nonblocking self-pipe; full pipe is successful coalescing. The handlers are process-global but the `alignc` watch binary owns the whole process, installs them before workers, never overlaps a second registration, and exits instead of restoring them. No signal handler allocates, locks, formats, closes, or touches compiler state. | Bounded child-process owner signals both idle and each active phase, waits for conventional exit, proves the link child was reaped and no staging residue/partial executable remains, and separately documents SIGKILL as the force-stop escape. |
...
| W11 | Platform and installation | Supported release hosts are Linux and macOS on the architectures already built by CI. An unsupported native backend, signal-handler/pipe setup failure, or backend initialization failure is a watcher error, never polling fallback. Release archives need no service file, launch agent, socket directory, or new executable. `alignc --version` and one-shot commands are byte-for-byte unchanged. | The native-watcher and signal-hook dependencies are linked into `alignc`; no runtime package or daemon ownership is added. | Linux x86_64/ARM64 and Apple Silicon CI build plus native replacement/signal smoke; release inventory remains unchanged. |
...
as the existing `walk_or_report` path does, then passes the returned
`PerUnitWalk` through the existing unique object stage, `build_thin_lto`, cache
statistics, deterministic library union, link, and atomic publication. ThinLTO
cannot combine with PGO by the existing CLI contract, so this sibling has no
target/profile/cache/PGO argument: those affect only the post-walk ThinLTO
codegen and existing keys, not observed project inputs. It does not consult the
persistent frontend cache, matching the current all-MIR ThinLTO path.
...
4  ordinary revision result and terminal marker
5  live-input/topology limit
6  watch-handle addition
7  post-registration topology and semantic snapshot
8  obsolete-handle removal
9  wait/backend delivery
```
