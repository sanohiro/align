...
W1–W14 names decoded record payload. W3 instability/alias text uses `notice`,
...
failed post-reap queued-byte query, read, or EOF
...
15 watch/repair-handle addition
16 post-registration input/repair topology and semantic snapshot
17 obsolete-handle removal
18 event/timer delivery
```
...
| D3 read/register/publication race | A semantic/topology change around original/final reads or registration cannot publish stale/torn bytes, replace a supplying resource, or wait on the wrong bytes. Existing, exact-missing, and filesystem-folded both-missing output/input slots are rejected before link. Rejected output aliases remain repair dependencies until their topology changes and a recovery revision revalidates them. | One semantic/graph authority serves observation, finalization, repair token, existing alias validation, W14 common-parent collation proof, and add-snapshot-remove; barriers cover writes/replacements/folding/hard links, both-absent parent/leaf combinations, probe cleanup, alias-time-to-install removal, and dropped-event audit recovery. |
| D4 event lifecycle | Startup-before-init, initialization, pre-`started`, normal/identical/burst/max-debounce/full-pipe/overflow events, event during compile, the complete prior/current 4x4 W5 state product including readability recovery, every fatal class/first-error order, periodic audit, failed-set and repair-output merge, idle/active shared-pipe signal wake, signal/fatal/synchronous-transition precedence, and terminal cleanup wake/stop exactly once. | Injectable event/timer/error source, one shared pipe, dirty/uncertain/fatal/signal atomics, startup and every before/after-transition control snapshot, total semantic classifier, logical/repair merge owner, bounded-read poll owner, backend shutdown, and process teardown; native adapters stay thin. |
...
| D8 resource bound | Exact/rejected-next length/NUL/absolute-path/input/combined-input-repair-graph/evidence/finalization/symlink/encoding/message/frame/post-reap limits and each bounded offending identity; W14 descriptor `NAME_MAX`, 1,023-byte effective component limit, 36-byte suffix, 16 attempts, one directory fd plus one probe fd and two fixed 1,024-byte buffers; W13 250-ms abort grace; shared wake pipe/fatal slot; non-overlapping scan; 64 KiB hash and fixed transcript/pump buffers; nonblocking special-file rejection; checked counters; and memo refusal are reachable. | Parameterized limit/repair/encoding/frame/pump/snapshot/overflow/fatal/FIFO/sparse/RSS/timer/probe owners assert the encoded logical path or path pair, or fixed length/hash for the overlong case, on every refusal. |
| D9 observable transcript | The single stderr protocol stream fixes startup-without-`started`, marker/prerequisite order, marker atomicity, write/flush/sink/status/poll/read failure, no-stdout behavior, reversible frames for every nonterminal record, instability/alias/collation/resource/backend/wake/pump errors with offending identity, decoded in-process/child logical channels, object/link order, behavior, and exits for success/failure/recovery/watcher error. Linked executables are not byte-compared on macOS. | One-pipe golden transcript with every startup checkpoint, kind/state/chunk boundary, all-byte and marker-shaped diagnostic/cache/message/path/child injection, decoded bounded parity, short marker writes, full/partial/flush/sink/status/poll/read errors, mutation/repair/collation, exact limits/classes/identities, and one-shot object/link/program comparison. |
