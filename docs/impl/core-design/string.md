...
- `template` results are arena-regioned `str` (it allocates; `guard_lambda_alloc_leak` applies).

## Effects

Pure (no I/O). The *allocation-visibility* rules are enforced structurally, not via effects:
`str + str` and `template` inside a pipeline lambda are hard errors ("would silently leak" —
`lambda.rs`), while the same concat inside the lambda's own `arena {}` is fine.
...
byte-oriented (`str + str` on a UTF-8 pair cannot produce non-UTF-8).

## Regions

`region_of(trim*/s[a..b]) = region_of(s)` — sub-views inherit. `region_of(a + b) =
Arena(depth)`. `clone` → owned, region-free. A `string` struct field read borrows as a
...
- P2 — the pipeline-lambda concat rejection is a *policy*, not a limitation: the fix is builder
  or the lambda's own arena. Never weaken it to a lint.
...
`m5.rs` (methods incl. find/rfind pairs, trim family, concat arena rules, builder incl. fuse,
template, escapes, UTF-8 byte lengths, print type coverage); `lambda.rs:271/280/287/294`
(lambda alloc rejections + arena-in-lambda allowance); `hash.rs` (view acceptance);
`fuzz_fmt.rs` (formatter round-trips string-heavy sources); examples `strings.align`,
`template.align`. SIMD scan pin: #310 differential oracle.
