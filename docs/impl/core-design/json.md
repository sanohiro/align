...
- **`json.scan` (J5):** streaming typed rows, binding-annotation-typed, pipeline source only.
  ...
  **Generic inference boundary.** Request 6 covers only an ordinary generic call whose scanner row
  ...
  travels through local/parameter inference and lambda captures, so a generic wrapper cannot erase
  the exact diagnostic identity; it never enters HIR. After all arguments, the checker finalizes every bound
  ...
  conflict boundary, and `m5::json_scan_generic_argument_source_spelling` owns local and lambda
  capture spelling propagation. The matrix also owns the exact Copy diagnostic through a bare
