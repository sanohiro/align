...
- `Result` に対する `else` は設計上拒否される(「`else` unwrap は Option を期待する」) — `Result` は
  読むか渡すべき理由を運ぶものであって、黙ってデフォルトで消し去るものではない。
...
(generic な関数内の `o else d`); `lint_unhandled_result.rs`; #308 の main-error 制限テスト; 例
