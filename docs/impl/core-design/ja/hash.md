...
- 初等関数 `exp`/`exp2`/`log`/`log2`/`log10` は plan 75 で一括して決着済み（実装待ち）。LLVM の scalar/vector intrinsic へ直接 lower し、ULP または cross-target の bit 同一性は保証しない。命令選択前の scalarize は可視化し、provider のない retained IR には後段でも scalarize され得ると警告する。最終的な machine SIMD は object の検査で確認する。三角関数は引き続き未提供。
