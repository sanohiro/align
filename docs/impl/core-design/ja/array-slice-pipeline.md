...
- P5 — 束縛されていない owned 一時値には 2026-07-15 から経路別の synthetic owner があり、
  view の生存期間と loop 各反復の cleanup も追跡する。`chunks(n)` は直接 `.len()`、index、pipeline
  へ渡す場合にも header 配列を常に materialize する。この残るコストは
