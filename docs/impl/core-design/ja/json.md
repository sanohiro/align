...
- **`json.scan`（J5）:** 型付き行ストリーミング。binding annotation で型付け、v1 は pipeline
  ...
  **Copy** でなければならない。`json.scan` は、到達可能な struct・option・union の完全な型
  ...
  | Acceptance と benchmark | 下記の現行 owner test と `json_scan_copy_row_no_owned_alloc` allocation probe が live contract を閉じる。`scripts/compare-json-scan-identity.sh` は implementation-time の固定 cross-compiler evidence を保存する。性能主張はなく benchmark は N/A。 |
  ...
  **Ownership closure matrix（implementation gate）。** 次の cell は implementation PR の開始前に閉じる。
  ...
  `cache_codegen::json_scan_generic_return_context_no_publication` とする。historical な implementation-time evidence は
  `json_scan_cross_compiler_identity` であり、
