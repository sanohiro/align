...
`region_of(trim*/s[a..b]/s.bytes()) = region_of(s)` — サブビューは継承する。`clone` → owned、region 無し。
...
- `s[i]` の直接バイトアクセスは無い — UTF-8 義務を外すことが呼び出し箇所で明示されるよう、
  `s.bytes()[i]` という明示的なバイトビューを使う。
...
`m5.rs`(find/rfind ペア、trim ファミリ、zero-copy bytes view、fuse を含む builder、template、escape、
