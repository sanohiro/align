...
a + b                      -> str        // arena-backed concat; REJECTED inside pipeline lambdas
...
- `template` の結果は arena に region 付けされた `str` である(確保を伴う。`guard_lambda_alloc_leak`
  が適用される)。

## Effects

Pure(I/O 無し)。*確保の可視性* のルールはエフェクトではなく構造的に強制される — パイプラインのラムダ
内での `str + str` と `template` はハードエラーである(「黙って漏らすことになる」 — `lambda.rs`)。一方、
ラムダ自身の `arena {}` 内での同じ連結は問題ない。
...
保つ(UTF-8 のペアに対する `str + str` は非 UTF-8 を生み出しえない)。

## Regions

`region_of(trim*/s[a..b]) = region_of(s)` — サブビューは継承する。`region_of(a + b) = Arena(depth)`。
`clone` → owned、region 無し。`string` の struct フィールド読み取りは Frame region の `str` として借用
...
- P2 — パイプラインのラムダにおける連結の拒否は *方針* であって制約ではない。修正は builder かラムダ
  自身の arena である。lint へ弱めてはならない。
...
`m5.rs`(find/rfind ペア、trim ファミリ、連結の arena ルール、fuse を含む builder、template、escape、
...
往復); 例 `strings.align`、`template.align`。SIMD スキャンの固定: #310 differential oracle。
