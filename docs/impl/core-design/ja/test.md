...
| Cache and sources of truth | Test mode は distinct versioned cache domain。unit key はcanonical span-erased production semantic/codegen identity、complete local overlay suffix、ordered test catalog/body/assertion location/test static descriptor/mode version/target/profile/codegen input/imported interface hash/runtime ABI fingerprint を含む。harness key は complete ordered canonical-id/symbol catalog と launch/acknowledgement/completion/terminal-commit protocol version を含む。production HIR projectionはdomain `align-production-codegen-v1`で始まり、Program table/function/statement/expressionをstored orderでexhaustiveにvisitし、全non-`Span` fieldをexisting canonical scalar/sequence encoderでencodeする。各expression直後に、そのfunctionのcurrent span-keyed `drop_individual_exprs`からMIRが得るexact tri-state `absent | arena | individual` lookup resultもencodeする。validatorはtraversed expressionに一致しないside-table keyをrejectし、raw map iteration/orderはkeyへ入れない。ordered production static-descriptor projectionはunit、item、descriptor id、visibility、consumer、driver、source tagと`File.path_literal`または`Inline.decoded_sql`、params/row type、complete reachable contract、static optionをencodeし、constructor/common/native option spanとsource variantのpath/literal spanだけをomitする。lowering memo keyはHIR projectionと既存visibility/toggle inputをconsume。production codegen/artifact identityはHIR/descriptor projectionの後に既存mode/target/profile/resolved-artifact inputをconsumeする。raw `Debug`、compiler source-file path、source span、diagnostic、located metadata、raw side-table iteration、test overlayは含めない。public interface identityはproduction prefixの既存public-surface encoderを保ち、overlayをexcludeするがprivate-body/descriptor inputは得ない。既存complete-source frontend lookupはtest-only edit後missし得る。offset shiftでcurrent production HIR/descriptor span/located outputは変化し得るが、semantic ownership stream、descriptor projection、MIR codegen graph、object key、link input、executable bytesは不変。まずこの file を更新し、その後 `draft.md`、`docs/language-spec.md`、`docs/design-notes.md`、`docs/open-questions.md`、`docs/impl/02-frontend.md`、`docs/impl/07-roadmap.md`、`docs/impl/10-cache-first-optimization.md`、`docs/impl/19-hir-validation-ledger.md`、runtime ABI row が land するときの ledger、同期 Japanese mirror を更新する。 | Exhaustive HIR/descriptor projection match、canonical span-erased prefix/overlay key encoder、interface/implementation hash が invalidation を所有。new HIR field/variantまたはsemantic side tableはcompile-time projection update。ownerは全ownership fact/descriptor semantic fieldを独立mutation、orphan ownership keyをreject、全descriptor-only spanをvaryし、earlier test width changeでlater production spanをshiftする。changed source/located metadataとidentical semantic/descriptor projection/public interface/object key/link input/executable、必要箇所のchanged overlay object/harnessをpin。 |
...
| --- | --- | --- |
| Ready/acquire | first graceful signal; capture allocation; stdout/stderr pipe; control socket; close-on-exec/nonblocking flags; pre-spawn clock sample; spawn; process-group proof | signal は stage 除去後 conventional exit。pipe/socket/flag/remap failureは全acquired descriptorをclose、両storeをreleaseし、childなしinfrastructure。successful spawn と `getpgid(child_pid) == child_pid` proofでleader PIDをretainしAwaitAckへ。proof failureはそのPIDだけへSIGKILL、rowをdrain/close、そのchildをreapし、launchを送らずuser codeも実行せずspawn infrastructure failureで停止。 |
...
| Child lifecycle | exact preallocation/failure、spawn/pipe/control acquisition/flag failure、unverified-group direct-PID cleanup、nonblocking stdout/stderr/control emptyまでdrain、short-output idle child、exact/rejected-next bound、leader verified-group retained/moved、全terminal result x descendant absent/present/escaped、WNOWAIT pin、group-then-direct signal before reap、terminal control barrier、cleanup-error override + evidence preservation、interrupted wait/read | deterministic allocation/I/O-liveness/failpoint/process-tree owner |
...
implementation closure matrixはcomplete capture/control I/O livenessとsemantic side-table identity周囲で
再度re-openした。prior statement-placement、terminal-target、finalization closureは維持する。
