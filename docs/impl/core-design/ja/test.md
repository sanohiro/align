...
| Cache and sources of truth | Test mode は distinct versioned cache domain。unit key は frozen production-prefix identity、complete local overlay suffix、ordered test catalog/body/assertion location/test static descriptor/mode version/target/profile/codegen input/imported interface hash/runtime ABI fingerprint を含む。harness key は complete ordered canonical-id/symbol catalog と launch/acknowledgement/completion/terminal-commit protocol version を含む。production interface/codegen key は frozen production prefix だけをencodeする。既存 source-keyed frontend lookup は test-only edit 後 miss し得るが、生成する production HIR table/descriptor/MIR/object key/link input/executable bytes は不変。まずこの file を更新し、その後 `draft.md`、`docs/language-spec.md`、`docs/design-notes.md`、`docs/open-questions.md`、`docs/impl/02-frontend.md`、`docs/impl/07-roadmap.md`、`docs/impl/19-hir-validation-ledger.md`、runtime ABI row が land するときの ledger、同期 Japanese mirror を更新する。 | Canonical prefix/overlay key encoder と interface/implementation hash が invalidation を所有。named input を独立に mutate し、byte-identical production prefix/artifact と必要箇所の changed overlay object/harness を pin。 |
...
prefixをclose/freezeする。test-only edit は source-keyed frontend lookup を invalidate し得るが、その後
byte-identical production HIR table/descriptor/MIR/object key/link input/executable を再現しなければならない。
test-mode keyはそのprefix identityに加え、全overlay suffix、local body、ordinal、canonical id、assertion
location、static descriptor、mode versionをencodeする。harness keyはcomplete ordered catalog/symbol mapping、
3つのcontrol protocol、terminal-commit versionを覆う。import order changeはpublic module interfaceを変えず
harness orderを変え得る。
...
| Checked artifact partition | test前にproduction fixed pointをfreeze、catalog root/back-reference、overlay closure equality、test lifted/generic helper、production/test shared monomorph reuse、全nominal/interned type classのsuffix、test static descriptor/capability、prefix mutation/suffix referenceなし | malformed prefix/overlay HIR matrix、whole/per-unit production-byte twin、generated-closure/descriptor/capability owner |
| Production isolation | 全 source command はfull resultをvalidateするがfrozen prefixを選択、located MIR/remark と `db prepare` descriptor はoverlayをomit、overlay capability/link/export/interface/cache influence なし、ordinary main 不変 | parameterized command matrix、production/test view twin、database descriptor owner、byte-identical HIR/MIR/object/link/executable owner |
...
| Runner state machine | Ready/AwaitAck/Running/Quiescing/Reporting/Between/Finalize x 全event、pre/post-ack deadline/output、drain-EAGAIN/terminal/drain barrier、全outcome group signal before direct reap、quiesced evidence through report/release、terminal commitまでcontroller-owned summary | deterministic Cartesian state/event/typestate/barrier/final-commit owner |
...
| Cache identity | 全production-prefix/overlay/harness key fieldの独立mutation、production frontend miss + byte-identical prefix/object hit、test descriptor/capability inclusion | cache hit/miss、prefix/suffix、canonical-key owner |

## Capability boundary and deferrals

implementation boundaryはcomplete test-generated artifact closureとterminal finalization周囲でre-openした。
