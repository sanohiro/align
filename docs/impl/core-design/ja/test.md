...
| Parent-to-harness launch ABI | 各spawn前にparentはcontrol socketpair/capture endpointを準備し、parent endpointをnonblockingにする。child spawn actionは最初にone-invocation suite cwdを`posix_spawn_file_actions_addchdir_np`でinstallし、fd 0を`/dev/null`、fd 1/2をcapture、fd 3をcontrolへsource-equals-targetを含めmapしてfinal mappingのclose-on-execをclearする。Linuxは全dup action後に`posix_spawn_file_actions_addclosefrom_np(4)`をappendする。macOSは代わりに`POSIX_SPAWN_SETPGROUP`とともに`POSIX_SPAWN_CLOEXEC_DEFAULT`をenableする。Appleの規則ではfile actionがcreateした全descriptorはそのflag下でinheritされるため4 mappingだけが残り、ambient/row-original descriptorはchild snapshotでcloseされる。両pathのfinal child tableはexactly 0..3だけ。supported Linux release hostはaddchdir/addclosefrom extension、supported macOS release hostはaddchdir/close-on-exec-default attributeを必須とする。unavailable/configuration failureはspawn前にcwd=`working directory`、remap/Linux close-from/macOS close-on-exec-default setup=`descriptor mapping`でreject。argv[0]はprivate stage pathのみ、environmentはinherit、cwdは全rowでsnapshot値のまま。残るnonblocking launch/ack、group proof、codec/validation ruleはEnglish ledgerどおり。 | Verification matrixがsuite-cwd snapshot/use/addchdir failpoint、fd 0/1/2/3全remap、Linux closefrom order、macOS close-on-exec-default configuration、ambient fd 4/soft-limit boundary、concurrent parent cwd/fd mutation、exact child fd inventory、全protocol/group edgeを覆う。 |
...
| --- | --- | --- |
| Ready/acquire | first graceful signal; capture allocation; pipe/socket/flags; suite-cwd/remap/platform-close setup; clock; spawn; group proof | runnerはimmutable executable/catalog/limits/cwd/cache reportだけを所有し、全build ownerはnormal Drop済み。completed spawn planはcwd install、fd 0..3 replace、Linux close-from actionまたはmacOS close-on-exec-default attributeによるfd 4以上closeの順。cwd/remap/platform-close failureはchild作成前infrastructure。残るsignal/group transitionはEnglish ledgerどおり。 |
...
| fd 0/1/2/3 remap、Linux fd 4以上close-from action、またはmacOS close-on-exec-default attribute configuration | `descriptor mapping` |
...
| Launch protocol | snapshotted cwdの`addchdir_np` install、source-equals-targetを含むfd 0..3 remap、最後のLinux `addclosefrom_np(4)`またはmacOS `POSIX_SPAWN_CLOEXEC_DEFAULT`、ambient fdなしexact inventory、parent capture/control nonblocking、concurrent parent cwd/fd mutation、argv/environment/stdin、group proof、launch/ack product | driver/harness protocol matrix + cwd/acquisition/flag/remap/platform-close failpoint |
...
replaceする。Linuxは`posix_spawn_file_actions_addclosefrom_np(4)`を最後にappendし、macOSは
`POSIX_SPAWN_CLOEXEC_DEFAULT`をsetして4 file-action mappingだけをinheritする。harnessのfdはexactly
...
| P2 childがambient descriptorをinheritし得る | fd 0..3 replace後に最後のLinux `posix_spawn_file_actions_addclosefrom_np(4)`またはmacOS `POSIX_SPAWN_CLOEXEC_DEFAULT`を使い、fd 4/soft-limit probeとexact child inventoryでambient fd不在を証明。 |
| P1 macOS planがunavailable addclosefrom symbolを参照 | Platform closure axisをreopen。Linuxはsupported trailing close-from actionを維持し、macOSはfile-action inheritance ruleで4 explicit mappingだけを残すsupported close-on-exec-default spawn attributeを使う。 |
| P2 per-unit test-import suppressionがdirect dependencyで停止 | Driverはreal source import graph上のreverse reachability closureを一度計算し、direct importのcomplete subtreeがcatalog testを含むときだけunused warningをsuppressする。test-free bridgeを挟む3-level ownerでtransitive caseを固定。 |
| P2 malformed pre-Ack recordが同一drain内のlater Ackで消え得る | Ack admissionはearlier pre-Ack record errorがfreezeされていないことを必須にする。drainはnonblocking boundaryまでconsumeし続けるが、malformed-then-Ackはunacknowledged launch infrastructureのままでcatalogをadvanceしない。 |
