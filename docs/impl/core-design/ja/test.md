...
| Cache and sources of truth | Test modeはdistinct versioned cache domain。unit keyはcanonical production identity、overlay suffix、catalog/body/assertion location、permitted non-DB test static descriptor、mode/target/profile/codegen/imported interface/runtime ABIを含む。harness keyはordered canonical-id/symbol catalog、sole-entry/source-main mapping、4 child-runtime ABI、launch/ack/completion/final protocolを含む。production HIR projectionは`align-production-codegen-v1`からstored orderで全non-`Span` fieldと各expression直後のexact `absent|arena|individual` ownership factをencodeし、orphan keyをreject。production descriptor projectionは全semantic fieldをencodeしdiagnostic locationだけomit。production lowering/artifactはprojection+既存mode/target/profile/resolved input、public interfaceは従来public surfaceだけをconsume。raw Debug/path/span/diagnostic/located metadata/raw map/test overlayはproduction key外。test-only offset shiftはcurrent spansを変え得るがsemantic projections/object/link/executable bytesは不変。更新順はこのfile、`draft.md`、language/design/open questions、frontend/roadmap/cache plan/test policy/HIR ledger/runtime ABI ledger/`docs/impl/22-repl-plan.md`/pkg.db design、同期Japanese mirrors。 | Projection match、prefix/overlay encoders、runtime registry、interface/implementation hashがowner。new field/variant/side tableはcompile-time update。全ownership/descriptor semantic mutation、orphan key、overlay DB consumer rejection、descriptor span、earlier test width shiftをownerが固定。 |
...
| --- | --- | --- |
| Ready/acquire | first graceful signal; capture allocation; stdout/stderr pipe; control socket; close-on-exec/nonblocking flags; pre-spawn clock sample; spawn; process-group proof | signalはstage除去後numeric `128 + signal`でraw exit。pipe/socket/flag/remap failureは全acquired descriptorをclose、両storeをreleaseし、childなしinfrastructure。successful spawn と `getpgid(child_pid) == child_pid` proofでleader PIDをretainしAwaitAckへ。proof failureはそのPIDだけへSIGKILL、rowをdrain/close、そのchildをreapし、launchを送らずuser codeも実行せずspawn infrastructure failureで停止。 |
...
| spawn action constructionまたはfd 0/1/2/3 remap plan | `descriptor mapping` |
...
| Test-callgraph process boundary | catalog-root direct/imported/function-value/lifted/generic reachability、deterministic first site、reachable `ProcessCommand` rejection、shared de-dup、unreachable production acceptance、malformed HIR、inert frozen-prefix command/runtime retention、production parity | whole/per-unit reachability matrix、checked-HIR negative、inert-prefix twin、unchanged production artifact owner |
| Launch protocol | source-equals-targetを含むfd 0/1/2/3 remap、all-original close-on-exec、parent capture/control spawn前nonblocking、argv/environment/stdin shape、pre-exec group setup/proof、launch/ack codec/golden、Ack/short-output idle-child poll return、malformed/order/ordinal product、pre/post-ack exit distinction | driver/harness protocol matrix + acquisition/flag/remap failpoint |
...
| Signal controller | shared lease、prior mask/disposition、SIGCHLD、setup/rollback、lock-free Idle/Writing/Selected/Pending arbitration、summary retention/final recheck、first-signal coalescing、signal before/during/after syscall | parameterized signal/writer/final-commit/wait-status owner |
...
| Child lifecycle | preallocation、`/dev/null`、spawn/pipe/socket/flag/remap/clock/poll failpoint、closed diagnostic operation mapping、nonblocking drains、group/direct lifecycle、barrier/reap/evidence | allocation/I/O-liveness/diagnostic-operation/process-tree owner |
...
implementation closure matrixはtest-callgraph process boundary/terminal exit product、すなわちreachable
process construction、default/imported module identity、`align-repl` item handling、signal/write arbitration、
exact numeric exit observation周囲でre-openした。preceding artifact-formation-to-execution mode/ABI closureと
