...
| Reporting | exact result bytes、failure-only evidence、all reason/signal/write product、terminal retention、nonrecursive failure、quiet-wrapper success/failure/interrupt | CLI stdout/stderr、consuming-row、sink/final-exit、repository quiet integration owners |
...
implementation closure matrixはcomplete artifact-formation-to-execution mode/ABI product、すなわち
production/test mode selection、entry identity、DB preparation reachability、child runtime reporting、option
consumer、runner diagnostic operation周囲で再度re-openした。prior statement-placement、terminal-target、
finalization、capture/control liveness、semantic side-table closureは維持する。
