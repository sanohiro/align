...
| fd 0/1/2/3 remap or fd-4-and-above close-from spawn-action construction | `descriptor mapping` |
...
| Launch protocol | one snapshotted suite cwd installed by `posix_spawn_file_actions_addchdir_np`; fd 0/1/2/3 remapping including source-equals-target; trailing `posix_spawn_file_actions_addclosefrom_np(4)`; exact child descriptor inventory with no ambient fd; parent capture/control nonblocking before spawn; argv/environment/stdin shape; concurrent parent cwd/fd mutation; pre-exec group setup/proof; launch/ack codecs and goldens; Ack/short-output idle-child return to poll; malformed/order/ordinal products; pre/post-ack exit distinction | driver/harness protocol matrix and cwd/acquisition/flag/remap/close-from failpoints |
...
Linux/macOS hosts, `posix_spawn_file_actions_addchdir_np` installs that cwd before exec; the fd
0/1/2/3 remaps follow, and `posix_spawn_file_actions_addclosefrom_np(4)` is appended last. Thus the
harness starts with exactly `/dev/null`, stdout capture, stderr capture, and fd-3 control, and cannot
observe an ambient parent fd. The snapshot remains stable if an embedding thread later changes the
parent cwd or descriptor table.
...
| P2 the child inherited unspecified ambient descriptors | The supported-host spawn plan replaces fd 0/1/2/3 and appends `posix_spawn_file_actions_addclosefrom_np(4)` last; fd-4/soft-limit probes and an exact child inventory prove no ambient descriptor survives. |
