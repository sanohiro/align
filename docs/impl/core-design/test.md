...
| P2 per-unit test-import suppression stopped at the direct dependency | The driver computes one reverse reachability closure over the real source import graph, so an otherwise-unused direct import is suppressed exactly when its complete subtree contains a catalog test; a three-level test-free bridge owner pins the transitive case. |
| P2 a malformed pre-ack record could be erased by a later acknowledgement in the same drain | Acknowledgement admission now requires that no earlier pre-ack record error was frozen. The drain still consumes through the nonblocking boundary, but malformed-then-Ack remains unacknowledged launch infrastructure and cannot advance the catalog. |
