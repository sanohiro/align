...
| spawn file-action construction or fd 0/1/2/3 remap plan | `descriptor mapping` |
...
| Test-callgraph process boundary | catalog-root reachability across direct/imported/function-value/lifted/generic edges; deterministic first site; direct and nested `ProcessCommand` rejection; shared site de-duplication; unreachable production helper acceptance; malformed-HIR rejection; inert frozen-prefix command-code/runtime-selection retention; production parity | whole/per-unit reachability matrix, checked-HIR negative, inert-prefix twin, and unchanged-production artifact owner |
| Launch protocol | fd 0/1/2/3 remapping including source-equals-target, all-original close-on-exec, parent capture/control nonblocking before spawn, argv/environment/stdin shape, pre-exec group setup/proof, launch/ack codecs and goldens, Ack/short-output idle-child return to poll, malformed/order/ordinal products, pre/post-ack exit distinction | driver/harness protocol matrix and acquisition/flag/remap failpoints |
...
The implementation closure matrix is re-opened around the test-callgraph process boundary and
terminal exit product: reachable process construction, default/imported module identity,
`align-repl` item handling, signal/write arbitration, and exact numeric exit observation. This
extends the preceding artifact-formation-to-execution mode and ABI closure and retains the earlier
statement-placement, terminal-target, capture/control I/O-liveness, and semantic side-table
closures. The accepted code
