...
| Time, output, and child cleanup | Before creating child pipes or spawning, the parent fallibly allocates one fixed raw-byte backing store of exactly the selected bound for each stream; zero allocates none. It never geometrically grows, replaces, or duplicates those stores after spawn. Reads fill the remaining range directly and use one fixed one-byte probe per full stream to detect the rejected next byte, so retained capture payload is exactly twice the selected bound plus two probe bytes and fixed pipe/control state, excluding allocator metadata/rounding, with no old/new-allocation transient. Allocation failure frees any first store and is runner infrastructure failure before user code. The deadline runs from the pre-spawn sample through acknowledgement, user execution, group quiescence, descriptor drain, and direct-child reap. Exact output fit succeeds; the first extra byte before acknowledgement is infrastructure failure and after acknowledgement is a test failure. After every successful spawn with verified group establishment and for every terminal path—pre-ack failure, Ok, Err, nonzero exit, signal, timeout, output excess, malformed/missing completion, infrastructure error, or interrupt—the leader PID stays unreaped until the complete pinned process group has been signalled. The unverified-group failure above instead uses direct-PID cleanup because no group target is trusted and no user code has run. Ordinary/test/infrastructure paths send SIGKILL. A graceful SIGHUP/SIGINT/SIGQUIT/SIGTERM first forwards that signal, permits exactly 250 ms, then sends SIGKILL. Direct-child terminal observation uses `waitid(..., WNOWAIT)`; a release host without non-reaping observation rejects before the first spawn. ESRCH from group kill is success only after non-reaping observation proved the leader terminal; otherwise it is infrastructure failure. The parent then drains accepted prefixes/control, closes descriptors, and reaps only the direct child with EINTR retry. Descendants are signalled, not reaped by this runner. Every kill/drain/close/wait/reap step is attempted in fixed order even after an earlier error. Any cleanup failure stops the suite after best effort; if an individual failure reason was already selected, its bounded block precedes the infrastructure diagnostic. The first graceful signal outranks simultaneous test/infrastructure outcomes, emits no new diagnostic or summary after cleanup, and exits 129/130/131/143 respectively. Only a fully cleaned ordinary test result permits the next catalog row. | The dedicated driver test-runner component alone owns signal snapshots, allocation, descriptors, deadline, pinned PGID, non-reaping observation, and stage cleanup. A Cartesian state/event owner crosses pre/post-ack, completion absent/present, leader running/terminal, descendants absent/present, every result class, four graceful signals, deadline/output at every boundary, each acquisition/cleanup failpoint, evidence preservation, and no-next-row-before-reap. |
| Reporting and exit | Passing child stdout/stderr is never emitted. A fully passing suite writes exactly `test result: ok. <N> passed; 0 failed` plus LF to runner stdout and exits 0. For each failure, in catalog order, runner stdout receives `FAIL <canonical-id>` plus LF, one fixed `reason: ...` line, then nonempty captured stdout and stderr under fixed `--- stdout ---` / `--- stderr ---` headers without decoding or rewriting their bytes; a missing final LF is followed by one LF before the next runner line. Runner stdout then receives `test result: FAILED. <P> passed; <F> failed` plus LF and the runner exits 1. Timeout, output-limit, signal/exit/record mismatch, returned Error, and malformed-record details use the closed formats below; assertion location is the bounded child-stderr line. Successful child output remains suppressed even when another test fails. Zero tests use the exact diagnostic above. Pre-ack timeout/output use the exact infrastructure lines below. Other build/compiler/catalog diagnostics retain their compiler-owned formats; runner infrastructure uses the closed stderr template below. Every such error exits 1 and never prints a complete-suite summary. A graceful interrupt emits no added line and no summary; prior failure blocks remain. | CLI reporter owns deterministic bytes and terse-success behavior. Golden stdout/stderr owners cover zero tests, all-success, mixed result, arbitrary bytes, missing LF, assertion, every termination reason, every pre/post-ack product, graceful interruption, and infrastructure before/after a selected result. |
| Ownership, allocation, and effects | A test declaration owns no runtime value. The test harness catalog and static assertion locations are compiler-owned immutable data. Test-mode object/harness allocation is driver-owned. The parent owns the two exact pre-spawn capture stores, never copies captured bytes for reporting, writes framing and stored ranges directly, and releases both stores after the row. Test functions and assertions are Impure and never exported. The bounded process-signal lease/self-pipe is the only new process-global state and exists only while one `alignc test` runner owns it; no test registry, reflection table, hidden source scan, persistent test history, retry, concurrency, shuffle, filter, fixture lifecycle, snapshot, coverage, benchmark, or network policy is introduced. | Sema effect inference, codegen data ownership, signal-lease RAII, and runner RAII are owners. Allocation accounting and failpoints pin exact requested live/transient capture bytes; capability-link twins prove production exclusion and test inclusion. |
...
| AwaitAck | first graceful signal; expired deadline; control datagram; non-reaping leader status; stdout; stderr | Deadline or first excess byte is launch infrastructure failure. Control is read before streams so a queued valid acknowledgement establishes the user-code boundary before causally later user output is classified. A valid exact acknowledgement enters Running. Malformed/missing/out-of-order control or leader termination selects launch infrastructure failure. Every terminal selection enters Quiescing without reaping. |
| Running | first graceful signal; stdout; stderr; completion datagram; non-reaping leader status; deadline | Stdout excess wins over stderr; either wins over completion/timeout. A complete valid completion/status product observed in one wake wins over that wake's deadline. Completion without status remains pending; terminal status without a valid product selects the closed mismatch reason. Every selected outcome enters Quiescing without reaping. |
| Quiescing | selected graceful signal or mandatory SIGKILL; remaining stdout/stderr/control; deadline; direct-child status; cleanup errors | The pinned group is signalled before `waitpid`. Accepted prefixes are drained and only the direct child is reaped. A pre-ack deadline remains launch infrastructure; a post-ack deadline remains a test timeout. Graceful signal remains highest precedence. Otherwise any cleanup failure becomes infrastructure failure while preserving an already selected test block. No result is reportable before descriptors close and capture stores release. |
| Between rows | first graceful signal; selected result reporting; next-row acquisition | There is no live or waitable direct child and no open row descriptor. Only a cleaned test result advances the catalog. Infrastructure stops; graceful signal removes the stage and exits. |
| Finalize | first graceful signal; stage removal; ordinary signal-controller teardown; summary | Stage removal and controller teardown precede the summary. Their failure is infrastructure failure with no summary. A graceful signal emits no new line/summary and exits conventionally after cleanup. |

After successful Quiescing the ordinary result order is selected output/timeout, completion length,
magic/version, outcome, tag, reserved bytes, conditional code, ordinal, record/status correlation,
then returned Error or Ok. This state-specific order closes every multi-ready product without using
scheduler timing as a retry or error-rewriting input.
...
| Signal controller | shared lease, prior mask/dispositions, SIGCHLD compatibility, setup/rollback/teardown, HUP/INT/QUIT/TERM ordering, first-signal coalescing, signal at every runner state | parameterized process-global signal owner |
| Runner state machine | Ready/AwaitAck/Running/Quiescing/Between/Finalize x every event, pre/post-ack deadline/output, completion/status timing, all-outcome group signalling before direct reap | deterministic Cartesian state/event and barrier owner |
| Child lifecycle | exact preallocation/failures, spawn/pipe/control acquisition failures, unverified-group direct-PID cleanup, concurrent drains, exact/rejected-next bounds, descendants absent/present for every verified-group terminal result, WNOWAIT pin, group-kill-before-reap, cleanup-error override with evidence preservation, interrupted wait/read | deterministic allocation/failpoint and process-tree owner |
| Reporting | exact zero-test and pre-ack infrastructure bytes, all pass one line, mixed failure, returned Error variant, assertion, exit/signal/timeout/output/malformed reason, four graceful signals, raw bytes/missing LF | exact CLI stdout/stderr golden |
...
The implementation boundary was re-opened around the runner lifecycle. The accepted code has two
strict internal proof domains: compiler formation owns syntax through the hidden harness and exact
control codecs; one dedicated driver test-runner component consumes only an immutable artifact,
ordered catalog, and validated limits and exclusively owns signal state, spawn/poll, deadline,
process groups, capture, reap, and reporting. Neither side may reproduce the other's validation or
lifecycle transition. Fake-stage protocol/process-tree owners validate the runner without compiling
Align source, while whole/per-unit owners validate the producer against that same codec boundary.
