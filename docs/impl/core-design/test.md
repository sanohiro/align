...
| Cache and sources of truth | Test mode is a distinct versioned cache domain. Its unit key includes the canonical span-erased production semantic/codegen identity, complete local overlay suffixes, ordered test catalog, bodies, assertion locations, test static descriptors, mode version, target/profile/codegen inputs, imported interface hashes, and runtime ABI fingerprint. Its harness key includes the complete ordered canonical-id/symbol catalog plus launch, acknowledgement, completion, and terminal-commit protocol versions. The one production projection starts with domain `align-production-codegen-v1`, exhaustively visits Program tables/functions/statements/expressions in stored order, encodes every non-`Span` field with the existing canonical scalar/sequence encoders, then encodes production static descriptors in order. Production interface and codegen keys consume that projection, never raw `Debug`, source spans/paths, diagnostics, located metadata, or the overlay; whole/per-unit modes add only their existing mode/target/profile inputs afterward. The existing complete-source frontend lookup may miss after a test-only edit; current production HIR spans and located output may change when offsets shift, while the span-erased semantic projection, descriptors, MIR codegen graph, object key, link inputs, and executable bytes remain unchanged. Update this file first, then `draft.md`, `docs/language-spec.md`, `docs/design-notes.md`, `docs/open-questions.md`, `docs/impl/02-frontend.md`, `docs/impl/07-roadmap.md`, `docs/impl/19-hir-validation-ledger.md`, the runtime ABI ledger when its row lands, and the synchronized Japanese mirror. | The exhaustive projection match, canonical span-erased prefix/overlay key encoders, and interface/implementation hashes own invalidation. A new HIR field/variant is a compile-time projection update. An earlier test edit changes byte width to shift a later production span; owners pin changed source/located metadata beside an identical semantic projection, object key, link inputs, and executable, plus changed overlay objects/harness where applicable. |
...
an earlier test edit may shift them. Production interface/object keys instead encode a canonical
span-erased projection of the complete semantic HIR plus production descriptors; diagnostic spans,
located metadata, and the overlay cannot enter that projection. The span-erased semantic projection,
descriptor vector, MIR codegen graph, object key, link inputs, and executable must therefore remain
byte-identical. The projection is the versioned exhaustive structural encoder above; it replaces
the current total-`Debug` lowering fingerprint rather than filtering rendered text. Test-mode keys
...
key covers the complete ordered catalog, symbol mapping, all three control protocols, and
terminal-commit version. An import-order change may therefore change the harness order without
...
| Checked artifact partition | production fixed point frozen before tests; catalog root/back-reference; overlay closure equality; test lifted and generic helper; shared production/test monomorph reuse; suffixes for every nominal/interned type class; test static descriptor and capability; no prefix mutation/reference to suffix | malformed prefix/overlay HIR matrix, whole/per-unit span-erased production-semantic twin, generated-closure and descriptor/capability owners |
| Production isolation | every source command validates the full result but selects the frozen prefix; located MIR/remarks and `db prepare` descriptors omit overlay; no overlay capability/link/export/interface/cache influence; ordinary main unchanged | parameterized command matrix, production/test view twin, database descriptor owner, current-span/located-metadata change owner, and byte-identical span-erased semantic/MIR-codegen/object/link/executable owner |
...
| Launch protocol | fd 3 datagram mapping, both-end close-on-exec plus parent nonblocking before spawn, argv/environment/stdin shape, pre-exec group setup/proof, launch/ack codecs and goldens, Ack-only idle-child return to poll, malformed/order/ordinal products, pre/post-ack exit distinction | driver/harness protocol matrix and acquisition/flag failpoints |
...
| Runner state machine | Ready/AwaitAck/Running/Quiescing/Reporting/Between/Finalize x every event, pre/post-ack deadline/output, nonblocking drain-empty/terminal/drain barrier, all-outcome pinned-group then direct-PID signalling before reap, quiesced evidence through report/release, controller-owned summary through terminal commit | deterministic Cartesian state/event, typestate, nonblocking/barrier, and final-commit owner |
| Child lifecycle | exact preallocation/failures, spawn/pipe/control acquisition/flag failures, unverified-group direct-PID cleanup, concurrent drains, exact/rejected-next bounds, leader retained in/moved out of verified group, descendants absent/present/escaped for every terminal result, WNOWAIT pin, group-then-direct signalling before reap, terminal control barrier, cleanup-error override with evidence preservation, interrupted wait/read | deterministic allocation/failpoint and process-tree owner |
...
| Cache identity | independent mutations of every span-erased production and overlay/harness key field, earlier variable-width test edit causing frontend/located miss and changed production spans but identical semantic projection/object hit, test descriptor/capability inclusion | cache hit/miss, span-erased projection, prefix/suffix, and canonical-key owner |

## Capability boundary and deferrals

The implementation closure matrix was re-opened again around semantic statement normalization,
event-loop liveness, complete terminal targeting, and span-free artifact identity. The accepted code
...
the complete span-erased semantic HIR and descriptor projection. Current spans remain in the checked
prefix for diagnostics and located output but cannot perturb object identity.

Inside the runner, `LiveRow` owns child, descriptors, stores, protocol state, and deadline. Its
parent control endpoint is nonblocking before spawn, so every control drain has a typed
`Datagram | Empty(EAGAIN/EWOULDBLOCK) | Error` result and Empty returns to poll. Pinned-group then
...
failure-block write or silent pass discard consumes and releases the stores before catalog advance. A terminal
