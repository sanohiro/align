...
| Child control runtime ABI | Test artifacts alone may declare five compiler-private unkeyed symbols: `i32 @align_rt_test_launch_recv_v1(i32 fd, ptr out_ordinal)`, `i32 @align_rt_test_fd_cloexec_v1(i32 fd)`, `i32 @align_rt_test_containment_install_v1(i32 fd)`, `i32 @align_rt_test_ack_v1(i32 fd, i32 ordinal)`, and `i32 @align_rt_test_report_v1(i32 fd, i8 outcome, i8 error_tag, i32 code, i32 ordinal)`, each `nounwind`. Rust receives the ordinal fields as `u32`, outcome/tag as `u8`, code/fd as `i32`, and the first output as `*mut u32`. Launch receive requires a non-null four-byte-aligned output, zeros it before I/O, receives once with a fixed 17-byte capacity while retrying EINTR, validates exact length/magic/version/reserved bytes, and publishes the little-endian ordinal only on success; catalog range remains harness-owned. The fd helper adds close-on-exec without changing other flags. Containment install validates an open fd 4, adds close-on-exec, and changes one runtime atomic exactly once from disabled to that retained borrowed descriptor number before acknowledgement; repeat, wrong-fd, or closed-fd input is `EINVAL`, and no ordinary runtime path resets it. Test-mode `process.command` may read the number but may neither close nor replace the harness-owned descriptor. Ack/report validate their closed semantic products, encode the exact stack-resident 16/20-byte records, and perform one datagram send with EINTR retry; a short send is `EIO`. All five return zero on success or a positive raw OS code, using `EINVAL` for invalid ABI input and `EPROTO` for malformed launch bytes. They allocate nothing and retain no pointer; only containment install retains the integer descriptor identity and changes child-process-global state, while no function closes fd 3 or fd 4. The harness maps launch receive/range failure to reserved exit 120, fd-3 close-on-exec or fd-4 containment-install failure to 121, acknowledgement send failure to 122, and completion send failure to 123; before acknowledgement 120/121/122 become `launch`/`descriptor flags`/`control write` infrastructure respectively, while post-ack 123 becomes `control write`, all with OS code zero because the failed child channel cannot transport its native code. Reserved-status interpretation is phase-specific; the same status produced by user code after acknowledgement remains an ordinary missing-record exit except for 123. | `align_codegen_llvm`, `align_runtime`, and the runtime ABI ledger own declarations/definitions atomically; none receives a language-callable `RuntimeKey`. ABI semantic/byte goldens, null/alignment, install-once/wrong-fd/closed-fd state, every invalid tag/product, EINTR/short/error send, malformed receive, fd retention/CLOEXEC/exec, reserved-status phase, symbol collision, base-export parity, and whole/per-unit link owners are required. |
| Time, output, and child cleanup | Before row pipes/spawn, the parent allocates one fixed store of exactly the selected bound per stream; zero allocates none. Direct reads plus one fixed rejected-next probe per full stream give exactly twice the bound plus two probe bytes and fixed pipe/control/witness state, excluding allocator metadata, with no growth/replacement transient. The deadline spans pre-spawn through Ack, user execution, signalling, witness EOF, drain, and direct-harness reap. Control/witness/capture drains are nonblocking. After `waitid(..., WNOWAIT)` observes the harness leader terminal, a second control drain precedes missing-record classification. Every verified terminal path keeps that leader unreaped while signalling the pinned row group then direct PID; ordinary/test/infrastructure paths use SIGKILL, while HUP/INT/QUIT/TERM forward the signal, allow exactly 250 ms, then use SIGKILL. The unverified-group path targets only its direct PID. All later observe/drain/close/reap steps run after a target error. In test mode an untimed, unbounded `process.command` target remains in the row group. A positive-timeout or explicitly bounded command first forks a sentinel; before any external target exists, the sentinel leaves the row group, retains fd 4 plus one private close-on-exec abort pipe and one private close-on-exec status pipe, and becomes the sole supervisor and target parent. It then forks the external target as a distinct group leader and keeps that target behind a pre-exec gate until target-group proof and all private-fd closures succeed. The target inherits none of those descriptors. Harness death or abort-pipe EOF makes the sentinel signal the target group then its unreaped direct target with SIGKILL, reap that target with EINTR retry, and poll group existence through exact `ESRCH` before closing fd 4 and exiting. Normal target terminal status follows the same residual-group cleanup. The sentinel then writes one exact 16-byte status record: bytes 0..7 are `41 4c 54 43 4d 44 53 01`; byte 8 is kind `0` for result or `1` for runtime error; bytes 9..11 are zero; bytes 12..15 are little-endian i32, restricted to public command code 0..=255 for kind 0 and a positive raw OS code for kind 1. It performs one blocking pipe write with EINTR retry; short write is internal `EIO`. The live harness validates field order, maps kind 0 to the ordinary output code and kind 1 to `Error.Code`, observes status-pipe EOF, and reaps the sentinel before `run` returns. Selected timeout/output/hard-capture failure or harness-death abort closes the abort writer; that path emits no status record, performs the same target cleanup, and exits. After harness death the OS orphan reaper owns only the sentinel's terminal status. A setup failure never releases it. Deliberate `setsid` escape retains the existing exclusion. The outer runner cannot quiesce, close its witness reader, classify/release evidence, advance, remove the stage, or reap its harness before exact fd-4 EOF, which proves every sentinel completed target/group cleanup. Quiescence retains both stores until complete failure reporting or silent pass discard. Cleanup failure stops the suite after best effort; a selected graceful signal outranks simultaneous outcomes and emits no new diagnostic/summary. | The outer driver owns row state; the test-aware process runtime owns each sentinel, target group/direct PID, gate, and reap. One Cartesian owner crosses pre/post-Ack, witness empty/EOF/error, leader retained/moved, bounded/unbounded commands at pre-sentinel/pre-target/pre-exec/running/target-terminal/group-absence states, nested descendants absent/present/escaped, harness normal/exit/exec/abort/SIGKILL, all result/signal/deadline/output/failpoint products, evidence retention, and no quiescence before witness EOF. |
...
fd 4 pipe is a write-only lifetime witness: the harness and every active bounded-command sentinel
retain a writer but never write a byte, while the runner owns the nonblocking reader. The parent
passes no compiler-private argv or environment value: the child observes only its stage path as
argv[0], the inherited environment, `/dev/null` stdin, captured stdout/stderr, and those two fixed
descriptors. The harness sets fd 3 close-on-exec and installs fd 4 as a close-on-exec containment
witness before acknowledging launch, so a successful `process.exec` inherits neither completion
authority nor witness ownership. The parent's endpoints are nonblocking before spawn, and every
receive/read returns to the poll/deadline loop at `EAGAIN`/`EWOULDBLOCK`; the child control endpoint
alone uses blocking receive while awaiting the single launch record. The child mapping explicitly
clears close-on-exec on fd 3 and fd 4, including either source-equals-target case, before the harness
sets it again after launch. The parent's fixed 21-byte control capacity distinguishes both valid
record sizes from any long datagram. A valid acknowledgement distinguishes harness setup from user
execution; only a normal test return is the safe completion producer. Missing completion bytes make
even exit zero fail, closing the `process.exit(0)` bypass. Witness EOF is independently mandatory
after leader termination and proves that no test-aware bounded-command sentinel remains. Unsafe code
can violate the descriptor boundary, as it can every safe compiler invariant; malformed or
self-inconsistent control or witness state still fails closed.

Test containment is installed before acknowledgement and changes no language-callable process API.
An untimed, unbounded command remains in the row group. For each bounded/timed command, the runtime
first forks a sentinel that leaves the row group and retains fd 4 plus a private close-on-exec abort
pipe. Only that armed sentinel creates the external target as a distinct group leader and releases
it from a pre-exec gate. Harness death or pipe EOF makes the sentinel kill the target group and its
direct target, reap the target, and wait for exact group absence before closing fd 4. Normal target
completion follows the same residual-group cleanup while preserving the public command code; a live
harness reaps the sentinel before `run` returns. After harness death the OS orphan reaper owns only
the sentinel status, while fd-4 EOF proves its target cleanup completed. This test-only supervisor
path prevents an outer row kill from losing the nested subgroup. A target that deliberately escapes
with `setsid` remains outside the existing `process.command` process-group contract.
...
| --- | --- | --- |
| Ready/acquire | first graceful signal; capture allocation; stdout/stderr pipe; control socket; containment-witness pipe; close-on-exec/nonblocking flags; pre-spawn clock sample; spawn; process-group proof | A signal removes the stage and exits conventionally. Any pipe/socket/flag/remap failure closes every acquired descriptor, releases both stores, and is infrastructure with no child. Successful spawn plus `getpgid(child_pid) == child_pid` proof retains the leader PID and enters AwaitAck. Failed proof sends SIGKILL only to that PID, drains/closes the row including witness EOF/close, reaps it, and stops with spawn infrastructure failure without sending launch or running user code. |
| AwaitAck | first graceful signal; nonblocking control/witness drain through empty; nonblocking stdout; nonblocking stderr; non-reaping leader status; terminal control barrier; deadline | Control and witness drain before streams and always return to the event loop when empty. Witness data or EOF before terminal status is launch infrastructure. Each ready capture stream drains only through empty, EOF, excess, or hard error, so a short write from a live child returns to the event loop. A valid acknowledgement proves fd-4 installation and enters Running immediately; the same drain continues under Running rules so queued acknowledgement + completion is consumed in one wake. An acknowledgement followed by an otherwise idle child cannot block witness/stream/status/deadline processing. A pre-ack malformed/order error, deadline, or first excess byte is launch infrastructure. Leader terminal status is recorded without reaping or declaring completion missing; it enters Quiescing, whose post-terminal control and witness drains are required barriers. |
| Running | first graceful signal; nonblocking stdout; nonblocking stderr; nonblocking control/witness drain through empty; non-reaping leader status; terminal control barrier; deadline | Stdout excess wins over stderr; either wins over completion/timeout. Each ready stream, control socket, and witness reader drains only its queued state through empty, EOF, excess, or hard error and returns to poll. Witness data or EOF while the harness leader is live is infrastructure. Every queued datagram is consumed in arrival order, so acknowledgement/completion coalescing and repetition are recorded before status classification. Completion without status remains pending. Empty sources immediately return to poll. Terminal status enters Quiescing without reaping or selecting a missing-record reason. After its control and witness barriers, a complete valid completion/status product observed before the same wake's deadline wins over that deadline; otherwise the closed precedence below applies. |
| Quiescing | selected graceful signal or mandatory signal; pinned-group then direct-PID targets; non-reaping terminal observation; final nonblocking control drain; containment-witness EOF; remaining stdout/stderr; deadline; descriptor close; direct-child reap; cleanup errors | The pinned group and then the still-unreaped direct PID are both signalled before reap; the second target closes a leader that moved out of its verified group. The runner obtains or retains terminal `WNOWAIT` status, drains control again through empty, drains accepted stdout before stderr, and continues polling until fd 4 reaches exact EOF. That EOF proves the harness and every active bounded-command sentinel have closed their witness writers; data, a hard read error, or descriptor close before EOF is infrastructure. Only then may the runner close every row descriptor and reap its direct child. The second control drain and witness barrier are mandatory even if both first drains returned empty immediately before terminal observation. A pre-ack deadline remains launch infrastructure; a post-ack deadline remains test timeout. Graceful signal remains highest precedence. Otherwise cleanup failure becomes infrastructure while preserving the selected outcome and stores. Successful cleanup produces one immutable quiesced row with no child/descriptor and with both stores still owned. |
...
`pipe`, `control socket`, `descriptor flags`, `descriptor mapping`, `clock`, `control write`,
...
| stdout/stderr capture-pipe or containment-witness-pipe creation | `pipe` |
| fd-3 socketpair creation | `control socket` |
| close-on-exec, nonblocking, or other descriptor-flag operation | `descriptor flags` |
| spawn file-action construction or fd 0/1/2/3/4 remap plan | `descriptor mapping` |
...
| parent acknowledgement/completion receive/drain or containment-witness read | `control read` |
| launch/ack codec, reserved bytes, linked ordinal, or phase validation | `launch` |
| process creation after a complete mapping plan | `spawn` |
...
key covers the complete ordered catalog, the sole-entry/source-main/root symbol mapping, the five
child runtime ABI records, launch/ack/completion/containment-witness/command-sentinel-status protocol versions, and
terminal-commit version. An import-order
change may therefore change the harness order without changing a public module interface.

The executable stays below an `ArtifactStage` retained by the runner through final witness EOF and
direct-child reap.
...
| Production isolation and mode product | every source command validates the full result but selects the frozen prefix; located MIR/remarks omit overlay; all database descriptors are prefix-owned; no overlay capability/link/export/interface/cache influence; ordinary main unchanged | parameterized command × accepted watch/profile/target/runtime-LTO/ThinLTO/PGO/jobs/cache-stats matrix; actual watch-rebuild/ThinLTO/PGO artifacts; database policy × driver offline metadata; production/test view, current-span/located-metadata, and byte-identical semantic/MIR-codegen/object/link/executable owners |
| Test options and artifact | complete accepted option product reaches its named terminal consumer; no-main test program; sole harness `main`; all four source-main ABIs encoded without wrapper; source main absent/unreferenced/directly called; one link; one immutable inode; exact `align_test$<8hex>` ordinal dispatch and hidden symbols | CLI terminal-consumer Cartesian owner plus driver artifact, reserved-symbol collision, and whole/per-unit semantic/execution owners |
| Launch protocol | fd 0/1/2/3/4 remapping including source-equals-target, all-original close-on-exec, parent capture/control/witness nonblocking before spawn, argv/environment/stdin shape, pre-exec group setup/proof, launch/ack codecs and goldens, fd-4 install before Ack, Ack/short-output idle-child return to poll, witness empty/data/EOF/error by phase, malformed/order/ordinal products, pre/post-ack exit distinction | driver/harness protocol/containment matrix and acquisition/flag/remap failpoints |
| Child control runtime ABI | exact five symbols/signatures; output-pointer initialization/alignment; borrowed descriptor/pointer ownership; containment install exactly once on fd 4; no allocation or descriptor close; launch receive/CLOEXEC/install/ack/report success and every error; reserved exits 120..123 and phase mapping | LLVM declaration/Rust export parity, independent codec goldens, install-state owner, registry/collision/attribute owner, and whole/per-unit link matrix |
...
| Signal controller | shared lease, prior mask/dispositions, SIGCHLD compatibility, setup/rollback/returning teardown, lock-free `Idle/Writing/Selected/WritingPending` arbitration, controller retained through summary, terminal mask/recheck/direct exit, HUP/INT/QUIT/TERM ordering, first-signal coalescing, signal before permit/during syscall/after commit and at every runner state | parameterized process-global signal, writer-arbitration, and final-commit owner |
| Runner state machine | Ready/AwaitAck/Running/Quiescing/Reporting/Between/Finalize x every event, pre/post-ack deadline/output, nonblocking control/capture/witness empty/terminal barriers, all-outcome pinned-group then direct-PID signalling before reap, mandatory witness EOF before quiescence, quiesced evidence through report/release, controller-owned summary through terminal commit | deterministic Cartesian state/event, typestate, nonblocking/barrier, containment-witness, and final-commit owner |
| Child lifecycle | exact preallocation/failures, `/dev/null`, spawn/pipe/control/witness acquisition/flag/remap/clock/poll failures, every closed diagnostic-operation mapping, unverified-group direct-PID cleanup, nonblocking stdout/stderr/control/witness drains through empty, short-output idle child, exact/rejected-next bounds, leader retained in/moved out of verified group, untimed/unbounded command in row group, outside-row bounded-command sentinel created before and parenting the external target; exact status codec; normal/abort cleanup through target reap and group absence; nested descendants absent/present/escaped, harness return/exit/exec/abort/SIGKILL, WNOWAIT pin, group-then-direct signalling before reap, terminal control plus witness-EOF barriers, cleanup-error override with evidence preservation, interrupted wait/read | deterministic allocation/I/O-liveness/failpoint, closed diagnostic-operation, nested-process-tree, and witness-closure owners |
| Reporting | exact zero-test and pre-ack infrastructure bytes, all pass one line with/without cache stats, mixed failure, returned Error variant, assertion, exit/signal/timeout/output/order/repetition/malformed reason, four graceful signals, raw bytes/missing LF, capture lifetime through last byte or terminal exit, SIGPIPE-safe full/short/zero/EINTR/error writes, `Idle/Writing/Pending/Selected` permit transitions, partial-prefix-before-selection, terminal evidence/controller retention, pre/post-final-block signal, nonrecursive diagnostic failure, and quiet-wrapper success/failure/interrupt behavior | exact CLI stdout/stderr, consuming-row, sink-failpoint, writer-arbitration, final-exit, and repository `run-quiet` integration owners |
...
The implementation closure matrix is re-opened around the complete row-execution containment and
output-commit product: outer harness group, every safe `process.command` subgroup, witness
ownership, leader/direct/sentinel cleanup, and signal-selection versus raw-write permission. This
extends the preceding artifact-formation-to-execution mode/ABI closure (production/test selection,
entry identity, database reachability, child ABI, option consumers, and diagnostic operations) and
retains the earlier statement-placement, terminal-target, capture/control I/O-liveness, and semantic
side-table closures. The accepted code
...
while jobs/cache statistics and timeout/output limits terminate at build scheduling, diagnostics,
and runner state respectively. Production consumers exercise their full admitted one-shot/watch,
whole/per-unit, ThinLTO, and PGO state product against the prefix-only selector.

Child control crosses one compiler/runtime seam containing exactly five unkeyed functions. The
runtime owns launch receive, fd-3 close-on-exec, install-once fd-4 containment, acknowledgement
encoding/send, and completion encoding/send; the harness owns catalog range/dispatch and
reserved-status choice; the driver owns the independent launch encoder,
acknowledgement/completion decoders, and containment reader. No side may duplicate the other's
codec or infer an ABI from symbol spelling. The ABI rows, codec/witness versions, and reserved exit
mapping enter the harness/runtime cache identity together.

Inside the runner, `LiveRow` owns child, descriptors, stores, protocol state, witness state, and
deadline. Its parent capture, control, and witness endpoints are nonblocking before spawn, so every
drain has a typed `Data | Empty(EAGAIN/EWOULDBLOCK) | Eof | Error` result and Empty returns to poll.
The test-aware process runtime puts untimed/unbounded commands in the outer group and arms an
fd-4-retaining sentinel outside that group before it creates/releases every bounded/timed external
target as a distinct group leader. Harness death makes each sentinel kill/reap its direct target and
prove target-group absence; normal command completion performs the same residual cleanup before
returning. Pinned outer-group then direct-PID signalling,
non-reaping terminal observation, the required second control drain, exact witness EOF, descriptor
closure, and direct-child reap consume `LiveRow` into `QuiescedRow`, which owns only immutable
...
retains the controller through the last summary write, whose raw syscalls are serialized against
signal selection by the lock-free arbitration state, then blocks the four graceful signals,
rechecks that state, and directly exits. It has no ordinary teardown/return edge. This terminal
...
| P1 child completion reporting had bytes but no native ABI | Five exact unkeyed runtime declarations fix launch receive, fd close-on-exec, containment installation, acknowledgement, and completion signatures, validation, ownership, allocation, return/error, descriptor close, reserved-status, registry, and cache rules. |
...
| P1 a bounded `process.command` subgroup could outlive row cleanup | Test mode arms an fd-4-retaining sentinel outside the row group before creating each bounded/timed external target group. On harness death or normal completion the sentinel kills/reaps its direct target, proves group absence, then closes the witness; the runner cannot quiesce until aggregate EOF. |
| P2 signal selection raced report writes | One lock-free atomic serializes `Idle -> Writing` permits against `Selected`; a handler during a permitted syscall records `WritingPending`, and the writer selects it before any later syscall can begin. |
