...
Status: **IMPLEMENTED for align-llm Request 55. Consumer adoption remains external.**
...
| --- | --- | --- |
| Formation and effects | Exact imported name, two arguments, source-order `str` coercion, Impure result, borrowed owned-string operands, and no mode flag or alias on `open_beneath`. | `m9_fs::retained_root_formation_diagnostics_and_per_unit_generic_paths` and `retained_root_open_and_create_round_trip`. |
| Root/relative validation | Output slot, complete lexical validation, grammar, traversal order, and native error mapping are byte-for-byte shared with `open_beneath`. | `align_runtime::fs_beneath_validation_types_and_same_final_matrix` parameterizes both constructors over malformed pairs and `.`, `/`, relative, and absolute roots. |
| Root/intermediate/final type | No symlink following; directory identity revalidation; final regular-file and device/inode revalidation all precede link count. | `align_runtime::fs_beneath_validation_types_and_same_final_matrix` and `fs_beneath_observe_open_races_and_retained_ancestor`. |
| Link-count predicate | The existing opened-final-descriptor `fstat` supplies the link count; after all existing open checks, exactly one is accepted and zero or greater than one maps to `Invalid`. No second syscall or path metadata substitutes. | `align_runtime::fs_beneath_single_link_rejects_every_alias_until_one_name_remains` plus the opened-descriptor barriers in `fs_beneath_observe_open_races_and_retained_ancestor`. |
| Same-inode race | The link predicate and returned reader refer to the same opened inode, even if the public final name is replaced after the check. | The post-predicate barrier and old-inode readback in `align_runtime::fs_beneath_observe_open_races_and_retained_ancestor`. |
| Ownership and cleanup | Root, intermediate, final, and result-construction owners close exactly once; failure publishes neither reader nor bytes. Existing reader move/Drop identity is unchanged. | `align_runtime::fs_beneath_descriptor_cycles_balance`, link-rejection null-output assertions, and the existing reader control-flow/Drop owners. |
| HIR and replay | Every new HIR producer/consumer validates operand types, exact result, and effect; malformed nodes fail closed, while the distinct valid operation remains separate in replay and compiler identity. | `align_mir::validate_hir_tests::hir_body_validator_native`, the compile-time variant sweep, per-unit replay, and MIR identity owner. |
| MIR, producer, LLVM, and ABI | Distinct MIR spelling and runtime key use A12 with exact root-relative-output order and existing reader `Result` CFG. | `m9_fs::retained_root_mir_and_llvm_abi_are_distinct`, runtime ABI golden/registry owners, and runtime source-export parity. |
| Whole/per-unit and cache | Whole-program and per-unit behavior/declarations match; definition edit/substitution changes identity and exact revert restores it. | `m9_fs` whole-program round trip and imported/generic per-unit check, plus existing serialized-HIR edit/revert owners over every enum discriminator. |
