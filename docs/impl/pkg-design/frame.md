...
The implementation activates both rows atomically:

| Runtime row | Exact symbol | ABI shape | Exact Rust ABI |
|---|---|---|---|
...
attribute. The final output pointer is an
...
checked-HIR discriminators, package interface, and owners are active atomically. They move the
keyed/base/maximum-optional-probe inventories from 328/345/353 to 330/347/355 and make A123 the
...
The capability is expected to exceed roughly 1,000 hand-written changed lines. One atomic boundary
is lower risk than splitting it: the public wrappers have no useful behavior until both checked
discriminators, their validators/lowerers, A121/A122, result ownership, and the runtime count/fill
engine agree, while activating a producer or consumer alone would create a dormant or aborting path
and duplicate ABI/cleanup proof in the next PR. The two key kinds share one status, allocation,
ordering, and hash-chain engine, so separating them would duplicate the same failure domain rather
than isolate independently useful capabilities.
