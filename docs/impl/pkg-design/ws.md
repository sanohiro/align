...
| `http_headers.contains_token_exact(name: str, token: str) -> bool` | The same receiver/name/token evaluation and detectable native validation order as `contains_token`. | Pure and allocation-free. Header names remain ASCII-case-insensitive, but every trimmed comma member is compared to `token` byte-exactly across all repeated rows. It likewise does not certify neighboring members. | Copy result; borrows only for the call. | `std.http`; runtime key `HttpHeadersContainsTokenExact`, existing A120. | Repeated-row/member/OWS, exact-match/case-negative, malformed-neighbor, whole/per-unit, native hard-abort, and ABI owners. |
...
| `pkg.web.types.UpgradeHandler { values_valid: bool, prepare: fn(Ctx, slice<str>) -> UpgradeDecision, pump: fn(Ctx, http_upgrade, string) -> Result<(), Error> }` | Exact field/source order. `values_valid` is the protocol package's already-computed validation result; the two callbacks are noncapturing/copyable function values. | Copy. During `serve`, the stored boolean is read once per Upgrade row after common method/pattern/prefix and handler-storage checks but before segment and pair checks. False aborts before bind with exact diagnosis `pkg.web: route N (METHOD PATTERN) has invalid upgrade values`. No arbitrary callback runs during startup validation. `prepare` runs once after middleware and before an Upgrade write. `pump` runs once only after a successful 101, owning the transport and selected string. Pump `Ok` emits no log; pump `Err` uses the same exact method/path/error diagnostic as Stream and cannot send another HTTP response. | One boolean and two 16-byte function values; no record allocation/Drop. The pump owns and must consume or Drop the handle before return. | `pkg.web.types`; field/function signatures and the complete nominal graph enter interface identity. | stored true/false, exact startup precedence/diagnostic, absence of a startup callback, direct/imported prepare/pump values, pump Ok/Err logging, whole/per-unit dispatch, and no-capturing-environment owners. |
| `pkg.web.upgrade(method: str, pattern: str, values: slice<str>, values_valid: bool, prepare: fn(Ctx, slice<str>) -> UpgradeDecision, pump: fn(Ctx, http_upgrade, string) -> Result<(), Error>) -> Route` | Arguments once left-to-right. No defaults. The protocol package computes `values_valid` before this call; any effects in that argument expression occur visibly at the call site and are never retained or replayed. | Pure constructor. Produces one ordinary route with `Handler.Upgrade`, empty `stream_type`, and the exact values, boolean, and callbacks. Ordinary route validation reads the boolean before bind/tree construction. It participates in the same radix priority, 405 `Allow`, prefix grouping, and middleware chain. HEAD fallback applies only to `Respond`, never Stream/Upgrade. | Returned Route is Copy and borrows method/pattern/values for the `serve` lifetime. No allocation. It stores no validator function. | `pkg.web` source and `pkg.web.types.Route`, whose exact new trailing field is `upgrade_values: slice<str>`. Router cache/source identity includes it. | left-to-right/evaluate-once validation-result construction, stored true/false startup order, no callback dispatch, mixed Respond/Stream/Upgrade routing, group/group_with copying, 404/405/HEAD/middleware, and zero hot-path allocation owners. |
...
| `pkg.ws.route(pattern: str, protocols: slice<str>, pump: fn(pkg.web.types.Ctx, http_upgrade, string) -> Result<(), Error>) -> pkg.web.types.Route` | Arguments once left-to-right. No defaults. Construction evaluates the package-owned Pure `protocols_valid(protocols)` exactly once, retains the opaque protocol list, and passes only that Copy boolean to `pkg.web.upgrade`; no validator function is stored. The check requires every server protocol to be a nonempty RFC token and rejects duplicate byte-exact names. Empty list means select none and ignore valid client offers. A nonempty list requires a client offer match; the first server-list entry offered by the client wins. Pattern validation remains `pkg.web`'s startup owner. | Pure and allocation-free. Returns a GET Upgrade route using the computed validation result and handshake prepare function. Invalid protocol configuration is diagnosed only when `pkg.web.serve` reads the stored false value, before bind or tree construction. Middleware runs first and may answer before handshake. The selected protocol reaches the pump as an owned string; `""` means none. | Route retains pattern/protocol views for `serve`; no constructor allocation or retained validation callback. The handshake clones only a nonempty selected protocol. | Ordinary `pkg.ws` source over `pkg.web.upgrade`; SHA-1 helper stays private and absent from the public interface. Root/internal source hashes and `pkg.web` dependency identity drive caches. | valid/invalid/duplicate protocol tables, constructor purity and evaluate-once result, exact bind-before validation/diagnosis, no startup callback, empty/required/first-server selection, mixed web routing/middleware, whole/per-unit, and vendorable subtree owners. |
...
| `HttpHeadersContainsTokenExact` | `align_rt_http_headers_contains_token_exact` | A120 | `unsafe extern "C" fn(*mut HttpRequestCtx, *const u8, i64, *const u8, i64) -> i32` |
| `HttpCtxUpgradeReady` | `align_rt_http_ctx_upgrade_ready` | A03 | `unsafe extern "C" fn(*mut HttpRequestCtx) -> i32` |

No A124 shape is consumed. The implementation activation grows the keyed inventory by eleven as one
...
|---|---|---|
| Formation and type placement | Type name; exact 71/47 codec leaves; raw same-frame local/parameters; unnested same-frame Result Ok carrier from constructor/`map_err`; function-value pump signature; reject every other collection/aggregate/tag/capture/task/parallel/out/extern/global/return path and malformed future variant. Method operands are checked before an exclusive-receiver diagnostic, matching sibling handle methods. | Exact bidirectional root/scalar/Result bytes and unknown/truncated/trailing rejection; parameterized sema + checked-HIR positive/negative placement sweep, variant tripwire, whole/per-unit pump compilation; shared-receiver plus invalid-operand diagnostic twin. |
...
| Web dispatch | Respond/Stream/Upgrade x static/param/wildcard x method/HEAD/405 x group/group_with x middleware Proceed/Respond/Failed. Protocol-owned validation-result evaluation once, stored true/false startup selection, and no retained validator callback; false follows common method/pattern/prefix/storage validation and precedes segment/pair validation; prepare Accept/Reject/Failed; pre-write validation failure versus committed-write failure; pump Ok/Err and exact logging/fallback behavior. | Dedicated `apps_web_upgrade` raw-HTTP/socket owner over static/param/wildcard, group/group_with, all middleware and prepare decisions, successful Upgrade, pre-write fallback, committed-write no-second-response, and pump logging; stored-false multi-invalid twins against one common, one segment, and one pair defect; direct/imported whole/per-unit construction and callback-absence assertions; existing web suites remain unchanged. |
| RFC handshake | Every seven-step validation row, duplicate Host/version/key/protocol headers, token case/grammar, malformed offer list, canonical key, version rejection header, empty/duplicate/multiple server protocol lists, first-server-order selection, extensions ignored, residual bytes, and SHA-1/base64 golden. | Independent raw HTTP oracle plus RFC 6455 accept vector; startup config twins and browser client interop. |
...
| Message/control | Text/Binary/fragmented UTF-8, Ping automatic Pong, Pong consume/ignore, Close empty/one-byte/code/reason including 123/124 reason edges and 1010 empty acknowledgment, outgoing send, timed complete close handshake, every allowed/forbidden code. Protocol-error Close, automatic Pong, and peer-Close reply failures are independently forced. | Raw peer wire captures and typed result assertions, including reset-on-write subprocess/socket twins for each automatic reply family. |
| Outbound close | Validate every server-sendable/forbidden code, 123/124-byte reasons, and timeout 0/1/max/max+1 before handle transfer; emit exact unmasked big-endian Close; retain one cumulative deadline; while closing, Pong/data are discarded, Ping is echoed, valid peer Close completes, and protocol/invalid Close/EOF/timeout/partial write or read returns the winning error with source nulling and close exactly once. | Dedicated `apps_ws_close` owner: source-level invalid-input abort twins, raw-socket wire/control/state matrix, timeout/EOF/reset twins, and existing MIR/runtime move/drop/fd/failpoint owners. |
| Bounds and precedence | negative/zero/exact/next message cap, exact/rejected-next fixed source-work allowance including zero-length continuation and nonempty Ping/Pong floods, every length arithmetic edge, malformed-plus-oversized, cap-plus-EOF, protocol/Ping/peer-Close reply and shutdown failure, caller-invalid x live/spent/poisoned transport operations, no overread. | Pairwise multi-invalid matrix; reduced-source-work continuation owner plus a separate nonempty-control owner whose exact/rejected-next outcomes fail if payload charging is removed; source-read/work/state/syscall/clock/buffer counters and exact boundary twins. |
| Allocation and cleanup | Exact `H`-byte Upgrade response head coexisting with builder storage and the preallocated handle shell; header stack storage, fixed scratch, partial-message storage across control policy, result ownership, no send payload copy, OOM abort, every unpublished failure, fd close once; exact `1073774720` producer-requested receive live-heap ceiling including simultaneous old/new growth and Text staging/result payloads plus the 128-byte shell budget. | Allocation/copy/fd counters and failpoints after each allocation/read/write/publication boundary; checked-`H`/head-allocation high-water owner; compile-time receive-shell assertions; maximum formula derived from the shipped `MAX_MESSAGE_BYTES` and `SCRATCH_BYTES`; live-byte probes for steady Binary/Text transfer and a scaled fragmented Binary growth path that observes old+new realloc overlap. |
| ABI and lowering | Eleven exact keys/symbols/shapes and the reused shapes' exact empty curated function-attribute sets; pointer/null/alignment/length/capacity/output validation; header/readiness hard-abort policy; HIR/MIR operation records; LLVM calls; rt-LTO on/off. The platform accept prerequisite checks `SO_NOSIGPIPE`, closes on failure, and publishes no ctx. | Registry/export/compatibility goldens, direct runtime/subprocess matrix, socket-option failpoint, malformed HIR/MIR, optimized/unoptimized and whole/per-unit parity. |
...
| P1/P2 route validation purity | Compute the protocol-owned validation result once inside the Pure protocol constructor, store only that boolean in the generic Upgrade record, and let existing pre-bind route validation select it with the exact diagnostic. No effect-unqualified validator callback crosses the package/interface boundary or executes during startup validation. |
...
| P1 owner operation matrix | Split shared framework dispatch into a dedicated `apps_web_upgrade` owner and keep WebSocket frame/resource evidence in `apps_ws`; close group/middleware/prepare/fallback/pump branches, nonempty control charging, Close edges, and protocol/Ping/peer-Close reply failures with mutation-discriminating raw sockets. |
| P2 maximum resource evidence | Derive the exact ceiling from the shipped message/scratch constants and add a scaled fragmented Binary path whose measured peak includes simultaneous non-null old/new builder buffers. |
| P2 activation prose drift | Synchronize active type/HIR/MIR/backend status, eleven keys, exact web Handler/Route records, English/Japanese package prose, and adjacent source comments in one pass. |
| P1 outbound-close boundary | Split the public `pkg.ws.close` contract into dedicated `apps_ws_close` ownership: outbound validation/wire, cumulative deadline, closing-state control/data policy, transport/protocol/EOF precedence, source nulling, and close-once cleanup. |
| P2 validation and handshake products | Cross stored false with invalid common/segment/pair rows in exact phase order; cover empty/duplicate/multiple server protocols and duplicate/malformed/residual client handshake rows. |
...
| P1 outbound-close evidence strength | Make the close owner reject negative and above-u16 codes, distinguish one cumulative 600 ms budget from a per-frame reset with a shorter client read bound, and fault-inject after the frame head/code write. After receiving the synchronization byte, the twin gives the abortive reset time to become observable, while synchronization-read failure and successful reason write map to two distinct sentinel errors, so only the reason write can produce the asserted transport error. |
...
| P2 fail-safe owner reaping | Give every affected test socket a shared deadline for writes as well as reads, and transfer every spawned child to a kill-and-wait reaper before a bounded owner returns or unwinds. The reaper retains the child and retries every wait error until reap even after the caller deadline expires; polling errors cannot discard process ownership. |
| Gate ratchets | Read compiled Align package sources through runtime fixtures, diagnose missing LLVM call values instead of panicking, keep malformed `http_upgrade` sizing total, and use address/checked integer conversions at the native boundary. Lock the improved runtime cast count into the ratchet baseline. |
