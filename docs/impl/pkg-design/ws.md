...
| Web dispatch | Respond/Stream/Upgrade x static/param/wildcard x method/HEAD/405 x group/group_with x middleware Proceed/Respond/Failed. Protocol-owned validation-result evaluation once, stored true/false startup selection, and no retained validator callback; prepare Accept/Reject/Failed; pre-write validation failure versus committed-write failure; pump Ok/Err and exact logging/fallback behavior. | Dedicated `apps_web_upgrade` raw-HTTP/socket owner over static/param/wildcard, group/group_with, all middleware and prepare decisions, successful Upgrade, pre-write fallback, committed-write no-second-response, and pump logging; direct/imported whole/per-unit construction and callback-absence assertions; existing web suites remain unchanged. |
