...
| P1 outbound-close evidence strength | Make the close owner reject negative and above-u16 codes, distinguish one cumulative 600 ms budget from a per-frame reset with a shorter client read bound, and fault-inject after the frame head/code write while mapping synchronization-read failure and successful reason write to two distinct sentinel errors, so only the reason write can produce the asserted transport error. |
