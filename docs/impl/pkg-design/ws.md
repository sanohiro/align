...
| Web dispatch | Respond/Stream/Upgrade x static/param/wildcard x method/HEAD/405 x group/group_with x middleware Proceed/Respond/Failed. Protocol-owned validation-result evaluation once, stored true/false startup selection, and no retained validator callback; prepare Accept/Reject/Failed; upgrade validation/write failure; pump Ok/Err. | `apps_web_upgrade` owner with exact startup-order/diagnostic, route-table, callback-absence, and socket assertions; direct/imported whole/per-unit construction; existing web suites remain unchanged. |
