...
excluded collection, box, tuple, sum-payload, materializer, builder, and decode edge, and both
directions of the Move-slice edge: a formation negative for retaining sums, and the admitted
borrowed view for retaining structs together with the reads, escapes and source moves it forbids.
