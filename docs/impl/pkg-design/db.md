...
target open. Both change no application-callable package surface or ABI and must merge in that
order. The third PR adds direct delivery, the rows ABI, and the streamed protocol below. The
fourth adds `rows_stmt_native` plus exact prepared-state parameter-name authority and reuses that
...
| prerequisite result-status safety closure | Before changing the stream ABI or libpq mode, route every shipped package-owned PGresult consumer through one exhaustive sealed `apps/db/pkg/db/internal/postgres_status.align` authority shared by `postgres.align`, internal execution, and resource cleanup without an import cycle. Before clear, copy the exact error selected by that consumer's existing non-success mapping; a previously owned error remains primary and silent cleanup remains silent. COPY, `PGRES_PIPELINE_SYNC`, `PGRES_PIPELINE_ABORTED`, or an unknown numeric status then clears the current result exactly once, immediately poisons/closes, and permits no later result/COPY/pipeline-exit/cancel/transaction-state/blocking-restore call. Clean all owners and the lease after close and never permit reuse; do not invent a new public error category, item, message, or query identity. Audit the Rust prepare/migration PGresult consumers in the same pass. Migration alone sends complete user SQL through `PQexec`, so extend the canonical PostgreSQL statement screening from §17.6 to reject first token COPY with its exact diagnostic before URL access or any native work; preparation cannot execute COPY through `PQprepare`, and all remaining tool SQL is fixed. | independently mergeable prerequisite PR; exact package/tool consumer/error inventory; sealed-authority exported-surface negative; COPY/pipeline/unknown x synchronous/timeout-completion/recovery/Drop no-call/clear-close/error-identity matrix; `postgres_migration_copy_is_rejected_before_native_work`; retained Q2/Q4a/Q4b/Q5a/Q5b2 owners |
...
stream-only error-drain state machine and validation-order correction remain inseparable from direct
...
| PostgreSQL result-status safety prerequisite | Audit every shipped package-owned PGresult consumer, including synchronous and timeout direct/prepared rows, one, command, prepare, transaction boundary, catalog, EXPLAIN, recovery drain, and silent cleanup. Before clear, retain the consumer's exact existing non-success error; an earlier error stays primary and silent cleanup stays silent. COPY, `PGRES_PIPELINE_SYNC`, `PGRES_PIPELINE_ABORTED`, or an unknown numeric status then clears the current result exactly once, immediately poisons/closes, performs no later result/COPY/pipeline-exit/cancel/transaction-state/blocking-restore call, cleans every owner, and never permits reuse. Audit both Rust tool PGresult modules; reject top-level PostgreSQL migration COPY through canonical statement screening before URL/native work and record why preparation/fixed tool SQL cannot enter that subprotocol. Add no `pkg.db` public symbol or error surface, ABI field, delivery option, libpq-17 dependency, or rows-state change; §17.6 owns the exact new migration diagnostic. | parameterized every-package-consumer COPY/pipeline/unknown injection and no-call/clear-close/error-identity owners; `postgres_migration_copy_is_rejected_before_native_work`; prepare/migration SQL-origin inventory; retained Q2/Q4a/Q4b/Q5a/Q5b2 suites; local database gate; normal code preflight/review/CI |
