...
The three-times-reopened implementation boundary remains three independently mergeable vertical
PRs. The new error-drain invariant cannot form a fourth mergeable prerequisite: publishing any
explicit direct stream without its validation/decode/storage cleanup would be incorrect, while the
helper has no independently callable surface or consumer before that stream exists.
...
Run one fresh independent adversarial review of this three-times-redesigned ledger and the unchanged
three PR boundaries
