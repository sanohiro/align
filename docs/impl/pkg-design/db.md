...
| namespaced builtin type identity | Generalize type lookup rather than renaming `db.Error`. Q2 shipped `Error`/`core.Error` (always-in-scope syntactic core), `argon2_params`/`crypto.argon2_params` (`import std.crypto`), and `regex_match`/`regex.regex_match` (`import std.regex`). The implemented asymmetric suite adds six bare/`crypto.*` key pairs to the same table; bare forms need no import and qualified forms require `std.crypto`. A non-entry same-module declaration wins bare lookup; a local miss falls back to the alias; exact explicit spelling retains builtin identity; std-qualified type use satisfies the unused-import lint; `error(c)` always targets `core.Error.Code(c)`; entry canonical collisions reject. No package name appears in the rule. | shipped parameterized sweep for all nine aliases; qualified importer construction/match; exact explicit builtin use and missing-import negatives; unchanged bare builtin fallback; local-Error `error(c)`; entry collision rejection; whole/per-unit interface and executable parity |
