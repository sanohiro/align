...
The initial D1--D12 PostgreSQL sums are fixed:
...
Post-release D13 adds this one execution variant without changing the initial-release gate:
...
- applies each whole screened file as a migration script in that order; the one-statement Query
  rule does not apply to migration scripts;
...
transaction-control statements; the runner owns the boundary. PostgreSQL migration files also may
not contain a top-level statement whose first token is `COPY`: the migration executor owns no COPY
data/termination protocol and must not enter libpq COPY mode. The driver-authoritative script
preparation/screening rejects both classes before opening the target or mutating the database.
...
4. classify every first-line policy, screen every complete statement, reject an empty file, the
   first transaction-control statement, the first PostgreSQL top-level `COPY` statement, or a
   Forbidden file whose database-authoritative count is not one;
...
Within one file, policy classification precedes lexical completeness and empty-script validation.
One classification pass then visits each complete statement exactly once in source order, checks
transaction control before PostgreSQL `COPY` within that statement, and stops at the first
prohibited statement. Forbidden-count validation runs only after that pass.
Phase 5 uses §16.2's ambient
...
CHARACTERISTICS AS TRANSACTION`. PostgreSQL additionally rejects first token `COPY` with exact
diagnostic `migration \`<filename>\` contains a PostgreSQL COPY statement`. Within one numeric file,
one statement-ordered classification pass selects the first prohibited statement in source order;
transaction-control classification precedes COPY classification for that statement, then the
Forbidden count check runs. SQLite statement
...
| complete row validation and decode | Before decoder invocation, validate exact column count, declared UTF-8 name bytes in order, exact driver-native type metadata, NULL disposition, and the value representation for every first-release type. Decode by ordinal through generated typed callbacks without reflection, boxing, maps, artifact/source I/O, or post-validation fallible conversion. Multi-invalid rows report the first declared ordinal; each native malformed pointer/length/UTF-8 value fails before a safe view exists. D13's binary-format ledger supersedes this ordering with one result-generation metadata phase over count and every declared name/OID/format before zero-row/cardinality handling, followed by one value phase over each delivered row's NULL disposition and representation. | both complete driver matrix owners, `pkg_db_q4b::malformed_native_view_values_fail_before_safe_view_formation`, cumulative Q2 name/type/count/null twins, generated MIR inspection, callback version/context misuse negatives, and the D13 metadata-before-value winner owner |
...
| `one` cardinality compatibility | Common and driver-qualified `one` reuse stream formation. Validate and decode only row zero and clone it into `out`; a malformed first row beats multiplicity. D13 supersedes the older second-row winner: the driver-private probe validates each newly acquired result generation's metadata before determining whether it contains a second row. Malformed second-generation count/name/OID/format therefore returns Decode before Cardinality; valid metadata plus any second row returns Cardinality without running Value mode or decoding that row. Exhaustion and multiplicity both close through the same stream cleanup path. | `pkg_db_q2::sqlite_native_command_and_one_execute_generated_i64_thunks`, `postgres_native_command_and_one_own_buffered_results`, both common-surface dispatch owners, and the D13 second-generation malformed-metadata versus valid-metadata Cardinality twin |
...
| catalog and policy screening | Reuse the Q3 `ALIGNMIG` catalog bytes/digest. Parse the exact first physical line, count complete driver statements, and reject empty, transaction-control, NUL, invalid UTF-8, and multi-statement Forbidden files before target mutation. After lexical completeness, one source-ordered classification pass rejects the first transaction-control or PostgreSQL top-level `COPY` statement before target open; transaction control precedes COPY within one statement, COPY uses the exact §17.6 diagnostic, and quoted/comment/dollar-body COPY text remains data. | cumulative Q3 catalog goldens plus `pkg_db_q5a::migration_policy_and_statement_screening_is_exact` and `postgres_migration_copy_is_rejected_before_native_work`, including both prohibited statement orders |
...
| A1 status-closure review found PostgreSQL migration user SQL could enter COPY mode and then issue rollback on the same connection | PostgreSQL migration screening rejects every top-level first-token `COPY` in statement order before URL access, target open, lock, history publication, or libpq. The exact diagnostic and lexical near-misses are Q5a owners retained by the A1 status prerequisite. |
...
The implementation also has two independently correct prerequisites. The first PR closes the
pre-existing PostgreSQL libpq-consumer lease gap in the Q5b2 matrix: every catalog call and
common/native EXPLAIN acquires the same connection lease as typed execution, holds it through
result/context cleanup, and rejects overlap without calling libpq. The second PR routes every
shipped package-owned PGresult consumer through one sealed status authority and fails closed on
COPY, pipeline, or unknown numeric statuses. The same prerequisite closes the only shipped
compiler-side user-SQL gap by rejecting every top-level PostgreSQL migration `COPY` statement before
target open. It also routes every Rust prepare/migration PGresult consumer through one private
exhaustive tool-status authority so a null or deferred result closes and nulls the connection before
rollback, deallocation, row access, or any later libpq call. Both change no application-callable
package surface or ABI and must merge in that order. The third PR adds direct delivery, the rows
ABI, and the streamed protocol below. The fourth adds `rows_stmt_native` plus exact prepared-state
parameter-name authority and reuses that
protocol without forking it. The prerequisite splits close independently useful shipped safety
gaps without hiding either repair inside the streamed state-machine PR.
...
| operation and result | With `Delivery` absent, direct and prepared paths delegate to the shipped caller-synchronous BufferedFull executor, complete the libpq protocol, restore blocking mode, and surface any server failure before publishing the complete buffered rows. Without `TimeoutNs` this is the existing one `PQexecParams`/`PQexecPrepared` call with zero nonblocking/send/mode-selector calls. With `TimeoutNs` it is the existing `PQsetnonblocking` + `PQsendQueryParams`/`PQsendQueryPrepared` + wait/cancel/drain path, still with zero row-mode-selector calls and no rows publication before completion. With explicit delivery, validated direct execution calls `PQsendQueryParams`; prepared parity calls `PQsendQueryPrepared`. The selector occurs immediately after a successful send and before flush, input consumption, or result retrieval, after which the constructor returns live rows without waiting for the first row. `one_native` validates/decodes only the first Row and performs exactly one ordinary `row.clone_in(out)` into the caller-supplied region before probing multiplicity. It notes a second mode-valid row as pending Cardinality 2 without validating or decoding it, then continues draining normally under the original absolute deadline. Clean completion publishes Cardinality 0/2 or the one Row as appropriate; a late native/sequence/timeout/cleanup failure wins and no Row is returned. The caller region is monotonic: once the first clone occurred, its exact successful-singleton clone bytes remain allocated but unreachable until the caller's arena scope ends on Cardinality 2 or any later failure; the package never rewinds, reuses, or drops those `RegionPlain` bytes. Zero rows and a first-row validation/decode failure allocate zero bytes in `out`. Cardinality never cancels the statement. |
| physical delivery and order | `SingleRow` accepts only `PGRES_SINGLE_TUPLE` data results containing exactly one row. `PortalBatch(n)` accepts only `PGRES_TUPLES_CHUNK` data results containing `1..=n` rows. Both preserve server row order. After zero or more data results, require one zero-row `PGRES_TUPLES_OK`, clear it, then call `PQgetResult` until null before declaring exhaustion, exactly as [libpq 17 §32.6](https://www.postgresql.org/docs/17/libpq-single-row-mode.html) specifies. For each later non-null result, protocol-state validation precedes status-to-error mapping. Once the zero-row terminal has been observed, any further result records the exact `postgres.rows.delivery` invalid-sequence error below before its status is classified for cleanup; therefore a post-terminal `PGRES_BAD_RESPONSE`, `PGRES_NONFATAL_ERROR`, or `PGRES_FATAL_ERROR` does not replace that error with `execution_error`. Before terminal, those three statuses use the existing `execution_error` SQLSTATE/native-detail mapping. `PGRES_EMPTY_QUERY`, `PGRES_COMMAND_OK`, the other mode's data status, an empty/oversized data result, or a nonempty/missing terminal result also records the invalid-sequence error, clears that result once, and continues the ordinary no-decode drain. `PGRES_COPY_OUT`, `PGRES_COPY_IN`, `PGRES_COPY_BOTH`, `PGRES_PIPELINE_SYNC`, `PGRES_PIPELINE_ABORTED`, and every numeric status outside the complete known libpq 17 set are instead fail-closed because their protocol or connection-global state cannot be assumed drainable and reusable without the deferred COPY or pipeline APIs: preserve an earlier owned error, otherwise use the already-recorded post-terminal invalid-sequence error or record it now, clear the current result exactly once, and immediately poison/close the physical connection. After observing one of those statuses, never call `PQgetResult`, `PQgetCopyData`, `PQputCopyData`, `PQputCopyEnd`, `PQexitPipelineMode`, a cancel API, `PQtransactionStatus`, or blocking-mode restoration on that connection. No cancel handle may remain owned when main-result classification begins. Clear the wrapper's mutable native state, destroy any unpublished package-owned batch, make an unpublished cloned Row unreachable without rewinding `out`, free the binder context and any unpublished direct Query-ID copy, and release the lease only after close. Earlier published values remain visible; an earlier validation/decode/storage/mode/timeout error remains primary, and Drop remains silent. No COPY, unknown, command, pipeline, or multi-statement status is reinterpreted as rows. |
| `next` and view lifetime | `next` returns at most one validated Row and never fetches a new PGresult while the current result has an unread row. A mutating advance invalidates the previous rows generation before clearing that result; therefore no returned `str`, `slice<u8>`, or enclosing view remains usable after the next advance. Reaching the end of one data result clears it exactly once before waiting for the next. Zero-row completion returns `Ok(None)` only after the terminal result and null protocol marker are consumed and the connection is synchronized. A row validation/decode error becomes the first owned error, publishes no Row, and enters the error-drain rule below before it returns. |
| `next_batch` and atomicity | `next_batch(rows, max_rows)` preserves the common A1 validation and materialization contract. It may consume part of one PGresult or span several PGresults, but stops exactly at the caller's `max_rows` without probing the next native row. Every view-bearing field is copied into the unpublished batch before its source result is cleared. A native/decode/storage error before the caller bound destroys that whole unpublished batch, retains that first owned error, and enters the error-drain rule below before returning it. If the caller bound is reached before a later server error becomes observable, the completed batch is published and that later error is returned by a subsequent advance. When clean EOF is observed before the bound, result drain and blocking-mode restoration complete before a nonempty partial batch is published. If that cleanup fails with no earlier error, destroy the unpublished batch, poison/close, and return `Error.Unsupported(ContractError { query_id: Some(query_id), item: "db.rows.cleanup", message: "PostgreSQL streamed rows cleanup failed and the connection was closed" })`; zero-row EOF returns the same error instead of `Ok(None)`. Earlier published rows/batches are never retracted or implicitly rolled back. |
...
| parameter ownership and allocation | The generated binder still performs one execution-owned Text copy per parameter according to §5.6.1. The context retains all parameter pointer/length/format arrays and every copied payload from send through terminal `PQgetResult == null`, fail-closed connection close, mode-selection failure cleanup, timeout cleanup, or early Drop. Original text/blob source owners may be moved, mutated, or dropped after the rows constructor returns. Direct execution owns one copied Query ID: a constructor failure frees it if already allocated, while a published terminal rows wrapper retains and later frees it on rows Drop. Prepared execution always borrows the statement-owned Query ID; no prepared success or failure frees it. The delivery rail allocates no per-row container and no hidden whole-result buffer. Its steady live set is at most the binder context, current PGresult, libpq transport storage, direct Query-ID copy, and an explicit common batch; cancellation additionally owns one temporary `PGcancelConn`. `one_native` adds only the public `clone_in(out)` allocation described above: zero clones for zero/invalid-first-row, exactly one clone for a valid first row, and no later package allocation proportional to the remaining result. Success and post-clone failure/Cardinality have the same `out` allocation count and byte/alignment pattern for that first Row. |
| validation and error precedence | Direct Query execution uses the settled §13.4 phases: validate the complete descriptor header and plan-shape agreement before reading Query identity; validate common options in source order, native options in source order, and the driver restriction; validate the exact live exec/connection state; allocate one context; invoke the generated static-option/field-shape validator; acquire the execution lease; bind values; check deadline representability; execute; and select the explicit mode. A static-validation failure frees the context without acquiring the lease or calling libpq; an overlap failure also frees that context. Prepared execution separately validates the complete statement v3 header, Query identity, batch plan, parameter resolver, common options, native options, driver/live state, and then acquires the lease before context allocation, bind, deadline, send, and selection. Its native-option validator calls only the compiler-private `pkg.db.internal.descriptor.prepared_parameter_ordinal(statement_ref, name) -> i32` bridge; the bridge accepts exactly a concrete `resource_ref<db.stmt<P,R>>` plus `str`, loads offset 80 only under a synthesized `pkg.db.internal.resource.stmt_header_valid(wrapper)` control-dependent guard, invokes the retained `fn(name: str) -> i32`, and returns zero for an unknown name. Application source, the wrong resource/type, an unguarded load/call, or malformed HIR is rejected. A nonpositive result is the existing unknown-name error before lease or allocation. At each `Delivery` source position, command rejection runs first because a command cannot use any Delivery payload. For a Query, validate the current variant payload before duplicate detection and registration, exactly as §13.4 requires; `SingleRow` has no payload and every `PortalBatch` validates its size even when its tag is a duplicate. A duplicate between valid values returns query-specific `Unsupported`, item `postgres.execute.delivery`, message `duplicate PostgreSQL row delivery option`; an invalid PortalBatch size uses the same item and message `PostgreSQL portal batch size must be between 1 and 2147483647 rows`. Thus both `[SingleRow, PortalBatch(0)]` and `[PortalBatch(0), SingleRow]` report size, while `[SingleRow, PortalBatch(1)]` reports duplicate. `execute_native` rejects its first `Delivery`, including `PortalBatch(0)`, with the same item and message `PostgreSQL row delivery requires a Query result`; Query-specific payload validation does not run for a command. All occur before lease, execution-context allocation, bind, or send. Existing earlier Text-format and common-option errors retain their exact category and precedence. |
| post-send invariant failures | Send failure retains the existing connection error. If libpq rejects the immediately selected mode, cancel/drain using the cleanup rule below and return query-specific `InvalidQuery`, item `postgres.rows.delivery`, message `PostgreSQL client rejected the selected row delivery mode`. An invalid result sequence uses the same category/item and message `PostgreSQL streamed result sequence is invalid`. A COPY, pipeline, or unknown numeric status uses that error only if no earlier primary exists and always takes the immediate close branch above; an observing connection is never reusable. Every ordinary known-status connection is reusable only if drain, transaction-state synchronization, and blocking-mode restoration all succeed; otherwise it is poisoned/closed before return. |
| deadline, cancellation, error drain, and Drop | One requested common `TimeoutNs` becomes one overflow-checked monotonic absolute deadline before explicit-mode send. After enabling nonblocking mode and immediately before send, read the monotonic clock again. If the deadline has expired, send and mode selection remain at zero; restore blocking mode and return the existing `Timeout`, or poison/close if restoration fails. This pre-send expiry path creates no cancel handle. Both the deadline and original positive duration otherwise remain attached until clean protocol completion. Caller think time, `one_native` pending-cardinality drain, and validation/decode/storage error drain all count. Later expiry returns the existing `Timeout` when no earlier error exists, after nonblocking native cancel and drain. A non-timeout server cancellation remains `Cancelled`. After a validation, decode, or batch-storage error with protocol results pending, destroy any unpublished package-owned value, make any cloned `one_native` Row unreachable under the monotonic-`out` rule, retain the first error as primary, and drain without decoding under the original absolute deadline. With no deadline or time remaining, continue normal drain. Clean completion commits a connection-target DML `RETURNING` and leaves a transaction target `PQTRANS_INTRANS` with effects retained. A later server fatal remains secondary to the first row/storage error but rolls back the connection-target statement at `PQTRANS_IDLE` or leaves the explicit transaction `PQTRANS_INERROR` with rollback required. If the original deadline expires during drain, issue cancellation with D9's `max(original_duration, 1_000_000 ns)` recovery budget but still return the earlier validation/decode/storage error. The cancel race may leave connection-target effects committed or rolled back at `PQTRANS_IDLE`; a transaction target is `PQTRANS_INTRANS` with effects retained if completion won, or `PQTRANS_INERROR` with rollback required if cancellation won. Early rows Drop and mode-selection failure instead issue cancellation immediately with exactly `1_000_000_000` ns of monotonic recovery; cardinality never does. The cleanup helper owns a non-null `PGcancelConn` from `PQcancelCreate` through `PQcancelStart`/`PQcancelPoll`. Before the first poll it reacquires `PQcancelSocket` and waits writable with `PQsocketPoll`; after each `PGRES_POLLING_READING`/`WRITING` it reacquires the possibly changed socket and waits for exactly that readiness under the remaining monotonic recovery budget before polling again. Socket lookup, readiness wait, or an undocumented polling tag fails closed. The helper reads or copies any `PQcancelErrorMessage` bytes before invoking `PQcancelFinish` exactly once on every start/poll success or failure, and no cancel-owned view escapes. It then drains and clears every ordinary main-connection PGresult to null, checks transaction synchronization, restores blocking mode or poisons/closes, frees the binder context and any unpublished direct Query-ID copy, clears the rows mutable native state when a wrapper exists, and only then releases the lease. At every result classification in normal completion, first-error drain, deadline recovery, mode-failure recovery, or early Drop, a COPY, pipeline, or unknown numeric status overrides that remaining sequence with the exact immediate-close rule above: clear only the current result, perform no further drain/pipeline-exit/cancel/probe/restore, preserve an earlier error, and close before owner cleanup and lease release. A connection target is reusable only after ordinary known-status drain reports `PQTRANS_IDLE`. An explicit transaction is reusable after ordinary known-status drain at `PQTRANS_INTRANS` when the statement completed before cancellation took effect, with its SQL effects still in that transaction; `PQTRANS_INERROR` means cancellation or a server error aborted the transaction and only caller-visible rollback/Drop or a server-rejected further operation may follow. The package performs no hidden rollback in any outcome. Timeout, mode-selection failure, or early Drop retains its primary observable result regardless of which synchronized transaction state wins; whether statement effects occurred remains observable only through subsequent database or transaction state. A published direct terminal wrapper retains its Query ID; a prepared path never frees the statement-owned ID. Failed cancel creation/dispatch, recovery expiry, I/O failure, any other transaction state, or restoration failure poisons/closes before release without overwriting an earlier owned error. With no earlier owned error, failure after an otherwise clean terminal returns the exact PostgreSQL `db.rows.cleanup` error above; timeout/native/sequence/decode/storage wins over any unpublished Row or pending Cardinality. Drop cannot report and only poisons/closes. No cleanup path issues SQL or overwrites the first owned error. |
...
| artifacts, ABI, and cache identity | Runtime delivery selection does not alter Query/command descriptor bytes, batch-plan bytes, checked metadata, static Query artifact semantics, or their semantic fingerprints; there is no per-mode descriptor or cache identity. Landing the public `Delivery` enum changes the PostgreSQL module interface hash, and landing the rows/statement ABI implementation changes the relevant producer implementation hashes. Per-unit importers therefore record new dependency-interface hashes and the affected codegen/object cache entries invalidate once at those implementation boundaries; only modules that neither import nor reach the changed interface may retain their prior cache keys. The direct PR makes the shared SQLite/PostgreSQL rows state one 120-byte, 8-aligned v3 record. Offsets 0--95 retain v2 meaning. Offset 96 is delivery `u8` (`0=PostgreSQL BufferedFull`, `1=PostgreSQL SingleRow`, `2=PostgreSQL PortalBatch`, `3=SQLite Step`); 97 is protocol-pending `u8` (`0/1`); 98--99 are zero `u16`; 100 is `i32 portal_max_rows` (zero except positive for PortalBatch); 104 is `i64 deadline_ns` (`-1` absent, otherwise nonnegative); and 112 is `i64 timeout_duration_ns` (`-1` absent, otherwise positive). Active SQLite direct/prepared rows require driver SQLite, delivery 3, pending 0, a non-null current statement mirrored at binder-context offset 8, portal size 0, and both deadline fields -1. Active PostgreSQL BufferedFull requires driver PostgreSQL, delivery 0, pending 0, a non-null current result mirrored at context offset 8, portal size 0, and both deadline fields -1 because its protocol completed before publication. Active PostgreSQL streamed rows require driver PostgreSQL, delivery 1/2, pending 1, portal size respectively zero/positive, and a null-or-live current result mirrored at context offset 8. Its deadline fields must be either the exact pair `(-1, -1)` or a nonnegative absolute deadline plus the original positive requested duration. No other driver/mode/deadline product is valid. Terminal rows for either driver set delivery/pending/portal/result/context to zero/null and both deadline fields to `-1`, while retaining the existing immutable Query identity and batch plan. The prepared-parity PR separately makes shared statement state an 88-byte, 8-aligned v3 record: offsets 0--79 retain statement v2 meaning and offset 80 is the non-null producer-owned `fn(name: str) -> i32` parameter-ordinal resolver already stored in every Query descriptor. Both SQLite and PostgreSQL preparation retain that static resolver. HIR validation accepts statement formation only when offset 80 receives `parameter_resolver` from the same concrete Query descriptor generation that supplies the retained binder, row validator, stream decoder, and batch plan; a cross-Query resolver splice or raw replacement is rejected. `stmt_header_valid` requires exact version 3, driver 1/2, live state, zero reserved bytes, every existing v2 pointer/layout invariant, a valid batch plan, and the non-null resolver before any accessor or indirect bridge call. Statement Drop marks closed and nulls native/connection/binder/identity/deallocate/row-validator/stream-decoder/batch-plan/resolver fields before free. Every SQLite/PostgreSQL constructor, accessor, `next`, `has_next`, `next_batch`, and Drop path validates its complete record before native use. Exact semantic-to-byte and byte-to-semantic goldens pin every valid direct/prepared active/terminal rows state, both-driver statement state, and malformed version/driver/tag/reserved/size/deadline/duration/pointer combinations. |
...
| acceptance and measurement | Requires the merged libpq-consumer lease and result-status safety prerequisites, shipped D8/D9 row generations and deadline/cancel recovery, A1 common batch, and libpq >=17. The direct PR crosses conn/tx x absent-Delivery BufferedFull/SingleRow/PortalBatch x `rows_native`/`one_native`; the prepared PR crosses a statement prepared from conn/tx x the same three modes x `rows_stmt_native`, with no prepared `one_native` cell. `next` and `next_batch` run on each rows axis. Both cover zero/one/many rows, exact/partial/multi-result boundaries, the official zero-row terminal, every known libpq 17 status plus unknown numeric sentinels, every ordinary error/invalid/deferred/unknown status after that terminal with protocol-state-before-status error precedence and the status-specific drain-or-close action, one-row-then-fatal/two-rows-then-fatal/two-rows-then-clean, DML `RETURNING` effect parity on connection and transaction targets, pre-send/post-row and pending-cardinality timeout, final-partial-batch restore failure, validation/decode/storage failure followed by normal completion or deadline expiry, early Drop plus cancellation races that synchronize a connection at `IDLE` or a transaction at `INTRANS`/`INERROR`, statement/connection reuse, malformed shared rows/statement v3 state including every deadline/duration pair and SQLite siblings, and whole/per-unit linking. The status-safety prerequisite separately injects every COPY status, both pipeline statuses, and an unknown numeric status into every shipped synchronous, timeout-completion, timeout-recovery, direct, prepared, command, rows, one, prepare, transaction-boundary, catalog, EXPLAIN, and silent-cleanup package PGresult consumer; each owner asserts one current-result clear, physical close, zero later result/COPY/pipeline-exit/cancel/transaction-state/blocking-restore calls, correct first-error or silent-Drop behavior, balanced owners/lease, and no reuse. The same prerequisite inventories compiler-side PGresult consumers: migration is the only path that sends complete user SQL through `PQexec`, so PostgreSQL screening rejects a top-level COPY in numeric-file/statement order before URL access, target open, lock, history publication, or any libpq call; `alignc db prepare` uses `PQprepare`/`PQdescribePrepared` and cannot execute COPY, while all other tool SQL is fixed producer-owned text. The migration owner crosses Required/Forbidden, lowercase/comment-leading COPY, COPY after an ordinary statement, quoted/comment/dollar-body COPY text, transaction control before later COPY, COPY before later transaction control, a later Forbidden-count error, and unchanged SQLite screening; the two prohibited-order cases prove the single statement-ordered classification pass and every rejected case proves zero environment reads, target/connection opens, locks, history publication, and libpq calls. Stream owners repeat the package fail-closed matrix for first result, after delivered data, and during `next`, `next_batch`, `one_native`, validation/decode/storage drain, deadline recovery, mode-failure recovery, and early Drop, crossing Timeout absent/present and Conn/Tx. Direct and prepared explicit-mode owners delay the clock across nonblocking enablement and assert expired-before-send has zero send/selector/cancel, exact Timeout, blocking restoration or poison/close, and no reuse after failed restoration. Direct phase-order owners cross every multi-invalid pair and failpoint with context-allocation/free, lease, binder, and libpq counters, proving live state precedes context/static validation and static validation precedes lease. Query Delivery owners enforce payload-before-duplicate with both invalid orderings and a valid duplicate; command owners enforce tag rejection before Query-only payload validation. `one_native` region owners assert zero `out` allocation for zero/invalid-first-row, exactly one first-Row clone for singleton/Cardinality 2/one-or-two-rows-then-fatal/timeout/COPY/pipeline/unknown/cleanup failure, equal byte/alignment deltas across those post-clone outcomes, and no rewind or second clone. Error-drain owners assert the first validation/decode/storage error, no further decode, clean-completion effects for Conn/Tx, deadline-expiry cancel effects for Conn/Tx, the retained original-duration recovery budget, and poison-on-unsynchronized cleanup. Transaction owners distinguish completed-before-cancel effects at `INTRANS` from canceled/aborted state at `INERROR`, with no hidden rollback. BufferedFull owners separately assert no-timeout `PQexec*` count one and zero nonblocking/send/selector calls, and timeout `PQsend*` count one with the shipped wait/cancel/drain calls, zero selector calls, blocking restoration, and server-error-before-publication. Cache owners pin one deterministic interface/implementation invalidation when the public Delivery/rows/statement ABI lands and no additional descriptor/cache identity split between runtime modes. A live stream rejects every catalog/EXPLAIN/typed/transaction sibling with no libpq call. The required PostgreSQL job runs against server 16.4 with client >=17 and is non-skippable. After correctness, the direct-libpq comparison records time-to-first-row, full-scan throughput, peak native/package bytes, PGresult and cancel-handle counts, max rows/result, transported/decoded/published rows, batch copies, and cancellation/Drop recovery; it is not a semantic gate. |

The status prerequisite's Rust tool acceptance owner is
`postgres_tool_results_fail_closed_before_followup_native_work`. It crosses migration
command/query plus preparation command/query/prepare/describe consumers with a null result, all
three COPY statuses, `PGRES_SINGLE_TUPLE`, `PGRES_TUPLES_CHUNK`, both pipeline statuses, and unknown
numeric sentinels. One private `crates/align_driver/src/db_postgres_status.rs` authority exhaustively
classifies the complete libpq 17 numeric set before row access or follow-up SQL. It loads no new
symbol and does not raise the prerequisite's current client floor. Each adapter copies its
existing diagnostic first, clears a present current result exactly once, immediately finishes and
nulls its mutable connection owner, and returns that first error. The closed owner makes rollback,
deallocation, result retrieval, row access, and every later libpq call zero; Drop cannot finish it
again. Expected success and known complete error controls retain their existing mapping and cleanup.
This tool matrix is cumulative with the migration screening axes in the public record above; fixed
tool SQL and `PQprepare` not executing COPY do not weaken it.
...
| prerequisite package result-status safety closure | Before changing the stream ABI or libpq mode, route every shipped package-owned PGresult consumer through one exhaustive sealed `apps/db/pkg/db/internal/postgres_status.align` authority shared by `postgres.align`, internal execution, and resource cleanup without an import cycle. Before clear, copy the exact error selected by that consumer's existing non-success mapping; a previously owned error remains primary and silent cleanup remains silent. COPY, `PGRES_PIPELINE_SYNC`, `PGRES_PIPELINE_ABORTED`, or an unknown numeric status then clears the current result exactly once, immediately poisons/closes, and permits no later result/COPY/pipeline-exit/cancel/transaction-state/blocking-restore call. Clean all owners and the lease after close and never permit reuse; do not invent a new public error category, item, message, or query identity. The adjacent Rust tool closure and migration screening are mandatory cells of this same prerequisite PR and do not weaken the package rule. | independently mergeable prerequisite PR; exact package consumer/error inventory; sealed-authority exported-surface negative; COPY/pipeline/unknown x synchronous/timeout-completion/recovery/Drop no-call/clear-close/error-identity matrix; retained Q2/Q4a/Q4b/Q5b2 owners |
| prerequisite Rust tool result-status safety closure | Route migration `execute_raw`/`query` and preparation `execute_command`/`query_rows`/`PQprepare`/`PQdescribePrepared` through one private exhaustive Rust status authority before any result field/row access or follow-up SQL. A null result, COPY, partial single/chunk row result, pipeline status, or unknown numeric status copies the available existing diagnostic, clears the present current result once, finishes and nulls the mutable connection owner, and permits no later libpq call; known complete results preserve current mappings. Canonical PostgreSQL migration screening additionally uses one source-ordered statement-classification pass to reject first-token COPY with the exact §17.6 diagnostic before URL/native work. | `postgres_tool_results_fail_closed_before_followup_native_work`; module x consumer x null/COPY/single/chunk/pipeline/unknown clear-close/error/no-row-access/no-rollback/no-deallocate matrix; `postgres_migration_copy_is_rejected_before_native_work` with both transaction-control-before-COPY and COPY-before-transaction-control cases; prepare/migration SQL-origin inventory |
| staged public surface and option disposition | The direct PR adds exactly `Delivery` plus one `ExecuteOption.Delivery` variant and routes `rows_native`/`one_native` through one validator; commands reject it and common calls cannot name it. Query validation applies §13.4 payload-before-duplicate order to every occurrence; command rejection is tag-first because no Delivery payload applies to a command. The prepared PR adds exactly `rows_stmt_native` and reuses the same validator after statement-specific header/resolver validation. Text/Binary behavior is otherwise unchanged. Keep Delivery solely in the post-release D13 inventory, never the initial D1--D12 surface. | per-PR exported-surface and initial/post-release inventory goldens; parameterized common/native source-order, reversed-invalid/valid-duplicate, and command/direct/prepared disposition owners |
...
| explicit direct construction and move-in | Preserve §13.4 exactly: complete header/identity/common/native/restriction validation, validate live exec/connection state, allocate one context, run generated static validation, acquire the lease, bind once, compute/check the absolute deadline, set nonblocking, recheck the clock, send once only when time remains, immediately select the explicit mode, then publish one rows owner dependent on conn/tx. Expiry after nonblocking enablement restores blocking and returns Timeout with zero send/selector/cancel, or poisons/closes if restoration fails. Static failure frees the context before return; overlap frees that context with zero bind/libpq calls. Every later failpoint frees Params/context and any unpublished copied Query ID once and recovers or poisons before lease release; a published wrapper owns its ID until Drop. | complete pairwise phase-order matrix with context allocation/free, lease, binder, and libpq counters; direct conn/tx pre-send expiry/restore and send/mode failpoint counters; direct Query-ID counters; Params Move/Drop and post-constructor mutation owners |
| prepared state and explicit construction | In the prepared-parity PR, bump both-driver statement state to exact 88-byte v3 and retain the descriptor's non-null parameter resolver at offset 80. Add the exact sealed `prepared_parameter_ordinal(resource_ref<db.stmt<P,R>>, str) -> i32` compiler bridge and its `stmt_header_valid` control dependency beside the existing prepared binder bridge; both bridges accept only a concrete statement reference, never an application call or raw pointer, and whole/per-unit compilation retains the resolver thunk. Statement formation must bind the resolver, binder, row validator, stream decoder, and batch plan from the same concrete Query producer generation. Validate the complete statement/plan/resolver and every native option before lease/context/bind. Explicit mode computes the deadline, enables nonblocking mode, performs the same immediate pre-send expiry/restore-or-poison transition, then uses one prepared send and the shared stream constructor; success/failure only borrows the statement-owned Query ID/resolver and never frees either. | statement semantic/byte goldens; sema/MIR wrong-resource, application-source, unguarded-call, cross-Query resolver-splice, malformed-resolver no-call owners; SQLite/PG preparation and whole/per-unit resolver retention; prepared conn/tx pre-send expiry/restore and send/mode failpoints; unknown/duplicate parameter-name precedence; statement reuse/Drop counters |
| result advancement | Put PGresult acquisition/status/count/current-row transitions in one helper used by `next`, `has_next`, and `next_batch`. Clear each ordinary result exactly once, implement the exhaustive known libpq 17 status table and mode-specific sequence, and never fetch past a caller-visible bound. For every non-null result after the zero-row terminal, record invalid sequence before status-to-error mapping; ordinary statuses then clear/drain, while COPY, pipeline, and unknown numeric statuses clear only that current result and close without another protocol/pipeline-exit/cancel/probe/restore call before owner cleanup and lease release. `one_native` validates/decodes and clones only the first Row, records multiplicity only as pending state, drains without decoding further rows, and publishes Cardinality only after clean completion; any later native/timeout/cleanup error wins. Once cloned, caller-region bytes remain allocated on every non-success outcome because an arena is never rewound. Audit all three stream consumers, every cleanup caller, and the merged status-safety prerequisite whenever this helper changes. | parameterized every-status/count/sequence owner including post-terminal ordinary-error/invalid/COPY/pipeline/unknown error-precedence x cleanup-action cases, COPY/pipeline/unknown first/after-data/every-cleanup-cause no-call/clear-close counters, zero/one/many and one/two-fatal/two-clean/DML-effect owners, plus `out` allocation byte/alignment/clone-count parity for singleton and every post-clone failure/Cardinality |
...
| failure, timeout, and cleanup | Deduplicate mode failure, deadline expiry, fatal result, validation/decode/storage error, early Drop, and clean-terminal restoration around one drain/cancel/synchronize/restore helper; cardinality and first-owned row/storage errors begin with normal no-decode drain, while only deadline expiry, early Drop, and mode failure enter cancellation. Persist the original duration needed by deadline-expiry recovery. Give each created `PGcancelConn` one owner through exactly one `PQcancelFinish`, copy no cancel error after finish, and finish it before classifying main results. Preserve the first owned error, clear ordinary main results, restore then release or poison then release, and accept only `IDLE` for a connection cancellation. If any cleanup cause instead observes COPY, pipeline, or an unknown numeric status, clear that current result, stop the helper without further drain/pipeline-exit/cancel/probe/restore, immediately poison/close, clean unpublished/wrapper/context/ID owners, and release the lease while preserving the earlier error or silent Drop. For an explicit transaction, accept `INTRANS` only as completed with effects retained, or `INERROR` as canceled/server-aborted with rollback required; perform no hidden rollback and poison any other state. A clean-terminal restore failure becomes the exact PostgreSQL `db.rows.cleanup` error; `next_batch` destroys an unpublished partial final batch before returning it. Audit existing BufferedFull timeout paths, the merged result-status safety helper, and the common A1 publication contract when this helper changes. | cancel create/start/poll/error-message/finish/drain/reset failpoint and allocation matrix; COPY/pipeline/unknown x mode/deadline/row/storage/Drop x Timeout absent/present x Conn/Tx first-error/no-call/clear-close matrix; validation/decode/storage first-error x clean-completion/later-fatal/deadline-expires x Conn/Tx effect matrix; original-duration recovery budget; clean zero/partial-final restore failure; fatal-after-data; pre-send/post-row/pending-cardinality timeout; early Drop and timeout conn/tx `IDLE`/`INTRANS`/`INERROR` race/effect owners; reuse/rollback-or-poison owners |
| rows and statement ABI malformed input | Construct and validate the exact shared 120-byte rows v3 in one authority. SQLite direct/prepared Step plus PostgreSQL BufferedFull/SingleRow/PortalBatch each have exact active and terminal records; all consumers validate before native state and terminal stores both `-1` deadline sentinels. A live explicit stream alone may carry the exact absolute-deadline/original-duration pair. Prepared parity separately constructs statement v3 with the exact non-null resolver and updates every sibling accessor/Drop. | rows and statement semantic/byte goldens; parameterized deadline/duration and every other field mutation with no native-call counters; v2 rejection; driver x direct/prepared x active/terminal rows matrix; both-driver stmt live/closed matrix; retained SQLite rows/stmt owners |
| artifact and cache invalidation | Keep runtime Delivery values out of static Query/command descriptor, batch-plan, checked-metadata, and semantic fingerprint bytes. When the direct public enum/rows ABI and prepared statement ABI land, allow the owning interface/implementation hashes and every affected per-unit dependency/codegen/object key to invalidate exactly once; do not create per-mode artifacts or let an importer retain a stale dependency-interface key. | descriptor/plan/metadata byte parity across runtime modes; public-interface and implementation-hash before/after controls; whole/per-unit affected-importer miss plus unrelated-module retention; no per-mode artifact/cache split |
| FFI and native-version closure | Declare the exact libpq 17 send, row-mode, result, nonblocking cancel-connection, cancel-socket/readiness, transaction-state, and cleanup signatures/constants. Pin signedness for native `int` chunk size, `pg_usec_time_t`, and every polling/status tag with compiled C signature probes. This rail adds no COPY data/termination or pipeline-exit FFI; COPY, pipeline, and unknown-status behavior is current-result clear plus ordinary connection close. Keep the ordered `pq`/`ssl`/`crypto`/`zstd`/`z` closure on whole/per-unit Linux and macOS links; reject a client below 17 at build/evidence setup rather than at first request and fail closed on a newer unknown status. | C signature/status/unknown-sentinel probe; cancel-socket readiness counter; forbidden COPY/pipeline-exit symbol/call inventory; client-version assertion; whole/per-unit link inventory; x86_64/ARM64/macOS build owners |
...
The five-times-reopened implementation boundary is four independently mergeable vertical PRs.
The latest consumer sweep proved that COPY, pipeline, and unknown-status fail-closed behavior is not confined
to the future stream: shipped synchronous and timeout executors already receive `PGresult` values
and can otherwise release a connection while libpq remains in an undrainable protocol. That existing
correctness repair is a separate application-package-surface/ABI-neutral prerequisite after the lease repair.
The compiler-side sweep found one additional user-SQL entrance: PostgreSQL migration `PQexec`.
Canonical screening rejects top-level COPY before native work in that same prerequisite. The
following tool-consumer sweep requires the same PR to close and null a prepare or migration
connection on null, COPY, partial row-mode, pipeline, or unknown results before rollback,
deallocation, row access, or any later libpq call. The stream-only error-drain state machine and
validation-order correction remain inseparable from direct
delivery, while prepared formation remains independently mergeable after the direct engine.
...
| PostgreSQL result-status safety prerequisite | Audit every shipped package-owned PGresult consumer, including synchronous and timeout direct/prepared rows, one, command, prepare, transaction boundary, catalog, EXPLAIN, recovery drain, and silent cleanup. Before clear, retain the consumer's exact existing non-success error; an earlier error stays primary and silent cleanup stays silent. COPY, `PGRES_PIPELINE_SYNC`, `PGRES_PIPELINE_ABORTED`, or an unknown numeric status then clears the current result exactly once, immediately poisons/closes, performs no later result/COPY/pipeline-exit/cancel/transaction-state/blocking-restore call, cleans every owner, and never permits reuse. Route every Rust prepare/migration PGresult consumer through the private exhaustive tool classifier; null, COPY, partial single/chunk, pipeline, and unknown results copy the available error, clear when present, finish and null the owner, and forbid row access, rollback, deallocation, or later libpq. Reject top-level PostgreSQL migration COPY through one canonical source-ordered statement pass before URL/native work. Add no `pkg.db` public symbol or error surface, ABI field, delivery option, libpq-17 dependency, or rows-state change; §17.6 owns the exact new migration diagnostic. | parameterized every-package-consumer COPY/pipeline/unknown injection and no-call/clear-close/error-identity owners; `postgres_tool_results_fail_closed_before_followup_native_work`; `postgres_migration_copy_is_rejected_before_native_work` with both prohibited statement orders; prepare/migration SQL-origin inventory; retained Q2/Q4a/Q4b/Q5a/Q5b2 suites; local database gate; normal code preflight/review/CI |
| PostgreSQL direct streamed delivery | Add `Delivery` for direct rows/one, libpq-17 calls, shared rows v3, streamed advancement, normal-drain cardinality, cancellation, batch publication, pre-send deadline recheck, and the direct callable matrix; preserve absent Delivery on both shipped BufferedFull timeout subpaths. | direct delivery and pre-send expiry/restore owner matrices, retained dual-driver rows/batch suites, required local/CI PostgreSQL evidence, normal code preflight/review/CI |
...
| duplicate plus invalid PortalBatch had no intra-option precedence | The first closure chose command, duplicate, then first-payload order. The later §13.4 consistency review below supersedes that choice with canonical payload-before-duplicate order while retaining command tag rejection. | invalid PortalBatch in both orders, valid duplicate, and command-disposition owners |
...
The fresh review of that redesign found one new P1 and two P2 gaps. The new P1 reopened the matrix a
third time around errors observed while an effectful protocol remains pending; it does not add a
fourth PR for the inseparability reason above:

| Review claim | Disposition and redesign | Required owner/source |
|---|---|---|
| validation/decode/storage errors had no effect-aware pending-protocol cleanup | Valid. Preserve the first row/storage error, normal-drain without decoding under the original deadline, cancel only if that deadline expires, and pin Conn/Tx completion-versus-cancel effects. | error x deadline state x Conn/Tx effect matrix plus first-error/no-more-decode counters |
| timeout recovery lost the original requested duration after rows publication | Valid. Replace rows-v3 offset 112's reserved tail with exact `timeout_duration_ns`; require it to pair with the absolute deadline and use it for D9 recovery. | semantic/byte deadline-pair goldens, malformed pair no-native owner, recovery-budget boundaries |
| F31 called per-result chunking bounded total transport | Valid. Describe the rail as bounded result delivery/peak buffering; total transport remains query-defined and `one_native` normal drain may consume every row. | §2.4/F31/ledger consistency pass plus transported/max-rows-per-result counters |

The next fresh review found one new P1 and one P2. The new P1 reopened the matrix a fourth time and
requires a state-machine redesign, not an invalid-status line patch:

| Review claim | Disposition and redesign | Required owner/source |
|---|---|---|
| COPY statuses cannot reach a terminal result through generic `PQgetResult` drain | Valid. COPY support remains deferred, so every COPY status clears its current result and immediately poisons/closes without a further drain, COPY call, cancel, transaction probe, or blocking restoration. Preserve any earlier owned error and clean owners only after close. | COPY status x first/after-data/every cleanup cause x Timeout absent/present x Conn/Tx no-call/clear-close matrix |
| direct static validation was ordered before live state/context formation | Valid. Restore the settled §13.4 and shipped Q2 sequence: header/options/restriction, live state, context allocation, generated static validation, lease, then bind/native work. | pairwise multi-invalid winner table and context/lease/bind/libpq failpoint counters |

No fourth PR is added. COPY classification and cleanup have no consumer before explicit direct
streaming, and the validation-order correction belongs to the same direct constructor. The prior
three vertical boundaries remain independently correct and mergeable.

The fresh review of the fourth redesign found three P2 consistency gaps and no new P1. They amend
the ledger and owners without reopening or re-splitting the matrix:

| Review claim | Disposition and closure | Required owner/source |
|---|---|---|
| failed streamed `one_native` hid caller-region allocation | Valid. Keep the shipped one-copy behavior and specify monotonic `out` precisely: a valid first Row allocates the same clone bytes on singleton success, Cardinality 2, and every later failure; zero/invalid-first-row allocate none and no path rewinds. | `out` allocation count/byte/alignment parity across all pre/post-clone outcomes |
| Delivery duplicate-before-payload contradicted §13.4 | Valid. Canonical payload-before-duplicate wins for Query options; both invalid orderings report size, a valid duplicate reports duplicate, and command rejection remains tag-first. This supersedes the first review's local disposition. | §13.4 plus invalid-order/valid-duplicate/command owner matrix |
| Delivery appeared in the initial-release option inventory | Valid. Keep D1--D12 sums exact and list Delivery separately as the post-release D13 addition. | initial-versus-post-release exported-surface inventory golden |

The base-refreshed fresh review then found two P1 omissions and one P2 status gap. The P1s reopened
the matrix a fifth time; the shipped-result consumer sweep changes the PR boundary, while the
pre-send timeout closure stays in each explicit delivery constructor:

| Review claim | Disposition and redesign | Required owner/source |
|---|---|---|
| COPY fail-close covered only future streamed executors | Valid. Shipped BufferedFull, command, prepared, one, timeout recovery, catalog/EXPLAIN, transaction, and silent-cleanup consumers can already observe a COPY result. Split one application-package-surface/ABI-neutral result-status prerequisite that closes the complete consumer class before streaming. | every shipped PGresult consumer x COPY status x synchronous/timeout/recovery/Drop no-call/clear-close/no-reuse matrix |
| explicit delivery omitted the post-nonblocking pre-send deadline check | Valid. After `PQsetnonblocking`, re-read the clock before either direct or prepared send; expiry restores or poisons and returns Timeout with zero send, selector, and cancel calls. | direct/prepared x SingleRow/PortalBatch x Conn/Tx pre-send expiry and restore-failure counters |
| unknown numeric status used ordinary drain | Valid. libpq is supported at 17 or newer, so an unrecognized status cannot be assumed protocol-drainable. Treat it like COPY in both the prerequisite and stream engine rather than imposing an artificial maximum client version. | known-status exhaustive table plus unknown-sentinel x every consumer/cleanup cause clear-close/no-later-call owners |

The fresh review of the fifth redesign found two P2 consistency gaps and no new P1:

| Finding | Disposition | Closure evidence |
|---|---|---|
| pipeline statuses used ordinary invalid-sequence drain | Valid. `PGRES_PIPELINE_SYNC` and `PGRES_PIPELINE_ABORTED` leave connection-global pipeline mode until `PQexitPipelineMode`; because this rail owns no pipeline-exit FFI, every current and streamed PGresult consumer clears the observed result and immediately closes exactly like COPY/unknown. | every consumer/cleanup cause x both pipeline statuses no-later-result/pipeline-exit/cancel/probe/restore, one clear, close, first-error/silent-Drop, no-reuse owners |
| introductory staging still named one prerequisite and three PRs | Valid. Name both application-package-surface/ABI-neutral prerequisites and the exact lease -> status -> direct -> prepared sequence in English and Japanese. | ledger/closure/staging/mirror four-boundary consistency sweep |

The next fresh review found one P2 and no new P1. The package PGresult matrix was complete, but the
same deferred-subprotocol class still reached shipped migration tooling:

| Finding | Disposition | Closure evidence |
|---|---|---|
| migration PGresults were outside the status closure | Valid. `screen_postgres_catalog` currently accepts top-level COPY, after which synchronous `PQexec` enters COPY mode and the error path attempts rollback. Reject COPY through the existing dollar-quote-aware statement screen before URL or native work, and inventory prepare/migration plus all fixed tool SQL in the same prerequisite. | Required/Forbidden COPY x lexical near-miss/multi-invalid order, zero env/open/libpq; prepare/migration SQL-origin inventory; retained Q5a screening matrix |

The following fresh review found one further P2 and no new P1. The SQL-origin inventory did not
itself make unexpected Rust tool results safe:

| Finding | Disposition | Closure evidence |
|---|---|---|
| Rust tool PGresult consumers could clear an unknown/deferred result and then issue follow-up SQL | Valid. Put every prepare/migration result site behind one private exhaustive Rust classifier. Null, COPY, partial single/chunk rows, pipeline, and unknown statuses copy the available error, clear a present result once, finish and null the owner, then forbid row access, rollback, deallocation, or later libpq. | `postgres_tool_results_fail_closed_before_followup_native_work`; module x consumer x synthetic-result clear/close/error/no-follow-up counters; known-complete controls |

This remains a P2 closure inside the existing status prerequisite. It neither reopens the matrix nor
changes the four PR boundaries.

The final base-bound review found three further P2 gaps and no new P1:

| Finding | Disposition | Closure evidence |
|---|---|---|
| a post-terminal fatal/error status had conflicting native-error and invalid-sequence dispositions | Valid. Validate protocol state before status-to-error mapping. Every non-null result after the zero-row terminal records invalid sequence; the status then selects ordinary drain or the COPY/pipeline/unknown immediate-close action without replacing that error. | post-terminal ordinary-error/invalid/COPY/pipeline/unknown x error identity x drain/close matrix |
| migration ownership omitted `COPY; BEGIN` | Valid. Require one source-ordered classification pass and both prohibited orders, so the first statement wins before Forbidden count regardless of category. | `BEGIN; COPY` and `COPY; BEGIN` exact diagnostic plus zero-env/open/lock/history/libpq counters |
| the runtime-mode identity claim hid dependency-cache invalidation | Valid. Static descriptor semantics remain mode-independent, but landing the public enum and rows/statement ABI changes their interface/implementation hashes and invalidates every affected importer key once. | semantic-byte parity plus interface/implementation and whole/per-unit cache invalidation controls |

These findings amend existing closure cells and do not reopen the matrix or change the four PR
boundaries.

Run one fresh independent adversarial review of this five-times-redesigned ledger and the four PR boundaries
...
This ledger is the source of truth for the next independently useful D13 rail. It completes the
already-published `Text|Binary` parameter and result choices for the first-release PostgreSQL type
matrix. It adds no public type, function, option, error variant, driver, Query shape, or dynamic
dispatch. The implementation is one capability boundary even though it changes generated
descriptor semantics, the package-owned binder context, prepared-statement validation, libpq
arguments, and row decoding: a binary encoder without the matching static type proof can send an
ill-typed value, while a producer proof without the package consumer is dormant. The expected
implementation may exceed 1,000 hand-written lines because splitting that strict producer-to-
consumer chain would duplicate the same wire, ABI, malformed-input, and whole/per-unit proof.

The exact application-callable inventory remains:
...
Absence of `ParameterFormat(name, ...)` selects Text for that parameter. Absence of
`ResultFormat(...)` selects Text for the complete result. A named parameter may have at most one
format option and an execution may have at most one result-format option. Result format is one
libpq-wide choice for all result columns; there is no per-column surface. `execute_native` passes
the selected result format even though a valid command has no result columns. `rows_native`,
`one_native`, and `rows_stmt_native` compose either format with `BufferedFull`, `SingleRow`, and
`PortalBatch`. Common operations remain Text-only by absence because they cannot name a
PostgreSQL-native option. No environment variable, server setting, row value, prepared-state
history, or heuristic changes a selected format.

The first-release binary mapping is exact:

| Align value shape | Required PostgreSQL canonical type / OID | Binary parameter and result payload |
|---|---|---|
| `bool` / `Option<bool>` | `bool` / 16 | one byte, exactly `00` or `01` |
...
| `str`/`string` and nullable forms | `text` / 25, `varchar` / 1043, or `name` / 19 | exact client-encoding bytes, no terminator in the recorded length |
| `slice<u8>`/`array<u8>` and nullable forms | `bytea` / 17 | exact bytes, including embedded zero and high bytes |

Owned `string` and `array<u8>` forms are parameter shapes only. Row fields remain borrowed `str`
and `slice<u8>` views (including their `Option` forms); `one_native` retains the existing explicit
region clone.

The connection remains pinned to client encoding UTF-8. Binary text input therefore copies the
...
The wire goldens are independently fixed in both directions: `false = 00`, `true = 01`,
`int2(-2) = ff fe`, `int4(0x01020304) = 01 02 03 04`, `int8(-2) = ff ff ff ff ff ff ff fe`,
`f32(1.5) = 3f c0 00 00`, `f64(-0.0) = 80 00 00 00 00 00 00 00`, UTF-8 `é = c3 a9`, and
`bytea([0, 255]) = 00 ff`. Float encode/decode preserves every bit, including signed zero,
infinities, and NaN payloads; it does not pass through decimal text. These layouts follow libpq's
...
`format_count` is the exact number of 8-byte, 4-aligned entries and is at most both the operation's
trusted parameter count (validated descriptor count/thunk for direct execution, validated statement
count thunk/copy for prepared execution) and `65_535`. `format_plan` is null exactly when
`format_count == 0`;
...
| checked parameter encoding or complete Bind-message budget overflow | `Encode`, item `db.parameter`, message `PostgreSQL parameters exceed the Bind message length limit` |
...
Parameter formation visits increasing protocol ordinal. Within Measure mode it checks the memory
representation, then text U+0000 when applicable, then exact encoding arithmetic and the remaining
Bind budget. Encode mode performs allocation and copy only after the complete Measure pass. The
language's ordinary allocation failure remains a hard abort, not another
`pkg.db.Error`. Each tuple-producing native result has two ordered validation phases. Before row
count, zero-row/cardinality/terminal handling, or result publication, metadata mode checks exact
column count and then increasing declared Row ordinal's name, OID, and format. Only after that
complete pass succeeds may value mode inspect a delivered row, checking increasing ordinal's NULL
disposition, negative length, non-null pointer for a present value, then fixed width before bool
value or UTF-8 payload validity. Thus any metadata error wins over every value/cardinality outcome;
within a phase the first declared ordinal wins. That ordering and the table above govern every
multi-invalid direct, prepared, buffered, and streamed input, including valid and malformed zero-row
results.

The context matrix's retained v2 range is exact and driver-specific. Bytes 0--39 keep the common v2
fields. SQLite uses offset 40 for its retained text/blob pointer vector, 48 for its `u32` vector
length, 52 for zero `u32`, and requires every byte 56--79 to be zero; it owns and frees only the
offset-40 vector and its entries. PostgreSQL uses offsets 40/48/56/64 for its value/OID/length/format
vectors, 72 for their `u32` length, and 76 for zero `u32`, with the existing ownership and cleanup.
Both v4 constructors initialize all 112 bytes explicitly. The complete validator checks this
driver-specific product before any load or free, and malformed cleanup never interprets a zero-only
SQLite slot as an owner.
...
descriptor or statement with that count returns the existing query-less `InvalidQuery`, item
`db.descriptor.header`, message `invalid static database descriptor`, before option access or
allocation. Before payload
allocation, generated binder-v2 Measure mode visits increasing protocol ordinal, validates the
memory representation and text U+0000 rule, computes the exact selected encoding length, and debits
one preinstalled remaining Bind budget. Text scalar measurement uses the same canonical formatter
against a count-only sink; Text bytea uses checked `2 + 2*source_len`; Binary uses the fixed table or
source byte length. The first ordinal whose encoding arithmetic or debit fails returns the exact Bind
...
and a nullable field passes its `Option` unchanged. Measure mode may call only the `measure_*_v1`
symbol in its row, while Encode mode may call only the existing `bind_*_v2` symbol:
...
Each uses the settled callback status model: exact zero on success and exact one after recording the
first context-owned failure. No valid callback returns another status, Measure never delegates to
...
Cardinality. A metadata-valid second generation returns Cardinality without Value mode, decoder
invocation, or a second Row allocation. This D13 rule supersedes the older D8 any-second-row winner.

The implementation closure matrix is:

| Closure cell | Required implementation closure | Exact owner evidence |
|---|---|---|
| public surface and operation matrix | Preserve the exact inventory above. Accept independently selected Text and Binary parameter formats on direct Query, direct command, and prepared Query execution; accept an independently selected Text or Binary result format on those same operations; add no prepared `one_native`, common native-option escape, per-column result selector, reflective codec, or compatibility alias. | exported-surface golden plus command/direct rows/direct one/prepared rows callable matrix; parameter-format plan and result-format axes remain separate from BufferedFull/SingleRow/PortalBatch |
| static type proof, binder, and resolver ABI | Bump the 8-aligned execution descriptor from 136-byte v5 to exact 144-byte v6. Offsets other than 64, 104, and the new 136 retain v5 meaning. Offset 64 becomes the non-null generated binder-v2 `fn(context: raw, borrow params: P, mode: u8) -> i32`: `0=Measure`, `1=Encode`, and every other mode fails before a package callback. Both modes visit the same increasing protocol ordinals and field types; Measure calls only type-matched count/debit callbacks and Encode calls only the matching allocation/bind callbacks. SQLite invokes Encode directly against its exact non-measuring context; PostgreSQL requires one complete Measure success for the same context/params/format vector before Encode. Offset 104 becomes the non-null generated parameter-resolver v2: zero means unknown, a positive ordinal means known but Text-only, and the corresponding negative ordinal means the parameter has one exact compatible PostgreSQL `ParameterType` from the table above and may use Binary; `i32::MIN` and an absolute ordinal outside `1..=parameter_count` are malformed. Offset 136 is a non-null generated `fn() -> u32` parameter-count v1 thunk whose body is exactly one constant return equal to offset 112's existing distinct-parameter count and at most `65_535`. The complete descriptor validator compares those two counts and enforces the protocol maximum before accepting the binder or resolver. Existing sealed `parameter_known` accepts either resolver sign and existing `parameter_ordinal` operations return the absolute ordinal. Add a sealed direct binary-ordinal operation that returns the absolute ordinal only for the negative form and zero otherwise. The prepared sibling additionally requires the retained effective-OID eligibility bit described below, so a differing `ParameterOid` override cannot reuse a stale static proof. Application calls, wrong descriptor/resource types, unguarded prepared loads, count-thunk or binder invocation outside the complete header guard, Measure/Encode mismatch, cross-descriptor binder/resolver/count-thunk splices, and malformed HIR fail closed. Query/command formation, interface serialization, monomorphization, whole-program, and per-unit compilation retain the matching binder v2, resolver v2, and count thunk from one producer generation. | exact v6 header/binder/resolver/count-thunk body/signature/relocation goldens; `65_535`/`65_536`, offset-count/thunk equality, and every zero/mismatch/malformed thunk product; mode/callback/ordinal parity and Measure-before-Encode owners; every type and missing-`ParameterType` resolver code; matching/differing/absent `ParameterOid` proof owners; sema/MIR application/wrong-kind/unguarded/splice/malformed rejection; whole/per-unit runtime-selected same-typed descriptor twins |
| option validation and precedence | Preserve §13.4 source-order phases. Direct command/Query execution validates the complete descriptor and identity, then common and native options before driver/live state. Prepared execution instead validates the complete statement v4 header, batch plan, resolver, count thunk/count equality, eligibility shape, and trusted identity before any common or native option because every prepared name/proof lookup depends on that guarded state. A malformed or closed statement combined with any invalid Binary option therefore returns query-less `InvalidQuery`, item `db.descriptor.header`, message `invalid static database descriptor`. After that prepared guard, both direct and prepared validation reject U+0000 in each `ParameterFormat` name, then an unknown name, then a Binary request lacking the direct negative proof or prepared negative-plus-eligibility proof, then a duplicate; current-payload validity beats duplicate registration. The missing proof uses `Unsupported`, the trusted Query/command ID, item `postgres.execute.parameter_format`, and exact message `binary PostgreSQL parameter requires an exact supported ParameterType`. `ResultFormat` has no payload beyond its closed enum and duplicate detection is exact. Delivery keeps its adjacent ledger order. After the operation-specific header guard, all option errors precede driver restriction, physical connection/live-state checks, lease/context/bind, and SQL send; a valid SQLite statement plus an invalid Binary option reports the option error before PostgreSQL driver mismatch. | parameterized source-order and reversed-invalid/valid-duplicate owners across command/direct/prepared; malformed/closed statement plus every invalid Binary option exact header winner; valid SQLite statement plus invalid Binary exact option winner; unknown/NUL/missing-proof zero execution-context/format-plan/libpq counters; retained Delivery precedence owners |
| package context and prepared ABI | The shared opaque binder/decode context becomes one exact 112-byte v4 record for both drivers. Offsets 0--95 retain the exact v3 meanings above. Offset 96 is signed remaining Bind-payload budget; offset 104 is measurement state (`0=unmeasured`, `1=measuring`, `2=measured`); and 105--111 are zero. SQLite requires state 0 and budget `-1`. A new PostgreSQL context also starts at `(0,-1)`; only the sealed budget initializer may store state 1 and the nonnegative result of the complete fixed Bind formula after header/options/count/Parse validation and before a parameter payload allocation. Measure callbacks require state 1 and atomically debit an exact length without going negative. A complete successful binder-v2 Measure return changes state to 2; PostgreSQL Encode requires state 2, and neither failure nor a second Measure/Encode pass can publish it. Offset 80 remains result format `u8` (SQLite requires `0=Text`; PostgreSQL allows `0=Text`, `1=Binary`); offset 81 remains metadata state (`0=unvalidated`, `1=validated`); 82--87 remain zero; and offset 88 remains the metadata native pointer. Metadata state 0 requires offset 88 null. A sealed native-generation setter installs offset 8 and resets 81/88 before every row-producing SQLite Query execution and every newly acquired tuple-producing PGresult, including zero-row terminal results. Successful metadata validation stores state 1 and the exact non-null offset-8 pointer; value validation requires that equality. Clearing/replacing native state resets both metadata fields, so pointer-address reuse cannot inherit validation. The one context validator rejects every driver/version/format/metadata-state/measurement-state/budget/reserved/pointer product before dereference, debit, bind, or free. Shared prepared statement state becomes an exact 112-byte, 8-aligned v4 record: offsets 0--79 retain v3 meaning; offset 80 retains resolver v2; offset 88 is a package-owned byte-per-parameter Binary eligibility vector; offset 96 is the non-null producer-owned parameter-count v1 thunk from descriptor offset 136; offset 104 is the copied exact `u32` parameter count; and 108--111 are zero. The one complete statement-header validator first establishes the fixed fields and non-null thunk, invokes that guarded thunk, and requires a result at most `65_535` exactly equal to offset 104 before using it to bound eligibility validation. SQLite requires a null eligibility pointer for every count. PostgreSQL requires null eligibility for zero parameters and otherwise a non-null vector with exactly the trusted count's readable bytes, all 0/1. At PostgreSQL preparation, byte `ordinal-1` is one exactly when static validation installed a supported nonzero canonical OID and the effective OID after any `ParameterOid` override equals it. Arbitrary nonzero overrides remain valid for Text, but a differing override makes that ordinal Binary-ineligible. The sealed prepared binary-ordinal bridge requires a complete guarded v4 header, negative resolver result within the trusted count, and eligibility byte one. HIR statement formation requires binder v2, resolver, count thunk, row validator, decoder, and batch plan from one Query producer. Statement Drop nulls the vector/thunk/count before freeing the vector once. The context/statement validators, every accessor/bridge/constructor/Drop path, and semantic/byte goldens move together. | driver x context-v4 result-format/metadata-state/measurement-state/budget/reserved/native-pointer matrix with new-result pointer-reuse reset and Measure replay rejection; statement v4 both-driver active/closed/malformed bytes; `65_535`/`65_536`, thunk null/body/count mismatch, and eligibility pointer/byte products with no out-of-bound scan/callback; cross-Query binder/thunk/resolver splice; matching/differing/absent override; constructor/accessor/Drop no-call and free counters; cumulative SQLite prepared/rows suite |
| normalized format formation and ownership | After complete native-option validation, form at most one call-local normalized 8-byte entry (`i32 ordinal`, `i32 format`) per explicit parameter option. The public native wrapper owns that 4-aligned scratch storage through the exact synchronous internal ABI above and frees it on success or error; a published rows resource never borrows it. After complete plan/count validation, the internal path allocates exactly one `N`-entry execution-owned format vector initialized to Text and copies each explicit code to its ordinal. This is the non-null vector that makes libpq emit `N` parameter format codes and that both binder-v2 modes read; zero parameters use a null vector. Zero explicit options use the canonical `(null, 0)` scratch plan but still allocate the full vector when `N > 0`. Result format needs no separate allocation. | zero/one/many option and zero/positive-`N` scratch/vector allocation/free counters; every malformed result-tag/count/pointer/size/ordinal/tag/duplicate product with exact `InvalidQuery` and zero context/bind/send; full-vector default/explicit code bytes and libpq format-count parity; rows publication followed by scratch destruction/use-after-free probe; direct/prepared parity; sema/MIR call-formation negatives |
| parameter measurement, encode, and retention | Run generated static validation before installing the normalized plan and exact full format vector on direct execution; prepared execution requires both the retained negative resolver proof and its retained effective-OID eligibility bit. Before payload allocation, validate the operation's Parse formula where applicable, initialize the exact Bind budget, and run binder-v2 Measure once. Measure callbacks read only the selected code for their own ordinal, validate and debit exactly as above, and cannot allocate, bind, send, broadcast a code to a sibling, or mutate result format. Binder-v2 Encode then reads the same code per ordinal and emits exactly the table above. Each non-null scalar/text/bytea value has exactly one execution-owned payload allocation and NULL has none. A nonempty Binary bytea allocates exactly `len` bytes; an empty Binary bytea allocates one zero sentinel byte, passes its non-null pointer with recorded length zero, and remains distinct from `None`. Direct and prepared BufferedFull retain through synchronous completion; SingleRow/PortalBatch retain until `PQgetResult` returns null or fail-closed cleanup destroys the context. Every Measure, partial Encode, or option-plan failure frees installed context/vector/payload owners once and sends nothing. | exact Parse/Bind formula and every encoding-class budget boundary; `65_535`/`65_536`; per-type/nullable measure-versus-wire-byte/length/format/OID goldens; every ordered two-parameter default/explicit-Text/explicit-Binary pair plus three-parameter heterogeneous control; direct/prepared `Some(empty bytea)` versus `None`; U+0000; accepted-limit/rejected-next multi-parameter and statement-name twins with zero payload allocation/send on rejection; count-only scalar formatter parity; mode replay/wrong-mode; mutation/source-Drop retention; partial-bind allocation/free matrix; synchronous/streamed context lifetime counters |
| libpq call parity | Pass the exact independently formed per-parameter format vector and result-format code to every `PQexecParams`, `PQexecPrepared`, `PQsendQueryParams`, and `PQsendQueryPrepared` call. Do not route an explicit Binary call through `PQexec`, text fallback, or a second SQL statement. Timeout, cancellation, normal drain, status fail-close, transaction effect, and delivery selection remain those of the adjacent D8/D9/A1 ledger. | stub capture of every ordered two-parameter format pair x every result-format state x synchronous/asynchronous direct Query and prepared Query path; separate direct-command capture; required live PostgreSQL mixed-format controls; retained deadline/delivery/status/cancellation matrices |
| result metadata and decode | Add exact `PQfformat` FFI use and make descriptor offset 80, statement offset 56, and rows offset 40 the same generated row-validator v2 pointer with exact ABI `fn(context: raw, mode: u8) -> i32`, where `0=Metadata` and `1=Value`; every other mode returns failure before a package callback. The changed rows pointer signature bumps the shared 120-byte, 8-aligned rows record from v3 to v4 with every byte offset otherwise unchanged. Metadata mode calls `validate_row_count_v3` once, then `validate_field_metadata_v3` in declared order. SQLite validates count/name; PostgreSQL validates count/name/OID/requested format. On success it publishes the context metadata state above. Value mode requires that state, then calls `validate_field_value_v3` in declared order for NULL and representation/payload; SQLite also validates its row-dynamic native type. Text uses the existing parsers. Binary rejects wrong fixed width, bool bytes other than 0/1, negative length, null pointer for a present value, or invalid UTF-8 before a safe view or decoder call. Integer and float decode use endian-explicit bit operations. Every row-producing SQLite Query execution and every tuple-producing PGresult generation invokes Metadata exactly once before row count, zero-row terminal disposal, cardinality, `has_next`, decode, or batch append; Value runs once per decoded/appended row and never for cardinality-only drain. Binary text/bytea views point directly into the current PGresult generation; Text bytea retains its owned decode buffer. `one_native` clones the validated first Row into `out` exactly as before. The descriptor producer, statement/rows constructors, one complete rows-v4 validator, every rows consumer/Drop path, sealed direct/prepared/current-metadata/current-value bridges, HIR validator, MIR, LLVM preflight, and whole/per-unit retention all recognize the v2 signature and reject v1, rows v3, cross-Query, invalid-mode, value-before-metadata, and malformed-thunk paths. | zero-row valid/wrong count/name/OID/format exact Decode-before-exhaustion/cardinality owners for BufferedFull/SingleRow/PortalBatch and direct/prepared; every new PGresult generation metadata call count including terminal; metadata-before-value multi-invalid matrix; rows-v4 and validator-v2 body/signature/relocation semantic-byte goldens plus sema/MIR malformed-call owners; per-type result-format/NULL/malformed width/bool/UTF-8/pointer; zero-copy pointer identity; view-generation and one-clone owners; cumulative SQLite metadata/value suite |
...
| artifact, cache, and documentation identity | Application-facing module interfaces and static Query semantic fingerprints do not change because the Text/Binary surface already exists. The added/changed exported compiler-private measurement and prevalidated signatures in `pkg.db.internal` and `pkg.db.internal.postgres` change those internal module interface hashes and every importing module's dependency-interface key once. Descriptor v6's 144-byte image, binder-v2/row-validator-v2/resolver/count-thunk bodies, package callback names/semantics, rows v4's 120-byte implementation, statement v4's 112-byte implementation, and shared context v4's 112-byte implementation also change their exact compiler/package implementation hashes and invalidate affected codegen/object/dependency cache entries once. Checked artifacts, metadata, SQL identity, and driver restriction remain unchanged. English/Japanese package designs and the live handoff agree on the next/shipped boundary. | both internal-module interface-hash and importer dependency-key before/after twins; descriptor semantic-byte parity plus binder/row-validator/resolver/count-thunk relocation/body and implementation/cache invalidation twins; rows 120-byte/statement 112-byte/context 112-byte goldens; application-interface/artifact/metadata identity controls; English/Japanese consistency check; handoff updated only with the implementation milestone |
| acceptance and measurement | Require merged D8/D9 streaming/cancellation, A1 batch and direct/prepared delivery, libpq client >=17, and the required PostgreSQL server job. Parameter format and result format are independent axes: for two named Query parameters, cross every ordered pair from `{default Text, explicit Text, explicit Binary}` (nine plans) with `{default Text, explicit Text, explicit Binary}` result format (27 products) on both direct and prepared stub paths; include a three-parameter heterogeneous control and the applicable direct-command parameter/result-code cases. Separately cross every mapped type/nullable form through Text and Binary measure/bind/decode, plus BufferedFull and both explicit delivery modes. Cross exact Parse/Bind formulas, direct/prepared statement-name lengths, single/multiple/NULL payloads, all encoding classes, accepted-limit/rejected-next, and parameter counts `65_535`/`65_536` through no-allocation stubs; then cross timeout absent/present, Conn/Tx, rows/one/command where callable, zero/one/many rows, valid/malformed zero-row and nonempty metadata/payload, and whole/per-unit linking without collapsing the two format axes. Run `scripts/db-verify-local.sh` before push. Record the named local Text-versus-Binary parameter/row measurement with payload bytes, copied bytes, allocations, time-to-first-row, and full-scan throughput after correctness; it is not a semantic gate. | focused 27-product Query format-plan/result owner plus command and mixed live controls; protocol-budget/count boundary and Measure/Encode parity owners; metadata-only zero-row/new-generation owners; per-type package/compiler owners; retained `pkg_db_q4b`/`pkg_db_a1`/`pkg_db_q5b2`; required live PostgreSQL suite, local database parity gate, and non-gating measurement record |
...
The next fresh review found two further P1 closure failures. The eligibility length had only a
self-reported statement count, so descriptor v6 and statement v4 now retain one generated constant
count thunk and validate it under the complete header before any vector scan. Prepared option
validation also depended on state that the earlier precedence sentence placed after options; the
single order is now complete statement header and identity, common/native options, driver and
physical live state. The exact malformed-statement/invalid-option and valid-wrong-driver/invalid-
option winners close that multi-invalid product. This reopens the ABI and validation-order rows;
the one implementation boundary remains unchanged.

The redesigned review then found two more P1s, so the closure matrix is reopened on the
`result-generation-metadata x independent-format-product` axis rather than patched locally. Result
schema is now a generation-level obligation with an exact row-validator-v2 Metadata mode that runs
before zero-row/cardinality/terminal handling and a guarded Value mode for delivered rows. The same
reopen makes parameter-plan state and result-format state independent: one parameterized owner
closes the complete two-parameter 27-product instead of a coupled `default/Text/Binary` shorthand.
The implementation remains one boundary because the changed producer thunk, shared context ABI,
every result consumer, and format propagation must land together to avoid either dormant proof or
unchecked empty results.

The next redesigned review found two P1 and two P2 omissions, so the matrix is reopened on the
`cross-driver-context-bytes x internal-interface-cache x metadata-cardinality x encoded-length`
axis. SQLite now has canonical zero bytes and no ownership in context offsets 56--79, internal
prevalidated signature changes explicitly invalidate interface/dependency keys, every result
generation's metadata wins before second-row Cardinality, and each selected wire encoding has an
accepted maximum plus rejected next length. These are one closure axis because constructors,
validators, consumers, cleanup, cache identity, and observable error precedence must agree on the
same ABI generation; splitting them would leave either an unreadable context image or a stale caller.

The protocol-boundary review then found two P2 omissions: libpq's C `int` is not the complete Bind
envelope, and Parse/Bind parameter counts are 16-bit protocol fields. The ledger now owns the exact
Parse/Bind length formulas and the `65_535` maximum. Binder v2 and context v4 provide one no-payload-
allocation Measure pass before Encode, rather than promising impossible per-value maxima or relying
on a late libpq failure. This changes the wire-validation strategy, so the matrix is reopened on the
`protocol-count x aggregate-message-budget x measure-before-encode` axis while retaining the same
one producer-to-consumer implementation boundary.

The implementation references are [PostgreSQL protocol message formats](https://www.postgresql.org/docs/17/protocol-message-formats.html),
...
PostgreSQL built-in send/receive functions named above. Run one fresh independent adversarial
review of this ledger and its one-PR capability boundary before implementation. Close findings
ledger-first. Before code review, extract every normative `must`/`exact`/`every`/`before`/
...
a NULL type. Unwrapping the returned `Result<Option<row>, Error>` through `else`, `match`, or `?`
must preserve that selected cleanup bit. The resulting arena-owned `row` may be inspected through
`borrow row: row`, but an ordinary by-value call must reject it before entering a callee whose
parameter Drop contract assumes free-standing storage.
...
null pointer only while `sqlite3_errcode` is `SQLITE_OK` or the current successful
`SQLITE_ROW`. Any other null/length/error-state
...
| returned-row ownership and call transfer | Preserve the path-selected cleanup bit from direct, imported, and compiler-private calls through `Result`/`Option` success projection. A row produced into `out` remains arena-owned after `else`, `match`, and `?`; shared-borrow helpers may inspect it, while an ordinary by-value call rejects it before callee entry. First-class function signatures cannot carry or capture a `region` capability, and free-standing Move call results retain the existing legal transfer. | mutation-discriminating generic direct/compiler-private/imported call-result owners; A2 `dynamic_next` `else`/borrow-helper acceptance and by-value rejection; whole/per-unit parity; runtime no-double-free control |
...
SQLite scalar functions and collations without retaining a source closure, exposing a native
pointer, permitting callback reentry into the owning connection, or relying on an unchecked C
comparator. PostgreSQL notice/COPY callbacks remain a separate rail. The public callback producers,
their generated C-ABI trampolines, registration, replacement, removal, and both callback consumers
form one capability: landing the compiler producer without registration is dormant, while landing
either native consumer without the shared capture/effect/ABI proof duplicates the same safety
boundary. The implementation may therefore exceed 1,000 changed hand-written lines; this one-PR
boundary has less duplicated proof and lower integration risk than splitting the strict
...
`scalar_function` and `collation` are nominal compiler-produced Copy descriptors. Their fieldless
source declarations are not constructible as ordinary values. `function` and `collation_key` are
compile-time producers despite their source spelling: their sole argument must resolve to one exact
...
A collation key callback must be `Pure` at descriptor formation. Purity does not mean total: an
allocation failure, bounds failure, invalid integer division, explicit `process.abort`, or another
language hard error terminates the process exactly as it does outside SQLite. No panic/unwind or
ordinary error crosses the C boundary.
...
signature `fn(function_args) -> Result<pkg.db.value, str>` and kind 1 has exact signature
`fn(collation_input) -> array<u8>` plus `Pure`. The generated identity is nominal: canonical module
...
`function_args` and `collation_input` are Copy views formed only inside a generated trampoline.
Their fields are readable ordinary values, but their source provenance is the callback invocation:
they cannot be returned, stored into a longer-lived aggregate, captured by an escaping function
value, moved to a task, or used after the callback returns. `function_args.values` is one ordered
slice with exactly `arity` elements. `collation_input.bytes` is the exact possibly non-UTF-8 SQLite
byte sequence and length supplied to `xCompare`; it is never guessed to be `str`. A scalar callback
may return Text/Bytes borrowed from its arguments or static storage because the trampoline consumes
and transient-copies that result before the invocation ends. A collation callback returns one owned
`array<u8>` key. The left key is produced first, the right key second, both are compared unsigned
byte-lexicographically with length as the final tiebreaker, and both are dropped exactly once after
comparison. Equal keys define one valid equivalence class. Because one proved-Pure function maps
each input to a key and the package alone compares keys, symmetry, equality transitivity,
antisymmetry, and order transitivity do not depend on an unchecked application comparator.
...
the descriptor's registered arity, the argv null/count product, and every argument in increasing
ordinal before invoking application code. It forms one fixed-capacity 127-value stack scratch and
one borrowed slice of its first `argc` entries; it performs no package heap allocation and invokes
the callback at most once. On `Ok`, it emits exactly one result call. On `Err` or pre-callback
failure, it emits exactly one error result and no value result. It never obtains the connection,
prepares/steps/resets/finalizes a statement, calls registration, or exposes context/argv to source.

Each generated collation trampoline has exact C ABI
`fn(application: raw, left_len: i32, left: raw, right_len: i32, right: raw) -> i32`. It requires
null application data, nonnegative lengths, and a non-null pointer for each nonempty input. It forms
the two callback views without UTF-8 conversion, invokes the same proved-Pure key callback exactly
twice in left-to-right order, compares the two owned keys as specified, drops both, and returns
exactly -1, 0, or 1. Invalid native pointer/length products cannot be reported through SQLite's
collation ABI; they are a native-contract violation and call `process.abort` before any source
callback. No Align unwind crosses C.
...
SQLite's case-insensitive name plus exact arity and UTF-8 encoding. Collation identity is SQLite's
case-insensitive UTF-8 name. Registration deliberately follows SQLite replacement semantics for an
...
The package passes null `pApp`/`pArg` and null `xDestroy` for every registration. Descriptor and
trampoline storage is immutable program-lifetime data, so replacement, removal, close, and either
native registration failure have no application allocation or destructor edge. This also avoids
depending on SQLite's deliberately different failed-registration destructor rules for functions
and collations.
...
4. for functions, validate arity, then the option slice in source order;
5. authenticate the complete descriptor and require the operation's kind;
6. require Pure for `Deterministic` or any collation key;
7. load the guarded trampoline and make exactly one native registration/removal call;
8. re-prove native autocommit and wrapper idle state before success; poison/close on contradiction.
...
| post-call idle proof fails | first native error if present; otherwise `Connection(ContractError { query_id: None, item: "sqlite.callback.cleanup", message: "SQLite callback registration left the connection unusable" })` |
...
[`sqlite3_create_collation_v2`](https://sqlite.org/c3ref/create_collation.html), and
...
| scalar trampoline | Validate context/argc/argv and all values before one callback; preserve ordered invocation views and returned provenance; map every value/result/error exactly once with transient copies and no package heap. | argc -1/0/127/128 and argv null products; every storage class/value variant; malformed pointer/length/UTF-8/NUL/NaN; Ok/Err/hard-error IR; call/result/allocation counters |
| collation trampoline | Require null app data and valid byte views; invoke one Pure key target left then right; compare owned keys byte-lexicographically; drop both and return -1/0/1. | empty/NUL/non-UTF-8/high-byte/prefix/equal keys; allocation/Drop counts; antisymmetry/equality/transitivity parameterized owner; impure/captured target rejection; invalid native product hard-error IR |
| registration and removal | Enforce exact connection/version/name/arity/option/descriptor order, UTF-8/DIRECTONLY flags, deterministic proof, one native call, replacement/removal identity, and post-call idle proof. | pairwise multi-invalid no-call matrix; SQLite 3.29.99/3.30.0/newer; name 0/1/255/256/NUL; arity -1/0/127/128; options; built-in/user replacement/removal; autocommit before/after; fake-native call order |
| lifetime, cleanup, and reentrancy | Retain only program-lifetime trampoline data; pass null application/destructor pointers; keep callbacks connection-local through replace/remove/close; forbid pool-origin registration and access to the invoking connection; preserve first error and poison once on failed reuse proof. | registration failure/replace/remove/close destructor-zero counters for both APIs; direct/pool/tx/lease/dependent-resource matrix; typed/dynamic invocation; distinct-connection nested callback; no same-connection source route; poison/no-call-after-close |
...
105. SQLite callbacks are formed only from one exact noncapturing target through nominal static
...
107. Collations use a proved-Pure `collation_input -> array<u8>` key callback; the package alone
     compares and drops keys, so safe source cannot publish an order-law-violating C comparator.
108. Callback registration is fixed-arity, UTF-8, connection-local, direct-SQLite-only state. It
...
The minimal common dynamic `db.value`/`db.row` set and the SQLite scalar-function/collation
callback surface are settled by §19 and the A2 ledgers in §23. They do not settle the wider
logical/native type mappings above or the separate PostgreSQL callback surface.
