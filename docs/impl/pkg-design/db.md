...
| allocation and ABI parity | Query direct/prepared paths carry the same plan into v2 statement and rows states; fake, SQLite, and PostgreSQL use one v1 batch state and one producer plan layout. Every size/offset/signature and cleanup provenance agrees across producer, package, HIR validator, MIR, and LLVM lowering. LLVM preflight recognizes the v5 Query descriptor's offset-128 nested plan as one semantic record: it validates the exact 72-byte image, callback ordinal set, stored declaration signatures, Row/rows/batch resource pairing, field count, and SoA flag before emission. It accepts `ColumnBatchSoa` only for the same nonempty concrete `SoaPlain` struct accepted by sema; zero fields cannot pass by vacuous field iteration. | descriptor/plan/state byte goldens, parameterized wrong-signature tests for every plan callback ordinal, static-data relocation tests, HIR ABI tests, empty-struct `ColumnBatchSoa` rejection, and alloc-count parity probe |
