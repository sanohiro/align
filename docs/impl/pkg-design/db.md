...
schema, compares the complete history to that snapshot, replaces all history rows with that
snapshot, and rereads it. Only an unchanged observed snapshot proceeds to Applied or Failed; any
row change restores Applying and fails visibly. If the worker removed the owned table, this migrate
