...
COPY, pipeline, or unknown numeric statuses. Both change no public surface or ABI and must merge in
that order. The third PR adds direct delivery, the rows ABI, and the streamed protocol below. The
...
correctness repair is a separate public/ABI-neutral prerequisite after the lease repair. The
...
| PostgreSQL result-status safety prerequisite | Audit every shipped package-owned PGresult consumer, including synchronous and timeout direct/prepared rows, one, command, prepare, transaction boundary, catalog, EXPLAIN, recovery drain, and silent cleanup. Before clear, retain the consumer's exact existing non-success error; an earlier error stays primary and silent cleanup stays silent. COPY, `PGRES_PIPELINE_SYNC`, `PGRES_PIPELINE_ABORTED`, or an unknown numeric status then clears the current result exactly once, immediately poisons/closes, performs no later result/COPY/pipeline-exit/cancel/transaction-state/blocking-restore call, cleans every owner, and never permits reuse. Add no public symbol, error surface, ABI field, delivery option, libpq-17 dependency, or rows-state change. | parameterized every-consumer COPY/pipeline/unknown injection and no-call/clear-close/error-identity owners, retained Q2/Q4a/Q4b/Q5b2 suites, local database gate, normal code preflight/review/CI |
...
| Review claim | Disposition and redesign | Required owner/source |
|---|---|---|
| COPY fail-close covered only future streamed executors | Valid. Shipped BufferedFull, command, prepared, one, timeout recovery, catalog/EXPLAIN, transaction, and silent-cleanup consumers can already observe a COPY result. Split one public/ABI-neutral result-status prerequisite that closes the complete consumer class before streaming. | every shipped PGresult consumer x COPY status x synchronous/timeout/recovery/Drop no-call/clear-close/no-reuse matrix |
...
| introductory staging still named one prerequisite and three PRs | Valid. Name both public/ABI-neutral prerequisites and the exact lease -> status -> direct -> prepared sequence in English and Japanese. | ledger/closure/staging/mirror four-boundary consistency sweep |
