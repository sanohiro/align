...
| Closure cell | Required implementation closure | Exact owner evidence |
|---|---|---|
| public type and option formation | Add every §18 `MetaDetail`, metadata discriminator, ref, and exact flat common record declaration plus the D12-owned common/SQLite/PostgreSQL metadata and EXPLAIN option sums. Publish only `meta_query` in Q5b1; the remaining exact signatures stay deferred together to Q5b2 rather than acquiring stubs or hidden overloads. | `pkg_db_q5b1::q5b1_publishes_exact_query_surface_without_catalog_stubs` |
...
| explicit Q5b2 deferral | Database/schema/table/column/key/index catalog operations, typed identifier U+0000 precedence, native option application, canonical constraint/index ordering, and common/native EXPLAIN including visible PostgreSQL ANALYZE remain one subsequent Q5b2 capability. Q5b1 does not claim those D12 acceptance cells or add placeholder implementations. | `pkg_db_q5b1::q5b1_publishes_exact_query_surface_without_catalog_stubs` |
