...
| FFI and native-version closure | Declare the exact libpq 17 send, row-mode, result, nonblocking cancel-connection, transaction-state, and cleanup signatures/constants. Pin signedness for native `int` chunk size and every polling/status tag with compiled C signature probes. This rail adds no COPY data/termination or pipeline-exit FFI; COPY, pipeline, and unknown-status behavior is current-result clear plus ordinary connection close. Keep ordered `pq`/`ssl`/`crypto` closure on whole/per-unit Linux and macOS links; reject a client below 17 at build/evidence setup rather than at first request and fail closed on a newer unknown status. | C signature/status/unknown-sentinel probe; forbidden COPY/pipeline-exit symbol/call inventory; client-version assertion; whole/per-unit link inventory; x86_64/ARM64/macOS build owners |
