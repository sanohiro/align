...
| namespaced builtin type identity | Generalize type lookup rather than renaming `db.Error`: a non-entry module may own a local type named by a compiler-provided bare alias, bare lookup prefers that same-module declaration, and the builtin remains available by its provider-qualified spelling (`core.Error`). Keep entry-module canonical collisions rejected. No package name appears in the rule. | same-module local `Error` signatures/constructors, qualified importer construction/match, explicit `core.Error` use beside the local type, unchanged bare builtin fallback elsewhere, entry collision rejection, whole/per-unit interface and executable parity |
