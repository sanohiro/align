...
| physical delivery and order | `SingleRow` accepts only `PGRES_SINGLE_TUPLE` data results containing exactly one row. `PortalBatch(n)` accepts only `PGRES_TUPLES_CHUNK` data results containing `1..=n` rows. Both preserve server row order. After zero or more data results, require one zero-row `PGRES_TUPLES_OK`, clear it, then call `PQgetResult` until null before declaring exhaustion, exactly as [libpq 17 §32.6](https://www.postgresql.org/docs/17/libpq-single-row-mode.html) specifies. The complete libpq 17 disposition is fixed: `PGRES_BAD_RESPONSE`, `PGRES_NONFATAL_ERROR`, and `PGRES_FATAL_ERROR` use the existing `execution_error` SQLSTATE/native-detail mapping. `PGRES_EMPTY_QUERY`, `PGRES_COMMAND_OK`, `PGRES_PIPELINE_SYNC`, `PGRES_PIPELINE_ABORTED`, an unknown numeric status, the other mode's data status, an empty/oversized data result, a nonempty/missing terminal result, or any result after the terminal one records the exact `postgres.rows.delivery` invalid-sequence error below, clears that result once, and continues the ordinary no-decode drain. `PGRES_COPY_OUT`, `PGRES_COPY_IN`, and `PGRES_COPY_BOTH` are instead fail-closed statuses because COPY termination is outside this rail: record that invalid-sequence error only when no earlier owned error exists, clear the current result exactly once, and immediately poison/close the physical connection. After observing COPY, never call `PQgetResult`, `PQgetCopyData`, `PQputCopyData`, `PQputCopyEnd`, a cancel API, `PQtransactionStatus`, or blocking-mode restoration on that connection. No cancel handle may remain owned when main-result classification begins. Clear the wrapper's mutable native state, destroy any unpublished package-owned batch, make an unpublished cloned Row unreachable without rewinding `out`, free the binder context and any unpublished direct Query-ID copy, and release the lease only after close. Earlier published values remain visible; an earlier validation/decode/storage/mode/timeout error remains primary, and Drop remains silent. No COPY, command, pipeline, or multi-statement status is reinterpreted as rows. |
...
| result advancement | Put PGresult acquisition/status/count/current-row transitions in one helper used by `next`, `has_next`, and `next_batch`. Clear each ordinary result exactly once, implement the exhaustive libpq 17 status table and mode-specific sequence, and never fetch past a caller-visible bound. COPY is a distinct terminal-to-Align fail-closed branch: clear only that current result, close without another protocol/cancel/probe/restore call, then clean owners and release the lease. `one_native` validates/decodes and clones only the first Row, records multiplicity only as pending state, drains without decoding further rows, and publishes Cardinality only after clean completion; any later native/timeout/cleanup error wins. Once cloned, caller-region bytes remain allocated on every non-success outcome because an arena is never rewound. Audit all three consumers and every cleanup caller whenever this helper changes. | parameterized every-status/count/sequence owner, COPY first/after-data/every-cleanup-cause no-call/clear/close counters, zero/one/many and one/two-fatal/two-clean/DML-effect owners, plus `out` allocation byte/alignment/clone-count parity for singleton and every post-clone failure/Cardinality |
...
| FFI and native-version closure | Declare the exact libpq 17 send, row-mode, result, nonblocking cancel-connection, transaction-state, and cleanup signatures/constants. Pin signedness for native `int` chunk size and every polling/status tag with compiled C signature probes. This rail adds no COPY data/termination FFI; its only COPY behavior is current-result clear plus ordinary connection close. Keep ordered `pq`/`ssl`/`crypto` closure on whole/per-unit Linux and macOS links; reject a client below 17 at build/evidence setup rather than at first request. | C signature/status probe; forbidden COPY-symbol/call inventory; client-version assertion; whole/per-unit link inventory; x86_64/ARM64/macOS build owners |
...
The four-times-reopened implementation boundary remains three independently mergeable vertical
PRs. The error-drain and COPY fail-closed invariants cannot form another mergeable prerequisite:
publishing any explicit direct stream without them would be incorrect, while the helper has no
independently callable surface or consumer before that stream exists. Restoring the settled direct
validation phase order changes only that direct constructor and likewise has no prerequisite
consumer. The lease repair remains the sole independently useful pre-stream closure.
...
Run one fresh independent adversarial review of this revised four-times-redesigned ledger and the unchanged
three PR boundaries
