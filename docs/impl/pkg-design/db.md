...
count thunk/copy for prepared execution) and `2_147_483_647`. `format_plan` is null exactly when
`format_count == 0`;
...
Both v3 constructors initialize all 96 bytes explicitly. The complete validator checks this
...
Length bounds apply to the selected encoded wire payload, not merely the source view. Binary bytea
accepts source and wire length `2_147_483_647` subject to the language's ordinary allocation rule and
rejects `2_147_483_648` before allocation or send. Text bytea has wire length `2 + 2 * source_len`,
so it accepts source length `1_073_741_822` and rejects `1_073_741_823`; the trailing C sentinel is
outside the recorded wire length but remains in checked allocation arithmetic. Text strings accept
UTF-8 byte length `2_147_483_647` subject to the same ordinary allocation rule and reject the next
length. Parameterized length-preflight owners use an allocation/send stub to prove each accepted-max
and rejected-max-plus-one pair without constructing a multi-gigabyte fixture.
...
| static type proof and resolver ABI | Bump the 8-aligned execution descriptor from 136-byte v5 to exact 144-byte v6. Offsets other than 104 and the new 136 retain v5 meaning. Offset 104 becomes the non-null generated parameter-resolver v2: zero means unknown, a positive ordinal means known but Text-only, and the corresponding negative ordinal means the parameter has one exact compatible PostgreSQL `ParameterType` from the table above and may use Binary; `i32::MIN` and an absolute ordinal outside `1..=parameter_count` are malformed. Offset 136 is a non-null generated `fn() -> u32` parameter-count v1 thunk whose body is exactly one constant return equal to offset 112's existing distinct-parameter count and at most `2_147_483_647`. The complete descriptor validator compares those two counts before accepting the resolver. Existing sealed `parameter_known` accepts either resolver sign and existing `parameter_ordinal` operations return the absolute ordinal. Add a sealed direct binary-ordinal operation that returns the absolute ordinal only for the negative form and zero otherwise. The prepared sibling additionally requires the retained effective-OID eligibility bit described below, so a differing `ParameterOid` override cannot reuse a stale static proof. Application calls, wrong descriptor/resource types, unguarded prepared loads, count-thunk invocation outside the complete header guard, cross-descriptor resolver/count-thunk splices, and malformed HIR fail closed. Query/command formation, interface serialization, monomorphization, whole-program, and per-unit compilation retain the matching resolver v2 and count thunk from one producer generation. | exact v6 header/function-body/relocation goldens; offset-count/thunk equality and every zero/max/mismatch/malformed thunk product; every type and missing-`ParameterType` resolver code; matching/differing/absent `ParameterOid` proof owners; sema/MIR application/wrong-kind/unguarded/splice/malformed rejection; whole/per-unit runtime-selected same-typed descriptor twins |
...
| package context and prepared ABI | The shared opaque binder/decode context becomes one exact 96-byte v3 record for both drivers. Offsets 0--79 retain v2 meaning; offset 80 is result format `u8` (SQLite requires `0=Text`; PostgreSQL allows `0=Text`, `1=Binary`); offset 81 is metadata state (`0=unvalidated`, `1=validated`); 82--87 are zero; and offset 88 is the metadata native pointer. State 0 requires offset 88 null. A sealed native-generation setter installs offset 8 and resets 81/88 before every row-producing SQLite Query execution and every newly acquired tuple-producing PGresult, including zero-row terminal results. Successful metadata validation stores state 1 and the exact non-null offset-8 pointer; value validation requires that equality. Clearing/replacing native state resets both fields, so pointer-address reuse cannot inherit validation. The one context validator rejects every driver/version/format/state/reserved/pointer product before dereference. Shared prepared statement state becomes an exact 112-byte, 8-aligned v4 record: offsets 0--79 retain v3 meaning; offset 80 retains resolver v2; offset 88 is a package-owned byte-per-parameter Binary eligibility vector; offset 96 is the non-null producer-owned parameter-count v1 thunk from descriptor offset 136; offset 104 is the copied exact `u32` parameter count; and 108--111 are zero. The one complete statement-header validator first establishes the fixed fields and non-null thunk, invokes that guarded thunk, requires a result at most `2_147_483_647` exactly equal to offset 104, and only then uses that trusted result to bound eligibility validation. SQLite requires a null eligibility pointer for every count. PostgreSQL requires null eligibility for zero parameters and otherwise a non-null vector with exactly the trusted count's readable bytes, all 0/1. At PostgreSQL preparation, byte `ordinal-1` is one exactly when static validation installed a supported nonzero canonical OID and the effective OID after any `ParameterOid` override equals it. Arbitrary nonzero overrides remain valid for Text, but a differing override makes that ordinal Binary-ineligible. The sealed prepared binary-ordinal bridge requires a complete guarded v4 header, negative resolver result within the trusted count, and eligibility byte one. HIR statement formation requires resolver, count thunk, binder, row validator, decoder, and batch plan from one Query producer. Statement Drop nulls the vector/thunk/count before freeing the vector once. The context/statement validators, every accessor/bridge/constructor/Drop path, and semantic/byte goldens move together. | driver x context v3/result-format/metadata-state/reserved/native-pointer matrix with new-result pointer-reuse reset; statement v4 both-driver active/closed/malformed bytes; thunk null/body/count mismatch/max and eligibility pointer/byte products with no out-of-bound scan/callback; cross-Query thunk/resolver splice; matching/differing/absent override; constructor/accessor/Drop no-call and free counters; cumulative SQLite prepared/rows suite |
...
| parameter encode and retention | Run generated static validation before applying the normalized plan on direct execution; prepared execution requires both the retained negative resolver proof and its retained effective-OID eligibility bit. Binder callbacks read only the selected code for their own ordinal and encode exactly the table above; no ordinal may broadcast its code to a sibling or mutate result format. Enforce the exact selected-wire length bounds above before allocation and send. Each non-null scalar/text/bytea value has exactly one execution-owned payload allocation and NULL has none. A nonempty Binary bytea allocates exactly `len` bytes; an empty Binary bytea allocates one zero sentinel byte, passes its non-null pointer with recorded length zero, and remains distinct from `None`. Direct and prepared BufferedFull retain through synchronous completion; SingleRow/PortalBatch retain until `PQgetResult` returns null or fail-closed cleanup destroys the context. Every partial bind or option-plan failure frees installed values once and sends nothing. | per-type/nullable wire-byte and length/format/OID goldens; every ordered two-parameter default/explicit-Text/explicit-Binary pair plus three-parameter heterogeneous control; direct/prepared `Some(empty bytea)` versus `None`; U+0000; Binary bytea/text/Text-bytea accepted-max and rejected-max-plus-one preflight twins with zero allocation/send on rejection; mutation/source-Drop retention; partial-bind allocation/free matrix; synchronous/streamed context lifetime counters |
...
| artifact, cache, and documentation identity | Application-facing module interfaces and static Query semantic fingerprints do not change because the Text/Binary surface already exists. The added/changed exported compiler-private `pkg.db.internal.postgres` prevalidated signatures change that internal module's interface hash and every importing module's dependency-interface key once. Descriptor v6's 144-byte image, row-validator-v2/resolver/count-thunk bodies, package callback names/semantics, rows v4's 120-byte implementation, statement v4's 112-byte implementation, and shared context v3's 96-byte implementation also change their exact compiler/package implementation hashes and invalidate affected codegen/object/dependency cache entries once. Checked artifacts, metadata, SQL identity, and driver restriction remain unchanged. English/Japanese package designs and the live handoff agree on the next/shipped boundary. | internal-module interface-hash and importer dependency-key before/after twins; descriptor semantic-byte parity plus row-validator/resolver/count-thunk relocation/body and implementation/cache invalidation twins; rows 120-byte/statement 112-byte/context 96-byte goldens; application-interface/artifact/metadata identity controls; English/Japanese consistency check; handoff updated only with the implementation milestone |
| acceptance and measurement | Require merged D8/D9 streaming/cancellation, A1 batch and direct/prepared delivery, libpq client >=17, and the required PostgreSQL server job. Parameter format and result format are independent axes: for two named Query parameters, cross every ordered pair from `{default Text, explicit Text, explicit Binary}` (nine plans) with `{default Text, explicit Text, explicit Binary}` result format (27 products) on both direct and prepared stub paths; include a three-parameter heterogeneous control and the applicable direct-command parameter/result-code cases. Separately cross every mapped type/nullable form through Text and Binary bind/decode, plus BufferedFull and both explicit delivery modes. Cross timeout absent/present, Conn/Tx, rows/one/command where callable, zero/one/many rows, valid/malformed zero-row and nonempty metadata/payload, and whole/per-unit linking without collapsing the two format axes. Run `scripts/db-verify-local.sh` before push. Record the named local Text-versus-Binary parameter/row measurement with payload bytes, copied bytes, allocations, time-to-first-row, and full-scan throughput after correctness; it is not a semantic gate. | focused 27-product Query format-plan/result owner plus command and mixed live controls; metadata-only zero-row/new-generation owners; per-type package/compiler owners; retained `pkg_db_q4b`/`pkg_db_a1`/`pkg_db_q5b2`; required live PostgreSQL suite, local database parity gate, and non-gating measurement record |
