...
| required PostgreSQL CI found first-run bootstrap treated only an absent table as missing history | Every blind history lock now recognizes both PostgreSQL absent-object SQLSTATEs: `42P01` for a missing table and `3F000` for a missing schema. The following exact inventory query still decides whether both owned objects are absent. |
