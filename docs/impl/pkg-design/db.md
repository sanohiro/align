...
| prerequisite result-status safety closure | Before changing the stream ABI or libpq mode, route every shipped package-owned PGresult consumer through one exhaustive sealed `apps/db/pkg/db/internal/postgres_status.align` authority shared by `postgres.align`, internal execution, and resource cleanup without an import cycle. Before clear, copy the exact error selected by that consumer's existing non-success mapping; a previously owned error remains primary and silent cleanup remains silent. COPY or an unknown numeric status then clears the current result exactly once, immediately poisons/closes, and permits no later result/COPY/cancel/transaction-state/blocking-restore call. Clean all owners and the lease after close and never permit reuse; do not invent a new public error category, item, message, or query identity. | independently mergeable prerequisite PR; exact consumer/error inventory; sealed-authority exported-surface negative; COPY/unknown x synchronous/timeout-completion/recovery/Drop no-call/clear-close/error-identity matrix; retained Q2/Q4a/Q4b/Q5b2 owners |
...
| result advancement | Put PGresult acquisition/status/count/current-row transitions in one helper used by `next`, `has_next`, and `next_batch`. Clear each ordinary result exactly once, implement the exhaustive known libpq 17 status table and mode-specific sequence, and never fetch past a caller-visible bound. COPY and an unknown numeric status are distinct terminal-to-Align fail-closed branches: clear only that current result, close without another protocol/cancel/probe/restore call, then clean owners and release the lease. `one_native` validates/decodes and clones only the first Row, records multiplicity only as pending state, drains without decoding further rows, and publishes Cardinality only after clean completion; any later native/timeout/cleanup error wins. Once cloned, caller-region bytes remain allocated on every non-success outcome because an arena is never rewound. Audit all three stream consumers, every cleanup caller, and the merged status-safety prerequisite whenever this helper changes. | parameterized every-status/count/sequence owner, COPY/unknown first/after-data/every-cleanup-cause no-call/clear-close counters, zero/one/many and one/two-fatal/two-clean/DML-effect owners, plus `out` allocation byte/alignment/clone-count parity for singleton and every post-clone failure/Cardinality |
...
| FFI and native-version closure | Declare the exact libpq 17 send, row-mode, result, nonblocking cancel-connection, transaction-state, and cleanup signatures/constants. Pin signedness for native `int` chunk size and every polling/status tag with compiled C signature probes. This rail adds no COPY data/termination FFI; COPY and unknown-status behavior is current-result clear plus ordinary connection close. Keep ordered `pq`/`ssl`/`crypto` closure on whole/per-unit Linux and macOS links; reject a client below 17 at build/evidence setup rather than at first request and fail closed on a newer unknown status. | C signature/status/unknown-sentinel probe; forbidden COPY-symbol/call inventory; client-version assertion; whole/per-unit link inventory; x86_64/ARM64/macOS build owners |
...
The latest consumer sweep proved that COPY and unknown-status fail-closed behavior is not confined
...
| PostgreSQL result-status safety prerequisite | Audit every shipped package-owned PGresult consumer, including synchronous and timeout direct/prepared rows, one, command, prepare, transaction boundary, catalog, EXPLAIN, recovery drain, and silent cleanup. Before clear, retain the consumer's exact existing non-success error; an earlier error stays primary and silent cleanup stays silent. COPY or an unknown numeric status then clears the current result exactly once, immediately poisons/closes, performs no later result/COPY/cancel/transaction-state/blocking-restore call, cleans every owner, and never permits reuse. Add no public symbol, error surface, ABI field, delivery option, libpq-17 dependency, or rows-state change. | parameterized every-consumer COPY/unknown injection and no-call/clear-close/error-identity owners, retained Q2/Q4a/Q4b/Q5b2 suites, local database gate, normal code preflight/review/CI |
