...
| artifact, cache, and documentation identity | Public module interfaces and static Query semantic fingerprints do not change because the Text/Binary surface already exists. Descriptor v6's 144-byte image, row-validator-v2/resolver/count-thunk bodies, package callback names/semantics, rows v4's 120-byte implementation, statement v4's 112-byte implementation, and shared context v3's 96-byte implementation change their exact compiler/package implementation hashes and invalidate affected object/dependency cache entries once. Checked artifacts, metadata, SQL identity, and driver restriction remain unchanged. English/Japanese package designs and the live handoff agree on the next/shipped boundary. | descriptor semantic-byte parity plus row-validator/resolver/count-thunk relocation/body and implementation/cache invalidation twins; rows 120-byte/statement 112-byte/context 96-byte goldens; artifact/metadata identity controls; English/Japanese consistency check; handoff updated only with the implementation milestone |
