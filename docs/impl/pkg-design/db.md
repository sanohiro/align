...
control/PostgreSQL-COPY rejection in statement order, and finally Forbidden-count validation.
...
the first prohibited statement in source order wins; transaction-control classification precedes
COPY classification for that statement, then the Forbidden count check runs. SQLite statement
...
| catalog and policy screening | Reuse the Q3 `ALIGNMIG` catalog bytes/digest. Parse the exact first physical line, count complete driver statements, and reject empty, transaction-control, NUL, invalid UTF-8, and multi-statement Forbidden files before target mutation. PostgreSQL additionally rejects every top-level `COPY` statement after lexical completeness and before target open with the exact §17.6 diagnostic; quoted/comment/dollar-body COPY text remains data. | cumulative Q3 catalog goldens plus `pkg_db_q5a::migration_policy_and_statement_screening_is_exact` and `postgres_migration_copy_is_rejected_before_native_work` |
...
| physical delivery and order | `SingleRow` accepts only `PGRES_SINGLE_TUPLE` data results containing exactly one row. `PortalBatch(n)` accepts only `PGRES_TUPLES_CHUNK` data results containing `1..=n` rows. Both preserve server row order. After zero or more data results, require one zero-row `PGRES_TUPLES_OK`, clear it, then call `PQgetResult` until null before declaring exhaustion, exactly as [libpq 17 §32.6](https://www.postgresql.org/docs/17/libpq-single-row-mode.html) specifies. The complete known libpq 17 disposition is fixed: `PGRES_BAD_RESPONSE`, `PGRES_NONFATAL_ERROR`, and `PGRES_FATAL_ERROR` use the existing `execution_error` SQLSTATE/native-detail mapping. `PGRES_EMPTY_QUERY`, `PGRES_COMMAND_OK`, the other mode's data status, an empty/oversized data result, a nonempty/missing terminal result, or any result after the terminal one records the exact `postgres.rows.delivery` invalid-sequence error below, clears that result once, and continues the ordinary no-decode drain. `PGRES_COPY_OUT`, `PGRES_COPY_IN`, `PGRES_COPY_BOTH`, `PGRES_PIPELINE_SYNC`, `PGRES_PIPELINE_ABORTED`, and every numeric status outside the complete known libpq 17 set are instead fail-closed because their protocol or connection-global state cannot be assumed drainable and reusable without the deferred COPY or pipeline APIs: record that invalid-sequence error only when no earlier owned error exists, clear the current result exactly once, and immediately poison/close the physical connection. After observing one of those statuses, never call `PQgetResult`, `PQgetCopyData`, `PQputCopyData`, `PQputCopyEnd`, `PQexitPipelineMode`, a cancel API, `PQtransactionStatus`, or blocking-mode restoration on that connection. No cancel handle may remain owned when main-result classification begins. Clear the wrapper's mutable native state, destroy any unpublished package-owned batch, make an unpublished cloned Row unreachable without rewinding `out`, free the binder context and any unpublished direct Query-ID copy, and release the lease only after close. Earlier published values remain visible; an earlier validation/decode/storage/mode/timeout error remains primary, and Drop remains silent. No COPY, unknown, command, pipeline, or multi-statement status is reinterpreted as rows. |
...
| prerequisite Rust tool result-status safety closure | Route migration `execute_raw`/`query` and preparation `execute_command`/`query_rows`/`PQprepare`/`PQdescribePrepared` through one private exhaustive Rust status authority before any result field/row access or follow-up SQL. A null result, COPY, partial single/chunk row result, pipeline status, or unknown numeric status copies the available existing diagnostic, clears the present current result once, finishes and nulls the mutable connection owner, and permits no later libpq call; known complete results preserve current mappings. Canonical PostgreSQL migration screening additionally rejects first-token COPY with the exact §17.6 diagnostic before URL/native work. | `postgres_tool_results_fail_closed_before_followup_native_work`; module x consumer x null/COPY/single/chunk/pipeline/unknown clear-close/error/no-row-access/no-rollback/no-deallocate matrix; `postgres_migration_copy_is_rejected_before_native_work`; prepare/migration SQL-origin inventory |
...
| result advancement | Put PGresult acquisition/status/count/current-row transitions in one helper used by `next`, `has_next`, and `next_batch`. Clear each ordinary result exactly once, implement the exhaustive known libpq 17 status table and mode-specific sequence, and never fetch past a caller-visible bound. COPY, pipeline, and unknown numeric statuses are distinct terminal-to-Align fail-closed branches: clear only that current result, close without another protocol/pipeline-exit/cancel/probe/restore call, then clean owners and release the lease. `one_native` validates/decodes and clones only the first Row, records multiplicity only as pending state, drains without decoding further rows, and publishes Cardinality only after clean completion; any later native/timeout/cleanup error wins. Once cloned, caller-region bytes remain allocated on every non-success outcome because an arena is never rewound. Audit all three stream consumers, every cleanup caller, and the merged status-safety prerequisite whenever this helper changes. | parameterized every-status/count/sequence owner, COPY/pipeline/unknown first/after-data/every-cleanup-cause no-call/clear-close counters, zero/one/many and one/two-fatal/two-clean/DML-effect owners, plus `out` allocation byte/alignment/clone-count parity for singleton and every post-clone failure/Cardinality |
