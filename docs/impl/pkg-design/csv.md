...
| Parallel-worker eligibility | `CsvDecode` remains Pure for sequential in-memory use, but `region` is non-Send independently of effect. One shared fail-closed source/HIR capture authority rejects an exact `ArenaHandle` from every parallel worker boundary, including `spawn` and `ArrayParMap`, before lifted-worker publication, MIR, generated identity, runtime call, or allocation. Codegen defensively rejects the same type in handcrafted parallel MIR before context layout or kernel/runtime publication. Sequential direct calls, local closures, `map`, and `reduce` may retain the capability under the ordinary lexical region rule. | Direct and helper-wrapped `decode` captures in `spawn` and `par_map`; raw malformed `ArrayParMap` HIR and `ParMapParallel`/`ParMapReduce` MIR `ArenaHandle` captures; whole-program/per-unit and generic monomorph parity; existing spawned-region negative retained through the shared authority; sequential Pure direct/closure/map/reduce positive twins; failing paths prove no worker/capability/context/kernel/call/allocation publication. |
...
- Pure-effect replay and the independent non-Send capture rule reject `region` at every `spawn` and
  `par_map` worker boundary while leaving sequential calls and closures admitted;
