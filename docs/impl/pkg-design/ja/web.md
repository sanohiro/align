...
`/a/featured/versions` を `:id` 行にルーティングする。試行順の優先度とスコア fold の base-3
左→右支配により、最初に成功した枝 = 線形オラクルの最大 `match_score` が**全テーブル**で成立 —
構築時の曖昧性 abort を要するルート集合の形は存在しない（旧稿は「バックトラックなし + abort」
としていたが、その abort はまさにこの種の現実的なテーブルを拒否してしまい、tree 化以前の本番
ディスパッチ = 線形スキャンはそれらを正しくマッチしていた）。(method, path) 重複行と param 名
衝突は引き続き構築時 abort。`web.param(c, "name")` = 高々 n_params 個の名前 view の線形走査
（n は極小。map なし）。
