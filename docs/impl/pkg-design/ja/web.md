...
//   workers >  process.cpu_count() -> 同じ abort（実装中に発見）: worker は決して return せず、
//                    task_group は利用可能並列度でサイズされたプール上でタスクを走らせるので、その
//                    数を超えたタスクはそもそも起動しない。黙って少ないループで serve する代わりに
//                    abort し、このパラメータが与える約束を守る。
```
