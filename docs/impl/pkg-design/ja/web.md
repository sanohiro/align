...
**出荷済み（W1–W7 完了、2026-07-22）。** 本書は現在のパッケージ契約であり、実装に至った
設計理由も残している。失われた会話記録から復元し、二度と
...
だけ」）— 本設計を規定しない。過去の実行計画: `../../15-pkg-web-plan.md`。以前のハードな
コンパイラ前提 **F1 フィールド許可拡張**は出荷済み（下の土台の節参照）。
...
- **F0 — pkg-foundation 規則。** `internal`、pkg 層 import 検査、パッケージ仕様が出荷され、
  radix 実装を置く `pkg.web.internal.*` が利用可能になった。
- **std.http 床。** query アクセス/percent-decode と WHATWG SSE フレーミングは、
  消費者向け HTTP 作業とともに出荷済み。
...
## multipart/form-data — 兄弟モジュール、2026-07-22 出荷

`pkg.web.multipart`（`apps/web/pkg/web/multipart.align`）は RFC 7578 のアップロードボディを歩く。
計画の REST 完全性バックログに残っていた最後の `TODO` を閉じるものだ。ファイルアップロードは
Web サーバの標準機能なので**作る** — ただし、あるべき場所に作る。

**なぜ兄弟モジュールでコア表面ではないのか。** 上の最小主義制約が、ルート表面をルーティング・
アクセサ・レスポンダ・middleware・ストリーミングに固定している。multipart はボディの**コーデック**
であってルーティングの関心事ではない — フォームが乗ってきてもディスパッチは何も変わらない。
よって `pkg.web.cookie` / `pkg.web.cors` の公開兄弟として出荷する。`internal` ではなく公開なのは
`Part`/`Step` がアプリケーションの**名前で呼ぶ**型だからであり、`web.align` に配線しないのは
モジュールを依存ゼロに保つためだ（import は皆無で、触るのは `slice<u8>` と `str` だけ）。両者の
接続はアプリケーションが自分で書く — `pkg.web` が既に渡している `header(c, "content-type")` と
`body(c)` を渡すだけであり、リクエストのバイトがフレームワーク側に迂回することはない。
...
//   `multipart/form-data` の Content-Type 値の boundary。歩けるマルチパートフォームでなければ ""。
//   `content_type` へのビュー。
multipart.next(body: slice<u8>, boundary: str, from: i64) -> Step
//   `from` 以降の次のパート。`from` は区切り子の先頭 '-' のバイト位置。呼び出し側は 0 から始め、
//   以後は `Part.next` をそのまま渡し返す。
```
...
fn upload(c: pkg.web.types.Ctx) -> Result<response_builder, Error> {
  ...
  if bd.len() == 0 {
    return pkg.web.status(400)                     // マルチパートフォームではない
  }
  body := pkg.web.body(c)
  mut off := 0
  mut files := 0
  loop {
    match pkg.web.multipart.next(body, bd, off) {
      ...
      Done => { break }
      Invalid => { return pkg.web.status(400) }    // 壊れたボディ
    }
  }
...
- **ゼロアロケーション・ゼロコピー — 不変条件 1 はコーデックを貫通する。** 返る name / filename /
  content-type / data はすべて呼び出し側の入力へのビューなので、200 MB のアップロードは既に届いて
  いるバッファ以上のコストを持たない。反復の状態はパーサハンドルではなく呼び出し側が持つ `i64`
  一個（`Part.next`）— これが、この walk を drop する物のない純関数の素朴な `loop` に保っている。
- **結果は 3 つ、明示的に。** `Result<Option<Part>, Error>` は現状表現できない（集約ペイロードの
  入れ子）し、`Done` を `Invalid` に潰すと**切り詰められた**ボディが整った反復終了に見えてしまう
  — 途中で切れたアップロードを完全なファイルとして保存させる、まさにその取り違えだ。だから
  `Step` は or-kind であり、`types.Middleware` と同じ理屈で導かれている。
- **エスケープは拒否する、推測しない。** `\` の quoted-pair を含む quoted な `filename`/`boundary`
  は、送信側が意図したバイト列と**異なる**。ゼロコピーで返せないし、エスケープされたまま返せば
  誤ったファイル名を黙って渡すことになる。よって `boundary` は `""`、`next` は `Invalid` を返す。
  アンエスケープはアロケーションを意味する — このモジュールが呼び出し側に隠れてやらない唯一の
  ことだ。
- **区切り子は行そのもの**（RFC 2046 §5.1.1）: CRLF、`--`、boundary、任意の transport padding、
  CRLF — 終端区切り子なら `--`。行全体を検査することが、boundary で**始まるだけ**のデータ行
  （boundary が `SEP` のときの `--SEPARATE`）をパート内に留め、正当な内容をそこで切らずに済ませる。
  最初の区切り子より前の preamble と終端区切り子より後ろの epilogue は RFC どおり無視する。
- **HTTP が決着している所では厳格に。** `name` は必須（RFC 7578 §4.2）、disposition は `form-data`
  でなければならず、`obs-fold` の継続行は解釈せず拒否し（RFC 9110 §5.2 が削除した機能であり、
  折り畳まれたヘッダの意図を推測することがスマグリングバグの出発点だ）、**引用符なし**のパラメータ値は
  RFC 9110 §5.6.6 の `token` でなければならず（`boundary=A,B` や `boundary=abc def` は拒否する。
  Go の `mime` は両方を拒否するので、受理すれば手前のプロキシとこちらでパートの区切り位置の解釈が
  食い違う）、フィールド名とパラメータ属性は ASCII のみ大小同一視する。未知のヘッダはバイト比較で
  読み飛ばすので有効な UTF-8 である必要すらない。**認識する**ヘッダが UTF-8 でなければ `Invalid` —
  `Part` のフィールドが `str` だからだ。
- **重複は拒否する、解決しない。** 1 つのパートに `Content-Disposition`（または `Content-Type`）の
  フィールド行が 2 本ある、あるいは 1 つのリストに同じパラメータ属性が 2 回現れる場合は `Invalid`。
  先勝ち（Go）も後勝ちも推測であり、両者は食い違う。手前のパーサがもう一方を選んでいれば、その差が
  スマグリングの原材料になる。規則は 1 つ、両方の階層に適用する。
- **`filename` は参考値であり、サニタイズしない**（RFC 7578 §4.2）— このモジュールがアプリケーション
  に渡す唯一の攻撃面だ。届いたバイト列がそのまま返る: `../`、絶対パス、バックスラッシュ、
  クライアントが選んだ任意の UTF-8。アプリケーションが検証せずに、パスとして（またはパスの一部として）
  使ってはならない。推測を拒むのは意図的だ — 黙って書き換えるのはトラバーサルと同じくらい誤りであり、
  そもそもゼロコピーのビューは何も書き換えられない。なお `filename=""`（ブラウザがファイル未選択の
  入力に対して送る形）は `filename` 不在と区別できない。どちらも `""` なので、`p.filename.len() > 0`
  は「ファイル名が付いているか」を分けるのであって、「ファイルパートかフィールドか」ではない。
- **`boundary()` の失敗の答えは 1 つ。** メディアタイプ違い、`boundary` パラメータ不在、その重複、空、
  70 バイト超（§5.1.1）、制御バイト、`token` でない引用符なしの値、SP/HTAB で終わる値（§5.1.1 の
  `bcharsnospace`。区切り子の後ろには transport padding が続くので、末尾の空白は「boundary か padding
  か」を曖昧にする）、閉じない引用符、エスケープ — どの理由でも `""`（「歩けるマルチパートフォームでは
  ない」）なので、アプリケーションの検査は一度で済む。忘れた呼び出し側のために `next` は空の boundary
  を拒否する: `--` だけでは無意味な位置でボディを割ってしまう。
- **RFC 文法の最小形については Go より厳格 — 意図的に。** パートのデータは次の区切り子に属する CRLF で
  終わらなければならないので、最小の空パート
  `--SEP\r\nContent-Disposition: form-data; name="a"\r\n\r\n--SEP--`（ヘッダ部と終端区切り子の間に
  CRLF が 1 つも無い形）はここでは `Invalid` であり、Go の `mime/multipart` は受理する。文法がそこに
  置いているバイトを要求することで、「パートがどこで終わるか」の読み方が 1 つに保たれる。緩い読み方は
  2 つ目を必要とする。

**v1 に意図的に入れないもの:** フィールドマップや「配列に集める」便利関数（アロケートするし、
どのフィールドが要るかはアプリケーションの都合）、ストリーミング/チャンク multipart（ボディは
今は 1 バッファであり、`next` はその純関数なので将来のストリーミング形は追加で済む）、
`Content-Transfer-Encoding` の復号（RFC 7578 §4.7 が使用禁止としている）、RFC 5987 の `filename*`
の復号（パーセント復号はアロケートする。必要な消費者が現れたら `std.encoding` が見える一手）。

**テスト:** `crates/align_driver/tests/apps_web_multipart.rs` — 出荷ソースそのもの（`include_str!`）
に対する 16 ケース: テキスト + ファイルの 2 パート、長さ 0 のパート、preamble/epilogue/transport
padding、大小無視の名前、quoted と素のパラメータ、quoted なファイル名の中の `;`、`name` 欠落 /
不正な disposition / コロン無し行、obs-fold 継続行（それ単独を切り分けるボディ）、重複のすべての形、
引用符なし `token` 規則と boundary 末尾空白の規則、切り詰めが `Done` ではなく `Invalid` になること、
パートのデータ中に boundary が現れる場合、バイナリデータのビューが逐語的であること（NUL / 0xFF /
裸の CRLF）、`boundary()` の受理/拒否の行、空 boundary と範囲外 `from` のガード。上の `upload`
...
- **W1 — router コア。完了。** パターン解析 + 検証。**radix tree**（static/param/wildcard ノード、
  ...
  走査オラクルとの差分テスト。出荷済みの F1① を利用する。
- **W2 — Ctx + serve + ディスパッチ。完了。** `web.Ctx`。std.http 上の accept ループ。
  自動 404/405/400/500。`group`。統合テストは in-process サーバパターン
  （`crates/align_driver/tests/m11_http.rs`）。
- **W3 — アクセサ + レスポンダ。完了。** param/query/header/body/body_str、
...
- **W4 — 堅牢化。** 第 1 スライス 2026-07-21 出荷: **起動時テーブル検証**
  ...
  E2E で固定する。W4 は完了。
- **W5 — router/e2e ベンチゲート。完了。** `bench/web_e2e` でフレームワーク上乗せは同じ応答を
  `std.http` へ直接書く場合に対して 0.8 µs/request。dispatch は 35 ns で Align 自身の
  protocol path の 0.9%。`bench/web_router` と CI ceiling が回帰を固定する。
- **W6 — middleware-lite + ストリーミング。完了。** stream は E2E 固定済み。`group` /
  `group_with`、prefix-aware な zero-copy radix dispatch、順序付き pre-handler、
  short-circuit response、500 logging、serve-loop 生存も 2026-07-22 に出荷済み。
- **W7 — 外部比較。完了。** 同一環境・prefork 条件で pkg.web 491,505 req/s、
  Fiber 374,393 req/s（1.31×）。単一接続 latency は 1.19×、p99 は 2.5×。
  条件と注意点は `bench/web_e2e/README.md` に記録。
