...
- **HTTP が決着している所では厳格に。** `name` は必須（RFC 7578 §4.2）、disposition は `form-data`
  ...
  折り畳まれたヘッダの意図を推測することがスマグリングバグの出発点だ）、フィールド名とパラメータ
  属性は ASCII のみ大小同一視する。未知のヘッダはバイト比較で読み飛ばすので有効な UTF-8 である
  必要すらない。**認識する**ヘッダが UTF-8 でなければ `Invalid` — `Part` のフィールドが `str` だからだ。
- **`boundary()` の失敗の答えは 1 つ。** メディアタイプ違い、パラメータ不在、空、70 バイト超
  （§5.1.1）、制御バイト、`"`、閉じない引用符、エスケープ — どの理由でも `""`（「歩けるマルチパート
  フォームではない」）なので、アプリケーションの検査は一度で済む。忘れた呼び出し側のために `next`
  は空の boundary を拒否する: `--` だけでは無意味な位置でボディを割ってしまう。
...
に対する 12 ケース: テキスト + ファイルの 2 パート、長さ 0 のパート、preamble/epilogue/transport
padding、大小無視の名前、quoted と素のパラメータ、quoted なファイル名の中の `;`、`name` 欠落 /
不正な disposition / obs-fold / コロン無し行、切り詰めが `Done` ではなく `Invalid` になること、
パートのデータ中に boundary が現れる場合、バイナリデータのビューが逐語的であること（NUL / 0xFF /
裸の CRLF）、`boundary()` の受理/拒否の行、空 boundary と範囲外 `from` のガード。
