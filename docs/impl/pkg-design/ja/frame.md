...
| `pub JoinError { InvalidLimit, LimitExceeded }` | 閉じた payload なし sum。ordinal は `InvalidLimit=0`、`LimitExceeded=1`。 | 前者は `max_pairs < 0`。後者は結果が上限、i64 長、または対象で表現可能な出力 byte 範囲を超えること。OOM は hard abort。 | 通常の `{ i32 tag }`。借用・割当・Drop なし。 | `pkg.frame` が定義と ordinal を所有する。通常の enum/Result と exact mapping owner が受理する。 |
| `pkg.frame.inner_join_i64(left: codec.i64_column, right: codec.i64_column, max_pairs: i64) -> Result<array<RowPair>, JoinError>` | 引数は左から 1 回ずつ評価。入力は生きた正規 `core.codec` i64 view。長さは異なってよく、空でも、同じ batch/storage generation でもよい。`max_pairs` に既定値はなく、範囲は正確に `0..=i64::MAX`。値は decode 後の符号付き i64 `==` で比較。 | 全一致対を left-row-major、同一 left 内は right ordinal 昇順で返す。重複は安定 Cartesian 積。負の上限は入力読取・割当前に `InvalidLimit`。次の 1 対が上限、i64 長、出力 byte 範囲を超える時は出力を公開せず `LimitExceeded`。Pure。 | 入力は呼出中だけ借用し保持しない。成功は通常の `array<RowPair>` 1 個を所有し、空は `{null,0}`。right hash/index scratch は処理所有で全 return 前に解放。error は出力なし。 | package wrapper と exact signature、専用 checked operation、MIR/LLVM、inactive A121 候補を一境界で有効化する。exact/empty/unequal/duplicate/self-storage/order/limit、割当 parity、malformed-HIR、whole/per-unit/cache、ABI/export が owner。 |
...
4. right hash table capacity arithmetic を scratch 割当前に検証する。
...
6. 非空出力を正確に 1 回割当、再 probe、正準順で fill 後に `{ptr,len}` を公開する。空は無割当。
...
vendor 可能な `pkg.frame` が 2 定義・2 関数を所有する。compiler は通常の module resolution が
正準 public signature を証明した場合だけ exact qualified function を認識する。同名 local/module
関数は通常の call。Sema は signature/evaluation/purity/region と 2 checked discriminator、checked
...
|---|---|---|
| 公開 formation/identity | 正準 vendored module、2 定義/2 関数、同名非捕捉、ordinal/field order、whole/per-unit/generic reconstruction。 | import/name/signature positive と wrong-module/type negative、interface bytes/hash、malformed definition。 |
...
| limit/precedence | negative/zero/exact/next/overflow、固定 validation order、部分公開なし。 | direct ABI multi-invalid、failpoint、全 error で null/zero header。 |
