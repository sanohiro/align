...
| `pkg.web.types.UpgradeHandler { values_valid: bool, prepare: fn(Ctx, slice<str>) -> UpgradeDecision, pump: fn(Ctx, http_upgrade, string) -> Result<(), Error> }` | field/source order exact。values_valid は protocol package が計算済みの Copy 値、二 callback は noncapturing Copy。 | serve の pre-bind validation が common checks 後・segment/pair checks 前に boolean を一回読む。false は exact diagnostic。startup 中に任意 callback は実行しない。prepare は middleware 後、pump は successful 101 後だけ一回。 | bool と 16-byte fn value 二つ、allocation/Drop なし。pump は handle を consume/Drop。 | field/function signature と nominal graph は interface identity。 | stored true/false/order/diagnostic、startup callback 不在、pump logging、whole/per-unit、capture 禁止。 |
| `pkg.web.upgrade(method, pattern, values, values_valid, prepare, pump) -> Route` | 左から一度、default なし。protocol package が call 前に validation result を計算し、argument expression の effect は call site にだけ現れて保持・再実行されない。 | Pure。serve の pre-bind route validation が stored false を bind/tree construction 前に abort。Handler.Upgrade、empty stream_type。既存 radix/405/group/middleware、HEAD fallback は Respond だけ。 | Copy Route が method/pattern/values/bool/callbacks を保持。validator callback と allocation なし。 | `pkg.web` + `Route` trailing `upgrade_values`。 | evaluate-once/stored true/false/no callback、mixed routing/group/404/405/HEAD/middleware、zero hot-path allocation。 |
...
| `pkg.ws.route(pattern, protocols, pump) -> pkg.web.types.Route` | 左から一度、default なし。constructor は Pure `protocols_valid(protocols)` を一回評価し、protocol list と Copy boolean だけを generic Upgrade に渡す。callback は保持しない。check は各 protocol の nonempty RFC token と byte-exact uniqueness を要求。empty は selection なし、nonempty は client offer match 必須、server-list 最初の match。 | Pure/zero-allocation。invalid config は `pkg.web.serve` が stored false を読む pre-bind validation だけで診断。GET Upgrade route。middleware が handshake より先。selected は owned string、`""` は none。 | Route が views を serve 中 retain。constructor allocation/validator callback なし、nonempty selection だけ clone。 | ordinary `pkg.ws` source。private SHA-1 は public interface 外。 | validation-result evaluate-once/selection、startup diagnostic/no callback、mixed routing/middleware、whole/per-unit、vendoring。 |
...
| P1/P2 route validation purity | Pure protocol constructor 内で validation result を一回計算し、generic Upgrade には boolean だけ保持する。既存 pre-bind validation が exact diagnostic を選び、effect-unqualified validator callback は interface を越えず startup 中にも実行されない。 |
...
| P1 outbound-close evidence strength | negative/above-u16 code を close owner で reject。600 ms cumulative budget と per-frame reset を短い client read bound で区別。frame head/code write 後の fault injection は synchronization byte 受信後に abortive reset が observable になるまで短く待ち、synchronization-read failure と reason write success を別々の sentinel error に map して、assert する transport error を reason write だけが生成可能にする。 |
...
| P2 fail-safe owner reaping | affected test socket の read/write 双方に shared deadline を設け、spawn child を bounded owner の return/unwind 前に kill-and-wait reaper へ transfer。reaper は caller deadline 後も child を保持し、全 wait error を reap まで retry するため poll error でも process ownership を破棄しない。 |
| Gate ratchets | compile する Align package source は runtime fixture で読む。LLVM call value missing は panic でなく diagnostic、malformed `http_upgrade` sizing は total、native boundary は address/checked integer conversion にし、改善した runtime cast count を ratchet baseline に固定。 |
