...
read/write/configure/retry をせず ordinary free/Drop path へ exact 1 回渡す。close は entry/partial state を
...
precondition、caller retirement、後続 close/Drop を固定する。mandatory base export、
...
| error・native status・poison state | I/O 前 Invalid。bounded UTF-8 Server と complete non-UTF-8 Decode は reusable。exact `0/1/2/3/4/>=5` status decode。reader `{invalid negative, admitted negative, zero, admitted positive, oversized positive}` x view length x `{null, non-null}` pointer representation と checked i32 narrowing、typed-slice construction 前の raw-header validation。invalid-negative/oversized-positive は header inspection 前に abort。Io/too-large/protocol/truncation/partial-write は close。selected terminal error を cleanup より保持。以後の全 call は zero I/O の Closed。全 impossible product は parser/publication/ownership change 前に `process.abort` へ到達。 | error-producer x command x before/during/after-frame x reuse table。early-abort/no-header inspection、両 empty pointer form、positive-null abort を含む全 category/representative code/width/count/length/pointer/malformed product、native call counter、explicit `ProcessAbort` IR/capability retention、no-import negative、selected-error/cleanup-failure probe。 |
...
2 回目の finding-ledger repair 後、2 回目の author-side ledger-to-prose および closure-matrix consistency
...
- resource record、RESP state machine、source-reachable timeout row の全 state/tag/reserved/pointer/length
  product、live/exclusive overlap exclusion、failed-second-option retirement、error preservation、Drop order が固定。
...
| P2 new-row malformed input | `TcpConnSetIoTimeout` は最初に null、その後 inclusive timeout range を検証し、fd access 前に `AL_INVALID`。全 non-null caller は live/unfreed/exclusive connection を渡す。direct runtime evidence は null x range、overlap/provenance precondition、exact `timeval`、pre-armed `{R0,S0}` から receive-fail/no-send/`{R0,S0}`、send-fail/`{T,S0}`、both-success/`{T,T}` の transition を覆う。range rejection は live connection を維持し、admitted-input option failure は caller retirement と後続 free/Drop を必須にする。package は publication/address retry なしで close。 |
...
| P2 timeout compatible-caller lifecycle | 全 non-null compatible caller は read/write/configure/free/Drop overlap のない live/unfreed/exclusive connection を渡す。pre-armed `{R0,S0}` は `{R0,S0}`/`{T,S0}`/`{T,T}` へ transition。どちらかの option failure は retirement と後続 1 回の free/Drop が必須で、validation rejection と success は usability を保持。direct/package owner は許可される range-rejection retry と禁止される option-failure retry を区別し、publication を禁止して close once を固定。 |
...
P1 が native validation order を変更し、P2 timeout finding が general source-reachable lifecycle を完成させるため、
acceptance には complete revised diff のもう 1 回の fresh full review が必要。candidate status はその review が
clean になるまで Open のまま。
