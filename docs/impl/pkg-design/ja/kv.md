...
builder/decode edge をすべて pin し、Move-slice edge は両方向を pin する: retaining sum では formation
negative、retaining struct では admitted borrowed view と、それが禁じる read/escape/source move。direct-runtime half は両 option state を pre-arm し、exact
