...
| complete row validation/decode | decoder前にexact column count、declared-order UTF-8 name bytes、NULL disposition、全first-release typeのexact driver-native representationをvalidateする。ordinalごとのgenerated typed callbackでreflection/boxing/map/artifact/source I/O/post-validation fallible conversionなしにdecodeする。multi-invalid rowは最初のdeclared ordinalをreportし、native pointer/length/UTF-8 malformed valueはsafe view形成前に失敗する。 | 両driver complete matrix owner、`pkg_db_q4b::malformed_native_view_values_fail_before_safe_view_formation`、cumulative Q2 name/type/count/null twin、generated MIR inspection、callback version/context misuse negative |
...
| `one` cardinality compatibility | common/driver-qualified `one`はstream formationを再利用するが、settled Q2 winner orderを変えない。row 0だけをvalidate/decodeして`out`へcloneし、second rowはdriver-private non-decoding probe 1回で存在だけを確認する。malformed first rowはmultiplicityより先に失敗し、valid first rowと任意のsecond rowはsecondをvalidate/decodeせずCardinalityを返す。exhaustion/multiplicityは同じstream cleanup pathでcloseする。 | `pkg_db_q2::sqlite_native_command_and_one_execute_generated_i64_thunks`、`postgres_native_command_and_one_own_buffered_results`、両common-surface dispatch owner |
...
implementationにはさらに1 independently correct prerequisiteがある。first PRはQ5b2 matrixに残る
...
public surface/ABIを変えず先にmergeする。second PRはdirect delivery/rows ABI/stream protocol、third
PRは`rows_stmt_native`とprepared parameter-name authorityを追加する。second design reviewがunleased
Q5b2 pathとのoverlapを発見したためprerequisiteを分けた。
...
| operation/result | Delivery absenceはshipped caller-synchronous BufferedFullへdelegateしprotocol completion、blocking restore、server failureをcomplete rows publication前に終える。Timeoutなしは既存1 `PQexecParams`/`PQexecPrepared`でnonblocking/send/selector zero。Timeoutありは既存`PQsetnonblocking`+`PQsendQueryParams`/`PQsendQueryPrepared`+wait/cancel/drainでselector zero、completion前rows publicationなし。explicit direct/preparedは同send後immediate selectorを呼びfirst rowを待たずlive rowsを返す。`one_native`はfirst validated Rowだけをunpublished outへcloneしsecond mode-valid rowをdecodeせずpending Cardinality 2として記録、original deadlineでnormal drainする。clean completion後だけCardinality 0/2またはone Rowをpublishし、late native/sequence/timeout/cleanup failureは勝ってpending valueをdiscardする。Cardinalityはcancelしない。 |
| physical delivery/order | `SingleRow`はexactly 1 rowの`PGRES_SINGLE_TUPLE`、`PortalBatch(n)`は`1..=n` rowsの`PGRES_TUPLES_CHUNK`だけを受理しserver orderを保持する。0個以上のdata後zero-row `PGRES_TUPLES_OK`を要求し、nullまで`PQgetResult`を呼ぶ。[libpq 17 §32.6](https://www.postgresql.org/docs/17/libpq-single-row-mode.html)のexact contractである。BAD/NONFATAL/FATALは既存native mapping。EMPTY/COMMAND/PIPELINE/unknown/other-mode/invalid count/nonempty-or-missing terminal/extraはexact invalid-sequence errorを記録しcurrent resultをonce clearしてordinary no-decode drainを続ける。`PGRES_COPY_OUT`/`PGRES_COPY_IN`/`PGRES_COPY_BOTH`はCOPY terminationが本rail外なのでfail-closedとする。earlier owned errorがない時だけinvalid-sequence errorを記録し、current resultをexactly once clearしてphysical connectionを即poison/closeする。COPY観測後はそのconnectionへ`PQgetResult`、`PQgetCopyData`、`PQputCopyData`、`PQputCopyEnd`、cancel API、`PQtransactionStatus`、blocking restoreを呼ばない。main-result classification開始時にcancel handleは残らない。close後にwrapper mutable native state、unpublished Row/batch、context、unpublished direct Query ID、leaseをcleanupする。published valueは残り、earlier row/storage/mode/timeout errorを保持しDropはsilent。COPY/command/pipeline/multi-statementをrowsへ再解釈しない。 |
...
| acceptance/measurement | merged lease prerequisite、D8/D9/common A1、libpq>=17前提。direct PRはconn/tx x BufferedFull/SingleRow/PortalBatch x rows/one、prepared PRはconn/tx-prepared stmt x同3 mode x rows_stmt、next/batchは各rows axis。zero/one/many、official zero terminal、全status、one/two-row-then-fatal/two-row-then-clean、Conn/Tx DML RETURNING effect、pending-cardinality timeout、partial-final restore、validation/decode/storage error後のnormal completion/deadline expiry、early Drop/cancel raceのConn `IDLE`とTx `INTRANS`/`INERROR`、全deadline/duration pairを含むmalformed rows/stmt v3とSQLite sibling、whole/per-unitをcover。COPY ownerは各COPY statusをfirst result/data後、next/batch/one、validation/decode/storage drain、deadline/mode recovery、Dropへinjectし、Timeout absent/present x Conn/Txでexact current clear+close、later getResult/COPY/cancel/tx-probe/restore zero、first error/silent Drop、unpublished destroy、balanced context/ID/wrapper/lease、no reuseをassert。direct phase-order ownerは全multi-invalid/failpointをcontext allocation/free、lease、binder、libpq counterとcrossしlive state→context/static→leaseを証明する。error-drain ownerはfirst error/no-more-decode、Conn/Tx clean effects、Conn/Tx expiry-cancel effects、original-duration budget、unsynchronized poisonをassert。Tx ownerはcompleted-before-cancel effectとcanceled/aborted stateを区別しhidden rollbackなし。BufferedFullはTimeoutなしのPQexec one/async+selector zeroとTimeoutありのPQsend one+existing wait/cancel/drain/selector zero/blocking restore/pre-publication errorを別々にassert。live overlap no-libpq。required server16.4/client>=17。measurementはnon-gating。 |
...
| staged public/option | direct PRはexact Delivery+1 variantを追加しrows/oneを1 validatorへroute、command reject/common name不可。prepared PRはexact rows_stmt_nativeを追加しstmt-specific validation後same validator。Text/Binary不変。 | per-PR surface golden、common/native source-orderとcommand/direct/prepared disposition |
...
| explicit direct construction | §13.4をexactに維持する。complete header/identity/common/native/restriction→live exec/connection state→one context allocation→generated static validation→lease→bind→deadline→nonblocking→send→immediate explicit mode→conn/tx-dependent rows。static failureはcontextをfree、overlapもcontextをfreeしてbind/libpq zero。later failpointはParams/context/unpublished copied IDをonce free、published IDはDropまでown。 | complete pairwise phase-order x context allocation/free/lease/binder/libpq counter、direct conn/tx send/mode failpoint、ID counter、Params Move/Drop/mutation |
| prepared state/construction | prepared PRでboth-driver stmtを88-byte v3へbumpしresolver@80をretain。existing prepared binder bridge隣にexact sealed `prepared_parameter_ordinal(resource_ref<db.stmt<P,R>>, str) -> i32`と`stmt_header_valid` control dependencyを追加する。両bridgeはconcrete stmt referenceだけを受けapplication/raw callを拒否し、whole/per-unitでresolver thunkをretainする。statement formationはresolver/binder/row validator/stream decoder/batch planを同じconcrete Query producer generationへbindする。complete stmt/plan/resolver/native optionをlease/context前にvalidateしprepared send+shared stream constructorを使う。statement-owned ID/resolverはborrowしfreeしない。 | stmt byte、sema/MIR wrong-resource/application/unguarded/cross-Query splice/malformed resolver no-call、SQLite/PG+whole/per-unit retention、prepared failpoint、unknown/duplicate name、reuse/Drop |
| result advancement | common helperでordinary resultをeach once clearしexhaustive status/mode/bound超fetchなし。COPYはdistinct fail-closed branchでcurrent resultだけclearし、追加protocol/cancel/probe/restoreなしでclose後owner cleanup+lease release。oneはmultiplicityをpendingにしlater rowをdecodeせずclean completionまでdrain、late native/timeout/cleanupが勝ちclean時だけCardinality publish。全consumer/cleanup callerをauditする。 | every-status/count/sequence、COPY first/after-data/every-cleanup-cause no-call/clear-close、zero/one/many、one/two-fatal、two-clean、DML effect |
...
| failure/timeout/cleanup | mode/deadline/fatal/validation-decode-storage/Drop/clean-terminal restoreを1 drain/cancel/sync/restore helperへ集約する。cardinalityとfirst row/storage errorはnormal no-decode drain、deadline expiry/Drop/mode failureだけcancelし、expiry budget用original durationをpersistする。PGcancelConn exactly-one finishをmain result classification前に完了し、first error、ordinary result、restore-or-poison後release。cleanup中COPYならcurrentだけclear、追加drain/cancel/probe/restoreなしで即close、earlier error/silent Dropを保持しowner cleanup+lease release。Conn cancelはIDLEだけ、Txはcompleted+effects retainedのINTRANSかcanceled/server-aborted+rollback requiredのINERRORだけを受理しhidden rollbackなし、他stateはpoison。clean restoreはexact cleanup、partial batch destroy。BufferedFull/common A1もaudit。 | full cancel failpoint、COPY x mode/deadline/row/storage/Drop x Timeout absent/present x Conn/Tx first-error/no-call/clear-close、error x clean/later-fatal/expiry x Conn/Tx effects、original-duration budget、zero/partial-final restore、fatal、pre/post/pending timeout、Drop/timeout Conn/Tx race、reuse/rollback-or-poison |
...
| FFI/version | exact libpq17 send/row-mode/result/nonblocking cancel/transaction/cleanup signaturesとconstantをdeclare。native `int` chunk size/polling status signednessをC probeでpin。本railはCOPY data/termination FFIを追加せず、COPY処理はcurrent-result clear+ordinary connection closeだけ。whole/per-unit Linux/macOSのordered pq/ssl/crypto closureを保持し、client <17はfirst requestでなくbuild/evidence setupでreject。 | C signature/status、forbidden COPY symbol/call inventory、client version、link inventory、x86_64/ARM64/macOS build |
...
four-times-reopened boundaryは3 independently mergeable vertical PRのままである。error-drainと
COPY fail-closedはexplicit direct streamなしにconsumer/callable surfaceを持たず、それらなしのstream
publicationはincorrectなため別prerequisiteへ安全に分割できない。settled direct validation phase-orderの
restorationも同direct constructorだけを変更しprerequisite consumerを持たない。lease repairだけが
independently usefulなpre-stream closureである。
...
| PostgreSQL direct streamed delivery | direct rows/oneへDelivery、libpq17、rows v3、advancement、normal-drain cardinality、cancel、batch、direct matrix。absenceはshipped BufferedFullの両Timeout subpathを維持。 | direct matrix、dual-driver rows/batch、required local/CI、normal preflight/review/CI |
...
duplicateはinvalid second payloadより先、callable matrixからprepared `one_native`を除外、全libpq17
...
implementation前にfour-times-redesigned ledgerとunchanged 3 PR boundaryをfresh independent
