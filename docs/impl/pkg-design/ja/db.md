...
| required PostgreSQL CIがfirst-run bootstrapでabsent tableだけをmissing historyとして扱い、到達したinventory queryがambiguousなinternal-`"char"` concatenationに依存していたことを発見した | 全blind history lockはPostgreSQLの両absent-object SQLSTATEを認識する。missing tableは `42P01`、missing schemaは `3F000` とし、後続のexact inventory queryが両owned objectのabsentを判定する。signature構築前に全internal-character discriminatorをtextへcastする。 |
