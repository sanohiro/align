...
| namespaced builtin type identity | `db.Error`をrenameせず一般type lookupを直す。non-entry moduleのlocal typeはcompiler-provided bare aliasと同名にでき、bare lookupはsame-module declarationを優先しbuiltinは`core.Error`等のprovider-qualified spellingで残る。entry canonical collisionはrejectする。 | local Error signature/constructor、qualified importer construct/match、同moduleのcore.Error、別module bare builtin、entry reject、whole/per-unit parity |
