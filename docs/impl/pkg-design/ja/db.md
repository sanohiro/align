...
| acceptance/measurement | merged lease prerequisite、D8/D9/common A1、libpq>=17前提。direct PRはconn/tx x BufferedFull/SingleRow/PortalBatch x rows/one、prepared PRはconn/tx-prepared stmt x同3 mode x rows_stmt、next/batchは各rows axis。zero/one/many、official zero terminal、全status、one/two-row-then-fatal/two-row-then-clean、Conn/Tx DML RETURNING effect、pending-cardinality timeout、partial-final restore、validation/decode/storage error後のnormal completion/deadline expiry、early Drop/cancel raceのConn `IDLE`とTx `INTRANS`/`INERROR`、全deadline/duration pairを含むmalformed rows/stmt v3とSQLite sibling、whole/per-unitをcover。COPY ownerは各COPY statusをfirst result/data後、next/batch/one、validation/decode/storage drain、deadline/mode recovery、Dropへinjectし、Timeout absent/present x Conn/Txでexact current clear+close、later getResult/COPY/cancel/tx-probe/restore zero、first error/silent Drop、unpublished non-return、balanced context/ID/wrapper/lease、no reuseをassert。direct phase-order ownerは全multi-invalid/failpointをcontext allocation/free、lease、binder、libpq counterとcrossしlive state→context/static→leaseを証明する。Query Delivery ownerは両invalid orderとvalid duplicateでpayload-before-duplicate、command ownerはQuery-only payload前のtag rejectをassert。`one_native` region ownerはzero/invalid-first-rowのzero `out` allocation、singleton/Cardinality 2/one-or-two-then-fatal/timeout/COPY/cleanup failureのexact one first-Row clone、全post-clone outcomeでequal byte/alignment delta、rewind/second cloneなしをassert。error-drain ownerはfirst error/no-more-decode、Conn/Tx clean effects、Conn/Tx expiry-cancel effects、original-duration budget、unsynchronized poisonをassert。Tx ownerはcompleted-before-cancel effectとcanceled/aborted stateを区別しhidden rollbackなし。BufferedFullはTimeoutなしのPQexec one/async+selector zeroとTimeoutありのPQsend one+existing wait/cancel/drain/selector zero/blocking restore/pre-publication errorを別々にassert。live overlap no-libpq。required server16.4/client>=17。measurementはnon-gating。 |
...
implementation前にrevised four-times-redesigned ledgerとunchanged 3 PR boundaryをfresh independent
