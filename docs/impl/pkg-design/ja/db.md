...
| live executionとoverlap | selected driverのexact versioned、unpoisoned connection prefixだけ受け付ける。SQLite catalog/EXPLAINはtyped executionと同じconnection-wide execution leaseを取得し、success/errorの全pathでcursor finalize後にreleaseする。PostgreSQLはclosed/poisoned targetをlibpq前にrejectする。transaction handlingは現在実装済みのexec-state contractを保ち、malformed stateをreinterpretしない。 | `apps/db/pkg/db.align`と`apps/db/pkg/db/internal/resource.align`、`pkg_db_q5b2::malformed_native_catalog_rows_close_once_and_sqlite_releases_the_lease`、SQLite plan owner、PostgreSQL bridge owner |
...
| construction、return、cleanup | metadata refとQuery/exec valueはsynchronous call中だけborrowする。successはfully initialized RegionPlain value/arrayを1つ返す。全prepare/bind/step/result errorはfirst owned errorを保ち、statement/result/context/leaseをnative orderでreleaseする。early `?`、branch join、malformed rowはcleanupをskipせずpartial resultをpublishしない。 | `apps/db/pkg/db.align`と`apps/db/pkg/db/internal/{sqlite,postgres,resource}.align`、malformed-result owner、SQLite catalog/plan runtime owner |
...
| deadline/cancellation/Drop | common `TimeoutNs`はsend前に1 overflow-checked monotonic absolute deadlineとなりclean completionまでrowsに付き、caller think timeもcountする。expiryはnonblocking native cancel+drain後に既存`Timeout`、別server cancelは`Cancelled`。early Drop/direct `one_native` cardinalityとmode-selection failureはexact `1_000_000_000` ns recovery、timeoutはD9の`max(original_duration, 1_000_000 ns)`。helperはnonnull `PGcancelConn`を`PQcancelCreate`からstart/pollまでownし、`PQcancelErrorMessage` bytesを必要なら`PQcancelFinish`前にread/copyし、全success/failureでfinishをexactly once呼びcancel viewをescapeさせない。その後main PGresultをnullまでclear/drain、transaction sync、blocking restoreまたはpoison、binder contextとunpublished direct ID free、wrapperがあればmutable native state clear、最後にlease releaseする。published direct terminal IDはretainし、prepared IDはfreeしない。create/dispatch/recovery/I/O/state/restore failureはrelease前poison/close。SQLなし、first error overwriteなし。 |
...
| acceptance/measurement | shipped D8/D9、common A1、libpq>=17が前提。directはconn/tx x SingleRow/PortalBatch x `rows_native`/`one_native`、preparedはconn/txからprepareしたstmt x mode x `rows_stmt_native`。`next`/`next_batch`は両rows axis、prepared `one_native` cellはない。zero/one/many、boundary、全libpq17 status、pre/post-row timeout、fatal-after-data、decode/storage、Drop、reuse、SQLite Stepを含むmalformed shared v3、whole/per-unitをcoverする。required PostgreSQLはserver16.4/client>=17。measurementはtime-to-first-row/full scan/peak bytes/PGresult+cancel-handle count/max rows/transported-decoded-published/batch copies/cancel recoveryを記録するがsemantic gateではない。 |
...
| result advancement | PGresult transitionを`next`/`has_next`/`next_batch`共通helperへ置き、each result once clear、exhaustive libpq17 status table、mode sequence、bound超fetchなし。変更時3 consumer audit。 | parameterized every-status/count/sequence + live zero/one/many |
...
| failure/timeout/cleanup | mode/deadline/fatal/Drop/cardinalityを1 helperへ集約。各`PGcancelConn`をexactly one finishまでownしfinish後cancel errorをcopyせず、first error、all main-result clear、restore→releaseまたはpoison→release。BufferedFull siblingもaudit。 | cancel create/start/poll/error/finish/drain/reset failpoint+allocation、fatal/timeout/Drop/reuse |
...
| operation/allocation parity | exact callable axisとしてdirect conn/tx x mode x rows/one、conn/tx-prepared stmt x mode x rows_stmtを実行し、両rows axisだけnext/batchを使う。one send/no hidden SQL/one lease/bounded PGresult+cancel/exact parameter+ID ownership/balanced allocation/sibling error identityをassertしSQLite Step matrixもretain。 | parameterized matrix、dual-driver rows、provisioned suite、measurement |

`e6fd66e` independent design reviewの7 findingは1 ownership/ABI/validation passでledger-firstに
