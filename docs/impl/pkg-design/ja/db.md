...
| PostgreSQL describe | library load前に全ambient `PG*`を拒否し、selected non-`PG*` env valueだけでUTF-8 connectionを開く。supported `ParameterType`をbinding protocol ordinalごとのexact `PQprepare` OID vectorへ写し、collision-free nameでprepare/describeしてOID/name/origin/search path/extensions/versionを記録する。unsupported mappingはstatement preparation前に失敗し、prepared state/result/connectionを全pathでexactly once cleanupする。 | `pkg_db_q3::postgres_rejects_ambient_connection_defaults_before_native_load`、`pkg_db_q3::postgres_native_prepare_describes_the_selected_query` とrequired PostgreSQL CI |
