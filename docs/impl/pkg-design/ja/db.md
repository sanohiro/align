...
| physical delivery/order | `SingleRow`はexactly 1 rowの`PGRES_SINGLE_TUPLE`、`PortalBatch(n)`は`1..=n` rowsの`PGRES_TUPLES_CHUNK`だけを受理しserver orderを保持する。0個以上のdata後zero-row `PGRES_TUPLES_OK`を要求し、nullまで`PQgetResult`を呼ぶ。[libpq 17 §32.6](https://www.postgresql.org/docs/17/libpq-single-row-mode.html)のexact contractである。BAD/NONFATAL/FATALは既存native mapping、EMPTY/COMMAND/COPY/PIPELINE/unknown/other-mode/invalid count/nonempty-or-missing terminal/extraはexact invalid sequence。全nonnull resultをclearする。 |
...
| deadline/cancellation/Drop | explicit modeのcommon Timeoutはsend前に1 absolute deadlineとなりclean completionまで付き、caller think timeとone pending-cardinality drainもcountする。expiryはcancel+drain後Timeout、別server cancelはCancelled。early rows Drop/mode failureはexact 1s recovery、cardinalityはcancelなし、timeoutはD9 budget。helperはnonnull PGcancelConnをcreate/start/pollからall-path exactly one finishまでownしerror bytesはfinish前にcopy、その後main results drain、transaction sync、blocking restore-or-poison、context/unpublished direct ID、wrapper、lease順。Conn targetはdrain後`PQTRANS_IDLE`だけreuse可。explicit Txはcancelよりstatement completionが先なら`PQTRANS_INTRANS`でeffectsをtransaction内に保持し、cancel成立なら`PQTRANS_INERROR`でcallerのrollback/Dropまたはserver-rejected operationだけを許す。どちらにもhidden rollbackなし。Timeout/mode failure/early Dropのprimary resultはrace stateに依存せず、statement effectはsubsequent DB/Tx stateだけでobservable。他stateはpoison/close。clean terminal cleanup failureはexact db.rows.cleanup。timeout/native/sequence/decode/storageはunpublished Row/pending Cardinalityより先。Dropはreportせずpoison/close。 |
...
| acceptance/measurement | merged lease prerequisite、D8/D9/common A1、libpq>=17前提。direct PRはconn/tx x BufferedFull/SingleRow/PortalBatch x rows/one、prepared PRはconn/tx-prepared stmt x同3 mode x rows_stmt、next/batchは各rows axis。zero/one/many、official zero terminal、全status、one/two-row-then-fatal/two-row-then-clean、Conn/Tx DML RETURNING effect、pending-cardinality timeout、partial-final restore、early Drop/cancel raceのConn `IDLE`とTx `INTRANS`/`INERROR`、malformed rows/stmt v3とSQLite sibling、whole/per-unitをcover。Tx ownerはcompleted-before-cancel effectとcanceled/aborted stateを区別しhidden rollbackなし。BufferedFullはTimeoutなしのPQexec one/async+selector zeroとTimeoutありのPQsend one+existing wait/cancel/drain/selector zero/blocking restore/pre-publication errorを別々にassert。live overlap no-libpq。required server16.4/client>=17。measurementはnon-gating。 |
...
| explicit direct construction | 全direct validation後lease→context→bind→deadline→nonblocking→send→immediate explicit mode→conn/tx-dependent rows。failpointはParams/context/unpublished copied IDをonce free、published IDはDropまでown。 | direct conn/tx send/mode failpoint、ID counter、Params Move/Drop/mutation |
...
| result advancement | common helperでeach result once clear/exhaustive status/mode/bound超fetchなし。oneはmultiplicityをpendingにしlater rowをdecodeせずclean completionまでdrain、late native/timeout/cleanupが勝ちclean時だけCardinality publish。 | every-status/count/sequence、zero/one/many、one/two-fatal、two-clean、DML effect |
...
| failure/timeout/cleanup | mode/deadline/fatal/Drop/clean-terminal restoreを1 helperへ集約しcardinalityはnormal drainでcancelなし。PGcancelConn exactly-one finish、first error、all result、restore-or-poison後release。Conn cancelはIDLEだけ、Txはcompleted-before-cancel+effects retainedのINTRANSかcanceled/aborted+rollback requiredのINERRORだけを受理しhidden rollbackなし、他stateはpoison。clean restoreはexact cleanup、partial batch destroy。BufferedFull/common A1もaudit。 | full cancel failpoint、zero/partial-final restore、fatal、pre/post/pending timeout、Drop/timeout Conn/Tx IDLE/INTRANS/INERROR race/effect、reuse/rollback-or-poison |
| rows/stmt ABI malformed | shared rows 120-byte v3を1 authorityでconstruct/validateしSQLite Step+PG3 mode active/terminalを固定。prepared PRはstmt v3 nonnull resolverと全sibling accessor/Dropを更新。 | rows/stmt byte golden、field mutation no-native、v2 reject、driver/direct-prepared/active-terminal、both-driver stmt、SQLite retention |
...
twice-reopened boundaryは3 independently mergeable vertical PR:
...
implementation前にtwice-redesigned ledgerと3 PR boundaryをfresh independent adversarial reviewし、findingは
