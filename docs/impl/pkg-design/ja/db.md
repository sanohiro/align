...
implementationは1つのindependently correct prerequisiteで分割する。first PRはQ5b2 matrixに残る
...
public surface/ABIを変えず先にmergeする。second PRだけがdelivery surface、rows ABI、streamed
protocolを追加する。second design reviewがunleased Q5b2 pathとのoverlapを発見したためこの再分割を
行い、independently usefulなsafety closureを大きいstream state-machine PRへ埋め込まない。
...
| operation/result | validation後、directは`PQsendQueryParams`、preparedは`PQsendQueryPrepared`。successful send直後かつflush/input consume/result retrieve前にmode callを行い、rows constructorはfirst rowを待たずlive rowsを返す。`one_native`はfirst Rowをcloneしexactly one more mode-valid rowをprobeする。rowなしclean protocol completionはCardinality 0、first後second前のnative/sequence failureはそのfailure、second mode-valid row観測時点でCardinality 2をownしlater drain/fatal/restore failureはoverwriteしない。second rowはdecode/publishしない。 |
| physical delivery/order | `SingleRow`はexactly 1 rowの`PGRES_SINGLE_TUPLE` data resultだけ、`PortalBatch(n)`は`1..=n` rowsの`PGRES_TUPLES_CHUNK`だけを受理しserver orderを保持する。0個以上のdata result後にzero-row `PGRES_TUPLES_OK`を1個要求してclearし、exhaustion前に`PQgetResult`がnullになるまで呼ぶ。complete libpq17 dispositionは、`PGRES_BAD_RESPONSE`/`PGRES_NONFATAL_ERROR`/`PGRES_FATAL_ERROR`を既存`execution_error` SQLSTATE/native-detail mappingへ、`PGRES_EMPTY_QUERY`/`PGRES_COMMAND_OK`/`PGRES_COPY_OUT`/`PGRES_COPY_IN`/`PGRES_COPY_BOTH`/`PGRES_PIPELINE_SYNC`/`PGRES_PIPELINE_ABORTED`/unknown numeric/other-mode data/empty-or-oversized data/nonempty-or-missing terminal/terminal後extraをexact `postgres.rows.delivery` invalid-sequence errorへmapする。COPY/command/pipeline/multi-statement statusをrowとしてreinterpretせず、全nonnull resultをclearしてdrainを続ける。 |
...
| `next_batch`/atomicity | common A1 contractを維持する。1 resultの一部または複数をconsumeできるがcaller boundでexactly stopしnext rowをprobeしない。source clear前にview-bearing fieldをunpublished batchへcopyし、bound前errorはwhole batchをdestroyする。bound到達後のlater errorはsubsequent advanceへ返す。clean EOFをbound前に見る場合はresult drain+blocking restore後だけpartial batchをpublishする。earlier errorなしでcleanup failureならunpublished batchをdestroy、poison/closeし、`Error.Unsupported(ContractError { query_id: Some(query_id), item: "db.rows.cleanup", message: "PostgreSQL streamed rows cleanup failed and the connection was closed" })`を返す。zero-row EOFも`Ok(None)`でなく同error。published batchはretractしない。 |
| partial server failure | data後fatalが来得る。観測advanceは既存exact SQLSTATE/native detailをcopy、result clear、nullまでdrain、rows failed化する。以前returnしたrow/batchはcaller-visibleでhidden compensation/rollback/mutationなし。`one_native`はfirst後second mode-valid row前のfailureを返すが、second row観測後はCardinality 2がlater fatal/invalid sequence/cleanupより先。ownerはone-row-then-fatalとtwo-rows-then-fatalを含む。 |
| parameter ownership/allocation | generated binderは§5.6.1どおりparameterごとに1 execution-owned Text copyを作る。contextはsendからterminal null、mode-selection cleanup、timeout、early Dropまで全pointer/length/format arrayとcopy payloadを保持する。constructor return後にoriginal text/blob source ownerをmove/mutate/dropできる。directは1 copied Query IDをownし、constructor failureはallocated済みならfree、published terminal rowsはrows Dropまでretainしてfreeする。preparedはstatement-owned Query IDを常にborrowしsuccess/failureのどちらもfreeしない。per-row container/hidden whole-result bufferなし。steady live setはbinder context/current PGresult/libpq transport/direct ID/explicit batchまでで、cancellation中だけ1 temporary `PGcancelConn`を追加する。 |
| validation/error precedence | direct Queryはcomplete descriptor header、Query identity、common options source order、native options source order、static Query/binder shape、exact live exec state、lease、bind、deadline representability、send、mode selectionの順。preparedはcomplete statement/plan header+Query identity後に同じsuffix。各Delivery位置でcommand rejectionが最初、Queryではduplicate detection後にfirst PortalBatchだけpayload sizeをvalidateする。duplicateはquery-specific `Unsupported` item `postgres.execute.delivery`、message `duplicate PostgreSQL row delivery option`。invalid first Portal sizeは同item、message `PostgreSQL portal batch size must be between 1 and 2147483647 rows`。したがって`[SingleRow, PortalBatch(0)]`はduplicate、reverseはsize。`execute_native`のfirst Deliveryはinvalid payloadでも同item、message `PostgreSQL row delivery requires a Query result`。全てlease/execution-context allocation/bind/send前。既存earlier Text/common errorのcategory/precedenceは不変。 |
...
| deadline/cancellation/Drop | common `TimeoutNs`はsend前に1 absolute deadlineとなりclean completionまでrowsに付きcaller think timeもcountする。expiryはcancel+drain後に既存`Timeout`、別server cancelは`Cancelled`。early Drop/direct one cardinality/mode failureはexact 1s recovery、timeoutはD9 budget。helperはnonnull `PGcancelConn`をcreate/start/pollから全path exactly one finishまでownしerror bytesはfinish前にcopyする。その後main resultsをnullまでclear/drain、transaction sync、blocking restoreまたはpoison、context/unpublished direct ID free、wrapper mutable state clear、lease release。published direct IDはretain、prepared IDはfreeしない。earlier errorなしのclean terminal後failureは上記exact PostgreSQL `db.rows.cleanup` error、既存timeout/native/sequence/decode/storage/Cardinalityはfirst errorとして維持する。Dropはreportせずpoison/close。SQLなし。 |
...
| artifact/ABI/cache | Query/command descriptor、batch plan、checked metadata、static Query bytes/cache identityはruntime optionなので不変。shared SQLite/PostgreSQL Rows stateは120-byte、8-align v3。0--95はv2 meaning。96 delivery u8 (`0=PostgreSQL BufferedFull`,`1=PostgreSQL SingleRow`,`2=PostgreSQL PortalBatch`,`3=SQLite Step`)、97 pending u8 (`0/1`)、98--99 zero u16、100 `i32 portal_max_rows`、104 `i64 deadline_ns`（absent `-1`）、112 zero u64。active SQLite direct/preparedはdriver SQLite/delivery3/pending0/non-null statementをcontext@8とmirror/portal0/deadline-1。active PostgreSQL BufferedFullはdriver PostgreSQL/delivery0/pending0/non-null result mirror/portal0/deadline-1。active streamedはdriver PostgreSQL/delivery1/2/pending1/portal0-or-positive/null-or-live result mirror。他productはinvalid。両driver terminalはdelivery/pending/portal/result/contextをzero/null、deadlineを`-1`にしimmutable Query identity/batch planをretain。全SQLite/PostgreSQL constructor/accessor/`next`/`has_next`/`next_batch`/Dropがnative use前にcomplete recordをvalidate。goldenは全direct/prepared active/terminalとmalformed version/driver/tag/reserved/size/deadline/pointerをpinする。 |
...
| acceptance/measurement | merged libpq-consumer lease prerequisite、shipped D8/D9/common A1、libpq>=17が前提。directはconn/tx x absent-Delivery BufferedFull/SingleRow/PortalBatch x rows/one、preparedはconn/tx-prepared stmt x同3 mode x rows_stmt。next/batchは両rows axisだけ。zero/one/many、boundary、全status、one-row/two-rows-then-fatal、timeout、partial-final restore failure、decode/storage/Drop/reuse、SQLite Step含むmalformed v3、whole/per-unitをcoverする。live streamは全catalog/EXPLAIN/typed/transaction siblingをlibpq callなしでreject。required PostgreSQLはserver16.4/client>=17。measurementはsemantic gateではない。 |
...
| direct/prepared construction | 全public/static input validation→lease→1 context/bind→deadline→nonblocking→1 send→immediate mode→dependent rows publication。failpointでParams/contextをonce free。direct failureはunpublished copied IDだけfreeしpublished IDはDropまでown、preparedはstatement IDをborrowしfreeしない。 | direct/prepared x conn/tx failpoint、ID allocation/free、Params Move/Drop/mutation |
| result advancement | PGresult transitionを`next`/`has_next`/`next_batch`共通helperへ置きeach result once clear、exhaustive status/mode sequence/bound超fetchなし。one probeはsecond前failureをownするがsecond mode-valid rowを即Cardinality 2へ変換しcleanupは置換不可。 | every-status/count/sequence + live zero/one/many + one-row/two-rows-then-fatal |
...
| failure/timeout/cleanup | mode/deadline/fatal/Drop/cardinality/clean-terminal restoreを1 helperへ集約。各`PGcancelConn`をexactly one finishまでownしfirst error/all main-result clear/restore-or-poison後release。clean terminal restore failureはexact PostgreSQL cleanup error、next_batchはunpublished partialをdestroy。BufferedFull/common A1 publicationもaudit。 | cancel full failpoint+allocation、zero/partial-final restore、fatal/timeout/Drop/reuse |
| rows ABI/malformed | exact shared 120-byte v3を1 authorityでconstruct/validate。SQLite direct/prepared StepとPostgreSQL 3 modeのactive/terminalを固定。native field前validateし、terminalはdelivery/pending/portal/result/context zero、deadline `-1`。 | byte golden、field mutation、v2 reject、driver x direct/prepared x active/terminal、SQLite consumer retention |
...
| operation/allocation parity | direct conn/tx x absent-Delivery BufferedFull/SingleRow/PortalBatch x rows/one、conn/tx-prepared stmt x同3 mode x rows_stmtを実行し、両rows axisだけnext/batch。one send/no hidden SQL/one lease/bounded native owner/exact parameter+ID/balanced allocationをassertしSQLite Stepもretain。 | default-and-explicit matrix、dual-driver rows、provisioned suite、measurement |

reopened implementation boundaryは2 independently mergeable vertical PR:
...
| PostgreSQL streamed delivery | reviewed option/function、libpq17、shared rows v3、advancement/cancel/batch publication/exact callable matrix。merged prerequisiteへ依存しfixをduplicateしない。 | full delivery matrix、dual-driver rows/batch、required local/CI PostgreSQL、normal code preflight/review/CI |
...
P2はsecond mode-valid rowでCardinality 2を即own、partial-final cleanup前publication禁止とexact error、
direct/prepared両方のabsent-Delivery BufferedFull axisで閉じる。ownerはconsumer inventory、one/two-row
fatal sequence、zero/partial-final restore failpoint、default-and-explicit operation matrixである。

implementation前にredesigned ledgerと2 PR boundaryをfresh independent adversarial reviewし、findingは
