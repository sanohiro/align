...
| Parallel-worker eligibility | `CsvDecode` は sequential in-memory use では Pure だが、`region` は effect と独立に non-Send。1 個の fail-closed source/HIR capture authority が `spawn`/`ArrayParMap` を含む全 parallel worker boundary で exact `ArenaHandle` を lifted-worker publication/MIR/generated identity/runtime call/allocation 前に拒否。codegen も handcrafted parallel MIR の同じ型を context layout/kernel/runtime publication 前に防御的に拒否。sequential direct/local closure/`map`/`reduce` は通常の lexical region rule で許可 | direct/helper-wrapped `decode` の `spawn`/`par_map` capture、raw malformed `ArrayParMap` HIR と `ParMapParallel`/`ParMapReduce` MIR の `ArenaHandle` capture、whole/per-unit/generic parity、shared authority 下の existing spawn negative、sequential Pure direct/closure/map/reduce positive、failure に worker/capability/context/kernel/call/allocation publication なし |
...
Pure effect と独立な non-Send capture rule による全 `spawn`/`par_map` worker の `region` rejection と sequential acceptance、
