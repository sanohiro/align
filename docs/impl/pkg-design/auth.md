...
status split propagates unchanged: provider/context/output-reserve failure is exactly
`Error.Code(0)`, while `EVP_KDF_derive` rejection is `Error.Invalid`. An allocation in
package-owned builder/string assembly follows ordinary hard-OOM behavior. There is no default work
...
| P2: Argon2 output allocation was described as aborting | Preserved the shipped native provider/context/output-reserve `Error.Code(0)` path, the distinct derive-rejection `Error.Invalid` path, and later package-owned hard OOM. |
...
The changed-slice review of those fixes produced one additional P2 finding:

| Finding | Ledger-first resolution and propagation |
|---|---|
| P2: the first Argon2 correction also classified derive rejection as `Code(0)` | Restored the shipped primitive's complete split: provider/context/output-reserve is `Code(0)`, derive rejection is `Invalid`, and later package-owned OOM aborts. The public rows, detailed precedence, closure owner, digest, roadmap, and handoff use the same split. |
