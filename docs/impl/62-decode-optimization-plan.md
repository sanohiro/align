...
| Bytes cannot reach a numeric pipeline without copying | `f32_le` already lowers to a bounds guard and alignment-1 integer load/float bitcast; it is not a per-element runtime decoder call. Plan 78 later settled `view_le<T>` as a distinct checked route: it validates natural alignment, whole-element length and native little-endian order while preserving authority. | Keep scalar accessors for packed data and implement the single checked view plus inverse from plan 78. Do not add per-type `as_f32_le()` aliases or pretend all byte storage is aligned and writable. |
