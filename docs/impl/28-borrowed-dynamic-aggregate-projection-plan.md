...
| Indexed shared borrow of an owning dynamic-array element | Existing call syntax `inspect(values[index])`, where the selected direct, imported, or function-value target's parameter mode is exactly `borrow`. `values` is a stable named local, borrowed parameter, projected borrowed binding, or struct-field path of one of those, and has an admitted ordinary dynamic scalar-array or AoS record-array type. `index` has the existing array-index type and semantics. No implicit `borrow`, temporary array materialization, or `borrow mut` element form is added. | Static checking resolves the call target and mode, checks the base place and concrete element type, reserves the complete source root, then checks the index expression in existing source order. A non-array, unsupported array class, non-stable base, type mismatch, by-value target, or `borrow mut` target is rejected before MIR. An index expression that might move, drop, replace, or mutably borrow the same or an overlapping source root is rejected; uncertain imported or indirect invalidation is conservatively overlapping. At runtime the base and index are each evaluated once, MIR emits the existing bounds-failure CFG before element-place formation and the call, and an invalid index keeps the existing hard bounds error. | The callee receives a non-null pointer to the existing element for the dynamic extent of the call. The caller and array remain owners; there is no element load, transfer, nulling, cleanup bit, temporary owner, allocation, or independent `Drop`. A view returned through the existing return-borrow summary, or retained in a mutable destination through the existing `BorrowMutRetentionSummary`, is rooted in the source array generation and every contained region root. Exact same-program direct summaries and conservative imported/indirect fallback both substitute the indexed source this way, and the destination must be able to outlive those roots. Moving, dropping, or validly replacing the array invalidates every derived view under existing rules. The element itself cannot be mutably borrowed, replaced, or retained as an owner by this surface. | `align_sema` admits and records the indexed shared-borrow argument, derives exact storage roots, reserves them across index evaluation, and applies direct/imported/indirect return and retention summaries. Checked HIR recomputes base stability, mode, type, index, target mode, and root reservation. MIR carries one bounds-guarded dynamic element place with the base borrowed place, evaluated index operand, and exact element type. MIR owns bounds-failure semantics and validation proves that the guard dominates every use; LLVM only computes the guarded element pointer and passes it through the existing borrowed parameter ABI. | The call changes only implementation/body identity. Function signatures, interface format, runtime ABI, and persisted formats are unchanged. Direct/local/field/projected bases, scalar-string and Move-record arrays, direct/imported/function-local/function-field/joined targets, returned and mutably retained views, source invalidation during and after index evaluation, malformed MIR, generic whole/per-unit parity, and cache edit/revert owners are required. |
...
existing left-to-right call evaluation. During that evaluation, a move, Drop, replacement, or
`borrow mut` operation that might invalidate the same or an overlapping root is rejected before
pointer formation; an uncertain imported or indirect effect uses the conservative overlapping
fallback. Mutation of an unrelated root remains valid. No array pointer or header field is retained
across an index expression that can invalidate the source. MIR then emits the existing signed
bounds-failure CFG, and only its success block forms the element place and calls the target. A
rejected static form creates no HIR/MIR element place. A runtime bounds failure invokes no callee
and creates no owner or cleanup action.
...
reservation across `index`. Missing, duplicate, unsorted, forged, or stale metadata rejects before
MIR.
...
CFG. `base` retains the root slot and ordered static struct/sum path; `index` is the once-evaluated
checked integer operand; `element_ty` is recomputed from the concrete dynamic-array class. Direct,
...
missing slot/path, non-`borrow` target mode, cleanup bit, malformed index operand, or any element
place use not dominated by its exact successful bounds guard before LLVM construction.

LLVM loads the base `{ptr,len}` parts through the already validated borrowed place, computes
`ptr + index * stride` using the canonical scalar or AoS record layout, and passes that element
pointer to the unchanged borrowed parameter ABI. Bounds failure is already explicit MIR control
flow; LLVM introduces no independent bounds decision. It does not load the whole
...
| Control flow and eager index evaluation | Match and indexed calls compose with `if`, nested ordinary blocks, loop joins, divergent arms, and early return without manufacturing moves or fallthrough. Reserve the source root before evaluating the index once in argument order; reject same/overlapping-root move, Drop, replacement, or `borrow mut`, conservatively reject uncertain imported/indirect invalidation, and permit unrelated-root mutation. `else`, `?`, and nested match over a projected binding retain their existing exclusions. | direct control-family owner matrix; same-root invalidating index negatives for every operation; uncertain indirect/imported negative; unrelated mutation positive; side-effecting index and multi-argument order runtime owner; unchanged plan-26 negatives and owning control owners. |
| MIR and checked HIR | Recompute exact payload eligibility and cleanup exclusions; validate every borrowed element base/index/type/mode and direct/indirect selected mode; include the place in exhaustive traversal, printing, fingerprinting, and variant tripwires. MIR emits the existing bounds-failure CFG before the element place, and validation requires its exact successful guard to dominate every use. | forged payload/static type/path/cleanup records; malformed element slot/path/class/index/type/mode/target/guard; bounds-failure no-call owner; print/fingerprint goldens and validator rejection. |
...
- `draft.md`, `docs/language-spec.md`, `docs/design-notes.md`, `docs/open-questions.md`, plans 26
  ...
  dynamic-array classes, shared-only indexed element rule, lifetime roots, and closed exclusions;
...
| P2: assigning bounds semantics to LLVM contradicted the locked backend-agnostic MIR rule | MIR emits the existing bounds-failure CFG and only the successful block may form/use the element place; checked validation proves exact guard dominance. LLVM performs pointer lowering only. The ledger, MIR contract, matrix, and implementation documentation now agree. |
