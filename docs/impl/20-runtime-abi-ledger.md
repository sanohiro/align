...
SIMD-surface consistency capability adds `BufferAppendFilled`. Current inventory: 447 keyed,
465 base, 472 alloc-count, 469 par-map-probe and 476 maximum exports.
...
are 447 `RuntimeKey` variants and a one-to-one native-symbol record. Relative to Am-c1, F-B added
...
`align_rt_tcp_conn_set_io_timeout`. The base native registry therefore has 465 records. R63 replaces Request 12's two bounded-only rows with four unified JSON builder rows
...
The compiler-visible native registry is always exactly the 465 base records.
...
records extend only the verification-time maximum runtime-export table to 476.
...
are 447 keyed records, 465 base records, and 476 records in the maximum optional-probe export table.
...
Every one of the 465 base rows has one `RuntimeEffects` record in
...
| `HttpServerMaxRequestBodyBytes` | `align_rt_http_server_max_request_body_bytes` | `HostState` |
...
- all 447 keys, mapped symbols, LLVM declaration types, and derived effects
  against this table through the checked-in
  `crates/align_codegen_llvm/tests/golden/runtime_abi_declarations.txt`;
- the 465 base native symbols against default-feature `align_runtime` exports,
...
- the 472 `alloc-count` and 469 `par-map-probe` native symbols against
  `align_runtime` built with each feature separately, including the eleven exact
  probe signatures above;
- the 476 maximum native symbols against `align_runtime` built with
...
- all 447 identities through the one `RuntimeAbiId`-keyed row iterator and all
  447 exact registry function types through the production compatibility
