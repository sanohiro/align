...
| `JsonEncodeBounded` | `env[parts.len]`; `child[parts in order, max_bytes]`; `post[the complete TemplatePart contract above; nonempty; max_bytes exactly i64; result exactly Result<String,builtin Error>; every part access is borrowed; source accesses precede the limit; success owns the String payload and failure owns no partial builder output]`. |
