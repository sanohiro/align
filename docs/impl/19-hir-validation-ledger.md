...
| abstract `soa<R>` boundary | Sema admits this template-only form exactly when the corresponding parameter has the closed `SoaPlain` bound, then substitutes and rechecks the ordinary concrete SoA field rule at instantiation. `Ty::Param` under SoA, `SoaPlain`, and every abstract batch thunk signature are forbidden in HIR; `hir_program_is_valid` accepts only the existing concrete `Ty::Soa(struct_id)` after all nominal ids and resource-rooted return provenance are fixed. |
