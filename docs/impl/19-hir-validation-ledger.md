...
remains deferred. Recompute them from authenticated types and roots; the internal
MIR `BorrowedElementPlace.field_path` and producer read-authority edge carry no
serialized field or optimistic producer evidence.
...
and globally reserved schema validation. The ExprKind sweep contains 342 variants.
...
compatible argument/capture. The worker-sendability authority consumes this same fact: a local
`ArenaHandle` root rejects, an explicit parameter root is
...
9. Require every `ArrayParMap` and widened parallel-stage callable to resolve
   to complete replayed `Pure`. Independently compute every staged and terminal capture's complete
   worker-transfer provenance using the same authority as
...
### `pkg.csv` record (implemented 2026-09-03)

The `pkg.csv` implementation adds one expression discriminator, `CsvDecode`, to the current enum,
canonical codec, validator, lowering, capability collector, and inventory. Those producers activate
atomically with canonical package source and runtime row A123.
...
| `MathFn::Exp`, `Exp2`, `Log`, `Log2`, `Log10` (plan 75) | Exactly one operand. Scalar `F` or vector `V(FS,n)`; result equals the exact operand type. Integers, masks, arrays and unresolved numeric variables reject. No accuracy, target-identity or machine-SIMD fact is stored in HIR. |
...
| `MatchArm` | `env[variants,values,bindings,borrowed_bindings]`: the parent `Match` selects exactly one domain. **Sum:** `values` is empty; `variants` contains distinct in-range tags in source or-pattern order, or is empty for wildcard. The table is user `Enum`, `Option` as `Some(T),None`, or `Result` as `Ok(T),Err(E)`. Wildcard/multi-tag arms have no bindings. A one-tag arm has exactly its payload count of distinct in-range locals with exact payload types. **Integer/char:** `variants`, `bindings`, and `borrowed_bindings` are empty; a non-wildcard `values` list contains only `Single(i128)` and inclusive `Range(i128,i128)` in source order, each value is representable in the scrutinee domain, each range has start <= end, and intervals are pairwise disjoint across the complete match. Empty `values` is the wildcard; integer coverage is complete or has one wildcard, while char has exactly one wildcard. **String (plan 72):** the scrutinee is `Str` or `String`; `variants`, `bindings`, and `borrowed_bindings` are empty; each non-wildcard value is `Str(String)` in source order, decoded values are unique across the complete match, and exactly one empty-values wildcard exists. Integer/char variants never occur in string mode and `Str` never occurs in numeric mode; string range syntax has no HIR form. In every domain a wildcard is last reachable, duplicate wildcards reject, and all non-wildcard arms contain at least one discriminator. Sum bindings enter before the body, are visible only in that arm, and leave before its sibling/enclosing tail. In borrowed sum mode, `borrowed_bindings` has one `BorrowedProjection { binding_local, variant, payload_ordinal, static_ty, path }` per binding; its canonical root/path is independently replayed, and projection-only locals cannot appear in any Drop set. Value modes never set parent `borrowed_place`; their text selection borrow is intrinsic to the `Match` and retains no arm binding. `child[body]`; `post[a reachable fallthrough body type equals the Match result under the structural body-type relation (including `FABI` for fresh function-value ids); a divergent body is context-polymorphic and contributes no result join]`. |
...
| `StructLit` | `env[struct_id,fields.len]`: in-range struct and exact field count. It is a general value expression and may occur in every consumer that accepts the resulting struct; `Stmt::Let.init` merely has a direct in-place lowering. `child[fields in declaration order]`; `post[field types equal definitions; result Struct(struct_id); each Move field transfers once]`. Source initializers written out of declaration order are normalized to an ordinary Block of source-ordered Let snapshots, followed by this layout-ordered literal of Local operands. Existing initialization/ownership replay validates those locals; already ordered source literals need no wrapper. |
...
| `RawNull` | `env[]; child[]; post[result Raw; copy; no unsafe lexical owner is required because forming the target null value has no memory effect and compile-time constant substitution erases its source origin; source sema still requires `unsafe` for an ordinary expression-level `raw.null()`]`. |
...
| `Len` | `env[]`; `child[recv]`; `post[recv is exactly Array(_,_), StructArray(_,_), Str, String, Slice(_), DynArray(_), DynVecArray(_,_), DynMaskArray(_,_), DynFixedArray(_,_), DynFixedStructArray(_,_), DynStructArray(_,_), DynSliceArray(_), DynResponseArray, or Soa(_); result i64; recv borrowed. Array(_,n) and StructArray(_,n) receivers retain their evaluated child and return constant n on continuing edges; direct literals use guarded slot initialization and temporary cleanup]`. |
...
| `ElemField` | `env[path,struct_id]`: nonempty valid path in struct_id. `child[recv,index]`; `post[index i64; recv fixed/dynamic StructArray(struct_id) or Soa(struct_id) where producer admits this path; result exact leaf; an indexed Move leaf is admitted only as an immediate shared-borrow call argument, using a checked runtime index for dynamic Slice/AoS and an integer-literal index for a source-formed fixed StructArray; result view/Copy fact inherits the complete direct/field/BorrowedProjection receiver generation and contained roots; a terminating index has no bounds/result fact; bounds/path action last]`. |
...
| recursively Move struct | Admit only when each element is constructed in place, including validated Block/Let source-order snapshots whose tail constructs a source-identical struct; reject copying a pre-existing value. One source type reaches checked HIR under several nominal ids — sema gives each callable origin its own function type — so the constructor is compared by `body_ty_matches`, exactly as the element flow beside it is; the in-place guarantee is carried by the expression form, which must still be a `StructLit`. | `owned_structs_arrays`, `client_admission_composition::reordered_record_initializers_in_fixed_arrays`, `analysis_coverage::monomorph_split_records_certify_every_owned_leaf` |
...
| `ReaderOpen` | `env[regular_only: bool]; child[path]`; `path Str; result ERR(Reader); successful owned fd; Impure`. False selects ordinary `fs.open`; true selects `fs.open_regular`, nonblocking descriptor regular-kind admission followed by blocking restoration. Replay cloning and MIR lowering preserve the discriminator exactly; native ABI shape remains A08. |
...
`Stmt`, all 342 `ExprKind` variants, `ArithMode`, `MathFn`, every
...
| Parallel-transfer consumer | A direct one-argument transparent call, an eager multi-argument direct call, and an indirect function-value call translate the callee's worker-transfer summary through completed pre-call argument facts before a mutable action advances or ends any destination generation. The callee body has already reduced its own source order to exact selected `params`/`captures` roots; `ClosureTarget`/`ClosureCapture` projections recursively retain a nested function value's environment, and an unresolved target selects every compatible argument and environment root. The caller does not re-read those selected facts after `borrow mut` invalidation, because ended parameter/storage markers are intentionally absent from the published summary. The `pkg.csv` worker-send consumer rejects a translated region local and preserves a translated explicit parameter or lifted environment slot in the corresponding summary set for the next caller. | `mutable_calls_publish_parallel_transfer_before_advancing_destinations` parameterizes transparent direct, eager direct, and indirect calls; the `pkg.csv` worker-send owners add direct and nested-closure worker environments plus malformed direct MIR capture rejection. |
...
| Whole/per-unit retention | A same-program body or validated imported mutable-retention record selects its exact stored roots, including known-empty retention. Only missing records and indirect calls conservatively expose contained and storage provenance from every compatible argument for each mutable destination. Every path still rejects a concrete shorter-lived source before it reaches longer-lived caller storage. | `out_str_retention_matches_whole_and_per_unit_checking`; `pkg_db_q6::borrow_mut_shaper_retention::{imported_interface_retention_parity, imported_storage_retention_parity}` safe/retaining twins; existing indirect fallback owners |
...
allocation, fd transfer, or I/O. These rows are active and owned by `pkg-design/ws.md`.
