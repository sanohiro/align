...
**R69–R76 provider implementation:** [R69–R76 plan 54](impl/54-r69-r76-prerequisite-batch-plan.md)
...
Self `access` is supported on Linux/macOS. Strict relative `access_at` supports
Linux; macOS returns `Error.Code(ENOTSUP)` after complete path/mask validation
...
**or-pattern** `A | B` (bare variant names, binds nothing). `match` also supports integer and `char`
values, inclusive ranges (`min..=max`), exact string literals, and literal or-patterns
(`1 | 2 | 5..=10`, `"get" | "head"`). Non-exhaustive integer matches and every `char`, `str`, or
`string` match require `_`. String patterns have no ranges; they compare decoded UTF-8 bytes exactly
and borrow the once-evaluated scrutinee without cloning, allocation, or moving an owned string.
`Option<T>` / `Result<T,E>` are sum
...
`fs.open_beneath_single_link(root: str, relative: str) -> Result<reader, Error>`, and
...
The single-link constructor runs the complete `open_beneath` sequence, then uses that sequence's
opened-descriptor `fstat` record to accept exactly `st_nlink == 1`; every other link count is
...
validation and wire rules are in `impl/pkg-design/s3-presign.md`; the accepted capability is implemented.
...
request setters require `borrow mut` on borrowed receivers. Response inspection accepts shared borrows; request serialization remains an
internal codec observed through sent bytes. The existing bound-owner
