...
Vector search is already usable through `pkg.db` when the database accepts a textual vector at its
SQL boundary and the result is shaped into ordinary supported fields. Align does not hide the
database contract behind a common vector API. The application owns embedding generation and the
`Params`/`Row` shape; each `Query` owns driver-pinned SQL, distance semantics, filtering, index use,
and tuning.

VC1 proves this path with PostgreSQL 16 and pgvector 0.8.6. The test database explicitly runs
`CREATE EXTENSION vector`; neither `pkg.db` nor `alignc` installs or upgrades the extension.
...
The vector is visibly constructed as text and parsed by the server. This is a compatibility path,
not a zero-copy or optimal-transport claim. Direct `pkg.db.rows`, `pkg.db.prepare` plus
`pkg.db.rows_stmt`, checked/offline metadata, `pkg.db.meta_query`, and `pkg.db.explain` all use the
same descriptor. Checked PostgreSQL metadata includes the canonical extension schema, name, and
version, so a database without pgvector fails preparation and a version change changes schema
identity instead of selecting a fallback.

## Vendor boundaries

This table records product contracts, not interchangeable Align capabilities. Follow the linked
vendor documentation when choosing a version or tuning a production index.

| Database | Storage and availability | Exact search and distance | Approximate index/search | Tuning boundary |
|---|---|---|---|---|
| PostgreSQL + [pgvector 0.8.6](https://github.com/pgvector/pgvector/tree/v0.8.6) | Separately installed `vector(N)` extension type. VC1 pins the official `pgvector/pgvector:0.8.6-pg16-bookworm` image. | Order by `<->` (L2), `<#>` (negative inner product), `<=>` (cosine), or `<+>` (L1) without an index for an exact scan. | HNSW and IVFFlat operator-class indexes; the query operator must match the index. | HNSW construction/search and IVFFlat lists/probes remain extension SQL/settings. |
...
## SQLite vec1 disposition

VC1 also ran an isolated x86-64 Linux probe from sqlite.org's `version-0.7` source. The probe built
`vec1.c` as a loadable extension, loaded it into the SQLite CLI, and observed:
...
The bytes are little-endian `f32` on that probe host; vec1 specifies machine byte order, so they are
not a portable persisted encoding. The extension had to be loaded outside `pkg.db`. Align's SQLite
connection deliberately keeps extension loading disabled, so VC1 ships no SQLite vector support.
A future bundled or statically linked vec1 proposal must separately own dependency provenance,
platform builds, initialization, migration behavior, and its public contract.

Direct native-vector binding is likewise future work. It needs a driver-qualified design for type
identity, dimensions, finite values, encoding, ownership, allocation, error precedence, and Row
lifetimes. Similar vocabulary across these databases is not evidence that one representation is
