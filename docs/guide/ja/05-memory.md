...
「この box は `.get()` で読み戻されるだけでスコープ外へエスケープしていないので、値を直接使えばよい（スタック上の値で足りる）」という意味です。`heap.new` が存在するのは、ある値が「それを生成したスタックフレームよりも長生きする必要があり、かつ、特定の arena のライフタイムに属する必要がある」場合のみです。意図的に、日常的に使う道具としては設計されていません。Align では、「すべてのオブジェクトをヒープに確保する」というコーディングスタイルは推奨されません。言語自体が、プログラマを「値」と「arena」の活用へと導き、そこから外れれば警告を出してくれるのです。
