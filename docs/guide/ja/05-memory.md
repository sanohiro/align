...
Align にはガベージコレクタも手動の `free` もなく、ライフタイム注釈も存在しません。その代わりに、**データがどこに配置されるか（どこに住むか）はプログラマが明示的に決定**し、所有権は**型の性質**として扱われます。コンパイラはすべてのライフタイムを推論し、ダングリング参照（宙ぶらりんの参照）を生むプログラムを拒否します。この章では、この小さくも強力なメモリモデルの中核を解説します。
