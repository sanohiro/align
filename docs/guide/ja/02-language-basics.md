...
残るもの —— 反復回数が*反復自身によって*決まるもの、たとえば EOF まで読む、成功するまでリトライする —— には、構文がきっかり 1 つだけあります。**`loop` 式**(**implementation in progress**)です。`break` が走るまでブロックを繰り返し、`break value` はその値でループを終えます。ですから `loop` も、`if` や `match` と同じく式です。
