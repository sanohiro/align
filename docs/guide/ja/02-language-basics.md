...
Align には `for` も `while` もありません。これは欠落ではなく、言語の重心そのものです。データに対する反復は**パイプライン**(`xs.map(f).where(p).sum()`、[06](06-pipelines.md) 章)であり、コンパイラはそれをベクトル化可能な単一ループへ融合します。コレクションを手作業で走査しそうになったら、手を止めてください。代わりに、その変換に名前を与えるのです。

残るもの —— 反復回数が*反復自身によって*決まるもの、たとえば EOF まで読む、成功するまでリトライする —— には、構文がきっかり 1 つだけあります。**`loop` 式**です。`break` が走るまでブロックを繰り返し、`break value` はその値でループを終えます。ですから `loop` も、`if` や `match` と同じく式です。
...
`continue` もラベル付き break もありません。次へスキップしたいなら本体の残りを `if` で囲めばよく、2 段階の脱出をしたいネストしたループは、関数として切り出されるのを待っている状態です。ループの状態は、ループの前に宣言した `mut` のローカル変数に置きます。

分担は厳格です。**パイプラインがデータの経路を受け持ち、`loop` が制御の経路を受け持ちます。** ループを書きたい衝動に駆られたら、まずその*変換*が何なのかを問うてください。十中八九それはパイプラインであり、`loop` の中で配列をインデックスで走査すると lint が付きます。(再帰も存在しますが、それは本当に再帰的な問題 —— パーサ、木構造 —— のためのものであり、反復のためではありません。Align は末尾呼び出し最適化を保証しないので、再帰による「ループ」はスタックを消費します。)
...
その型は **`array<T>` ではなく `slice<T>`** です。Align では所有権は型の性質であり、トップレベル定数は何も所有しません。文字列リテラル `"hello"` が所有権を持つ `string` ではなく `str`(読み取り専用バイト列へのビュー)であるのとまったく同じで、`PRIMES` はプログラムに焼き込まれた読み取り専用テーブルを見る `slice<i64>` です。インデックス(`PRIMES[3]`)、`.len()`、パイプライン(`PRIMES.where(...).sum()`)はいずれもアロケーションなしでそのテーブルを直接読み、定数インデックスはコンパイル時に要素へ畳み込まれます。テーブルは読み取り専用なので、そこへ書き込むことはできません(`PRIMES[0] = 9` はコンパイルエラーです)。変更したいときは、まず所有権を持つ配列へコピーしてください。注釈は `slice<T>` と書く(または省略する)こと。`array<T>` の注釈は拒否されます。
