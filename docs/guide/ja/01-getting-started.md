...
Align はまだリリース前です。リリース自動化により、タグ付きビルドが macOS (Apple Silicon) および Ubuntu 24.04 (x86_64 / ARM64) 向けにパッケージ化されます。最初のリリースとリポジトリの設定が完了した後に、パッケージマネージャからインストールできるようになります。それまでは、コンパイラをソースからビルドしてください。
