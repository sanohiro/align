...
`check` はエントリファイルと到達可能なインポート先をまとめて解析します。
`check-per-unit` は `build` と同じインターフェース単位のフロントエンド処理を使い、
検査済み HIR の検証、MIR への変換、値の生成元の認証まで行いますが、実行ファイルは出力しません。
各コマンドの診断範囲は次のとおりです。

| 診断の種類 | `check` | `check-per-unit` | `build` |
|---|---|---|---|
| ソース読み込み、構文、名前、型、lint、move、borrow、region、effect | プログラム全体で解析 | コンパイル単位ごとに解析 | コンパイル単位ごとに解析 |
| インポートしたインターフェースの契約と利用側のジェネリック具体化（borrow、region、可変参照先の保持関係、cleanup の復元を含む） | 独立したインターフェース境界の検査なし | 検査する | 検査する |
| 検査済み HIR の検証、MIR への変換と値の生成元の認証、インターフェース記述子の生成 | 後段に残る | 検査する | 検査する |
| 出力時の関数値・コールバック・並列カーネルの ABI 検証、ネイティブのレイアウト、LLVM 検証、最適化、オブジェクト出力 | 後段に残る | 後段に残る | コード出力時に検査する |
| ビルド専用のオプションと入力、ランタイム・PGO の成果物、ネイティブライブラリとシンボル解決、リンク、実行ファイルの配置 | 後段に残る | 後段に残る | 該当する場合に検査する |

全体解析と単位ごとの解析は、必ずしも同じ診断を出しません。後者は公開されたモジュール契約に
照らして呼び出しを検査します。特に、`check` の成功だけでは、インターフェース経由の
borrow・region 解析や MIR の認証を通過するとは限りません。`check-per-unit` はその段階までを
検査しますが、表の最後の2行にあるネイティブビルドの検査は行いません。
ビルドキャッシュは、各段階を繰り返す代わりに検証済みのフロントエンド結果やオブジェクトを再利用できます。

コンパイル可能な状態を確認するには、同じエントリファイルと依存ソースに対して次の2つを実行し、
ビルドには実際に使うターゲット・プロファイル・リンカーのオプションを指定してください。
...
検査の成功は、ネイティブコード生成やリンクの成功を保証しません。
正しいプログラムが内部の HIR・MIR 検証やバックエンド検証で失敗する場合は、引き続き
コンパイラの不具合として報告してください。この処理段階の違いは、言語の制約を増やしたり、
所有権検査の不整合を正当化したりするものではありません。
...
**フロントエンドキャッシュ**は、各コンパイル単位の検査済みインターフェース、診断、リンクライブラリを保存し、別プロセスでの再ビルドでも検査を省けるようにします。キャッシュの識別には、その単位のソースのバイト列、検査時に参照した直接・間接の依存インターフェース一式、コンパイラとインターフェース形式の識別子、解決済みのターゲットトリプルを使います。Apple ターゲットでは、このトリプルは deployment target を含み、カーネルのバージョンは含みません。プロファイル、`--target-cpu`、ランタイム LTO、PGO モードはフロントエンドの出力に影響しないため含めず、同じエントリを異なるビルド設定で使えます。
...
`emit-mir` はプログラムの意味を表す中間表現を表示します。最適化前の LLVM IR では低レベルな表現への変換結果を、最適化後の IR では LLVM が最適化した結果を確認できます。`explain-opt` は、コンパイラが現在選んだストレージ方式と明示的並列処理の実行方式を報告し、その後にベクトル化などの LLVM 最適化情報をソース行へ対応づけます。`size` は選択したプロファイルで `build` と同じ成果物を作り、サイズの内訳を報告します。単独のオブジェクトや IR を出力するときは、`--export name` を繰り返し指定すると、エントリ単位の特定の関数を外部に公開できます。

実行ファイルをリンクするためのルートと、単位を調査するためのルートは別の問題です。ビルドは `main` と `--export` で指定した関数をルートとし、`emit-obj` の出力はリンクされるものなので同じ集合を使います。調査用のコマンドは、指定したファイル自身のコードを報告します。そのファイルが `main` を定義せず `--export` も指定しない場合は、そのファイルが定義するすべての `pub` 関数がルートになります。そのため `emit-llvm --stage optimized` はその単位自身の関数本体を出力し、`explain-opt` は空のモジュールではなくその単位自身の最適化結果を報告します。追加されたルートは標準エラー出力に通知するので、リダイレクトした IR やレポートの内容は変わりません。`--export` は `emit-obj`、`emit-llvm`、`explain-opt` で使用でき、従来どおりルートを絞り込みます。`emit-mir` は低レベル表現をすべて出力するため、この規則は不要です。

CPU名の欠落、空文字、埋め込みNUL、未知の名前、別アーキテクチャ用の名前は、成果物の処理前にエラーになります。名前は対象アーキテクチャのLLVM情報で検証します。`emit-llvm` と `explain-opt` は選択したプロファイルのパイプライン、コード生成レベル、サイズ属性を使用します。raw IRにもサイズ属性は含まれますが、最適化前の状態です。`explain-opt` はruntime LTOを無効にした単位ごとの観測結果であり、ThinLTOでリンクした実行ファイルの結果ではありません。
...
--sdk-version VERSION                   # 計画中 (#1093): Apple オブジェクトに明示する SDK 来歴
...
`build`、`run`、`size`、`test` では `--cc /absolute/path/to/cc`
（または `--cc=/absolute/path/to/cc`）で C ドライバを明示できます。
`--` より前に一度だけ指定します。区切りの `--` を受け付けるのは `run` だけで、
以降はプログラムの引数です。他のコマンドでは処理前に区切りを拒否します。
UTF-8 の絶対パスで実行可能な通常ファイルを指定してください。値不足、相対パス、
重複、対象外のコマンドへの指定はコンパイル前にエラーになります。空白や
シンボリックリンクは使えますが、シェル展開は行いません。指定パスを直接実行し、
`PATH` へのフォールバックはしません。`build --watch` の再ビルドでも指定を保持します。
省略時は通常の `cc` 探索を維持します。対象はドライバだけであり、その内部の
リンカー探索、シンボル削除、サイズ調査には通常のツールチェーン設定が必要です。
実行ファイルの内容認証や、検証後のファイル差し替え防止は行いません。

`alignc` は既定ではリンクをシステムの C ドライバ経由で行います。ELF ターゲットではさらに、LLVM の `ld.lld` を使うよう選択したドライバへ指示します。`ld.lld` は `alignc` が元から必要とする LLVM ツールチェーンに同梱されているため、新たにインストールするものはありません。環境変数 `ALIGNC_LINKER` で選択を固定できます。
