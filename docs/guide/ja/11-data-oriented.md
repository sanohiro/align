...
`.agg(...)` は各キーを 1 度だけインターン化し、すべてのアキュムレータを 1 パスで畳み込みます。手書きの分析ループが取る形を、宣言から生成します。`str` key の AoS と SoA source を受け付けます。numeric-key SoA では個別の grouped reducer は使えますが、複数集約の `.agg(...)` 形式は保留中です。
