...
これで、`Disconnected`（切断状態）のときに `user_id` を持つことは物理的に不可能になりました。ソケットを持たずに `Authenticating`（認証中）になることも不可能です。データの「形」が、ドメインの「現実」と完全に一致したのです。（ペイロードは位置指定で、今日の Align ではスカラーか素の構造体です — 名前を与えたくなったら、`Connected(Session)` のように構造体をペイロードにします。）
