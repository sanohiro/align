...
システムを構築する際は、「実行したい処理を過不足なく表現できる、最も薄い（狭い）レイヤー」を選択してください。単なるバイト列の移動なら `reader` / `writer` を、ソケットレベルの制御が必要なら `std.net` を、HTTP のセマンティクスが必要なら `std.http` を選びます。それ以上の高度なルーティング、ミドルウェア、特定のアプリケーションプロトコル、Web フレームワークなどは、標準ライブラリ（`std`）ではなく `pkg` レイヤーが担います。現在は first-party の `pkg.web` と `pkg.jwt` が具体例として実装されており、第 [23](23-packages.md) 章で紹介します。
