...
現時点での実装上の制限についても触れておきます。変数に代入されるラムダ式では、パラメータの型注釈が必須となります。また、関数値を**戻り値として返す**ことや、構造体・配列に格納する機能は、リージョン機能に裏打ちされたクロージャ環境が実装されるまで保留されています。関数を「下へ（引数として）渡す」機能は、パイプラインと2つの並列処理構文のすべてをカバーしており、実用上の多くのユースケース（荷重を支えるケース）はこれで十分に満たせます。
