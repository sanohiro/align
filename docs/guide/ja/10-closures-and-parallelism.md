...
コンパイラはすべての関数について、それが **Pure（純粋）** であるかどうかを推論します。ここでの「純粋」とは、I/O 操作がない、乱数生成（RNG）がない、FFI 呼び出しがない、そして自身が所有しない外部状態への変更（ミューテーション）がないことを意味します（明示的な `borrow mut` パラメータとして渡された値の更新は、呼び出し元がまさにそのために引き渡したものなので、純粋のままです）。プログラマが `pure` のような注釈を書くことは決してなく、そのため注釈を書き間違えることもありません。コンパイラがあなたを危険から守ってくれた時にだけ、この純粋性推論の存在に気づくはずです。
