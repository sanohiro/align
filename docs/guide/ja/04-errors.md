...
`?` が伝播のすべてです。`Ok` をアンラップするか、`Err` を即座に呼び出し元へ返すか。ハッピーパスは上から下へ読み下せます。エラーパスは冷たい(cold)縁であり、コンパイラは文字どおりそれを cold ブランチとして配置します。ついに `Result` を*消費する*地点まで来たら、形は 2 つです。エラーの*中身*が問題になるなら `match`、ならないなら `else` —— `v := f() else fallback` は `Ok` の値を取り出すか、エラーを意図的に捨ててフォールバックを評価します(**実装進行中**)。意図ごとに形はちょうど 1 つ:`?` は伝播、`else` はフォールバック、`match` は検査です。
