...
バリアントを生成する際は、型名で修飾します（例：`Shape.Rect(3, 4)` や `Shape.Dot`）。これにより、コードを読む人はそのバリアントがどの型に属しているかを常に把握できます。ペイロードは位置で指定します。スカラー、借用した `str` ビュー、構造体、sum、所有 `string`、対応済みの所有配列を格納できます。所有ペイロードを持つと sum 値全体が Move 値になり、生きているアームだけが再帰的に破棄されます。1 つのバリアント内に複数の所有ペイロードがある場合、それらは同一のアロケーションモードを共有する必要があります。`Result<Option<T>, E>` のような入れ子のタグ付きペイロードも動作し、第 2 の仕組みではなく同じ 1 つのエラーモデルと 1 つの所有権モデルを再利用します。
