...
バリアントを生成する際は、型名で修飾します（例：`Shape.Rect(3, 4)` や `Shape.Dot`）。これにより、コードを読む人はそのバリアントがどの型に属しているかを常に把握できます。ペイロードは位置で指定します。スカラー、借用した `str` ビュー、素のデータを持つ構造体、対応済みの所有配列を格納でき、所有配列を持つ sum 値は、生きているアームをタグに従って破棄する Move 値になります。裸の所有 `string` ペイロードと、捨てられたアームをまだ深く破棄できない形は現在も拒否されます。
